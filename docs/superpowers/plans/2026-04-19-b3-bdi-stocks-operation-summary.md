# B3 BDI Stocks Operation Summary Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add `B3BdiStocksOperationSummary` — an ingestion class for the B3 BDI `StocksOperationSummary` endpoint that returns equity market activity broken down by transaction type (standard lots, fractional, etc.).

**Architecture:** POST to `https://arquivos.b3.com.br/bdi/table/StocksOperationSummary/{date}/{date}/{page}/{page_size}` with an empty JSON body `{}`. Response is the same `{"table": {"columns": [...], "values": [...]}}` envelope used by all BDI table endpoints. Class mirrors `B3BdiStocksSummary` exactly — same pagination loop, same PascalCase→UPPER_SNAKE_CASE column mapping via `StrHandler`, different URL and column schema.

**Tech Stack:** Python 3.11+, requests, backoff, pandas, pytest, pytest-mock
...
| Action | Path |
|--------|------|
| Create | `stpstone/ingestion/countries/br/exchange/b3_bdi_stocks_operation_summary.py` |
| Create | `tests/unit/test_b3_bdi_stocks_operation_summary.py` |
| Create | `examples/b3_bdi_stocks_operation_summary.py` |

---

### Task 1: Write unit tests (failing)
...
```python
"""Unit tests for B3BdiStocksOperationSummary class."""
...
from stpstone.ingestion.countries.br.exchange.b3_bdi_stocks_operation_summary import (
	B3BdiStocksOperationSummary,
)
...
@pytest.fixture
def instance(sample_date: date) -> B3BdiStocksOperationSummary:
	"""Fixture providing a B3BdiStocksOperationSummary instance.
	...
	"""
	return B3BdiStocksOperationSummary(date_ref=sample_date)
...
def sample_table_dict() -> dict:
	...
	-------
	dict
		Sample table dict mimicking the BDI StocksOperationSummary API response structure.
	"""
	return {
		"columns": [
			{"name": "TckrSymb"},
			...
			{"name": "ThousandTitles"},
			{"name": "Part1"},
			{"name": "ValueInThousand"},
			{"name": "Part2"},
			{"name": "OrderCol"},
		],
		"values": [
			["LOTE PADRAO ", 2373751, 4439747, 65.56, 36370026.07, 44.11, 0],
		],
	}
...
def test_init_with_valid_inputs(sample_date: date) -> None:
	...
	None
	"""
	inst = B3BdiStocksOperationSummary(date_ref=sample_date, int_page_size=500)
	...
	assert "StocksOperationSummary" in inst.url_tpl
...
def test_init_default_page_size(sample_date: date) -> None:
	...
	None
	"""
	inst = B3BdiStocksOperationSummary(date_ref=sample_date)
...
def test_init_without_date_ref(mocker: MockerFixture) -> None:
	...
	inst = B3BdiStocksOperationSummary()
...
def test_init_logger_propagated() -> None:
	...
	inst = B3BdiStocksOperationSummary(date_ref=date(2026, 4, 17), logger=mock_logger)
	assert inst.logger is mock_logger


def test_get_response_success(
	instance: B3BdiStocksOperationSummary, mocker: MockerFixture
) -> None:
	"""Test get_response posts to the correct URL and returns the response.

	Parameters
	----------
	instance : B3BdiStocksOperationSummary
		Initialized instance.
	mocker : MockerFixture
		Pytest-mock fixture.
...
def test_transform_data_normal(
	instance: B3BdiStocksOperationSummary, sample_table_dict: dict
) -> None:
	...
	assert df_["TCKR_SYMB"].iloc[0] == "LOTE PADRAO "
	assert df_["NMBR_TRADES_DAY"].iloc[0] == 2373751
	assert df_["VALUE_IN_THOUSAND"].iloc[0] == 36370026.07


def test_transform_data_all_rows(instance: B3BdiStocksOperationSummary) -> None:
	"""Test transform_data handles multiple transaction-type rows correctly.
	...
	table = {
		...
		"values": [
			["LOTE PADRAO ", 2373751, 4439747, 65.56, 36370026.07, 44.11, 0],
			["FRACIONARIO ", 289639, 82656, 1.22, 92155.91, 0.11, 1],
			["TOTAL ", 2663390, 4522403, 66.78, 36462181.98, 44.22, 2],
		],
	}
	df_ = instance.transform_data(table)
	assert len(df_) == 3
	assert set(df_["TCKR_SYMB"].tolist()) == {"LOTE PADRAO ", "FRACIONARIO ", "TOTAL "}


def test_transform_data_empty_values(
	instance: B3BdiStocksOperationSummary, empty_table_dict: dict
) -> None:
...
```bash
poetry run pytest tests/unit/test_b3_bdi_stocks_operation_summary.py -v
```
...
```python
"""B3 BDI Stocks Operation Summary ingestion."""
...
class B3BdiStocksOperationSummary(ABCIngestionOperations):
	"""B3 BDI Stocks Operation Summary ingestion class."""
	...
	def __init__(
		...
		int_page_max : int, optional
			...
			returns a small fixed set of transaction-type rows.
		...
		self.url_tpl = (
			f"https://arquivos.b3.com.br/bdi/table/StocksOperationSummary/"
	...
	def run(
		...
		str_table_name: str = "br_b3_bdi_stocks_operation_summary",
		...
		str_table_name : str, optional
			The name of the table, by default "br_b3_bdi_stocks_operation_summary".
		...
		dict_dtypes = {
			"TCKR_SYMB": str,
			"NMBR_TRADES_DAY": int,
			"THOUSAND_TITLES": float,
			"PART1": float,
			"VALUE_IN_THOUSAND": float,
			"PART2": float,
			"ORDER_COL": int,
			"URL": str,
		}
	...
	def _log_page_progress(self, int_page: int, int_rows: int) -> None:
		...
		self.cls_create_log.log_message(
			logger=self.logger,
			message=(
				f"B3BdiStocksOperationSummary: page {int_page} fetched ({int_rows} rows)"
			),
			log_level="info",
		)
...
### Task 3: Create the example file
...
cls_ = B3BdiStocksOperationSummary(
	date_ref=None,
	logger=None,
	cls_db=None,
)
...
```bash
make test_feat MODULE=b3_bdi_stocks_operation_summary
```

Expected: ruff + codespell + pytest all PASS, no lint or type errors.
