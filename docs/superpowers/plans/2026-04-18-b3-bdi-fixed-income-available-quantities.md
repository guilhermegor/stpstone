# B3 BDI Fixed Income Available Quantities — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Ingest the B3 BDI `Register` table, which reports the daily count of registered fixed-income instruments and their aggregate financial volume by instrument type.

**Architecture:** Single paginated-POST ingestion class following the identical scaffold used by `B3BdiFixedIncomeInstrumentsRegistration`. Pagination loop in `run()`, exponential-backoff on `get_response()`, PascalCase-to-UPPER_SNAKE column rename in `transform_data()`.

**Tech Stack:** Python 3.10+, `requests`, `backoff`, `pandas`, `pytest`, `pytest-mock`, `stpstone.ingestion.abc.ingestion_abc`
...
| Action | Path |
|---|---|
| Create | `stpstone/ingestion/countries/br/registries/b3_bdi_fixed_income_available_quantities.py` |
| Create | `tests/unit/test_b3_bdi_fixed_income_available_quantities.py` |
| Create | `examples/b3_bdi_fixed_income_available_quantities.py` |
...
from stpstone.ingestion.countries.br.registries.b3_bdi_fixed_income_available_quantities import (  # noqa: E501
...
@pytest.fixture
def instance(sample_date: date) -> B3BdiFixedIncomeAvailableQuantities:
	"""Fixture providing a B3BdiFixedIncomeAvailableQuantities instance.
	...
	"""
	return B3BdiFixedIncomeAvailableQuantities(date_ref=sample_date)
...
def sample_table_dict() -> dict:
	...
	-------
	dict
		Sample table dict mimicking the BDI Register API response structure.
	"""
	return {
		"columns": [
			{"name": "RptDt"},
			{"name": "TckrSymb"},
			{"name": "QuantityInstrument"},
			{"name": "RegisterVolume"},
		],
		"values": [
			["2026-04-17T00:00:00", "CDB", 1000847, 51456888798.04],
		],
	}
...
def test_init_with_valid_inputs(sample_date: date) -> None:
	...
	None
	"""
	inst = B3BdiFixedIncomeAvailableQuantities(date_ref=sample_date, int_page_size=500)
...
def test_init_default_page_size(sample_date: date) -> None:
	...
	None
	"""
	inst = B3BdiFixedIncomeAvailableQuantities(date_ref=sample_date)
...
def test_init_without_date_ref(mocker: MockerFixture) -> None:
	...
	inst = B3BdiFixedIncomeAvailableQuantities()
...
def test_init_logger_propagated() -> None:
	...
	inst = B3BdiFixedIncomeAvailableQuantities(date_ref=date(2026, 4, 17), logger=mock_logger)
	assert inst.logger is mock_logger


def test_get_response_success(
	instance: B3BdiFixedIncomeAvailableQuantities, mocker: MockerFixture
) -> None:
	"""Test get_response posts to the correct URL and returns the response.

	Parameters
	----------
	instance : B3BdiFixedIncomeAvailableQuantities
		Initialized instance.
	mocker : MockerFixture
		Pytest-mock fixture.
...
def test_transform_data_normal(
	instance: B3BdiFixedIncomeAvailableQuantities, sample_table_dict: dict
) -> None:
	...
	assert len(df_) == 1
	assert list(df_.columns) == ["RPT_DT", "TCKR_SYMB", "QUANTITY_INSTRUMENT", "REGISTER_VOLUME"]
	assert df_["TCKR_SYMB"].iloc[0] == "CDB"


def test_transform_data_empty_values(
	instance: B3BdiFixedIncomeAvailableQuantities, empty_table_dict: dict
) -> None:
...
```bash
poetry run pytest tests/unit/test_b3_bdi_fixed_income_available_quantities.py -v
```

Expected: `ImportError: cannot import name 'B3BdiFixedIncomeAvailableQuantities'`
...
```python
"""B3 BDI Fixed Income Available Quantities ingestion."""
...
class B3BdiFixedIncomeAvailableQuantities(ABCIngestionOperations):
	"""B3 BDI Fixed Income Available Quantities ingestion class."""
	...
	def __init__(
		...
		self.url_tpl = (
			f"https://arquivos.b3.com.br/bdi/table/Register/"
	...
	def run(
		...
		str_table_name: str = "br_b3_bdi_fixed_income_available_quantities",
		...
		dict_dtypes = {
			"RPT_DT": "date",
			"TCKR_SYMB": str,
			"QUANTITY_INSTRUMENT": int,
			"REGISTER_VOLUME": float,
			"URL": str,
		}
	...
	def _log_page_progress(self, int_page: int, int_rows: int) -> None:
		...
		self.cls_create_log.log_message(
			logger=self.logger,
			message=(
				f"B3BdiFixedIncomeAvailableQuantities: page {int_page} fetched ({int_rows} rows)"
			),
			log_level="info",
		)
...
```bash
git add stpstone/ingestion/countries/br/registries/b3_bdi_fixed_income_available_quantities.py \
        tests/unit/test_b3_bdi_fixed_income_available_quantities.py
git commit -m "feat(bdi-b3): Add B3 BDI fixed income available quantities ingestion

Implements Register table ingestion with paginated POST, backoff retry,
and UPPER_SNAKE_CASE column normalisation.
"
```
...
- [ ] **Step 2: Run full feature check**

```bash
make test_feat MODULE=b3_bdi_fixed_income_available_quantities
```

Expected: codespell, ruff check, ruff format, pydocstyle, and pytest all pass.

- [ ] **Step 3: Commit**

```bash
git add examples/b3_bdi_fixed_income_available_quantities.py
git commit -m "feat(bdi-b3): Add example for B3 BDI available quantities ingestion

Demonstrates zero-config usage of B3BdiFixedIncomeAvailableQuantities.
"
```
