...
class JobsCLI:
    ...
    def get_run_metadata(
        self, filename='run_metadata.json', outside_run_id=None
    ):
        complete_path = os.path.join(self.path, filename)
        if outside_run_id:
    ...
    def get_run_output(
        self, filename='run_output_metadata.json', outside_run_id=None
    ):
        complete_path = os.path.join(self.path, filename)
        if outside_run_id:
            command = 'databricks runs get-output --run-id {} > {}'.format(
                outside_run_id, complete_path
            )
        else:
            command = 'databricks runs get-output --run-id {} > {}'.format(
...
class DbfsCLI:
        ...
        if l == '-l':
            ...
            for x in output:
                while '' in x:
                    x.remove('')
            dict_infos = {
                x[2]: {'tipo': x[0], 'tamanho': x[1]} for x in output
            }
            return dict_infos
        else:
            output = [x.strip() for x in stream.readlines()]
    ...
    def copy_and_run(self, local_path, dbfs_path, job_id, int_seconds_wait=10):
        ...
        try:
            ...
            print(
                'Unable to fetch metadata from run: {}'.format(db_cli.run_id)
            )
        sleep(int_seconds_wait)
        while run_state['state']['life_cycle_state'] != 'TERMINATED':
            try:
                ...
                print(
                    'Unable to fetch metadata from run: {}'.format(
                        db_cli.run_id
                    )
                )
            sleep(int_seconds_wait)
