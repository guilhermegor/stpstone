...
class Databricks:
    def __init__(
        self, str_query, list_dsns, logger=None, max_error_attempts=3
    ):
    ...
    def conn_dsn_databricks(
        self, dsn_conn, int_timeout=108000, bl_autocommit=True
    ):
        """
    ...
    def conn_databricks(
        ...
        for dsn in self.list_dsns:
            try:
                ...
                df_databricks = func_timeout(
                    int_max_wait,
                    self.get_data_from_databricks,
                    args=(connection),
                )
                ...
                if self.logger != None:
                    CreateLog().warnings(
                        self.logger,
                        'Connection could not be established in '
                        + 'the DSN {}'.format(dsn),
                    )
            ...
            if self.logger != None:
                ...
                if bl_kill_process_when_databricks_down == True:
                    raise Exception(
                        ...
                        'Connection to Databricks could not be established '
                        + 'in any of the DSNs, please validate the stability of the service. List of DSNs '
                        + 'configured on the machine: {}'.format(
                            self.list_dsns
                        ),
                    )
            return str_error
    ...
    def retrieve_query_data(self, int_max_wait=480, i=0, df_databricks=''):
        ...
        while (type(df_databricks) == str) and (i <= self.max_error_attempts):
            ...
            if self.logger == None:
                print(
                    '#{} Attempting connection with DSNs to Databricks'.format(
                        i
                    )
                )
            else:
