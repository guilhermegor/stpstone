...
class AnbimaMTM:
    def general_req(self, url, bl_verify, method='GET', str_sep='@'):
        ...
        for row in list_tb:
            ...
            #   appending data
                ...
                dict_ = dict(
                    zip(list_headers, row.replace(',', '.').split(str_sep))
                )
    ...
    def br_treasury_bonds(self, wd_bef=1):
        ...
        url = YAML_ANBIMA['sec_mkt_prcs']['br_treasuries']['url'].format(
            DatesBR()
            .sub_working_days(DatesBR().curr_date, wd_bef)
            .strftime('%y%m%d')
        )
        # requesting secondary market prices
        df_br_tb = self.general_req(
            url, YAML_ANBIMA['sec_mkt_prcs']['br_treasuries']['bl_verify']
        )
        # columns
    ...
    def corporate_bonds(self, wd_bef=1):
        ...
        # requesting secondary market prices
        df_br_tb = self.general_req(
            url, YAML_ANBIMA['sec_mkt_prcs']['corporate_bonds']['bl_verify']
        )
        # columns
        list_headers = list(df_br_tb.columns)
        # chaning data types
        df_br_tb = df_br_tb.astype(
            {
                ...
                list_headers[
                    12
                ]: float,  ## pv @ par / VNE (future value - fv - or valor nominal
                ##     de emissão, pt-br)
