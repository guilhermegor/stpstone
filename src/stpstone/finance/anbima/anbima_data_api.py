...
class AnbimaDataGen:
    ...
    def generic_request(self, str_app, str_method):
        ...
        resp_req = request(
            method=str_method, url=str_url, headers=dict_headers
        )
        # checking wheter response code is 2xx, otherwise raise error
        resp_req.raise_for_status()
        # returning token
...
class AnbimaDataFunds(AnbimaDataGen):
    ...
    def funds_trt(self, i_pg=0, i_fnd=0):
        ...
        while True:
            ...
            for dict_cnt in json_funds[
                YAML_ANBIMA['anbima_data_api']['key_content']
            ]:
                #   setting variables
                dict_aux = dict()
                i_fnd += 1
        ...
        df_funds.fillna(
            YAML_ANBIMA['anbima_data_api']['str_fill_na'], inplace=True
        )
            ...
            for key_cnt, data_cnt in json_fnd_info.items():
                ...
                if isinstance(data_cnt, str) or data_cnt is None:
                    ...
                    for col_ in list(df_.columns):
                        #   date
                        if (
                            StrHandler().match_string_like(col_, '*data_*')
                            == True
                        ) and (len(col_) == 10):
                            df_[col_].fillna(
                                YAML_ANBIMA['anbima_data_api'][
                        ...
                        #   timestamp
                            ...
                            and (
                                StrHandler().match_string_like(col_, '*T*')
                                == True
                            )
                            and (len(col_) > 10)
                        ):
    ...
    @property
    def institutions(
        self, str_app='feed/fundos/v2/fundos/instituicoes', str_method='GET'
    ):
        """
