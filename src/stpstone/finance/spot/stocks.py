...
class ValuingStocks:
        ...
        if type_return == 'ln_return':
            return [
                ValuingStocks().continuous_return(
                    list_prices[i - 1], list_prices[i]
                )
                for i in range(1, len(list_prices))
            ]
        elif type_return == 'stnd_return':
            return [
                ValuingStocks().stnd_return(list_prices[i - 1], list_prices[i])
                for i in range(1, len(list_prices))
            ]
        else:
            raise Exception(
    ...
    def pandas_returns_from_spot_prices(
        ...
        df_[col_first_occurrence_ticker] = np.where(
            df_[col_dt_date] == np.min(df_[col_dt_date]), 'OK', 'NOK'
        )
        ...
        if type_return == 'ln_return':
            df_[col_stock_returns] = df_.apply(
                ...
                if row[col_first_occurrence_ticker] == 'NOK'
                else 0,
                axis=1,
            )
        else:
    ...
    def short_fee_cost(
        self, fee_short, nper_cd, short_price, quantities, year_cd=360
    ):
        """
        ...
        """
        return (
            FinancialMath().compound_interest(fee_short, nper_cd, year_cd)
            * short_price
            * quantities
        )
    ...
    def pricing_strategy(
        ...
        if type_return == 'ln_return':
            return {
                'mtm': (float(short_price) - float(long_price))
                * float(leverage)
                - float(operational_costs),
                'pct_retun': ValuingStocks().continuous_return(
                    float(short_price), float(long_price)
                ),
