...
class Sidra:
    ...
    def rl_dts_indicators(self, str_pool_name=None):
        ...
        for el in [el.get_text() for el in bs_html.find_all('p')]:
            if (
                StrHandler().match_string_like(
                    el,
                    '*'
                    + YAML_BR_MACRO['sidra']['st_indicators'][
                        'pool_names_like'
                    ]
                    + '*',
                )
                ...
                str_pool_name = StrHandler().get_string_until_substr(
                    el,
                    YAML_BR_MACRO['sidra']['st_indicators']['pool_names_like'],
                )
            elif (
                (str_pool_name != None)
                and len(el) > 1
        ...
        # id duplicates
        ds_id_dup = df_rl_dts_macro.duplicated(
            subset=[YAML_BR_MACRO['sidra']['st_indicators']['col_pool_name']],
            keep='last',
        )
    ...
    def ipca_in_index_number(self, aggregate='1737', variable='2266'):
        ...
        OUTPUTS: DICTIONARY (CLOSING MONTH AND INDICE NUMBER KEYS)
        """
        return self.sidra_variables(aggregate, variable)[0]['resultados'][0][
            'series'
        ][0]['serie']
...
class BCB:
    ...
    def sgs_bcb(self, int_sgs_tabel, data_inic, data_fim):
        ...
        url = 'http://api.bcb.gov.br/dados/serie/bcdata.sgs.{}/dados'.format(
            int_sgs_tabel
        )
        query_params = {
            'formato': 'json',
    ...
    def usd_brl(
        self,
        data_ref,
        formato_data_saida='%m/%d/%Y',
        int_dol_usd_security_id=9800508,
        key_dol_usd_security_id='securityIdentificationCode',
        key_value='value',
        key_last_update='lastUpdate',
        int_precision_decimals=4,
        ...
        if DatesBR().check_date_datetime_format(data_ref) == True:
            data_ref = DatesBR().datetime_to_string(
                data_ref, formato_data_saida
            )
        else:
            data_ref = DatesBR().datetime_to_string(
                DatesBR().str_date_to_datetime(data_ref), formato_data_saida
            )
        ...
        try:
            ...
            request_message = request(
                'GET', url, headers=headers, data=dict_payload
            ).content
            request_message = HandlingObjects().literal_eval_data(
                request_message, "b'", "'"
            )
            return {
                'cotacao_compra': request_message['value'][0]['cotacaoCompra'],
                'cotacao_venda': request_message['value'][0]['cotacaoVenda'],
                'timestamp_cotacao': request_message['value'][0][
                    'dataHoraCotacao'
                ],
            }
        except:
            ...
            dict_usd_brl = [
                dict_
                for dict_ in json_ref_taxes_b3
                if int(dict_[key_dol_usd_security_id])
                == int(int_dol_usd_security_id)
            ][0]
            # return the contingency dolbrl exchange
            return {
                'cotacao_compra': NumHandler().convert_thousands_decimals_separator(
                    dict_usd_brl[key_value],
                    precision_decimals=int_precision_decimals,
                ),
    ...
    def foreign_exchange_bcb(
        self,
        int_working_days_before_end=1,
        int_working_days_before_begin=None,
        ...
        #   (0 as default)
        datetime_end_ref = DatesBR().sub_working_days(
            DatesBR().curr_date, int_working_days_before_end
        )
        str_datetime_end_ref = datetime_end_ref.strftime('%m-%d-%Y')
        if int_working_days_before_begin != None:
            datetime_begin_ref = DatesBR().sub_working_days(
                DatesBR().curr_date, int_working_days_before_end
            )
            str_datetime_begin_ref = datetime_begin_ref.strftime('%m-%d-%Y')
        # header
        dict_header = {'accept': 'application/json;odata.metadata=minimal'}
        # payload for currencies available consulting
        dict_payload = {'format': 'json'}
        dict_payload = JsonFiles().dict_to_json(dict_payload)
        # retriving data
        resp_req = request(
            method,
            url.format(api_currencies),
            headers=dict_header,
            data=dict_payload,
        )
        # raises exception when not a 2xx response
        ...
        list_currencies = [
            str(dict_[key_simbolo]) for dict_ in list_ser_currencies
        ]
        ...
        for currency in list_currencies:
            ...
            if int_working_days_before_begin == None:
                ...
                resp_req = request(
                    method,
                    url.format(api_currecy_exchange_day.format(str_payload)),
                    headers=dict_header,
                )
            else:
                #   payload for currencies available consulting
                ...
                dict_params = {
                    ...
                    'dataFinalCotacao': "'{}'".format(
                        str(str_datetime_end_ref)
                    ),
                }
                #   coverting to string payload and params
                str_payload = (
                ...
                resp_req = request(
                    method,
                    url.format(
                        api_curency_exchange_period.format(str_payload)
                    ),
                    headers=dict_header,
                )
            #   raises exception when not a 2xx response
            ...
            dict_export[
                str_currency.format(currency)
            ] = json_currency_exchange[key_value]
        # retrieving foreign exchanges
        return dict_export

    @property
    def market_macro_expec(
        self,
        url='https://olinda.bcb.gov.br/olinda/servico/Expectativas/versao/v1/odata/ExpectativasMercadoAnuais?$top=100000&$orderby=Data%20desc&$format=json&$select=Indicador,IndicadorDetalhe,Data,DataReferencia,Media,Mediana,Minimo,Maximo,numeroRespondentes',
        col_indicator='Indicador',
        col_detailed_indicator='IndicadorDetalhe',
        col_date='Data',
        col_ref_year='DataReferencia',
        col_avg='Media',
        col_median='Mediana',
        col_min='Minimo',
        col_max='Maximo',
        col_num_answ='numeroRespondentes',
    ):
        """
...
class YFinanceMacroBR:
    ...
    def ipca_forecast(
        self,
        method='GET',
        url='https://sbcharts.investing.com/events_charts/eu/1165.json',
        ...
        json_brazillian_cpi = [
            {
                'datetime': DatesBR().timestamp_float_to_datetime(
                    int(
                        int(dict_['timestamp'])
                        / int_convert_miliseconds_seconds
                    ),
                    bl_format=True,
                ),
    ...
    def yforex(self, list_xcg_curr, wd_start_date=2, wd_end_date=0):
        ...
        inf_date = (
            DatesBR()
            .sub_working_days(DatesBR().curr_date, wd_start_date)
            .strftime('%Y-%m-%d')
        )
        sup_date = (
            DatesBR()
            .sub_working_days(DatesBR().curr_date, wd_end_date)
            .strftime('%Y-%m-%d')
        )
        ...
        for xcg_curr in list_xcg_curr:
            ...
            df_ = yf.download(
                ticker, start=inf_date, end=sup_date, progress=False
            )
            ...
            df_[
                ...
                name=YAML_BR_MACRO['exchange_rates_yahoo']['col_date']
            )
        ...
        # renaming columns
        df_xcg = df_xcg.rename(columns={x: x.upper() for x in df_xcg.columns})
        # sort dataframe
        df_xcg.sort_values(
            [
                YAML_BR_MACRO['exchange_rates_yahoo']['col_currency'],
