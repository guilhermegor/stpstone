...
class USAMacro:
    def fred_request(
        ...
        message_content = request(
            'GET', url_root.format(data_source), params=query_params
        ).content
    ...
    def non_farm_payroll(self):
        ...
        # requesting response
        resp_req = request(
            YAML_USA_MACRO['non_farm_payroll']['req_method'],
        ...
        # headers
        list_th_0 = [
            td.get_text().strip().upper() for td in bs_html.find_all('th')
        ][:7]
        # data
        list_td1 = [
            td.get_text().strip()
            for td in bs_html.find_all(
                'td', class_='datatable-item datatable-item-positive'
            )
        ]
        list_td2 = [
            td.get_text().strip()
        ...
        list_ser = HandlingDicts().pair_headers_with_data(list_th_0, list_td)
        ...
        for i in range(1, 3):
            ...
            if i == 1:
                ...
                raise Exception(
                    'Table of payroll not supported for data treatment, '
        ...
        df_payrolls = df_payrolls[
            [
                c
                for c in list(df_payrolls.columns)
                if (c not in list_th_0) and (c not in list_th_2)
            ]
        ]
        df_usa_labour_data = pd.DataFrame(list_ser_2)
        df_usa_labour_data = df_usa_labour_data[
            [
                c
                for c in list(df_usa_labour_data.columns)
                if (c not in list_th_1) and (c not in list_th_0)
            ]
        ]
        # looping through dataframes
        for i, df_ in enumerate([df_payrolls, df_usa_labour_data]):
            ...
            df_.rename(
                columns={
                    x: StrHandler().get_string_until_substr(x, str(i + 1))
                    for x in list(df_.columns)
                },
                inplace=True,
            )
            #   adding logging
