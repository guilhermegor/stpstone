...
class WGBD:
    ...
    def bonds_10y_rates_rating(self):
        ...
        str_last_update = (
            StrHandler()
            .get_string_after_substr(
                bs_html.find('article')
                .find(
            ...
            for bs_tr in bs_table.find('tbody').find_all('tr'):
                ...
                if len(list_th) > len(list_td):
                    if StrHandler().bl_has_numbers(list_td[1]) == True:
                        ...
                        raise Exception(
                            'Error - data missing in tbody unable to replace, please '
                            + 'validate the code'
                        )
    ...
    def inv_yc_economies(self):
        ...
        str_last_update = (
            StrHandler()
            .get_string_after_substr(
                bs_html.find('article').find('p', class_='-f14').get_text(),
            ...
            #   dealing with first table
                ...
                list_td = [
                    ...
                    * 1e-4
                    ...
                    else el.get_text().replace('\n', '').strip()
                    for el in bs_table.find('tbody').find_all('td')
                    if len(el.get_text().replace('\n', '').strip()) > 0
                ]
                #   creating list of dictionaries
                list_ser = HandlingDicts().pair_headers_with_data(
                    list_th, list_td
                ...
                raise Exception(
                    'Table not identified, of inverted yield curve countries, please '
                    + 'revisit the code'
                )
        #   adding logging columns
        for df_ in [df_inv_yc_ec, df_inv_yc_spreads]:
            #   adding logging
    ...
    def yield_spreads_nations(self):
        ...
        str_last_update = (
            ...
            .get_string_after_substr(
                [
                    el.get_text()
                    for el in bs_html.find('article').find_all('p')
                    if StrHandler().match_string_like(
                        el.get_text(),
                        YAML_WGBD['yield_spreads_nations'][
                            'data_last_update_like'
                        ],
                    )
                ][0],
        ...
        # getting headers
        list_th1 = [
            el.get_text().replace(' ', '_').strip().upper()
            for el in bs_html.find('table')
            .find('thead')
            .find_all('tr')[0]
            .find_all('th')
            if len(el.get_text()) > 0
        ...
        list_th.extend([list_th1[0] + '_' + x for x in list_th2[2:]])
        # getting data
        for el in bs_html.find('table').find('tbody').find_all('td'):
            ...
            #   collecting bps data
                ...
                list_td.append(
                    float(
                        ...
                        .replace('\n', '')
                        .replace(
                            YAML_WGBD['yield_spreads_nations']['data_bp'], ''
                        )
                        .strip()
                    )
                    * 1e-4
            ...
            #   collecting 10y data
                ...
                list_td.append(
                    float(
                        ...
                        .replace('\n', '')
                        .replace(
                            YAML_WGBD['yield_spreads_nations']['data_pct'], ''
                        )
                        .strip()
                    )
                    * 1e-2
            ...
            #   collecting other data, such as country name
                ...
                list_td.append(el.get_text().replace('\n', '').strip())
        # creating list of dictionaries
    ...
    def ratings(self):
        ...
        for el in bs_html.find('table').find('tbody').find_all('td'):
            ...
            list_td.append(el.get_text().strip())
            #   getting info regarding outlook
            try:
                list_td.append(el.find('i')['title'].replace(' ', '_').upper())
            except (AttributeError, TypeError):
                if bl_country == False:
                    list_td.append('NEUTRAL_OUTLOOK')
                else:
                    bl_country = False
        # creating list of dictionaries
        list_ser = HandlingDicts().pair_headers_with_data(list_th, list_td)
