...
class InitialSettings:
    def set_parameters(self, *params, opt_type='call'):
        ...
        # initial parameters
        list_params = [
            param[0] if isinstance(param, list) == True else param
            for param in params
        ]
        # return parameters
        return [
...
class BlackScholesMerton(InitialSettings):
    ...
    def d1(self, s, k, b, t, sigma, q):
        ...
        return (np.log(s / k) + (b + sigma**2 / 2) * t) / (
            sigma * np.sqrt(t)
        )
    ...
    def general_opt_price(self, s, k, r, t, sigma, q, b, opt_type):
        ...
        # return option price
        if opt_type == 'call':
            return s * np.exp((b - r) * t) * NormalDistribution().cdf(
                self.d1(s, k, b, t, sigma, q)
            ) - k * np.exp(-r * t) * NormalDistribution().cdf(
                BlackScholesMerton().d2(s, k, b, t, sigma, q)
            )
        elif opt_type == 'put':
            return k * np.exp(-r * t) * NormalDistribution().cdf(
                -self.d2(s, k, b, t, sigma, q)
            ) - s * np.exp((b - r) * t) * NormalDistribution().cdf(
                -self.d1(s, k, b, t, sigma, q)
            )
...
class Greeks(BlackScholesMerton):
    ...
    def delta(self, s, k, r, t, sigma, q, b, opt_type):
        ...
        if opt_type == 'call':
            ...
            return np.exp((b - r) * t) * (
                NormalDistribution().cdf(self.d1(s, k, b, t, sigma, q)) - 1
            )
    ...
    def strike_from_delta(self, s, r, t, sigma, q, b, delta, opt_type):
        ...
        if opt_type == 'call':
            return s * np.exp(
                -NormalDistribution().inv_cdf(delta * np.exp((r - b) * t))
                * sigma
                * t**0.5
                + (b + sigma**2 / 2.0) * t
            )
    ...
    def gamma(self, s, k, r, t, sigma, q, b):
        ...
        # return greek
        return (
            NormalDistribution().pdf(self.d1(s, k, b, t, sigma, q))
            * np.exp((b - r) * t)
            / (s * sigma * t**0.5)
        )
    ...
    def time_to_maturity_maximum_vega(self, s, k, r, sigma, b):
        ...
        return (
            ...
            * (
                1.0
                + (1.0 + (8.0 * r / sigma**2 + 1.0) * np.log(s / k) ** 2)
                ** 0.5
            )
            / (8.0 * r + sigma**2)
        )
    ...
    def vega_delta_relationship(self, s, k, r, t, sigma, q, b, opt_type):
        ...
        return (
            ...
            * NormalDistribution().pdf(
                NormalDistribution().inv_cdf(
                    np.exp((r - b) * t)
                    * np.abs(self.delta(s, k, r, t, sigma, q, b, opt_type))
                )
            )
            / (s * sigma * t**0.5)
        )
    ...
    def vanna_vol(self, s, k, r, t, sigma, q, b, opt_type):
        ...
        # return greek
        return (
            self.vanna(s, k, r, t, sigma, q, b, opt_type)
            * (1.0 / sigma)
            * (
    ...
    def color(self, s, k, r, t, sigma, q, b):
        ...
        s, k, r, t, sigma, q, b = self.set_parameters(s, k, r, t, sigma, q, b)
        # return greek
        return -self.gamma(s, k, r, t, sigma, q, b) * (
            r
            - b
    ...
    def vomma_positive_outside_interval(
        self, s_k, r, t, sigma, q, b, bl_spot=True
    ):
        """
        REFERENCES: THE COMPLETE GUIDE TO OPTION PRICING FORMULAS - ESPEN GAARDER HAUG
    ...
    def d_vega_d_time(self, s, k, r, t, sigma, q, b):
        ...
        s, k, r, t, sigma, q, b = self.set_parameters(s, k, r, t, sigma, q, b)
        # return greek
        return self.vega(s, k, r, t, sigma, q, b) * (
            r
            - b
            + b * self.d1(s, k, b, t, sigma, q) / (sigma * t**0.5)
            - (
        ...
        return (
            ...
            * (
                ...
                * (
                ...
                - (
                    self.d1(s, k, b, t, sigma, q) ** 2
                    + self.d2(s, k, b, t, sigma, q) ** 2
                )
            )
        )
    ...
    def dbsm_dohm(self, s, k, r, t, sigma, q, b):
        ...
        s, k, r, t, sigma, q, b = self.set_parameters(s, k, r, t, sigma, q, b)
        # return greek
        return s * NormalDistribution().pdf(
    ...
    def bleed_offset_volatility(self, s, k, r, t, sigma, q, b):
        ...
        s, k, r, t, sigma, q, b = self.set_parameters(s, k, r, t, sigma, q, b)
        # return greek
        return self.theta(s, k, r, t, sigma, q, b) / self.vega(
            s, k, r, t, sigma, q, b
        )
    ...
    def theta_gamma_relationship_driftless(self, s, k, r, t, sigma, q, b):
        ...
        return (
            -2.0
            * self.driftless_theta(s, k, r, t, sigma, q, b)
            / (s**2 * sigma**2)
        )
    ...
    def strike_given_risk_neutral_prob(
        self, s, k, r, t, sigma, q, b, p, opt_type
    ):
        """
        REFERENCES: THE COMPLETE GUIDE TO OPTION PRICING FORMULAS - ESPEN GAARDER HAUG
        DOCSTRING:
        ...
        if opt_type == 'call':
            return s * np.exp(
                -NormalDistribution().inv_cdf(p) * sigma * t**0.5
                + (b - sigma**2 / 2.0) * t
            )
        elif opt_type == 'call':
            return s * np.exp(
    ...
    def d_zeta_d_vol(self, s, k, r, t, sigma, q, b, opt_type):
        ...
        if opt_type == 'call':
            return (
                -NormalDistribution().pdf(self.d2(s, k, b, t, sigma, q))
                * self.d1(s, k, b, t, sigma, q)
                / sigma
            )
        elif opt_type == 'put':
    ...
    def risk_neutral_probability_density(
        self, s, k, r, t, sigma, q, b, opt_type
    ):
        """
        REFERENCES: THE COMPLETE GUIDE TO OPTION PRICING FORMULAS - ESPEN GAARDER HAUG
    ...
    def probability_ever_getting_itm(self, s, k, r, t, sigma, q, b, opt_type):
        ...
        if opt_type == 'call':
            return (k / s) ** (mu + lambda_) * NormalDistribution().cdf(-z) + (
                k / s
            ) ** (mu - lambda_) * NormalDistribution().cdf(
...
class IterativeMethods(Greeks):
    def binomial_pricing_model(
        self, s, k, r, t, n, u, d, opt_type, h_upper=None, h_lower=None
    ):
        """
        REFERENCES:
        ...
        array_s_nodes = (
            s * d ** (np.arange(n, -1, -1)) * u ** (np.arange(0, n + 1, 1))
        )
    ...
    def eqp_method(self, s, k, r, t, n, sigma, opt_type):
        ...
        dt = t / n
        nu = r - 0.5 * sigma**2
        dxu = 0.5 * nu * dt + 0.5 * np.sqrt(
            4 * sigma**2 * dt - 3 * nu**2 * dt**2
        )
        dxd = 1.5 * nu * dt - 0.5 * np.sqrt(
            4 * sigma**2 * dt - 3 * nu**2 * dt**2
        )
        pu = 0.5
        pd = 1 - pu
...
class EuropeanOptions(IterativeMethods):
    ...
    def implied_volatility(
        ...
        if method == 'newton_raphson':
            ...
            while epsilon > tolerance:
                ...
                dif_calc_market = (
                    self.general_opt_price(s, k, r, t, imp_vol, q, b, opt_type)
                    - cp0
                )
                ...
                try:
                    imp_vol = (
                        -dif_calc_market / self.vega(s, k, r, t, imp_vol, q, b)
                        + imp_vol
                    )
            ...
            while (float_high - float_low) > epsilon:
                if (
                    self.general_opt_price(
                        ...
                        r,
                        t,
                        float(float_high + float_low) / 2.0,
                        q,
                        b,
                        opt_type,
                    )
    ...
    def moneyness(self, s, k, r, t, sigma, q):
        ...
        # returning moneyness
        return (
            self.d1(s, k, r, t, sigma, q) + self.d2(s, k, r, t, sigma, q)
        ) / 2
    ...
    def iaotm(self, s, k, r, t, sigma, opt_type, pct_moneyness_atm=0.05):
        ...
        if abs(self.moneyness(s, k, r, t, sigma)) < pct_moneyness_atm:
            ...
            self.moneyness(s, k, r, t, sigma) < pct_moneyness_atm
            and opt_type == 'call'
        ) or (
            self.moneyness(s, k, r, t, sigma) > pct_moneyness_atm
            and opt_type == 'put'
