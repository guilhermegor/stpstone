...
class MarketRiskManagement:
    ...
    def parametric_var(
        ...
        if ro:
            ...
            h_corrected = h + 2 * ro * (1 - ro) ** (-2) * (
                (h - 1) * (1 - ro) - ro * (1 - ro ** (h - 1))
            )
        else:
            h_corrected = t / h
        # returning result of parametric var
        return {
            'financial_var': port_financial_value
            * (
                std_deviation
                * np.sqrt(h_corrected)
                * NormalDistribution().inv_norm(confidence_level)
                - (t / h) * float_mu
            ),
            'percentual_var': std_deviation
            * np.sqrt(h_corrected)
            * NormalDistribution().inv_norm(confidence_level)
            - (t / h) * float_mu,
            'drifted_adjustment_to_var': std_deviation
            * np.sqrt(h_corrected)
            * NormalDistribution().inv_norm(confidence_level)
            - (
                std_deviation
    ...
    def equity_var(
        ...
        beta_portfolio = NumHandler().sumproduct(
            list_betas, list_financial_exposures
    ...
    def normal_linear_interest_rate_var(
        self,
        confidence_level,
        array_pv01,
        array_yields,
        ...
        for i in range(len(array_yields)):
            for j in range(len(array_yields)):
                if i == j:
                    ...
                    cov_mtx[i, j] = (
                        correl_yields * array_yields[i] * array_yields[j]
                    )
        cov_mtx = (t / h) * cov_mtx
        # float_mu vector
        ...
        return NormalDistribution().inv_norm(confidence_level) * np.sqrt(
            LinearAlgebra().matrix_multiplication(
                LinearAlgebra().matrix_multiplication(array_pv01, cov_mtx),
                LinearAlgebra().transpose_matrix(array_pv01),
            )
        ) + LinearAlgebra().matrix_multiplication(
            float_mu, LinearAlgebra().transpose_matrix(array_pv01)
        )
    ...
    def expected_tail_loss(
        ...
        if any(
            ...
            raise Exception(
                'Please revisit original prices and returns arguments; only one '
                'of them ought be different from none'
            )
        # defining returns
        if list_prices:
            list_cacl_returns = ValuingStocks().calc_returns_from_prices(
                list_prices
            )
        elif list_returns:
        ...
        # nth lowest returns
        return {
            'percentage_etl': np.average(
                HandlingLists().nth_smallest_numbers(
                    list_cacl_returns, nth_lowest
                )
            ),
            'financial_etl': np.average(
                HandlingLists().nth_smallest_numbers(
                    list_cacl_returns, nth_lowest
                )
            )
            * port_financial_value,
        }

    def drawdown(
        self,
        tau,
        list_original_prices=None,
        list_returns=None,
        prices_base=100,
    ):
        """
        ...
        if list_returns:
            values = MarketRiskManagement().prices_base_normalizer(
    ...
    def max_drawdown(
        self, list_original_prices=None, list_returns=None, prices_base=100
    ):
        """
        ...
        for i in range(len_observations_data):
            drawdown_i = MarketRiskManagement().drawdown(
                i, list_original_prices, list_returns, prices_base
            )
    ...
    def ewma(self, list_daily_returns, int_wdy=252, accuracy=0.01):
        ...
        if accuracy in [0.01, 0.001, 0.0001]:
            if len(list_daily_returns) > curr_accuracy_list[-1]:
                ...
                lamba_smoothing_parameter = list_lambdas[
                    curr_accuracy_list.index(
                        HandlingLists().closest_bound(
                            curr_accuracy_list, len(list_daily_returns)
                        )
                    )
                ]
        else:
            return (
                'Poor defined variable accuracy, please make sure its within '
                + '[0.01, 0.001, 0.0001] possible values'
            )
        # periodic returns
        u_ln_returns = [
            ValuingStocks().continuous_return(
                list_daily_returns[p - 1], list_daily_returns[p]
            )
            for p in range(1, len(list_daily_returns))
        ]
        # array_weights
        array_weights = [
            (1 - lamba_smoothing_parameter)
            * lamba_smoothing_parameter ** (d - 1)
            for d in range(1, len(list_daily_returns))
        ]
        array_weights.reverse()
        # ewma daily contributions
        if len(u_ln_returns) == len(array_weights):
            ewma_daily_contributions_list = [
                array_weights[i] * u_ln_returns[i] ** 2
                for i in range(len(u_ln_returns))
            ]
        else:
            return (
                'Please check wheter daily returns and array_weights list are the same size and '
                'try again.'
            )
        ...
        dict_message = {
            ...
            'variance_ewma_yearly': (
                math.sqrt(sum(ewma_daily_contributions_list))
                * math.sqrt(int_wdy)
            )
            ** 2,
            'std_ewma_yearly': math.sqrt(sum(ewma_daily_contributions_list))
            * math.sqrt(int_wdy),
        }
        return JsonFiles().send_json(dict_message)

    def systematic_specific_risk(
        self,
        variance_portfolio,
        std_market,
        array_stocks_weights,
        array_stocks_beta,
    ):
        """
        ...
        """
        beta_portfolio = LinearAlgebra().matrix_multiplication(
            np.array(array_stocks_weights),
            LinearAlgebra().transpose_matrix(np.array(array_stocks_beta)),
        )
        systematic_risk = math.pow(beta_portfolio, 2) * math.pow(std_market, 2)
...
class MarkowitzEff:
    ...
    def __init__(
        ...
        self.df_mktdata[self.col_max_date] = self.df_mktdata.groupby(
            [self.col_ticker]
        )[self.col_dt].transform('max')
        # minimum w per asset
        if self.col_min_w not in self.df_mktdata.columns:
            self.df_mktdata[self.col_min_w] = (
                self.df_mktdata.groupby([self.col_ticker, self.col_max_date])[
                    col_close
                ].transform('last')
        ...
        self.array_close = np.array(
            [
                self.df_mktdata[
                    self.df_mktdata[self.col_ticker]
                    == ticker.replace('.SA', '')
    ...
    def sharpe_ratio(
        self, float_mu: float, float_sigma: float, float_rf: float
    ) -> float:
        """
        DOCSTRING:
        INPUTS:
    ...
    def sigma_portfolio(
        self, array_weights: np.array, array_returns: np.array
    ) -> float:
        """
        DOCSTRING:
        INPUTS:
        ...
        return np.sqrt(
            np.dot(array_weights.T, np.dot(array_cov, array_weights))
        )
    ...
    def returns_min_w_uids(
        ...
        array_returns = df_assets.pivot_table(
            index=col_dt, columns=col_id, values=col_returns
        ).to_numpy()
        ...
        array_min_w = np.array(
            df_assets.groupby(col_id)[col_min_w].unique(), dtype=float
        )
        array_min_w = array_min_w.T
        # list of uids
        list_uids = HandlingLists().remove_duplicates(
            df_assets[col_id].to_list()
        )
    ...
    def random_weights(
        ...
        if bl_constraints == True:
            ...
            if array_min_w is None:
                raise ValueError(
                    'MIN_INVEST_PER_ASSET MUST BE PROVIDED AS A LIST WHEN '
                    + 'CONSTRAINTS ARE ENABLED.'
                )
            if any(isinstance(x, str) for x in array_min_w):
                raise ValueError(
                    'MIN_INVEST_PER_ASSET MUST BE A LIST OF NUMBERS.'
                )
            if len(array_min_w) != int_n_assets:
                raise ValueError(
                    'THE LENGTH OF MIN_INVEST_PER_ASSET MUST MATCH THE '
                    + 'NUMBER OF ASSETS.'
                )
            ...
            while not bl_valid_weights:
                ...
                if (i_attempts >= nth_try) or (
                    all([sum(comb) >= 1.0 for comb in list_combs])
                ):
                ...
                if (bl_opt_possb_comb == False) or (
                    np.count_nonzero(array_w) == 1
                ):
    ...
    def random_portfolio(
        ...
        array_sigmas = self.sigma_portfolio(array_weights, array_r) * np.sqrt(
            int_wdy
        )
    ...
    def optimal_portfolios(
        ...
        mus = [
            10.0 ** (5.0 * float(t / n_attempts) - 1.0)
        ...
        list_portfolios = [
            opt.solvers.qp(float_mu * S, -pbar, G, h, A, b)['x']
            for float_mu in mus
        ]
        # calculating risk and return for efficient frontier
        array_returns = [
            opt.blas.dot(pbar, x) * int_wdy for x in list_portfolios
        ]
        array_sigmas = [
            np.sqrt(opt.blas.dot(x, S * x)) * np.sqrt(int_wdy)
            for x in list_portfolios
        ]
    ...
    def eff_frontier(
        ...
        for _, eff_risk in zip(array_eff_returns, array_eff_risks):
            while True:
                try:
                    ...
                    #       np.isclose
                    list_idx_sigma = np.where(
                        np.isclose(array_sigmas, eff_risk, atol=atol)
                    )
        ...
        df_porf = pd.DataFrame(
            {col_mu: array_mus, col_sigma: array_sigmas, col_w: array_weights}
        )
        # output the results
    ...
    def plot_risk_return_portfolio(self) -> None:
        ...
        if self.bl_debug_mode == True:
            ...
            print(
                'WEIGHTS: {}'.format(
                    self.array_weights[self.array_sharpes.argmax()]
                )
            )
            print(
                'RISK: {}'.format(
                    self.array_sigmas[self.array_sharpes.argmax()]
                )
            )
            print(
                'RETURN: {}'.format(
                    self.array_mus[self.array_sharpes.argmax()]
                )
            )
            print(
                'SHARPE: {}'.format(
                    self.array_sharpes[self.array_sharpes.argmax()]
                )
            )
        # prepare customdata for scatter plot
        customdata_portfolios = np.array(
            [
                [weights, ', '.join(self.list_securities)]
                for weights in self.array_weights
            ],
            dtype=object,
        )
        # Ppepare the subtitle with the list of securities
        subtitle_text = 'List of securities: ' + ', '.join(
    ...
    def max_sharpe(self) -> dict:
        ...
        if self.bl_non_zero_w_eff:
            array_valid_indices = np.where(
                (self.array_weights != 0).all(axis=1)
            )[0]
    ...
    def min_sigma(self) -> dict:
        ...
        if self.bl_non_zero_w_eff:
            ...
            int_argmin_risk = array_valid_indices[
                self.array_sigmas[array_valid_indices].argmin()
            ]
        else:
    ...
    def lot_shares_ticker_corr(
        self, dict_allocation: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        for ticker in dict_allocation['tickers']:
            float_qty_rnd = df_trad_sec[
                df_trad_sec['Symbol'] == ticker.replace('.SA', '')
            ]['MinOrderQty'].values[0]
            float_qty_quotient = (
                float(
                    dict_allocation['eff_quantities'][
                        self.list_securities.index(ticker)
                    ]
                )
                // float_qty_rnd
            )
            ...
            list_ser.append(
                {
                    ...
                    'qty': float_qty_quotient,
                }
            )
            list_ser.append(
                {
                    'ticker': ticker.replace('.SA', '') + 'F',
                    'qty': float_qty_remainder,
                }
            )
