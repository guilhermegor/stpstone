...
class FixedIncomeAppraisal:
    def __init__(
        ...
        delta_pv=None,
        delta_curve=None,
        annual_modified_duration=None,
        future_value=None,
        shift=0.0001,
        bl_exact_form_macaulay_dur=True,
        bl_exact_form_money_dur=True,
        bl_exact_form_convexity=True,
        bl_exact_form_pv_01=True,
        ...
        if self.bl_exact_form_macaulay_dur == False:
            mac_dur = (
                np.sum(
                    [
                        self.list_cfs[i]
                        * ((i + 1) / self.payments_per_year)
                        / np.power(1 + self.list_rates[i], i + 1)
                        for i in range(len(self.list_cfs))
                    ]
                )
                / self.present_value
            )
    ...
    def curve_durations(self):
        ...
        # returning durations
        return {'effective_duration': eff_dur}
    ...
    def money_duration(self):
        ...
        if self.bl_exact_form_money_dur == True:
            return (
                self.yield_durations['macaulay_duration'] * self.present_value
            )
        else:
        ...
        if self.bl_exact_form_convexity == True:
            return (
                np.sum(
                    [
                        self.list_cfs[i]
                        * ((i + 1) * (i + 2) / self.payments_per_year)
                        / np.power(1 + self.list_rates[i], i + 3)
                        for i in range(len(self.list_cfs))
                    ]
                )
                / self.future_value
            ...
            return (
                (self.pv_minus + self.pv_plus) - (2 * self.present_value)
    ...
    def dv01(self, ytm, nper, side='C', contract='DI1'):
        ...
        if contract == 'DI1':
            ...
            return FuturesPricingB3().pu_di1(
                ytm, nper, c_v_taxa=side
            ) + FuturesPricingB3().pu_di1(
                ytm + self.shift, nper, c_v_taxa=reverse_side
            )
        else:
    ...
    def approximate_delta_bond_price(self):
        ...
        if self.bl_consider_convexity_delta_pv == False:
            return (
                -self.money_duration(
                    self.annual_modified_duration, self.present_value
                )
                * self.delta_yield
            )
        else:
