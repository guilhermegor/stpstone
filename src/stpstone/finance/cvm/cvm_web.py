...
from stpstone.multithreading.mp_helper import mp_run_parallel, mp_worker
...
class CVMWeb_WS_Funds:
    ...
    def cookie_govbr(
        ...
        driver = cls_selenium.selenium_web_driver(
            url, webdriver_path, port, time_wait_page_load, bl_open_minimized
        )
        cls_selenium.selenium_element_is_clickable(
            driver, xpath_click_login, frame_name=str_frame_name
        )
        # click login button
        cls_selenium.selenium_find_element(driver, xpath_click_login).click()
        # fill TIN br - tax payer id number
        cls_selenium.selenium_wait_until_element_loaded(
            driver, xpath_fill_tin_br, time_wait_el
        )
        cls_selenium.selenium_find_element(
            driver, xpath_fill_tin_br
        ).send_keys(str_id)
        # fill password
        cls_selenium.selenium_wait_until_element_loaded(
            driver, xpath_fill_passw, time_wait_el
        )
        cls_selenium.selenium_find_element(driver, xpath_fill_passw).send_keys(
            str_passw
        )
        # get cookie
    ...
    def generic_req(
        ...
        tup_timeout: Tuple[float, float] = (12.0, 21.0),
    ) -> html.HtmlElement:
        """
        DOCSTRING:
        INPUTS:
        ...
        dict_headers = {
            ...
            'Referer': 'https://cvmweb.cvm.gov.br/SWB/Sistemas/SCW/CReservd/{}'.format(
                str_header_ref
            ),
    ...
    def available_funds(
        ...
        for el_option_fund in HtmlHndler().html_lxml_xpath(
            html_content, str_xpath_funds
        ):
            #   get text and split into fund's ein and name
            try:
                str_option_fund_raw = HtmlHndler().html_lxml_xpath(
                    el_option_fund, './text()'
                )
                ...
                str_option_fund_trt = StrHandler().remove_diacritics(
                    str_option_fund_raw
                )
                str_option_fund_trt = StrHandler().replace_all(
                    str_option_fund_trt,
                    {
                        ...
                        '- FUNDO DE INVESTIMENTO EM COTAS DE FUNDOS DE INVESTIMENTO': 'FUNDO DE INVESTIMENTO EM COTAS DE FUNDOS DE INVESTIMENTO',
                        '- FGTS ': 'FGTS ',
                        '- CRED PRIV': 'CRED PRIV',
            ...
            list_ser.append(
                {
                    self.key_fund_code: HtmlHndler().html_lxml_xpath(
                        el_option_fund, str_xpath_value
                    )[0],
        ...
        df_funds[self.key_fund_ein_unm] = DocumentsNumbersBR(
            df_funds[self.key_fund_ein].tolist()
        ).unmask_docs
    ...
    def available_dates_report_fund(
        ...
        list_dts = [
            el_
            for el_ in HtmlHndler().html_lxml_xpath(
                html_content, xpath_avl_dates
            )
            if el_ != ''
        ]
        ...
        list_avl_dts_fund = [
            {
                ...
                self.key_ref_date: DatesBR().str_date_to_datetime(
                    d, str_dt_fmt
                ),
        ...
        if (self.cls_db is not None) and (len(list_avl_dts_fund) > 0):
            self.cls_db._insert(
                list_avl_dts_fund,
                str_table_nane,
                bl_insert_or_ignore=self.bl_insert_or_ignore,
    ...
    def fund_daily_reports_raw(
        ...
        for str_dt in list_str_dts:
            ...
            dict_data = {
                ...
                el_eventargument: '',
                el_lastfocus: '',
                el_viewstate: HtmlHndler().html_lxml_xpath(
                    html_content_gen,
                    str_xpath_event_el_like.format(el_viewstate),
                ),
                el_eventvalidation: HtmlHndler().html_lxml_xpath(
                    html_content_gen,
                    str_xpath_event_el_like.format(el_eventvalidation),
                ),
                el_viewstategen: HtmlHndler().html_lxml_xpath(
                    html_content_gen,
                    str_xpath_event_el_like.format(el_viewstategen),
                ),
    ...
    def fund_daily_report_trt(
        ...
        # raise Exception('BREAK')
        if (len(list_str_dts) == 0) or (
            len(
                HtmlHndler().html_lxml_xpath(
                    html_content_gen, str_xpath_dropdown_box
                )
            )
            == 0
        ):
        ...
        dict_html_contents_dts = self.fund_daily_reports_raw(
            html_content_gen, str_fund_code, list_str_dts
        )
        ...
        for str_dt, html_content_dt in dict_html_contents_dts.items():
            ...
            dict_g_shareholders = {
                self.fstr_greatest_shareholders.format(i): el_ein[0]
                if el_ein is not None
                else ''
                for i, el_ein in enumerate(
                    HtmlHndler().html_lxml_xpath(
                        html_content_dt, str_xpath_greatest_shareholders
                    )
                )
            }
            #   daily infos
            try:
                dict_daily_infos = {
                    self.__dict__[f'key_{k}']: str(
                        HtmlHndler().html_lxml_xpath(html_content_dt, v)[0]
                    ).replace(',', '.')
                    for k, v in dict_xpaths.items()
                }
            except IndexError:
                raise Exception(
                    f'ERROR HTML XML XPATH - URL: {url_daily_report_fund}'
                )
            dict_daily_infos[self.key_fund_code] = str_fund_code
            dict_daily_infos[
                self.key_fund_daily_infos_url
            ] = url_daily_report_fund
            dict_daily_infos[
                self.key_ref_date
            ] = DatesBR().str_date_to_datetime(str_dt, str_fmt_date)
            #   appending to exporting list
            dict_ = HandlingDicts().merge_n_dicts(
                dict_daily_infos, dict_g_shareholders
            )
    ...
    def block_fund_fetch(
        ...
        html_content_gen, url_fund_daily_infos = self.fund_daily_report_gen(
            str_fund_code
        )
        # dates of daily report available
        list_str_dts_reports = self.available_dates_report_fund(
            html_content_gen, str_fund_code, url_fund_daily_infos
        )
        ...
        # backup in db, if is user's will
        if (self.cls_db is not None) and (len(list_ser) > 0):
            self.cls_db._insert(
                list_ser,
                str_table_nane,
                bl_insert_or_ignore=self.bl_insert_or_ignore,
    ...
    def funds_daily_reports_trt(
        ...
        for str_fund_code, list_dts in dict_dts_funds.items():
            for i, str_dt in enumerate(list_dts):
                if DatesBR().check_date_datetime_format(str_dt) == True:
                    ...
                    list_dts[i] = (
                        DatesBR()
                        .str_date_to_datetime(str_dt, format=str_dt_fmt_1)
                        .strftime(str_strftime_format)
                    )
        ...
        cols_greatest_shareholders = [
            ...
            if StrHandler().match_string_like(
                c, self.key_greatest_shareholders_like
            )
            == True
        ]
        df_funds_daily_reports[
            cols_greatest_shareholders
        ] = df_funds_daily_reports[cols_greatest_shareholders].astype(str)
