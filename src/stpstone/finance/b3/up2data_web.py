...
from stpstone.handling_data.lists import HandlingLists, StrHandler
...
class UP2DATAB3(metaclass=ValidateAllMethodsMeta):
    def __init__(
        ...
        req_session: Union[ReqSession, None] = None,
        ...
        self.dt_ref = dt_ref
        self.req_session = (
            ReqSession(bl_use_timer=bl_use_timer)
    ...
    def daily_trades_secondary_market(self) -> pd.DataFrame:
        ...
        file_daily_trades = DirFilesManagement().get_zip_from_web_in_memory(
            YAML_B3['up2data_b3']['url_daily_trades'].format(
        ...
        df_daily_trades.fillna(
            YAML_B3['up2data_b3']['value_nan'], inplace=True
        )
        ...
        df_daily_trades = DealingPd().change_dtypes(
            ...
            [
                YAML_B3['up2data_b3']['col_dt_ref'],
                YAML_B3['up2data_b3']['col_trd_dt'],
            ],
            errors='ignore',
        )
        # column asset
    ...
    def security_category_name(
        ...
        for security_division, col_ in [
            ...
            for market, list_source_names in YAML_B3['up2data_b3'][
                security_division
            ].items():
                ...
                if market not in dict_export:
                    dict_export[market] = HandlingLists().remove_duplicates(
                        df_[df_[col_].isin(list_source_names)][
                            key_ticker
                        ].tolist()
                    )
                else:
                    dict_export[market].extend(
                        HandlingLists().remove_duplicates(
                            df_[df_[col_].isin(list_source_names)][
                                key_ticker
                            ].tolist()
                        )
            ...
            return HandlingLists().remove_duplicates(
                df_[
                    ~df_[col_security_category_name].isin(
                        list_markets_classified
                    )
                ][col_security_category_name].tolist()
            )
        else:
    ...
    def loan_balace(
        self,
        wd_bef=1,
        url='https://arquivos.b3.com.br/tabelas/table/LoanBalance/{}/{}',
        method='GET',
        key_pg_count='pageCount',
        key_valores='values',
        list_cols_lb_b3=[
            'RptDt',
            'TckrSymb',
        ...
        data_ref_lb_b3 = (
            DatesBR()
            .sub_working_days_before(DatesBR().curr_date, wd_bef)
            .strftime('%Y-%m-%d')
        )
        # payload
        dict_payload = {}
        ...
        for i in range(1, int_pgs_lb_b3 + 1):
            ...
            for list_ in json_lb_b3[key_valores]:
                ...
                list_ser.append(dict(zip(list_cols_lb_b3, list_)))
        # builiding dataframe
        ...
        df_lb_b3[list_cols_lb_b3[0]] = [
            DatesBR().timestamp_separator_string_to_datetime(d)
    ...
    def lending_open_position(
        self,
        wd_bef=1,
        url='https://arquivos.b3.com.br/tabelas/table/LendingOpenPosition/{}/{}',
        method='GET',
        key_pg_count='pageCount',
        key_valores='values',
        list_cols_lop_b3=[
        ...
        for i in range(1, int_pgs_lb_b3 + 1):
            ...
            for list_ in json_lb_b3[key_valores]:
                #   appending to serialized list
                list_ser.append(dict(zip(list_cols_lop_b3, list_)))
        # builiding dataframe
    ...
    @property
    def bond_issuers_accepted_warranty_b3(
        self,
        url_banks_brazil='http://www.bcb.gov.br/pom/spb/estatistica/port/ParticipantesSTRport.csv',
        ...
        col_issuers='ISSUERS',
        col_ispb='ISPB',
        col_numero_cod='NUMERO_CODIGO',
        col_nome_extenso='NOME_EXTENSO',
        col_fr_total='FRTotal',
        ...
        # change column names
        df_banks_operating_domestically.rename(
            columns=dict(
                zip(
                    df_banks_operating_domestically.columns,
                    [
                        StrHandler().remove_diacritics(str(x).upper())
                        for x in df_banks_operating_domestically.columns
                    ],
                )
            ),
            inplace=True,
        ...
        df_banks_operating_domestically = (
            df_banks_operating_domestically.astype(
                {col_ispb: int, col_numero_cod: int}
            )
        )
        df_banks_operating_domestically = (
            df_banks_operating_domestically.astype(
        ...
        df_banks_operating_domestically[col_nome_extenso] = [
            StrHandler().remove_diacritics(
                StrHandler()
                .replace_all(str(x).upper(), dict_replace_str_name)
                .strip()
            )
        ...
        list_unziped_files = DirFilesManagement().get_zip_from_web_in_memory(
            url_bond_issuers_warranty_b3, bl_io_interpreting=bl_io_interpreting
        )
        ...
        list_banks_warranty_b3 = [
            str(x).upper() for x in list_banks_warranty_b3
        ]
        # creating dataframe with banks accepted as warranty in b3 clearing
        df_banks_accepted_warranty_b3 = pd.DataFrame(
            [{col_issuers: bank} for bank in list_banks_warranty_b3]
        )
        df_banks_accepted_warranty_b3.sort_values(
            [col_issuers], ascending=[True], inplace=True
        )
        # dealing with complete names from financial institutions
        df_banks_accepted_warranty_b3[col_issuers] = [
            StrHandler().remove_diacritics(
                StrHandler()
                .replace_all(str(x).upper(), dict_replace_str_name)
                .strip()
        ...
        df_banks_acc_warr = df_banks_operating_domestically[
            df_banks_operating_domestically[col_nome_extenso].isin(
                df_banks_accepted_warranty_b3[col_issuers]
            )
        ]
