...
class AlphaTools:
    def __init__(
        ...
        self.bl_debug_mode = (
            bl_debug_mode if bl_debug_mode is not None else True
        )
    ...
    def funds(self) -> pd.DataFrame:
        ...
        df_funds[
            YAML_INOA['alpha_tools']['funds']['col_origin']
        ] = self.str_instance
    ...
    def quotes(self, list_ids: List[int]) -> pd.DataFrame:
        ...
        dict_params = {
            ...
            YAML_INOA['alpha_tools']['quotes'][
                'key_start_dt'
            ]: self.dt_start.strftime('%Y-%m-%d'),
            YAML_INOA['alpha_tools']['quotes'][
                'key_end_dt'
            ]: self.dt_end.strftime('%Y-%m-%d'),
        }
        # requesting infos
        json_req = self.generic_req(
