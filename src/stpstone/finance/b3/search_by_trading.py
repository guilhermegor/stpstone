...
class TradingFilesB3:
    def __init__(
        ...
        int_wd_bef: int = 1,
        str_fillna: str = '-1',
        str_fillna_dt: str = '2100-12-31',
        str_fillna_ts: str = '2100-12-31 00:00:00',
    ...
    def price_report(self, bl_debug=False):
        ...
        url = YAML_B3['price_report']['url'].format(
            DatesBR()
            .sub_working_days(DatesBR().curr_date, self.int_wd_bef)
            .strftime('%y%m%d')
        )
        # mtm b3 on memry
        zipfile = DirFilesManagement().get_zip_from_web_in_memory(
            url,
            bl_io_interpreting=YAML_B3['price_report']['bl_io_interpreting'],
                    ...
                    for tag in value:
                        try:
                            ...
                            if (
                                ...
                                dict_[
                                    YAML_B3['price_report']['col_currency']
                                ] = tag_.attrs[
                                    YAML_B3['price_report']['attrb_currency']
                                ]
        ...
        df_pr = DBLogs().audit_log(
            ...
            DatesBR().utc_from_dt(
                DatesBR().sub_working_days(
                    DatesBR().curr_date, self.int_wd_bef
                )
            ),
        )
        # returning dataframe
    ...
    def tradable_securities(self):
        ...
        list_ser = list()
        # url
        url = YAML_B3['securities_tradable']['url'].format(
            DatesBR()
            .sub_working_days(DatesBR().curr_date, self.int_wd_bef)
            .strftime('%y%m%d')
        )
        ...
        zipfile = DirFilesManagement().get_zip_from_web_in_memory(
            url,
            bl_io_interpreting=YAML_B3['securities_tradable'][
                'bl_io_interpreting'
            ],
        ...
        for col_ in ['MaturityDate', 'IssueDate', 'SettlDate']:
            df_[col_] = [
                DatesBR().str_date_to_datetime(d, self.str_fmt_dt)
                for d in df_[col_]
            ]
        for col_ in ['SecurityValidityTimestamp']:
            df_[col_] = [
                DatesBR().timestamp_separator_string_to_datetime(
                    d, substring_datetime=' ', format=self.str_fmt_dt
                )
                for d in df_[col_]
            ]
        # adding logging
    ...
    def daily_liquidity_limits(self):
        ...
        """
        # url
        url = YAML_B3['daily_liquidity_limits']['url'].format(
            DatesBR()
            .sub_working_days(DatesBR().curr_date, self.int_wd_bef)
            .strftime('%y%m%d')
        )
        ...
        zipfile = DirFilesManagement().get_zip_from_web_in_memory(
            url,
            bl_io_interpreting=YAML_B3['daily_liquidity_limits'][
                'bl_io_interpreting'
            ],
        ...
        df_dll = df_dll.astype(
            {
                ...
                YAML_B3['daily_liquidity_limits'][
                    'col_instrument_origin'
                ]: str,
    ...
    def options_b3(
        self, float_value_error=-1000.0, bl_debug=False, float_days_year=365.0
    ):
        """
        DOCSTRING:
        INPUTS:
        ...
        # risk-free rate
        r = df_fin_ind[
            df_fin_ind[YAML_B3['financial_indicators']['col_si_code']]
            == YAML_B3['financial_indicators']['int_si_code_selic']
        ][YAML_B3['financial_indicators']['col_rate']].tolist()[0]
        if bl_debug == True:
            print('RISK-FREE RATE: {}'.format(r))
        # options traded in b3 exchange - in zip file
        url = YAML_B3['options_traded_b3']['url'].format(
            DatesBR()
            .sub_working_days(DatesBR().curr_date, self.int_wd_bef)
            .strftime('%y%m%d')
        )
        ...
        zipfile = DirFilesManagement().get_zip_from_web_in_memory(
            url,
            bl_io_interpreting=YAML_B3['options_traded_b3'][
                'bl_io_interpreting'
            ],
                ...
                for soup_content in soup_xml.find_all(key):
                    ...
                    if bl_debug == True:
                        print(
                            'ISIN: {}'.format(
                                soup_content.find(
                                    YAML_B3['options_traded_b3']['keys'][
                                        'isin'
                                    ]
                                ).get_text()
                            )
                    ...
                    #   spot ticker
                    str_isin = soup_content.find(
                        YAML_B3['options_traded_b3']['keys']['isin']
                    ).get_text()
                    dict_[
                        YAML_B3['options_traded_b3']['keys']['spot']
                    ...
                    dict_[
                        YAML_B3['options_traded_b3']['keys']['cd_exp']
                    ...
                    if bl_debug == True:
                        print(
                            'CALENDAR DAYS TO EXPIRATION: {}'.format(
                                DatesBR().delta_calendar_days(
                                    DatesBR().sub_working_days(
                                        DatesBR().curr_date,
                                        abs(-self.int_wd_bef - 1),
                                    ),
                                    DatesBR().str_date_to_datetime(
                                        str(
                                            dict_[
                                                YAML_B3['options_traded_b3'][
                                                    'keys'
                                                ]['xpr_dt']
                                            ]
                                        ),
                    ...
                    while (bl_break_spt_price == False) and (
                        bl_break_opt_price == False
                    ):
                    ...
                    #   risk-free rate
                    dict_[
                        YAML_B3['options_traded_b3']['keys']['risk_free']
                    ] = r
                    ...
                    if bl_debug == True:
                        ...
                        print(
                            ...
                            float(
                                dict_[
                                    YAML_B3['options_traded_b3']['keys']['k']
                                ]
                            ),
                            float(r),
                            float(
                    ...
                    try:
                        dict_[
                            YAML_B3['options_traded_b3']['keys']['imp_vol']
                    ...
                    try:
                        dict_[
                            YAML_B3['options_traded_b3']['keys']['delta']
    ...
    def carga_mtm_b3(self):
        ...
        url_mtm = YAML_B3['margens_teoricas_maximas_b3']['url'].format(
            DatesBR()
            .sub_working_days(DatesBR().curr_date, self.int_wd_bef)
            .strftime('%y%m%d')
        )
        ...
        zipfile = DirFilesManagement().get_zip_from_web_in_memory(
            url_mtm,
            bl_io_interpreting=YAML_B3['margens_teoricas_maximas_b3'][
                'bl_io_interpreting'
            ],
        ...
        list_rows_mtm = [
            ...
            .replace("'", '')
            .split(';')
            for str_ in name_file_extract.readlines()
            if len(
                str(str_)
                .replace(r'\n', '')
                .replace("b'", '')
    ...
    def mtm_compra_venda(
        ...
        list_ativos_bov_vista = HandlingLists().extend_lists(
            dict_ativos_bov_bmf_tipo[key_stocks],
            dict_ativos_bov_bmf_tipo[key_funds],
            dict_ativos_bov_bmf_tipo[key_etfs],
            dict_ativos_bov_bmf_tipo[key_bdrs],
        )
        # removendo eventuais duplicatas
        list_ativos_bov_vista = HandlingLists().remove_duplicates(
            list_ativos_bov_vista
        )
        ...
        for i, _ in enumerate(list_rows_mtm):
            if i < len(list_rows_mtm) - 3:
                ...
                if (
                    ...
                    and list_rows_mtm[i][4] != '0'
                    and (
                        len(list_rows_mtm[i][5]) == 5
                        or len(list_rows_mtm[i][5]) == 6
                    )
                ):
                    den_desagios = float(list_rows_mtm[i][4].replace(',', '.'))
                ...
                for dias_uteis_choque in range(1, 11):
                    try:
                        if (
                            int(list_rows_mtm[i + dias_uteis_choque][1])
                            == dias_uteis_choque
                            and list_rows_mtm[i + dias_uteis_choque][2] != ''
                        ):
                            list_num_desagios_compra.append(
                                float(
                                    list_rows_mtm[i + dias_uteis_choque][
                                        2
                                    ].replace(',', '.')
                                )
                            )
                        if (
                            int(list_rows_mtm[i + dias_uteis_choque][1])
                            == dias_uteis_choque
                            and list_rows_mtm[i + dias_uteis_choque][3] != ''
                        ):
                            list_num_desagios_venda.append(
                                float(
                                    list_rows_mtm[i + dias_uteis_choque][
                                        3
                                    ].replace(',', '.')
                ...
                if (list_num_desagios_compra != []) and (
                    list_num_desagios_venda != []
                ):
                    ...
                    if (
                        ...
                        and all(
                            [
                                type(x) == float
                                for x in list_num_desagios_compra
                            ]
                        )
                        and (type(den_desagios) == float)
                    ):
                        ...
                        for dias_uteis_choque in range(1, 11):
                            ...
                            if list_rows_mtm[i][5] in list_ativos_bov_vista:
                                dict_exportacao[
                                    YAML_B3['margens_teoricas_maximas_b3'][
                                        'col_incluir_margem_b3_compra_d_mais_n'
                                    ]
                                    + str(dias_uteis_choque)
                                ] = abs(
                                    den_desagios
                                    * (
                                        1
                                ...
                                dict_exportacao[
                                    YAML_B3['margens_teoricas_maximas_b3'][
                                        'col_incluir_desagio_b3_compra_d_mais_n'
                                    ]
                                    + str(dias_uteis_choque)
                                ] = min(
                                    [
                                ...
                                dict_exportacao[
                                    YAML_B3['margens_teoricas_maximas_b3'][
                                        'col_incluir_margem_b3_venda_d_mais_n'
                                    ]
                                    + str(dias_uteis_choque)
                                ] = abs(
                                    den_desagios
                                ...
                                dict_exportacao[
                                    YAML_B3['margens_teoricas_maximas_b3'][
                                        'col_incluir_desagio_b3_venda_d_mais_n'
                                    ]
                                    + str(dias_uteis_choque)
                                ] = min(
                                    [
        ...
        df_mtm_b3.sort_values(
            [YAML_B3['margens_teoricas_maximas_b3']['col_incluir_ticker']],
            ascending=[True],
            inplace=True,
        )
        # adding logging
        df_mtm_b3 = DBLogs().audit_log(
            df_mtm_b3,
            YAML_B3['margens_teoricas_maximas_b3']['url'].format(
                DatesBR()
                .sub_working_days(DatesBR().curr_date, self.int_wd_bef)
                .strftime('%y%m%d')
            ),
    ...
    def fatores_primitivos_risco_b3(self):
        ...
        url = YAML_B3['fatores_primitivos_risco_b3']['url'].format(
            DatesBR()
            .sub_working_days(DatesBR().curr_date, self.int_wd_bef)
            .strftime('%y%m%d')
        )
        ...
        zipfile_fpr_b3 = DirFilesManagement().get_zip_from_web_in_memory(
            url,
            bl_io_interpreting=YAML_B3['fatores_primitivos_risco_b3'][
                'bl_io_interpreting'
            ],
        ...
        reader = pd.read_csv(
            extracted_file_fpr_b3,
            sep=';',
            skiprows=1,
            header=None,
            names=YAML_B3['fatores_primitivos_risco_b3'][
                'cols_fatores_primitivos_b3'
            ],
        )
        df_fpr_b3 = pd.DataFrame(reader)
        # substituindo valores NA por valor padrão
        df_fpr_b3.fillna(
            YAML_B3['fatores_primitivos_risco_b3']['valor_na_substituir'],
            inplace=True,
        )
        # alterando tipo de colunas de interesse
        df_fpr_b3 = df_fpr_b3.astype(
            {
                YAML_B3['fatores_primitivos_risco_b3'][
                    'cols_fatores_primitivos_b3'
                ][0]: int,
                YAML_B3['fatores_primitivos_risco_b3'][
                    'cols_fatores_primitivos_b3'
        ...
        df_fpr_b3[
            YAML_B3['fatores_primitivos_risco_b3'][
                'col_incluir_nome_formato_variacao'
            ]
        ] = [
            str(
                YAML_B3['fatores_primitivos_risco_b3'][
                    'variacao_fatores_primitivos'
                ]['tipo'][int_formato_variacao]
            )
            for int_formato_variacao in df_fpr_b3[
                YAML_B3['fatores_primitivos_risco_b3'][
                    'cols_fatores_primitivos_b3'
                ][3]
            ]
        ...
        df_fpr_b3[
            YAML_B3['fatores_primitivos_risco_b3'][
                'col_incluir_nome_origem_instrumento'
            ]
        ] = [
            str(
                YAML_B3['fatores_primitivos_risco_b3'][
                    'variacao_fatores_primitivos'
                ]['origem_instrumento'][int(int_origem_instrumento)]
            )
            for int_origem_instrumento in df_fpr_b3[
                YAML_B3['fatores_primitivos_risco_b3'][
                    'cols_fatores_primitivos_b3'
                ][7]
            ]
        ...
        df_fpr_b3[
            YAML_B3['fatores_primitivos_risco_b3'][
                'col_incluir_criterio_capitalizacao'
            ]
        ] = [
            str(
                YAML_B3['fatores_primitivos_risco_b3'][
                    'variacao_fatores_primitivos'
                ]['criterio_capitalizacao'][int(int_criterio_cap)]
            )
            for int_criterio_cap in df_fpr_b3[
                YAML_B3['fatores_primitivos_risco_b3'][
                    'cols_fatores_primitivos_b3'
                ][10]
            ]
    ...
    @property
    def collateral_acc_spot_bov_b3(
        self,
        dict_repalce_str_erros={'ETF': ''},
        col_ticker='TICKER',
        col_isin='ISIN',
        col_limite_qtds='LIMITE_QUANTIDADES',
        col_mes_arquivo='MES_ARQUIVO',
        key_limite_qtds='Limite (quantidade)',
    ):
        """
        DOCSTRING:
        INPUTS:
        ...
        html_parser_acoes_units_etfs_aceitos_garantia_b3 = (
            HtmlHndler().html_lxml_parser(
                YAML_B3['collateral_acc_spot_bov_b3']['url'], bl_verify=False
            )
        )
        url_acoes_units_etfs_aceitos_garantia_b3 = (
            HtmlHndler().html_lxml_xpath(
                html_parser_acoes_units_etfs_aceitos_garantia_b3,
                YAML_B3['collateral_acc_spot_bov_b3'][
                    'xpath_url_acoes_units_etfs_aceitos_garantia_b3'
                ],
            )[0]
        )
        url_acoes_units_etfs_aceitos_garantia_b3 = (
            StrHandler().get_string_after_substr(
                url_acoes_units_etfs_aceitos_garantia_b3,
                YAML_B3['collateral_acc_spot_bov_b3'][
        ...
        for file_unz in list_unzipped_files[1:]:
            ...
            str_mes_arquivo_garantias_b3_corrente = (
                StrHandler()
                .find_between(str(file_unz), '_', '.xlsx')
                .replace(' ', '')
                .lower()
            )
            ...
            for nome_plan_ativa in [
                ...
                reader = pd.read_excel(file_unz, sheet_name=nome_plan_ativa)
        ...
        # removing duplicates
        df_coll_acc_spot_bov[col_ticker] = [
            StrHandler().replace_all(str(x), dict_repalce_str_erros)
            for x in df_coll_acc_spot_bov[col_ticker].tolist()
        ]
        # adding logging
        df_fpr_b3 = DBLogs().audit_log(
            df_fpr_b3,
    ...
    def cenarios_tipo_curva(self, list_cenarios_tipo_curva=None):
        ...
        if list_cenarios_tipo_curva != None:
            ...
            list_cenarios_tipo_curva = [
                str(x) for x in list_cenarios_tipo_curva
            ]
        # definindo url com cenários de risco - tipo de curva
        url_cenarios_tipo_curva = YAML_B3['cenarios_risco_tipo_curva'][
            'tipo_curva'
        ]['url'].format(
            DatesBR()
            .sub_working_days(
                DatesBR().curr_date,
                YAML_B3['cenarios_risco_tipo_curva']['tipo_curva'][
                    'self.int_wd_bef'
                ],
            )
            .strftime('%y%m%d')
        ...
        list_txts_tipos_curva_cenarios = (
            DirFilesManagement().get_zip_from_web_in_memory(
                url_cenarios_tipo_curva,
                bl_verify=YAML_B3['cenarios_risco_tipo_curva']['bl_verify'],
                bl_io_interpreting=YAML_B3['cenarios_risco_tipo_curva'][
                    'bl_io_interpreting'
                ],
                timeout=YAML_B3['cenarios_risco_tipo_curva']['tipo_curva'][
                    'timeout'
                ],
            )
        )
        ...
        for (
            extracted_file_tipo_curva_cenario
        ) in list_txts_tipos_curva_cenarios:
            ...
            if (list_cenarios_tipo_curva != None) and (
                all(
                    [
                        StrHandler().find_substr_str(
                            extracted_file_tipo_curva_cenario.name, substr
                        )
                        == False
                        for substr in list_cenarios_tipo_curva
                    ]
                )
            ):
                continue
            ...
            reader = pd.read_csv(
                extracted_file_tipo_curva_cenario,
                sep=YAML_B3['cenarios_risco_tipo_curva']['sep_instancias'],
                skiprows=1,
                header=None,
                names=YAML_B3['cenarios_risco_tipo_curva'][
                    'cols_cenarios_tipo_curvas'
                ],
                decimal=',',
            )
            ...
            df_cenarios_passagem.fillna(
                YAML_B3['cenarios_risco_tipo_curva']['fill_na_padrao'],
                inplace=True,
            )
            ...
            list_passagem = df_cenarios_passagem.to_dict(
                orient=YAML_B3['cenarios_risco_tipo_curva']['orient_df']
            )
        ...
        df_cenarios_tipo_curva = df_cenarios_tipo_curva.astype(
            {
                YAML_B3['cenarios_risco_tipo_curva'][
                    'cols_cenarios_tipo_curvas'
                ][0]: int,
                YAML_B3['cenarios_risco_tipo_curva'][
                    'cols_cenarios_tipo_curvas'
    ...
    def cenarios_risco_tipo_spot(self, list_cenarios_tipo_spot=None):
        ...
        url_cenarios_tipo_spot = YAML_B3['cenarios_risco_tipo_spot'][
            ...
            .sub_working_days(
                DatesBR().curr_date,
                YAML_B3['cenarios_risco_tipo_spot']['tipo_curva'][
                    'self.int_wd_bef'
                ],
            )
            .strftime('%y%m%d')
        ...
        # baixando em memória zip com cenário de tipo curva da b3
        list_txts_tipos_spot_cenarios = (
            DirFilesManagement().get_zip_from_web_in_memory(
                url_cenarios_tipo_spot,
                bl_verify=YAML_B3['cenarios_risco_tipo_spot']['bl_verify'],
                bl_io_interpreting=YAML_B3['cenarios_risco_tipo_spot'][
                    'bl_io_interpreting'
                ],
                timeout=YAML_B3['cenarios_risco_tipo_spot']['tipo_curva'][
                    'timeout'
                ],
            )
        )
        ...
        for extracted_file_tipo_curva_cenario in list_txts_tipos_spot_cenarios:
            ...
            if (list_cenarios_tipo_spot != None) and (
                all(
                    [
                        StrHandler().find_substr_str(
                            extracted_file_tipo_curva_cenario.name, substr
                        ...
                        == False
                        for substr in list_cenarios_tipo_spot
                    ]
                )
            ):
                continue
            ...
            reader = pd.read_csv(
                extracted_file_tipo_curva_cenario,
                sep=YAML_B3['cenarios_risco_tipo_spot']['sep_instancias'],
                skiprows=1,
                header=None,
                names=YAML_B3['cenarios_risco_tipo_spot'][
                    'cols_cenarios_tipo_curvas'
                ],
                decimal=',',
            )
            ...
            #   preenchendo com valor padrão campos com valor indisponível
            df_cenarios_passagem.fillna(
                YAML_B3['cenarios_risco_tipo_spot']['fill_na_padrao'],
                inplace=True,
            )
            #   alterando para dicionário dataframe corrente, visando consolidar em uma lista e
            #       importar para uma dataframe para exportação
            list_passagem = df_cenarios_passagem.to_dict(
                orient=YAML_B3['cenarios_risco_tipo_spot']['orient_df']
            )
            list_ser_exportacao_curvas.extend(list_passagem)
        # adicionando lista a um dataframe a ser exportado
        ...
        df_cenarios_tipo_spot = df_cenarios_tipo_spot.astype(
            {
                YAML_B3['cenarios_risco_tipo_spot'][
                    'cols_cenarios_tipo_curvas'
                ][0]: int,
                YAML_B3['cenarios_risco_tipo_spot'][
                    'cols_cenarios_tipo_curvas'
        ...
        df_cenarios_tipo_spot[
            YAML_B3['cenarios_risco_tipo_spot']['col_criar_nome_tipo_cenario']
        ] = [
            YAML_B3['cenarios_risco_tipo_spot'][
                'variaveis_tipo_cenarios_curvas'
            ][x]
            for x in df_cenarios_tipo_spot[
                YAML_B3['cenarios_risco_tipo_spot'][
                    'cols_cenarios_tipo_curvas'
                ][3]
            ].tolist()
    ...
    def fprs_cenarios_curva_spot_merge(
        self,
        list_cenarios_tipo_curva=[
            '2962',
            '2906',
        ...
        df_tipos_fprs = df_tipos_fprs.astype(
            {
                list(
                    YAML_B3['fatores_primitivos_risco_b3']['tipos_fprs'].keys()
                )[0]: int,
                list(
                    YAML_B3['fatores_primitivos_risco_b3']['tipos_fprs'].keys()
                )[1]: str,
        ...
        df_cenarios_tipo_curva = self.cenarios_tipo_curva(
            list_cenarios_tipo_curva
        )
        ...
        df_cenarios_tipo_curva = df_cenarios_tipo_curva[
            df_cenarios_tipo_curva[
                YAML_B3['cenarios_risco_tipo_curva']['col_id_cenario']
            ].isin(
                range(
                    YAML_B3['cenarios_risco_tipo_curva'][
                        'int_inf_range_cenarios'
                    ],
                    YAML_B3['cenarios_risco_tipo_curva'][
                        'int_sup_range_cenarios'
                    ],
                )
            )
        ...
        df_cenarios_tipo_curva = df_cenarios_tipo_curva[
            df_cenarios_tipo_curva['ID_CENARIO'].isin(
                range(1, int_cenario_sup_avaliacao)
            )
        ].reset_index(drop=True)
        if bl_debug == True:
            print('*** TIPO CURVA ***')
        ...
        df_cenarios_tipo_spot = self.cenarios_risco_tipo_spot(
            list_cenarios_tipo_spot
        )
        # removendo colunas de interesse
        ...
        df_cenarios_tipo_spot = df_cenarios_tipo_spot[
            df_cenarios_tipo_spot[
                YAML_B3['cenarios_risco_tipo_spot']['col_id_cenario']
            ].isin(
                range(
                    YAML_B3['cenarios_risco_tipo_spot'][
                        'int_inf_range_cenarios'
        ...
        # eliminando cenários prospectivos
        df_cenarios_tipo_spot = df_cenarios_tipo_spot[
            df_cenarios_tipo_spot['ID_CENARIO'].isin(
                range(1, int_cenario_sup_avaliacao)
            )
        ].reset_index(drop=True)
        if bl_debug == True:
        ...
        df_fpr_b3 = df_fpr_b3[
            df_fpr_b3[
                YAML_B3['fatores_primitivos_risco_b3']['col_id_fpr']
            ].isin(
                list(
                    YAML_B3['fatores_primitivos_risco_b3']['dict_fprs'].keys()
                )
            )
        ]
        if bl_debug == True:
        ...
        list_colunas_frp = [
            ...
            'ID_CENARIO',
            'TIPO_FPR',
        ] + list_colunas_frps.drop(['ID_FPR', 'NOME_FPR']).to_list()
        if bl_debug == True:
            print('*** TIPO FPR B3 ***')
            print(df_fpr_b3)
            print('*** TIPOS FPRS ***')
        ...
        df_cenarios_tipo_curva = pd.concat(
            [df_cenarios_tipo_curva, df_cenarios_tipo_spot],
            ignore_index=True,
            sort=False,
        )
        if bl_debug == True:
        ...
        df_fpr_b3['NOME_FPR'] = [
            YAML_B3['fatores_primitivos_risco_b3']['dict_fprs'][x]
        ...
        df_fpr_b3.rename(columns={'ID_CENARIO_y': 'ID_CENARIO'}, inplace=True)
        if bl_debug == True:
            print('*** DF FPRS CONSOLIDADO ***')
            print(df_fpr_b3)
        # adding logging
        df_fpr_b3 = DBLogs().audit_log(
            df_fpr_b3, None, DatesBR().utc_from_dt(DatesBR().curr_date)
        )
    ...
    def trading_report(
        ...
        el_scty_id='SctyId',
        el_ticker='TckrSymb',
        el_fin_ins_id='FinInstrmId',
        el_othr_id='OthrId',
        el_id='Id',
        el_tp='Tp',
        el_pr_try='Prtry',
        el_plc_of_listg='PlcOfListg',
        el_mkt_id_rcd='MktIdrCd',
        el_trad_dtls='TradDtls',
        el_trad_qty='TradQty',
        el_fin_att='FinInstrmAttrbts',
        el_mkt_data_strm_id='MktDataStrmId',
        el_nt_fin='NtlFinVol',
        el_int_fin='IntlFinVol',
        el_opn_instr='OpnIntrst',
        el_fin_instrm_qt='FinInstrmQty',
        el_best_ask='BestAskPric',
        el_first_pric='FrstPric',
        el_min_pric='MinPric',
        el_max_prec='MaxPric',
        el_avg_pric='TradAvrgPric',
        el_last_pric='LastPric',
        el_rglr_qt='RglrTxsQty',
        el_rglr_trad='RglrTraddCtrcts',
        el_ntl_rglr_vol='NtlRglrVol',
        el_intl_rglr_vol='IntlRglrVol',
        el_oscn_pctg='OscnPctg',
        col_dt_transacao='DT_TRANSACAO',
        col_ticker='TICKER',
        col_id='ID',
        col_id_prtry='ID_PRTRY',
        col_mercado='MERCADO',
        col_qtd_neg='QTD_NEGOCIOS',
        col_str_exercicio='STR_EXERCICIO',
        col_notional_neg_dom='NOTIONAL_NEG_MOEDA_DOM',
        col_notional_neg_int='NOTIONAL_NEG_MOEDA_INT',
        col_qtd_contratos_aberto='QTD_CONTRATOS_ABERTO',
        col_qtd_contratos_neg='QTD_CONTRATOS_NEG',
        col_melhor_ask='MELHOR_ASK',
        col_prim_prec='PRIMEIRO_PRECO',
        col_menor_preco='MENOR_PRECO',
        col_maior_prec='MAIOR_PRECO',
        col_preco_med='PRECO_MEDIO',
        col_ult_preco='ULT_PRECO',
        ...
        col_pct_osc_int='PCT_OSC_INT',
        formato_dt='AAAA-MM-DD',
        qtd_erro=-1,
        col_ticker_cd='Symbol',
        col_tipo_mercado_cd='TIPO_MERCADO',
    ):
        """
        DOCSTRING:
        INPUTS:
        ...
        url = YAML_B3['trading_report']['url'].format(
            DatesBR()
            .sub_working_days(DatesBR().curr_date, self.int_wd_bef)
            .strftime('%y%m%d')
        )
        ...
        for file_xml in list_zip_f:
            ...
            for content_xml_bn in soup_xml_bn.find_all(el_pric_rpt):
                ...
                dict_ = dict()
                #   data de transação
                dict_[col_dt_transacao] = (
                    content_xml_bn.find(el_trad_dt).find(el_dt).get_text()
                )
                #   ticker
                dict_[col_ticker] = (
                    content_xml_bn.find(el_scty_id).find(el_ticker).get_text()
                )
                #   id
                dict_[col_id] = (
                    content_xml_bn.find(el_fin_ins_id)
                    .find(el_othr_id)
                    .find(el_id)
                    .get_text()
                )
                ...
                #   mercado
                dict_[col_mercado] = (
                    content_xml_bn.find(el_plc_of_listg)
                    .find(el_mkt_id_rcd)
                    .get_text()
                )
                ...
                try:
                    dict_[col_qtd_neg] = (
                        content_xml_bn.find(el_trad_dtls)
                        .find(el_trad_qty)
                        .get_text()
                    )
                except:
                ...
                for col_, key_ in [
                    ...
                    try:
                        dict_[col_] = (
                            content_xml_bn.find(el_fin_att)
                            .find(key_)
                            .get_text()
                        )
                    except:
        ...
        for col_ in [col_dt_transacao]:
            df_trd_rpt[col_] = [
                DatesBR().str_date_to_datetime(d, formato_dt)
                for d in df_trd_rpt[col_].tolist()
            ]
        ...
        df_trd_rpt = DBLogs().audit_log(
            df_trd_rpt, url, DatesBR().utc_from_dt(DatesBR().curr_date)
        )
        # retornando dataframe de interesse
        return df_trd_rpt
