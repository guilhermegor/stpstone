...
from stpstone.finance.performance_apprraisal.financial_math import (
    FinancialMath,
)
...
class PrecificacaoTD:
    ...
    def ntn_f(
        self,
        ytm,
        lista_dus_fluxos_caixa,
        taxa_nominal_cupom=0.1,
        valor_nominal=1000,
        du_1_ano=252,
        periodicidade_pagmento_cupons=126,
        periodicidade_capitalizacao=1,
    ):
        """
        ...
        cupom_semestral = valor_nominal * FinancialMath().compound_interest(
            taxa_nominal_cupom, du_1_ano, periodicidade_pagmento_cupons
        )
        print(cupom_semestral)
        print(lista_dus_fluxos_caixa)
        list_fluxos_caixa = [
            FinancialMath().present_value(ytm_real, du, 0, cupom_semestral)
            for du in lista_dus_fluxos_caixa
        ]
        list_fluxos_caixa.append(
            FinancialMath().present_value(
                ytm_real, lista_dus_fluxos_caixa[-1], 0, valor_nominal
            )
        )
        # pu
        return {
    ...
    def pr1(
        self,
        data_negociacao=DatesBR()
        .add_working_days(DatesBR().curr_date(), 1)
        .strftime('%d/%m/%Y'),
        ...
        data_negociacao = DatesBR().str_date_to_datetime(
            data_negociacao, 'DD/MM/AAAA'
        )
        ...
        if (
            date(
                data_negociacao.year,
                data_negociacao.month,
                dia_atualizacao_vna,
            )
            > data_negociacao
        ):
        ...
        # retornar pr1
        return DatesBR().delta_calendar_days(
            data_negociacao, data_vna_anterior
        ) / DatesBR().delta_calendar_days(data_proximo_vna, data_vna_anterior)

    def vna_projetado_ntnb(
        self,
        vna_ultimo_disponivel_ntnb,
        ipca_projetado_aa,
        pr1,
        nper_dias_uteis_aa=252,
        nper_dias_corridos_am=30,
    ):
        """
        ...
        """
        ipca_projetado_adu = FinancialMath().compound_interest(
            ipca_projetado_aa, nper_dias_uteis_aa, nper_dias_corridos_am
        )
    ...
    def ntn_b_principal(
        ...
        # vna projetado
        vna_projetado_ntnb = PrecificacaoTD().vna_projetado_ntnb(
            vna_ultimo_disponivel_ntnb,
            ipca_projetado_aa,
            pr1,
            nper_dias_uteis_aa,
    ...
    def ntn_b(
        ...
        # cupom semestral
        cupom_semestral = FinancialMath().compound_interest(
            taxa_cupom_aa, nper_dias_uteis_aa, periodicidade_pagmento_cupons
        )
        ...
        lista_cotacao.append(
            ...
            / (
                1
                + FinancialMath().compound_interest(
                    ytm, nper_dias_uteis_aa, lista_dus_fluxos_caixa[-1]
                )
            )
        )
        ...
        return {
            ...
            'lista_fluxos_caixa_ntnb': [
                x * vna_projetado_ntnb for x in lista_cotacao
            ],
            'retorno_bruto_ntnb': vna_projetado_ntnb * cotacao,
        }

    def vna_projetado_lft(
        self,
        vna_ultimo_disponivel_lft,
    ...
    def lft(
        ...
        """
        # vna atualizado
        vna_projetado_lft = PrecificacaoTD().vna_projetado_lft(
            vna_ultimo_disponivel_lft,
            taxa_selic_projetada_aa,
            periodicidade_capitalizacao,
            nper_dias_uteis_aa,
        ...
        # cotacao
        cotacao = valor_nominal / (
            1
            + FinancialMath().compound_interest(
                ytm, nper_dias_uteis_aa, periodicidade_capitalizacao
            )
        )
        # pu
    ...
    def dus_vencimento(
        self, data_vencimento, data_referencia=DatesBR().curr_date()
    ):
        """
        DOCSTRING:
        INPUTS:
        ...
        if type(data_vencimento) == str:
            data_vencimento = DatesBR().str_date_to_datetime(
                data_vencimento, 'DD/MM/AAAA'
            )
        # lista últimos dias do ano entre o intervalo
        list_years = DatesBR().list_years_within_dates(
        ...
        return DatesBR().get_working_days_delta(
            data_referencia, data_vencimento
        ) + DatesBR().add_holidays_not_considered_anbima(
            data_referencia, data_vencimento, list_last_week_year_day
        )
    ...
    def dus_pagamento_cupons(self, prim_dia_util, papel, vencimento):
        ...
        if type(prim_dia_util) == str:
            prim_dia_util = DatesBR().str_date_to_datetime(
                prim_dia_util, 'DD/MM/AAAA'
            )
        if type(vencimento) == str:
            vencimento = DatesBR().str_date_to_datetime(
        ...
        if (
            ...
            prim_pagamento_cupom = DatesBR().add_working_days(
        ...
        while iterador == 'OK':
            ...
            if data_prox_pagamento.month == mes_pagamento_cupom_1:
                data_prox_pagamento = DatesBR().add_working_days(
                    DatesBR().sub_working_days(
                        date(
                            data_prox_pagamento.year,
                            mes_pagamento_cupom_2,
                            dia_pagamento_cupom,
                        ),
                        1,
            ...
            dict_cupons[data_corrente] = DatesBR().get_working_days_delta(
                ...
                prim_dia_util, data_corrente, list_last_week_year_day
            )
            # iterador
            iterador = DatesBR().testing_dates(data_prox_pagamento, vencimento)
