...
class ConsultaDadosTesouroDireto:
    def pu_taxa_abertura_contratos(self, contrato, data_mtm, data_vencimento):
        ...
        ano_vencimento = (
            DatesBR().str_date_to_datetime(data_mtm).strftime('%Y')
        )
        ...
        for index, row in df_contrato_vencimento.iterrows():
            if df_contrato_vencimento.loc[index, 'Dia'] == data_mtm:
                return {
                    ...
                    'taxa_compra_abertura': df_contrato_vencimento.loc[
                        index, 'Taxa Compra Manhã'
                    ],
                    'taxa_venda_abertura': df_contrato_vencimento.loc[
                        index, 'Taxa Venda Manhã'
                    ],
                    'pu_compra_abertura': df_contrato_vencimento.loc[
                        index, 'PU Compra Manhã'
                    ],
                    'pu_venda_abertura': df_contrato_vencimento.loc[
                        index, 'PU Venda Manhã'
                    ],
                    'pu_base_abertura': df_contrato_vencimento.loc[
                        index, 'PU Base Manhã'
                    ],
                }
        else:
            raise Exception(
                'Data de referência para marcação a mercado não encontrada para o '
                + 'vencimento'
            )
