...
class DocumentsNumbersBR:
    ...
    def validate(self, doc='CNPJ'):
        ...
        if doc.upper() == 'CNPJ':
            ...
            raise Exception(
                'Type of document provided not identified, please revisit '
    ...
    def get_public_info_cnpj(
        self, url='https://receitaws.com.br/v1/cnpj/{}', method='GET'
    ):
        """
        ...
        """
        return [
            request(method, url.format(cnpj_number)).json()
            for cnpj_number in self.list_docs
        ]
