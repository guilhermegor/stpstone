...
class ProbabilityDistributions:
    ...
    def chi_squared(
        ...
        p,
        df,
        probability_func='ppf',
        x_axis_inf_range=None,
        x_axis_sup_range=None,
        x_axis_pace=None,
    ):
        """
        ...
        if probability_func == 'ppf':
            ...
            raise Exception(
                'Error defining the probability function of interest. {} '.format(
                    probability_func
                )
...
class HansenSkewStudent(object):
    ...
    def __init__(self, eta=10.0, lam=-0.1):
    ...
    def const_b(self):
        ...
        b : float
        """
        return (1 + 3 * self.lam**2 - self.const_a**2) ** 0.5
    ...
    def const_c(self):
        ...
        return gamma((self.eta + 1) / 2) / (
            (np.pi * (self.eta - 2)) ** 0.5 * gamma(self.eta / 2)
        )
    ...
    def pdf(self, arg):
        ...
        return (
            ...
            * (
                ...
                + 1
                / (self.eta - 2)
                * ((b * arg + a) / (1 + np.sign(arg + a / b) * self.lam)) ** 2
            )
            ** (-(self.eta + 1) / 2)
        )
    ...
    def ppf(self, arg):
        ...
        ppf2 = t.ppf(
            0.5 + (arg - (1 - self.lam) / 2) / (1 + self.lam), self.eta
        )
        ppf = -999.99 * np.ones_like(arg)
        ppf = np.nan_to_num(ppf1) * cond + np.nan_to_num(
            ppf2
        ) * np.logical_not(cond)
        ppf = (
            ppf
            * (1 + np.sign(arg - (1 - self.lam) / 2) * self.lam)
            * (1 - 2 / self.eta) ** 0.5
            - a
        ) / b
    ...
    def plot_pdf(self, arg=np.linspace(-2, 2, 100)):
        ...
        arg : array
            Grid of point to evaluate PDF at
        """
        scale = (self.eta / (self.eta - 2)) ** 0.5
        plt.plot(
            arg, t.pdf(arg, self.eta, scale=1 / scale), label='t distribution'
        )
    ...
    def plot_cdf(self, arg=np.linspace(-2, 2, 100)):
        ...
        scale = (self.eta / (self.eta - 2)) ** 0.5
        plt.plot(
            arg, t.cdf(arg, self.eta, scale=1 / scale), label='t distribution'
        )
        plt.plot(arg, self.cdf(arg), label='skew-t distribution')
        plt.legend()
        plt.show()

    def plot_ppf(self, arg=np.linspace(0.01, 0.99, 100)):
        ...
        scale = (self.eta / (self.eta - 2)) ** 0.5
        plt.plot(
            arg, t.ppf(arg, self.eta, scale=1 / scale), label='t distribution'
        )
