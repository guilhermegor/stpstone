...
class StatiscalDescription:
    def statistical_description(self, array_data):
        ...
        return {
            ...
            'standard_deviation_sample': stats.describe(
                np.array(array_data)
            ).variance
            ** 0.5,
    ...
    def covariance(self, array_data_1, array_data_2):
        ...
        """
        return np.dot(
            np.array(array_data_2).T - np.array(array_data_2).mean(),
            np.array(array_data_1) - np.array(array_data_1).mean(axis=0),
        ) / (np.array(array_data_2).shape[0] - 1)
    ...
    def correlation(self, array_data_1, array_data_2):
        ...
        OUTPUTS:
        """
        return self.covariance(array_data_1, array_data_2) / np.sqrt(
            np.var(np.array(array_data_2), ddof=1)
            * np.var(np.array(array_data_1), axis=0, ddof=1)
        )

    def mean_squared_error_fitting_precision(
        self, array_observations, array_predictions
    ):
        """
        ...
        """
        float_mean_squared_error = mean_squared_error(
            array_observations, array_predictions
        )
        return {
    ...
    def mean_absolute_error_fitting_precision(
        ...
        """
        float_mean_absolute_squared_error = mean_absolute_error(
            array_observations, array_predictions
        )
        return {
            'mean_absolute_squared_error': float_mean_absolute_squared_error,
            'sqrt_mean_absolute_squared_error': np.sqrt(
                float_mean_absolute_squared_error
            ),
        }
