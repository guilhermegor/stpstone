...
class Fibonacci:
    ...
    def fibonacci_of_n(self, n):
        ...
        self.set_cache[n] = self.fibonacci_of_n(n - 1) + self.fibonacci_of_n(
            n - 2
        )
        # recursive case
        return self.set_cache[n]
    ...
    def __find_coefficients(self):
        ...
        for i in range(0, self.order + 1):
            self.coefficients.append(
                round(
                    derivative(self.f, self.center, n=i, order=self.d_pts)
                    / math.factorial(i),
                    5,
                )
            )
    ...
    def print_equation(self):
        ...
        for i in range(self.order + 1):
            if self.coefficients[i] != 0:
                eqn_string += (
                    str(self.coefficients[i])
                    + ('(x-{})^{}'.format(self.center, i) if i > 0 else '')
                    + ' + '
                )
        eqn_string = (
            eqn_string[:-3] if eqn_string.endswith(' + ') else eqn_string
        )
        print(eqn_string)
