...
class InputsClassification:
    def fetch_sklearn_database(
        self, database_name='mnist_784', version=1, bl_asframe=False
    ):
        """
        ...
        """
        return fetch_openml(
            database_name, version=version, as_frame=bl_asframe
        )
...
class Classification:
    def one_hot_vectorizer(self, list_corpus):
        ...
        list_labels = [
            StrHandler().remove_end_period_marks(x).lower()
        ...
        return {'labels': list_labels, 'one_hot_encoder': array_one_hot}
    ...
    def svm(
        ...
        array_y,
        kernel='rbf',
        float_regularization_parameter=1,
        multiclass_classification_strategy='best',
        gamma='auto',
        int_random_state_seed=42,
    ):
        """
    ...
    def random_forest(
        self, array_x, array_y, n_estimators=100, int_random_state_seed=42
    ):
        """
    ...
    def k_means(self, n_clusters, array_y, array_x, int_random_state_seed=0):
        ...
        # classifier
        model = KMeans(
            n_clusters=n_clusters, random_state=int_random_state_seed
        )
        # fitting model
        model.fit(array_x, array_y)
        # array predictions
