...
class DimensionalityReduction:
    def backward_elimination(
        ...
        array_x,
        array_y,
        str_estimator='linear_regression',
        int_cv=5,
        int_verbose=0,
    ):
        """
        ...
        if str_estimator == 'linear_regression':
            ...
            raise Exception(
                'Estimator {} not defined, please revisit this parameter'.format(
                    str_estimator
                )
            )
    ...
    def iv_woe_iter(
        ...
        target_col,
        class_col,
        name_missing_data='Missing',
        name_bins='_bins',
        col_min='min',
        col_max='max',
        col_non_event_count='non_event_count',
        col_sample_class='sample_class',
        col_event_count='event_count',
        col_min_value='min_value',
        col_max_value='max_value',
        col_sample_count='sample_count',
        col_event_rate='event_rate',
        col_non_event_rate='non_event_rate',
        col_feature='feature',
        col_sample_class_label='sample_class_label',
        name_category='category',
        col_pct_non_event='pct_non_event',
        col_pct_event='pct_event',
        col_first='first',
        col_count='count',
        col_sum='sum',
        col_mean='mean',
        ...
        if (
            name_bins not in class_col
            and name_missing_data in df_tmp_groupby[col_min_value]
        ):
            df_tmp_groupby[col_min_value] = df_tmp_groupby[
                col_min_value
            ].replace({name_missing_data: np.nan})
            df_tmp_groupby[col_max_value] = df_tmp_groupby[
                col_max_value
            ].replace({name_missing_data: np.nan})
    ...
    def var_iter(
        ...
        for c_i in df.columns:
            if c_i not in [target_col]:
                ...
                if (
                    np.issubdtype(df[c_i], np.number)
                    and df[c_i].nunique() > min_bins_monotonic
                ):
    ...
    def get_iv_woe(
        ...
        col_iv='iv',
        col_remarks='remarks',
        col_number_classes='number_of_classes',
        col_feature_null_pct='feature_null_percent',
        col_iv_sum='iv_sum',
    ):
        """
        REFERENCES: https://github.com/pankajkalania/IV-WOE/blob/main/iv_woe_code.py,
        ...
        df_iv = (
            df_woe_iv.groupby(col_feature)[[col_iv]]
            .agg([col_sum, col_count])
            .reset_index()
        )
        ...
        df_woe_iv = df_woe_iv.merge(
            df_iv[[col_feature, col_iv, col_remarks]].rename(
                columns={col_iv: col_iv_sum}
            ),
            on=col_feature,
            how='left',
        )
    ...
    def iv_label_predictive_power(
        self, df_iv, col_iv='iv', col_predictive_power='predictive_power'
    ):
        """
        REFERENCES: https://github.com/pankajkalania/IV-WOE/blob/main/iv_woe_code.py,
            https://gaurabdas.medium.com/weight-of-evidence-and-information-value-in-python-from-scratch-a8953d40e34#:~:text=Information%20Value%20gives%20us%20the,as%20good%20and%20bad%20customers.
