...
class ExploratoryDataAnalysis:
    def bendford_law(self, array_data, bl_list_number_occurrencies=False):
        ...
        if bl_list_number_occurrencies == False:
            ...
            for i in range(len(array_real_numbers)):
                array_real_numbers = [
                    str(x)[0] for x in array_real_numbers if str(x)[0] != '0'
                ]
            # count percentual occurrency of each int number
            for i in range(1, 10):
                array_percentual_occurrency_digits[
                    i - 1
                ] = array_real_numbers.count(str(i)) / len(
                    array_percentual_occurrency_digits
                )
    ...
    def prepare_bins(
        ...
        for n_bins in range(max_bins, 2, -1):
            try:
                df[c_i + name_bins] = pd.qcut(
                    df[c_i], n_bins, duplicates='drop'
                )
                array_data_monotonic = (
                    df.groupby(c_i + name_bins)[target_col]
                    .mean()
        ...
        if force_bin or (
            c_i + name_bins in df and df[c_i + name_bins].nunique() < 2
        ):
            _min = df[c_i].min()
            _mean = df[c_i].mean()
            _max = df[c_i].max()
            df[c_i + name_bins] = pd.cut(
                df[c_i], [_min, _mean, _max], include_lowest=True
            )
        ...
        if binned == True:
            return (
                c_i + name_bins,
                remarks,
                df[[c_i, c_i + name_bins, target_col]].copy(),
            )
        else:
