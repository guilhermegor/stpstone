...
from stpstone.quantitative_methods.statistical_description import (
    StatiscalDescription,
)
...
class MultipleRegressionHT:
    ...
    def linearity_test(
        self, array_x, array_y, list_cols_iv, r_squared_cut=0.8
    ):
        """
        ...
        for idx in range(array_x.shape[1]):
            ...
            float_r_squared = 1.0 - np.var(
                array_y - np.polyval(lr_model, array_x[:, idx])
            ) / np.var(array_x[:, idx])
            list_linearity.append(float_r_squared)
        # print(list_linearity)
        return {
            'r_squared': {
                list_cols_iv[idx]: list_linearity[idx]
                for idx in range(len(list_linearity))
            },
            'h0': 'linear relationship between X and y',
            'bl_reject_h0': {
                list_cols_iv[idx]: list_linearity[idx] < r_squared_cut
                for idx in range(len(list_linearity))
            },
        }
    ...
    def vif_iv(self, vector_x, array_y):
        ...
        # r squared caclulation
        float_r_squared = 1.0 - np.var(
            array_y - np.polyval(lr_model, vector_x)
        ) / np.var(vector_x)
    ...
    def calculate_vif_ivs(
        self, array_x, array_y, list_cols_iv, float_r_squared_mc_cut=0.8
    ):
        """
        ...
        tups_vif_ivs = [
            self.vif_iv(array_x[:, idx], array_y)
            for idx in range(array_x.shape[1])
        ]
        # return vif for ivs
        return {
            'vif_ivs': {
                col_: tups_vif_ivs[idx][0]
                for idx, col_ in enumerate(list_cols_iv)
            },
            'r_squared_mc_ivs': {
                col_: tups_vif_ivs[idx][1]
                for idx, col_ in enumerate(list_cols_iv)
            },
            'h0': 'lack of multicollinearity',
            'bl_reject_h0': {
                col_: tups_vif_ivs[idx][0] > float_r_squared_mc_cut
                for idx, col_ in enumerate(list_cols_iv)
            },
        }
    ...
    def normality_error_dist_test(self, array_x, array_y, alpha=0.05):
        ...
        dict_anderson_darling = NormalityHT().anderson_darling(
            model_fitted.resid, alpha
        )
    ...
    def durbin_watson_test(self, array_x, array_y):
        ...
        return {
            ...
            'bl_reject_h0': (float_dw_test_value <= 1.5)
            or (float_dw_test_value >= 2.5),
        }
    ...
    def breusch_pagan_test(self, array_x, array_y, alpha=0.05):
        ...
        if array_x.shape[0] != array_y.shape[0]:
            raise SystemExit(
                'Error: the number of samples differs between array_x and array_y.'
            )
        else:
...
class NormalityHT:
    def kolmogorov_smirnov_test(
        self, array_x, alpha=0.05, factor_empirical_func=10
    ):
        """
        ...
        empirical_func_array = [
            array_x / factor_empirical_func
            for array_x in range(1, len(array_x) + 1)
        ]
        # theoretical function array, with normal function distribution for cumulative values
        mean_array = StatiscalDescription().statistical_description(array_x)[
            'mean'
        ]
        standard_deviation = StatiscalDescription().statistical_description(
            array_x
        )['standard_deviation_sample']
        cdf_array = [
            NormalDistribution().cdf(array_x, mean_array, standard_deviation)
            for array_x in np.array(array_x)
        ]
        # test 1 array: |F(array_x(i)) - Fn(array_x(i-1))|
        test_1_array = [
            abs(empirical_func_array[i] - cdf_array[i])
        ...
        return {
            'dn': max(
                np.array(np.max(test_1_array)), np.array(np.max(test_2_array))
            ),
            'critical_value': stats.ksone.ppf(1 - alpha / 2, len(array_x)),
            'h0': 'nomally distributed data',
            'reject_h0': max(
                np.array(np.max(test_1_array)), np.array(np.max(test_2_array))
            )
            > stats.ksone.ppf(1 - alpha / 2, len(array_x)),
        }
    ...
    def anderson_darling(self, array_data, alpha=0.05):
            ...
            d_array.append(
                ...
                * np.log(
                    NormalDistribution().cdf(
                        array_x[i], mean_array, standard_deviation
                    )
                )
                + (2 * (len(array_x) - (i + 1)) + 1)
                * np.log(
        ...
        a_2 = -len(array_x) - 1 / len(array_x) * sum(d_array)
        # anderson-darling modified
        a_m_2 = a_2 * (1 + 0.75 / len(array_x) + 2.25 / len(array_x) ** 2)
        # p-value
        if a_m_2 < 0.2:
            p_value = 1 - np.exp(
                -13.436 + 101.14 * a_m_2 - 223.73 * a_m_2**2
            )
        elif a_m_2 < 0.340:
...
class StatisticalDistributionsHT:
    ...
    def friedman_test(self, x1_array, x2_array, alpha=0.05):
        ...
        return {
            ...
            'reject_h0': alpha
            > stats.friedmanchisquare(x1_array, x2_array).pvalue,
        }
