...
class LinearRegressions:
    def normal_equation(self, array_x, array_y, bl_optimize=True):
        ...
        if bl_optimize == True:
            ...
            return (
                np.linalg.inv(array_x.T.dot(array_x))
                .dot(array_x.T)
                .dot(array_y)
            )
        ...
        if array_x.shape[1] == 1:
            return {
                ...
                'theta_best': np.linalg.inv(
                    np.array(array_x).T.dot(np.array(array_x))
                )
    ...
    def polynomial_equations(
        self, array_x, array_y, int_degree=2, bl_include_bias=True
    ):
        """
    ...
    def ridge_regression(
        self, array_x, array_y, alpha=0, solver_ridge_regression='cholesky'
    ):
        """
        REFERENCES: “HANDS-ON MACHINE LEARNING WITH SCIKIT-LEARN, KERAS, AND TENSORFLOW,
            2ND EDITION, BY AURÉLIEN GÉRON (O’REILLY). COPYRIGHT 2019 KIWISOFT S.A.S.,
    ...
    def elastic_net_regression(
        self, array_x, array_y, alpha=0.1, l1_ratio=0.5
    ):
        """
        REFERENCES: “HANDS-ON MACHINE LEARNING WITH SCIKIT-LEARN, KERAS, AND TENSORFLOW,
            2ND EDITION, BY AURÉLIEN GÉRON (O’REILLY). COPYRIGHT 2019 KIWISOFT S.A.S.,
...
class NonLinearRegressions:
    ...
    def random_forest_regression(
        self, array_x, array_y, seed=None, n_estimators=100
    ):
        """
    ...
    def support_vector_regression(
        ...
        kernel='poly',
        int_degree=2,
        c_positive_floating_point_number=100,
        epsilon=0.1,
    ):
        """
...
class LogLinearRegressions:
    def logistic_regression_logit(
        ...
        solver='lbfgs',
        penalty='l2',
        mult_class_classifier='auto',
        float_tolerance=0.0001,
        intercept_scaling=1,
        random_state=0,
        verbose=0,
        bl_fit_intercept=True,
        bl_warm_start=False,
        class_weight=None,
        bl_dual=False,
        n_jobs=None,
        ...
        return {
            ...
            'confusion_matrix': confusion_matrix(
                array_y, model.predict(array_x)
            ),
            'classification_report': classification_report(
                array_y, model.predict(array_x), output_dict=True
            ),
            'log_likelihood': class_likelihood_ratios(
                array_y, model.predict(array_x)
            ),
        }
...
class NonLinearEquations:
    def differential_evolution(
        ...
        method='scipy',
        max_iter=1000,
        max_iterations_wo_improvement=100,
        int_verbose_monitor=10,
        bl_print_convergence_messages=False,
        bl_print_warning_messages=True,
        bl_inter_monitor=False,
        int_size_trial_solution_population=40,
        tolerance=5e-5,
    ):
        """
