...
class DataCleaning:
    ...
    def split_train_test(
        self, df_data, test_ratio=0.2, random_seed=42, stratify_col=None
    ):
        """
        REFERENCES: HANDS-ON MACHINE LEARNING WITH SCIKIT-LEARN, KERAS, AND TENSORFLOW,
            2ND EDITION, BY AURÉLIEN GÉRON (O’REILLY). COPYRIGHT 2019 KIWISOFT S.A.S.,
    ...
    def split_stratified_train_test(
        ...
        for train_index, test_index in split_class.split(
            df_data, df_data[col_name]
        ):
    ...
    def create_category_stratified_train_test_set(
        ...
        df_data[id_column_category] = pd.cut(
            df_data[id_column_original], bins=list_bins, labels=list_labels
        )
        return df_data

    def remove_category_stratified_train_test_set(
        self, df_train_set, df_test_set, id_column_category
    ):
        """
    ...
    def compare_stratified_random_samples_propotions(
        ...
        df_compare_props = pd.DataFrame(
            {
                'Overall': self.dataframe_id_column_prportions(
                    df_data_original, id_column
                ),
                'Stratified': self.dataframe_id_column_prportions(
                    df_data_stratified_set, id_column
                ),
                'Random': self.dataframe_id_column_prportions(
                    df_data_random_set, id_column
                ),
            }
        ).sort_index()
        df_compare_props['Rand. %error'] = (
            100 * df_compare_props['Random'] / df_compare_props['Overall']
            - 100
        )
        df_compare_props['Strat. %error'] = (
            100 * df_compare_props['Stratified'] / df_compare_props['Overall']
            - 100
        )
    ...
    def replace_nan_values(
        ...
        if n_neighbors == None:
            imputer = SimpleImputer(
                strategy=strategy, missing_values=missing_values
            )
        else:
            imputer = KNNImputer(
                n_neighbors=n_neighbors, missing_values=np.nan
            )
        # fitting data
    ...
    def feature_scaling(
        self, array_data, type_scaler='normalization', tup_feature_range=(0, 1)
    ):
        """
        REFERENCES:  “HANDS-ON MACHINE LEARNING WITH SCIKIT-LEARN, KERAS, AND TENSORFLOW,
            2ND EDITION, BY AURÉLIEN GÉRON (O’REILLY). COPYRIGHT 2019 KIWISOFT S.A.S.,
