...
class BTree:
    ...
    def search(
        self, int_key: int, node: Optional[Node] = None
    ...
    def delete_internal_node(
        self, node_x: Node, node_k: None, int_i: int
    ) -> None:
        """
        ...
        if len(node_x.children[int_i].keys) >= self.t:
            node_x.keys[int_i] = self.delete_predecessor(
                node_x.children[int_i]
            )
            return
        elif len(node_x.children[int_i + 1].keys) >= self.t:
            node_x.keys[int_i] = self.delete_successor(
                node_x.children[int_i + 1]
            )
            return
        else:
            self.delete_merge(node_x, int_i, int_i + 1)
            self.delete_internal_node(
                node_x.children[int_i], node_k, self.self.t - 1
            )
