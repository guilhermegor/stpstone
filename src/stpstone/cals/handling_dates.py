...
class DatesBR(BrazilBankCalendar):
    ...
    def excel_float_to_date(self, float_excel_date):
        ...
        return date.fromordinal(
            date(1900, 1, 1).toordinal() + float_excel_date - 2
        )
    ...
    def str_date_to_datetime(self, data_str, format='DD/MM/YYYY'):
        ...
        if format == 'DD/MM/YYYY':
            return date(
                int(data_str[-4:]), int(data_str[3:5]), int(data_str[0:2])
            )
        elif format == 'YYYY-MM-DD':
            return date(
                int(data_str[0:4]), int(data_str[5:7]), int(data_str[-2:])
            )
        elif format == 'YYMMDD':
            return date(
                int('20' + data_str[0:2]),
            ...
            return date(
                int(data_str[0:2]), int(data_str[2:4]), int(data_str[-4:])
            )
        elif format == 'DD/MM/YY':
            int_year = int(
                str(DatesBR().year_number(DatesBR().curr_date))[:2]
                + str(data_str[-2:])
            )
    ...
    def list_range_number_working_days_between_dates(
        self, data_inic, data_fim, du_ant_ref_date=0, format_datas_input=None
    ):
        """
        DOCSTRING:
        INPUTS:
        ...
        if type(data_inic) == str:
            data_inic = DatesBR().str_date_to_datetime(
                data_inic, format_datas_input
            )
        if type(data_fim) == str:
            data_fim = DatesBR().str_date_to_datetime(
                data_fim, format_datas_input
            )
    ...
    def list_working_days(
        ...
        for x in range(int((data_fim - data_inic).days) + 1):
            list_du.append(
                super().find_following_working_day(
                    day=data_inic + timedelta(days=x)
                )
            )
        if format_datas_saida != None:
            return [
                DatesBR().datetime_to_string(d, format_datas_saida)
                for d in list(unique_everseen(list_du))
            ]
        else:
            return list(unique_everseen(list_du))

    def list_calendar_days(
        self, data_inic, data_fim, format_data='DD/MM/YYYY'
    ):
        """
    ...
    def date_begin_end_month(
        self, date_, last_month_year=12, format='YYYY-MM-DD'
    ):
        """
        DOCSTRING:
        INPUTS:
    ...
    def find_working_day(self, date_test):
        ...
        OUTPUTS: DATE
        """
        return DatesBR().add_working_days(
            DatesBR().sub_working_days(date_test, 1), 1
        )
    ...
    def nth_weekday_month(
        ...
        return [
            DatesBR().add_working_days(DatesBR().sub_working_days(d, 1), 1)
            for d in list_working_days_within_dates
            if (
                DatesBR().week_number(d) == weekday_number
                and d.day
                >= (nth_repetition * number_days_week - number_days_week)
    ...
    def delta_working_hours(
        ...
        inf_time_stamp,
        sup_time_stamp,
        start_time_office_hours=8,
        end_time_office_hours=18,
        start_time_lunch_hours=0,
        end_time_lunch_hours=0,
    ...
    def add_holidays_not_considered_anbima(
        ...
        return len(
            [
                d
                for d in DatesBR().list_calendar_days(inf_date, sup_date)
                if (
                    d.strftime('%d/%m') in list_holidays_not_considered
                    and not DatesBR().week_name(d) in list_non_bzdays_week
    ...
    def timestamp_float_to_datetime(
        self, timestamp, format_timestamp='%Y-%m-%d %H:%M:%S', bl_format=True
    ):
        """
    ...
    def timestamp_separator_string_to_datetime(
        self, str_timestamp, substring_datetime='T', format='YYYY-MM-DD'
    ):
        """
        DOCSTRING:
        INPUTS:
        ...
        return self.str_date_to_datetime(
            StrHandler().get_string_until_substr(
                str_timestamp, substring_datetime
            ),
            format,
        )
    ...
    def timestamp_to_datetime(self, timestamp, bl_return_from_utc=False):
        ...
        if bl_return_from_utc == True:
            return pd.to_datetime(timestamp, unit='s', utc=True).tz_convert(
                'America/Sao_Paulo'
            )
        else:
            return pd.to_datetime(timestamp, unit='s', utc=True).strftime(
                '%Y%m%d'
            )
    ...
    def month_year_string(
        self, str_dt, format_input='%b/%Y', format_output='%Y-%m', bl_dtbr=True
    ):
        """
        DOCSTRING:
        INPUTS:
        ...
        if bl_dtbr == True:
            ...
            return datetime.strptime(str_dt, format_input).strftime(
                format_output
            )
