...
class MT5:
    ...
    def initialize(self, path, login, server, password):
        ...
        if not mt5.initialize(
            path=path, login=login, server=server, password=password
        ):
    ...
    def get_ticks_from(
        ...
        datetime_from,
        ticks_qty,
        type_ticks=mt5.COPY_TICKS_ALL,
        print_quantidade=False,
    ):
        """
        DOCSTRING:
        INPUTS:
    ...
    def get_ticks_range(
        ...
        OUTPUTS:
        """
        if not isinstance(datetime_from, datetime) or not isinstance(
    ...
    def get_last_tick(self, symbol):
        ...
        if last_tick:
            ...
            print(
                'mt5.symbol_info_tick({}) failed, error code ='.format(symbol),
                mt5.last_error(),
            )
            return None
    ...
    def get_market_depth(self, ticker, n_times=10):
        ...
        if mt5.market_book_add(ticker):
            ...
            print(
                'mt5.market_book_add({}) failed, error code ='.format(ticker),
                mt5.last_error(),
            )
        # shut down connection to the MetaTrader 5 terminal
        return items
