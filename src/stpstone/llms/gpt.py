        ...
        for tup_ in list_tuple:
            if tup_[0] == 'text':
                ...
                list_.append(
                    {
                        'type': str(tup_[0]).lower(),
                        'image_url': {'url': str(tup_[1]).lower()},
                    }
                )
        # creating the message content
        dict_content = HandlingDicts().merge_n_dicts(
            dict_content, {'content': list_}
        )
        # add context info, regarding the prompt
        if self.str_context is not None:
            list_prompt = [
                {'role': 'system', 'content': self.str_context},
                dict_content,
            ]
        else:
