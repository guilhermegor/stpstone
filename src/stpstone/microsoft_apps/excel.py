...
class DealingExcel:
    ...
    def autofit_range_columns(
        self, nome_plan, str_range, excel_app, active_wb
    ):
        """
        DOCSTRING: COLAR COMO VALOR COLUNA DE INTERESSE
        INPUTS: NOME DA PLAN, COLUNA COMO STR, XL APP E WORKBOOK ATIVO
    ...
    def delete_cells_w_specific_data(
        self, nome_plan, str_range, value_to_delete, excel_app, active_wb
    ):
        """
        ...
        for cel in excel_app.ActiveSheet.Range(str_range).SpecialCells(2):
            if str(cel) == value_to_delete:
                excel_app.ActiveSheet.Cells(
                    cel.row, cel.column + 1
                ).value = 'DELETAR'

        excel_app.Worksheets(nome_plan).Cells.Find(
            'DELETAR'
        ).EntireColumn.SpecialCells(2).EntireRow.Delete
    ...
    def copy_column_paste_in_other(
        self, nome_plan, str_range_a, str_range_b, excel_app, active_wb
    ):
        """
        DOCSTRING: CRIAR COLUNA EM UMA PLANILHA EXCEL
        INPUTS: NOME COMPLETO DO ARQUIVO, PLAN DE INTERESSE, RANGE DE INTERESSE (COLUNA) ANTES DA QUAL
    ...
    def attr_value_cel(
        self, nome_plan, str_range, value_inter, excel_app, active_wb
    ):
        """
    ...
    def number_format(
        self, nome_plan, str_range, format_inter, excel_app, active_wb
    ):
        """
        DOCSTRING: CHANGE A CEL VALUE
        INPUTS: NOME PLAN INTERESSE, RANGE (CEL), NEW VALUE, XL APP E ACTIVEWORKBOOK
...
class ExcelWriter(object):
    ...
    def addMultipleRows(
        self, start_row, list_data_tuples, start_col=1, sheet=None
    ):
        """
    ...
    def getRangeByCells(
        ...
        return sheet.Range(
            sheet.Cells(cell_start_row, cell_start_col),
    ...
    def fitCol(self, col_n, sheet=None):
        ...
        if sheet == None:
            sheet = self.default_sheet
        sheet.Range(
            sheet.Cells(1, col_n), sheet.Cells(1, col_n)
