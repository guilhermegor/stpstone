...
class ChartsRskManagement:
    def efficient_frontier_makowitz(
        self,
        list_stocks_weights_portfolio,
        list_stocks_tickers,
        date_inf,
        date_sup,
        risk_free_nominal_rate,
        num_portfolios=25000,
        num_working_days_year=252,
        country='brazil',
    ):
        """
        ...
        setup_markowitz_portfolio = (
            MarketRiskManagement().setup_markowitz_portfolio(
                list_stocks_weights_portfolio,
                list_stocks_tickers,
                date_inf,
                date_sup,
                num_working_days_year,
                country,
            )
        )
        ...
        x_axis_std = [
            d['markowitz_portfolio']['std_portfolio_nominal']
            for d in json_markowitz_random_portfolios['n_potfolios'].values()
        ]
        y_axis_returns = [
            d['markowitz_portfolio']['return_portfolio_stocks_nominal']
            for d in json_markowitz_random_portfolios['n_potfolios'].values()
        ]
        colorify_trigger = [
            d['markowitz_portfolio']['sharpe_ratio']
            for d in json_markowitz_random_portfolios['n_potfolios'].values()
        ]
        ...
        plt.scatter(
            json_markowitz_random_portfolios['max_sharpe_ratio_portfolio'][
                'std_nominal'
            ],
            json_markowitz_random_portfolios['max_sharpe_ratio_portfolio'][
                'return_nominal'
        ...
        # minimum volatility
        plt.scatter(
            json_markowitz_random_portfolios['min_volatility_portfolio'][
                'std_nominal'
            ],
            json_markowitz_random_portfolios['min_volatility_portfolio'][
                'return_nominal'
