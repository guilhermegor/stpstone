...
class StockMarket:
    def stock_price_evolution(
        ...
        for k, v in (
            B3Fcto()
            .cotacoes_serie_historica(
                list_stocks_tickers, date_inf, date_sup, country
            )
            .items()
        ):
            list_current_dates = [
                dict_current_price_data['date']
                for dict_current_price_data in v['historical']
            ]
            list_current_prices = [
                dict_current_price_data['close']
                for dict_current_price_data in v['historical']
            ]
