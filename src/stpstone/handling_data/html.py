    ...
    def parse_html_to_string(
        ...
        for _, dict_j in dict_.items():
            for j, _ in dict_j.items():
                dict_fill_blanks_td[j] = max(
                    [
                        dict_[i][j]
                        for i in list(dict_.keys())
                        if i in dict_ and j in dict_[i]
                    ]
                )
        # joining td's with a separator
        for i in range(len(list_tr_html)):
            #   filling blanks to construct columns of the same size
            str_body_html += join_td_character.join(
                [
                    list_tr_html[i][j]
                    + td_size_ajust_character
                    * (dict_fill_blanks_td[j] - len(list_tr_html[i][j]))
                    for j in range(len(list_tr_html[i]))
                ]
            )
            #   adding line scapes
...
class SeleniumWD:
    def __init__(
        ...
        path_webdriver: str,
        int_port: int,
        str_user_agent: str = 'Mozilla/5.0 (Windowns NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.101 Safari/537.36',
    ...
    def get_browser(self) -> WebDriver:
        ...
        browser = webdriver.Chrome(
            executable_path=self.path_webdriver,
            port=self.int_port,
            options=browser_options,
        )
        # open minimized
        if (self.bl_opn_min == True) and (self.bl_headless == False):
    ...
    def process_log(
        self, log: Dict[str, Union[str, dict]]
        ...
        if 'Network.response' in log['method'] and 'params' in log.keys():
            body = self.browser.execute_cdp_cmd(
    ...
    def process_browser_log_entry(
        self, entry: Dict[str, Union[str, dict]]
    ...
    def get_network_traffic(self) -> List[Dict[str, Union[str, dict]]]:
        ...
        # getting events
        list_events = [
            self.process_browser_log_entry(entry) for entry in browser_log
        ]
        list_events = [
            event
    ...
    def find_element(
        self, str_element_interest: str, selector_type: str = 'XPATH'
    ) -> WebElement:
        """
        ...
        try:
            return self.browser.find_element(
                getattr(By, selector_type), str_element_interest
            )
        except AttributeError:
            raise Exception(
                'Attribute, difined in selector type, not available in find element, '
    ...
    def wait_until_el_loaded(self, str_xpath: str) -> WebDriverWait:
        ...
        OUTPUTS:
        """
        return WebDriverWait(self.browser, self.int_delay).until(
            self.el_is_enabled(str_xpath)
        )
