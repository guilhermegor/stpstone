...
class NumHandler:
    ...
    def round_up(self, float_number_to_round, float_base, float_ceiling):
        ...
        float_number_to_round, float_base, float_ceiling = (
            float(x)
            for x in [float_number_to_round, float_base, float_ceiling]
        )
        # defining next multiple with a ceiling
        if (
            float(
                float_base
                + self.truncate(float_number_to_round / float_base, 0)
                * float_base
            )
            < float_ceiling
        ):
    ...
    def sumproduct(self, *lists):
        ...
        """
        return sum(
            functools.reduce(operator.mul, data) for data in zip(*lists)
        )
    ...
    def convert_thousands_decimals_separator(
        self,
        number_float,
        ...
        number_format_output = ':{}{}{}f'.format(
            thousands_outputs, decimals_outputs, precision_decimals
        )
