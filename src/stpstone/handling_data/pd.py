...
class DealingPd(DealingExcel):
    ...
    def append_df_to_Excel(
        ...
        bl_index=0,
        mode='w',
        label_sensitivity='internal',
        bl_set_sensitivity_label=False,
        bl_debug_mode=False,
    ):
        """
        DOCSTRING:
        ...
        # write excel
        with pd.ExcelWriter(
            filename, engine='xlsxwriter', mode=mode
        ) as writer:
            for df_, sheet_name in list_tup_df_sheet_name:
                ...
                df_.to_excel(
                    writer, sheet_name, index=bl_index, header=bl_header
                )
        # setting label
        if bl_set_sensitivity_label == True:
            self.xlsx_sensitivity_label(
                filename,
    ...
    def export_xl(
        ...
        if blame_exportacao_xlsx == 'NOK':
            CreateLog().warnings(
                logger,
                'Arquivo não salvo na rede: {}'.format(
                    nome_completo_xlsx_exportacao
                ),
            )
            raise Exception(
                'Arquivo {} não salvo na rede, favor validar'.format(
                    nome_completo_xlsx_exportacao
                )
            )
        else:
            if bl_adjust_layout == True:
                for _, plan_nome in list_tup_df_sheet_name:
                    ...
                    self.autofit_range_columns(
                        plan_nome, range_colunas, xla, wb
                    )
                    self.close_wb(wb)
    ...
    def convert_datetime_columns(
        self, df_, list_col_date, bl_pandas_convertion=True
    ):
        """
            ...
            for index, row in df_.iterrows():
                for col_date in list_col_date:
                    if '-' in row[col_date]:
                        ...
                        df_.loc[index, col_date] = DatesBR().build_date(
                            ano, mes, dia
                        )
                    else:
                        df_.loc[
                            index, col_date
                        ] = DatesBR().excel_float_to_date(int(row[col_date]))
    ...
    def merge_dfs_into_df(self, df_1, df_2, cols_list):
        ...
        merge_df = pd.merge(
            df_2, intersec_df, how='outer', on=cols_list, indicator=True
        )
    ...
    def change_dtypes(
        ...
        """
        if any(
            [col_ not in list(dict_dtypes.keys()) for col_ in list_cols_dt]
        ):
    ...
    def strip_all_obj_dtypes(
        self, df_: pd.DataFrame, list_cols_dt: List[str]
    ) -> pd.DataFrame:
        """
        DOCSTRING:
        INPUTS:
    ...
    def fillna_data(
        ...
        """
        list_cols = [
            col_ for col_ in list(df_.columns) if col_ not in list_cols_dt
        ]
    ...
    def pipeline_df_startup(
        ...
        OUTPUTS:
        """
        df_ = self.fillna_data(
            df_, list_cols_dt, str_dt_fillna, str_data_fillna
        )
