...
class JsonFiles:
    ...
    def load_message(
        self, json_file, errors='ignore', encoding=None, decoding=None
    ):
        """
        ...
        if encoding == None:
            ...
            with open(
                json_file, errors=errors, encoding=encoding
            ) as read_file:
                return json.loads(
                    read_file.read().encode(encoding).decode(decoding)
                )
    ...
    def byte_to_json(self, byte_message):
        ...
        jsonify_message = ast.literal_eval(
            StrHandler().find_between(str(byte_message), "b'", "'")
        )
        return JsonFiles().send_json(jsonify_message)

    def normalize_json_keys(
        self, json_: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
        """
