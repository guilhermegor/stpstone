...
class DirFilesManagement:
    ...
    def time_last_edition(self, object_path, bl_to_datetime=False):
        ...
        if os.path.exists(object_path):
            if bl_to_datetime == True:
                return (
                    datetime.fromtimestamp(os.path.getmtime(object_path)),
                    True,
                )
            else:
    ...
    def compress_to_7z_file(
        self, file_path_7z, object_to_compress, method='w'
    ):
        """
    ...
    def walk_folder_subfolder_w_rule(self, root_directory, list_name_like):
        ...
        for directory, _, files in os.walk(root_directory):
            for file in files:
                if any(
                    [
                        fnmatch.fnmatch(file, name_like) == True
                        for name_like in list_name_like
                    ]
                ):
...
class FoldersTree:
    def __init__(
        self,
        str_path,
        bl_ignore_dot_folders=False,
        list_ignored_folders=None,
    ...
    def generate_tree(
        ...
        if bl_include_root:
            str_tree_structure += (
                f'{os.path.basename(self.str_path)}{str_linebreak_md}\n'
            )
        ...
        for idx, str_entry in enumerate(list_entries):
            ...
            if bl_is_directory:
                ...
                str_new_prefix = str_prefix + (
                    '    ' if bl_is_last_entry else '│   '
                )
