...
class HandlingLists:
    def get_first_occurrence_within_list(
        ...
        obj_occurrence=None,
        bl_uppercase=False,
        bl_last_uppercase_before_capitalized=False,
        int_error=-1,
        int_error_obj_occurrence=-2,
        bl_regex_alphanumeric_false=False,
        bl_ignore_sole_letter=True,
        str_original_replace_1=',',
        str_original_replace_2='.',
        str_result_replace='',
        bl_audit=False,
    ):
        """
        ...
        if bl_uppercase == True:
            try:
                return list_.index(
                    next(obj for obj in list_ if obj.isupper() == True)
                )
        ...
        # first occurrence of a given object
            ...
            for el in list_:
                ...
                if StrHandler().match_string_like(
                    ...
                    if bl_audit == True:
                        print('LIST INDEX: {}'.format(list_.index(el)))
        ...
        # last occurrence of an uppercase before a capitalized one
            ...
            if bl_audit == True:
                ...
                print(
                    'BOOLEAN 1 - CAPITZALIDED: {}'.format(
                        StrHandler().is_capitalized(
                            StrHandler().remove_diacritics(list_[0])
                        )
                        == True
                    )
                )
                print(
                    'BOOLEAN 2 - LOWER: {}'.format(
                        (
                            StrHandler().remove_diacritics(list_[0]).islower()
                            == True
                        )
                    )
                )
            if (
                ...
                == True
            ) or (StrHandler().remove_diacritics(list_[0]).islower() == True):
                return int_error
            for i in range(len(list_) - 2):
                if (list_[i].replace(',', '').isupper() == True) and (
                    ...
                    or (list_[i + 1].islower() == True)
                ):
                    if bl_ignore_sole_letter == True:
                        if (
                            len(
                                StrHandler().remove_non_alphanumeric_chars(
                                    list_[i]
                                )
                            )
                            == 1
        ...
        # find first error to regex alphanumeric within a list
            ...
            for i in range(len(list_)):
                if (
                    StrHandler().regex_match_alphanumeric(
                        StrHandler()
                        .remove_diacritics(
                            list_[i]
                            .replace(
                                str_original_replace_1, str_result_replace
                            )
                            .replace(
    ...
    def get_list_until_invalid_occurrences(self, list_, list_invalid_values):
        ...
        list_invalid_values = [
            StrHandler().remove_diacritics(el) for el in list_invalid_values
        ]
        ...
        for el in list_:
            if any(
                [
                    StrHandler().match_string_like(el, str_) == True
                    for str_ in list_invalid_values
                ]
            ):
                break
            else:
    ...
    def get_lower_upper_bound(self, sorted_list, value_to_put_in_between):
        ...
        if (value_to_put_in_between in sorted_list) and sorted_list[
            -1
        ] != value_to_put_in_between:
            list_idx_lower_upper_bound = [
        ...
        if all(0 <= i <= len(sorted_list) for i in list_idx_lower_upper_bound):
            ...
            raise Exception(
                '{} value is outside the bounds of {}'.format(
    ...
    def get_lower_mid_upper_bound(self, sorted_list, value_to_put_in_between):
        ...
        # dictionary with responses
        if (
            all(0 <= i <= len(sorted_list) for i in list_idx_lower_upper_bound)
            and len(sorted_list) > 2
        ):
            try:
                dict_message = {
                    'lower_bound': sorted_list[list_idx_lower_upper_bound[0]],
                    'middle_bound': sorted_list[list_idx_lower_upper_bound[1]],
                    'upper_bound': sorted_list[
                        list_idx_lower_upper_bound[1] + 1
                    ],
                    'end_of_list': False,
                }
            except:
                dict_message = {
                    'lower_bound': sorted_list[
                        list_idx_lower_upper_bound[0] - 1
                    ],
    ...
    def closest_bound(self, sorted_list, value_to_put_in_between):
        ...
        return sorted_list[
            min(
                ...
                key=lambda i: abs(sorted_list[i] - value_to_put_in_between),
            )
        ]
    ...
    def closest_number_within_list(list_, number_):
        ...
        """
        return list_[
            min(range(len(list_)), key=lambda i: abs(list_[i] - number_))
        ]
    ...
    def first_occurrence_like(self, list_, str_like):
        ...
        return list_.index(
            next(
                x
                for x in list_
                if StrHandler().match_string_like(x, str_like) == True
            )
        )
    ...
    def extend_lists(self, *lists, bl_remove_duplicates=True):
        ...
        if bl_remove_duplicates == True:
            list_extended_lists = HandlingLists().remove_duplicates(
                list_extended_lists
            )
        else:
    ...
    def chunk_list(
        ...
        list_position_chunks = NumHandler().multiples(
            int_chunk, len(list_to_chunk)
        )
        ...
        if len(list_position_chunks) > 2:
            for lim in list_position_chunks[2:]:
                ...
                if str_character_divides_clients == None:
                    ...
                    list_chunked.append(
                        str_character_divides_clients.join(
    ...
    def cartesian_product(self, list_lists, int_break_n_n=None):
        ...
        if int_break_n_n != None:
            for tup in list_cartesian_product:
                if (tup[:int_break_n_n] not in list_export) and (
                    all(
                        [
                            tup[:int_break_n_n][i]
                            != tup[:int_break_n_n][i - 1]
                            for i in range(1, len(tup[:int_break_n_n]))
                        ]
                    )
                ):
