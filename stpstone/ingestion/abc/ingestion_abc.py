...
class CoreIngestion(metaclass=TypeChecker):
	...
	def standardize_dataframe(
		...
		df_ = self.cls_db_logs.audit_log(df_, url, date_ref, bool_format_log_as_str)
		return df_
...
class ContentAggregator(metaclass=TypeChecker):
	...
	def paginate_text_blocks(self, stream_file: fitz.Document, int_pages_join: int) -> list[str]:
		...
		for i in range(0, len(stream_file)):
			...
			if i % int_pages_join == 0 or i == len(stream_file) - 1:
				str_ = self.cls_str_handler.remove_diacritics_nfkd(str_, bool_lower_case=True)
...
class ContentParser(ContentAggregator):
	...
	def pdf_docx_regex(
		...
		list_matches = self._regex_patterns_match(
			list_blocks_pages=list_blocks_pages, dict_regex_patterns=dict_regex_patterns
		)
	...
	def _regex_patterns_match(
		...
		for str_page in list_blocks_pages:
			if len(dict_count_matches) > 0 and all(
				count > 0 for count in dict_count_matches.values()
			):
				break
