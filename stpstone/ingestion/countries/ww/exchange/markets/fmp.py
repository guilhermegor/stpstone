"""Financial Modeling Prep (FMP) worldwide market data ingestion."""
...
class FMPMktHoursExchanges(ABCIngestionOperations):
    """FMP market hours for all exchanges ingestion class."""
    ...
    def __init__(
        ...
        self.url = f"{self._BASE_URL}all-exchange-market-hours?apikey={self.token}"
    ...
    def run(
        ...
        str_table_name: str = "ww_fmp_mkts_hours_yesterday",
        ...
        str_table_name : str, optional
            The name of the table, by default 'ww_fmp_mkts_hours_yesterday'.
...
class FMPCryptoOhlcvYesterday(ABCIngestionOperations):
    """FMP cryptocurrency OHLCV quotes ingestion class."""

    _BASE_URL = "https://financialmodelingprep.com/stable/"
    _DEFAULT_SLUGS = ["BTCUSD", "SOLUSD", "ETHUSD"]
    ...
    def __init__(
        ...
        list_slugs : Optional[list[str]], optional
            List of crypto symbols to fetch, by default ['BTCUSD', 'SOLUSD', 'ETHUSD'].
        logger : Optional[Logger], optional
            The logger, by default None.
        cls_db : Optional[Session], optional
            The database session, by default None.
    ...
    def run(
        ...
        bool_verify: bool = False,
        bool_insert_or_ignore: bool = False,
        str_table_name: str = "ww_fmp_crypto_ohlcv_yesterday",
    ) -> Optional[pd.DataFrame]:
        """Run the ingestion process.
        ...
        bool_insert_or_ignore : bool, optional
            Whether to insert or ignore the data, by default False.
        str_table_name : str, optional
            The name of the table, by default 'ww_fmp_crypto_ohlcv_yesterday'.
        ...
        for slug in self.list_slugs:
            self.url = f"{self._BASE_URL}quote?symbol={slug}&apikey={self.token}"
            resp_req = self.get_response(timeout=timeout, bool_verify=bool_verify)
...
class FMPCommoditiesOhlcvYesterday(ABCIngestionOperations):
    """FMP batch commodity OHLCV quotes ingestion class."""
    ...
    def __init__(
        ...
        self.date_ref = date_ref or \
            self.cls_dates_br.add_working_days(self.cls_dates_current.curr_date(), -1)
        self.url = f"{self._BASE_URL}batch-commodity-quotes?apikey={self.token}"
    ...
    def run(
        ...
        bool_verify: bool = False,
        bool_insert_or_ignore: bool = False,
        str_table_name: str = "ww_fmp_commodities_ohlcv_yesterday",
    ) -> Optional[pd.DataFrame]:
        """Run the ingestion process.
        ...
        bool_insert_or_ignore : bool, optional
            Whether to insert or ignore the data, by default False.
        str_table_name : str, optional
            The name of the table, by default 'ww_fmp_commodities_ohlcv_yesterday'.
