"""B3 BDI historical USD/BRL exchange rates ingestion."""
...
class B3BdiFxDolHistory(ABCIngestionOperations):
	"""B3 BDI historical USD/BRL exchange rates ingestion class."""
	...
	def __init__(
		...
		int_page_max : int
			...
			showPagination true but returns all data in a single page.
		...
		self.url_tpl = (
			f"https://arquivos.b3.com.br/bdi/table/HistoricalExchange/"
	...
	def run(
		...
		str_table_name: str = "br_b3_bdi_fx_dol_history",
		...
		str_table_name : str
			The name of the table, by default "br_b3_bdi_fx_dol_history".
	...
	def _log_page_progress(self, int_page: int, int_rows: int) -> None:
		...
		self.cls_create_log.log_message(
			logger=self.logger,
			message=(f"B3BdiFxDolHistory: page {int_page} fetched ({int_rows} rows)"),
			log_level="info",
		)
	...
	def transform_data(self, data: dict) -> pd.DataFrame:
		...
		Trailing None values per row are a B3 API artifact and map to integer
		positional columns that are excluded from dict_dtypes.
		...
		col_names = [
			str_handler.convert_case(col["name"].strip(), "pascal", "upper_constant")
