...
class YahiiRentIndices(ABCIngestionOperations):
    ...
    def __init__(
        ...
        self.date_ref = date_ref or \
            self.cls_dates_br.add_working_days(self.cls_dates_current.curr_date(), -300)
        ...
        # Index mapping
        self.indices = [
            "INPC/IBGE", "IPC/FIPE", "IGP(DI)/FGV", "IGPM/FGV", 
            "IPCA/IBGE", "IPC/FGV", "IPCR"
        ]
    ...
    def transform_data(
        ...
        for month_idx, month_name in enumerate(self.months, start=1):
            ...
            # Get the data table (should be the first table after the month header)
            for table in tables:
