...
class BCBCurrenciesCodes(ABCIngestionOperations):
...
class BCBQuotesBulletins(ABCIngestionOperations):
    ...
    def __init__(
        ...
        self.date_end = date_end or \
            self.cls_dates_br.add_working_days(self.cls_dates_current.curr_date(), -1)
        self.cls_bcb_currencies_codes = BCBCurrenciesCodes(
    ...
    def run(
        ...
        df_currencies_codes = self._get_currencies_codes(timeout=30_000)

        for _, row in df_currencies_codes.iterrows():
            resp_req = self.get_response(currency_code=row["CODIGO_MOEDA"], 
    ...
    def _get_currencies_codes(self, timeout: int = 30_000) -> pd.DataFrame:
        ...
        """
        return self.cls_bcb_currencies_codes.run(timeout=timeout)
