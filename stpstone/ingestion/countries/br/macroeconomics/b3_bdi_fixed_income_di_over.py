"""B3 BDI DI Over (overnight interbank deposit rate) ingestion."""
...
class B3BdiFixedIncomeDiOver(ABCIngestionOperations):
	"""B3 BDI DI Over overnight interbank deposit rate ingestion class."""
	...
	def run(
		...
		str_table_name: str = "br_b3_bdi_fixed_income_di_over",
		...
		str_table_name : str
			The name of the table, by default "br_b3_bdi_fixed_income_di_over".
	...
	def _log_page_progress(self, int_page: int, int_rows: int) -> None:
		...
		self.cls_create_log.log_message(
			logger=self.logger,
			message=f"B3BdiFixedIncomeDiOver: page {int_page} fetched ({int_rows} rows)",
			log_level="info",
		)
	...
	def transform_data(self, data: dict) -> pd.DataFrame:
		...
		return df_.rename(columns={"TCKR_SYMB": "NUMBER_OF_OPERATIONS"})
