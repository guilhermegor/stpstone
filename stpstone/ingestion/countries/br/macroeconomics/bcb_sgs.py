...
class BCBSGS(ABCIngestionOperations):
	...
	def __init__(
		...
		self.date_start = date_start or self.cls_dates_br.add_working_days(
			self.cls_dates_current.curr_date(), -30
		)
		...
		self.url_data = (
			...
			+ "serie/bcdata.sgs.{}/dados?formato=json&dataInicial={}&dataFinal={}"
		)  # codespell:ignore
	...
	def get_series_metadata(self, series_code: int) -> tuple[str, ...]:
		...
		try:
			...
			self.cls_create_log.log_message(
				self.logger, f"Failed to fetch series name for {series_code}: {e}", "error"
			)
	...
	def _get_series_metadata_from_mapping(self, series_code: int) -> tuple[str, ...]:
		...
		"""
		series_mapping = {
			11: ("Taxa de juros - Selic", "Selic", "Diária", "% a.d.", "4/6/1986"),
			4390: (
			...
			1178: (
				"Taxa de juros - Selic em termos anuais (base 252)",
			...
			4189: (
				"Taxa de juros - Selic acumulada no mês em termos anuais (base 252)",
				"Selic acumulada no mês em termos anuais (base 252)",
				"Mensal",
				"% a.a.",
				"31/7/1986",
			),
			432: ("Taxa de juros - CDI", "CDI", "Diária", "% a.d.", "2/1/1986"),
			4391: (
			...
			1: ("Dólar americano (venda)", "Dólar (venda)", "Diária", "R$/US$", "1/7/1994"),
			3695: ("IPCA - Variação mensal", "IPCA mensal", "Mensal", "% a.m.", "31/1/1980"),
			433: (
			...
			189: ("IGP-M - Variação mensal", "IGP-M mensal", "Mensal", "% a.m.", "30/6/1989"),
			190: (
		...
		return series_mapping.get(series_code, (f"SERIE_{series_code}", "", "", "", ""))
	...
	def transform_data(self, json_: list[dict[str, Union[str, int, float]]]) -> pd.DataFrame:
