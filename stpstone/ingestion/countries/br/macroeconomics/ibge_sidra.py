...
class IBGESIDRA(ABCIngestionOperations):
	...
	def _validate_series_id_input(self, list_series_id: list[int]) -> None:
		...
		if list_series_id is not None:
			if not isinstance(list_series_id, list):
				raise TypeError(
					f"list_series_id must be of type list[int], got {type(list_series_id)}"
				)
			...
			for item in list_series_id:
				if not isinstance(item, int) or isinstance(item, bool):
					raise TypeError(
						f"All items in list_series_id must be of type int, got {type(item)}"
					)
				if item <= 0:
					raise TypeError(
						f"All items in list_series_id must be positive integers, got {item}"
					)
	...
	def get_metadata(self, int_series_id: int) -> tuple[str, str, str]:
		...
		return (
			...
			";".join(
				json_["Notas"]
				if isinstance(json_["Notas"], list)
				else [json_["Notas"]]
				if json_["Notas"]
				else ""
			),
