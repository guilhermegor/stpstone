"""Anbima IGPM Forecasts for the Last Twelve Months ingestion module."""
...
class AnbimaIGPMForecastsLTM(AnbimaIGPMCore, ABCIngestionOperations):
	...
	def transform_data(self, scraper_playwright: PlaywrightScraper) -> pd.DataFrame:
		...
		with scraper_playwright.launch():
			...
			list_td = [
				nan if len(str(dict_["text"])) == 1 else str(dict_["text"]).replace(",", ".")
		...
		df_["IGPM_EFETIVO_PCT"] = df_.groupby("MES_COLETA")["IGPM_EFETIVO_PCT"].ffill()
		return df_
	...
	def _debug_flat_list_structure(self, flat_list: list[str]) -> None:
		...
		if len(month_positions) > 1:
			distances = [
				month_positions[i] - month_positions[i - 1] for i in range(1, len(month_positions))
			]
			self.cls_create_log.log_message(
				self.logger,
