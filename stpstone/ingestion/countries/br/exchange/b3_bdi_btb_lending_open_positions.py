...
class B3BdiBtbLendingOpenPositions(ABCIngestionOperations):
	"""B3 BDI securities lending open positions ingestion class."""
	...
	def __init__(
		...
		int_page_max : int, optional
			...
			returns all open position rows on a single page.
		...
		self.url_tpl = (
			f"https://arquivos.b3.com.br/bdi/table/BTBLendingOpenPosition/"
	...
	def run(
		...
		str_table_name: str = "br_b3_bdi_btb_lending_open_positions",
		...
		str_table_name : str, optional
			The name of the table, by default "br_b3_bdi_btb_lending_open_positions".
	...
	def _log_page_progress(self, int_page: int, int_rows: int) -> None:
		...
		self.cls_create_log.log_message(
			logger=self.logger,
			message=(f"B3BdiBtbLendingOpenPositions: page {int_page} fetched ({int_rows} rows)"),
			log_level="info",
		)
	...
	def transform_data(self, data: dict) -> pd.DataFrame:
		...
		Column names are converted from PascalCase to UPPER_SNAKE_CASE. This endpoint
		returns exactly one value per column with no trailing extra nulls.
