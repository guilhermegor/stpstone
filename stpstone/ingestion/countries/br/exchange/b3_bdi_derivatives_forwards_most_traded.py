...
class B3BdiDerivativesForwardsMostTraded(ABCIngestionOperations):
	"""B3 BDI most traded equities in forward contracts ingestion class."""
	...
	def __init__(
		...
		int_page_max : int, optional
			...
			typically returns all forward market rows on a single page.
		...
		self.url_tpl = (
			f"https://arquivos.b3.com.br/bdi/table/Forward/"
	...
	def run(
		...
		str_table_name: str = "br_b3_bdi_derivatives_forwards_most_traded",
		...
		str_table_name : str, optional
			...
			"br_b3_bdi_derivatives_forwards_most_traded".
	...
	def _log_page_progress(self, int_page: int, int_rows: int) -> None:
		...
		self.cls_create_log.log_message(
			logger=self.logger,
			message=(
				f"B3BdiDerivativesForwardsMostTraded: page {int_page} fetched ({int_rows} rows)"
			),
			log_level="info",
		)
	...
	def transform_data(self, data: dict) -> pd.DataFrame:
		...
		row has 8 elements but only 7 column definitions — the 8th element is always
		...
		-------
		pd.DataFrame
			DataFrame with UPPER_SNAKE_CASE column names (7 columns), or empty
