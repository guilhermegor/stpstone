...
class B3BdiEquitiesGreatestUpsides(ABCIngestionOperations):
	"""B3 BDI cash market equities with the largest gains ingestion class."""
	...
	def __init__(
		...
		int_page_max : int
			...
			typically returns all equities with the largest gains in the cash
		...
		self.url_tpl = (
			f"https://arquivos.b3.com.br/bdi/table/InCashMarketBiggestHighs/"
	...
	def run(
		...
		str_table_name: str = "br_b3_bdi_equities_greatest_upsides",
		...
		str_table_name : str
			...
			"br_b3_bdi_equities_greatest_upsides".
	...
	def _log_page_progress(self, int_page: int, int_rows: int) -> None:
		...
		self.cls_create_log.log_message(
			logger=self.logger,
			message=(f"B3BdiEquitiesGreatestUpsides: page {int_page} fetched ({int_rows} rows)"),
			log_level="info",
		)
