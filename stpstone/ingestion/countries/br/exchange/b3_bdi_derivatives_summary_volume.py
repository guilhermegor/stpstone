"""B3 BDI derivatives daily average volume summary by period ingestion."""
...
class B3BdiDerivativesSummaryVolume(ABCIngestionOperations):
	"""B3 BDI derivatives daily average volume summary by period ingestion class."""
	...
	def __init__(
		...
		int_page_max : int
			...
			returns all period summary rows on a single page.
		...
		self.url_tpl = (
			f"https://arquivos.b3.com.br/bdi/table/DailyAverageDerivatives2/"
	...
	def run(
		...
		str_table_name: str = "br_b3_bdi_derivatives_summary_volume",
		...
		str_table_name : str
			...
			"br_b3_bdi_derivatives_summary_volume".
		...
		dict_dtypes = {
			"TCKR_SYMB": str,
			"TOTAL_WITH_MINIS": float,
			"TOTAL_NO_MINIS": float,
			"COL_ORDER": int,
			"URL": str,
		}
	...
	def _log_page_progress(self, int_page: int, int_rows: int) -> None:
		...
		self.cls_create_log.log_message(
			logger=self.logger,
			message=(f"B3BdiDerivativesSummaryVolume: page {int_page} fetched ({int_rows} rows)"),
			log_level="info",
		)
	...
	def transform_data(self, data: dict) -> pd.DataFrame:
		...
		row has 5 elements but only 4 column definitions — the 5th element is always
		...
		-------
		pd.DataFrame
			DataFrame with UPPER_SNAKE_CASE column names (4 columns), or empty
