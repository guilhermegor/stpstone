...
class B3BdiClearingAdrCustody(ABCIngestionOperations):
	"""B3 BDI equities in custody for the ADR program ingestion class."""
	...
	def __init__(
		...
		self.url_tpl = (
			f"https://arquivos.b3.com.br/bdi/table/Custody/"
	...
	def run(
		...
		str_table_name: str = "br_b3_bdi_clearing_adr_custody",
		...
		str_table_name : str
			The name of the table, by default "br_b3_bdi_clearing_adr_custody".
	...
	def _log_page_progress(self, int_page: int, int_rows: int) -> None:
		...
		self.cls_create_log.log_message(
			logger=self.logger,
			message=(f"B3BdiClearingAdrCustody: page {int_page} fetched ({int_rows} rows)"),
			log_level="info",
		)
