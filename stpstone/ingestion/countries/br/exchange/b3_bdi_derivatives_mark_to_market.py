"""B3 BDI derivatives mark-to-market settlement prices and open interest ingestion."""
...
class B3BdiDerivativesMarkToMarket(ABCIngestionOperations):
	"""B3 BDI derivatives mark-to-market ingestion class."""
	...
	def __init__(
		...
		int_page_max : int, optional
			...
			typically returns all mark-to-market rows on a single page.
		...
		self.url_tpl = (
			f"https://arquivos.b3.com.br/bdi/table/DerivativesMtM/"
	...
	def run(
		...
		str_table_name: str = "br_b3_bdi_derivatives_mark_to_market",
		...
		str_table_name : str, optional
			...
			"br_b3_bdi_derivatives_mark_to_market".
	...
	def _log_page_progress(self, int_page: int, int_rows: int) -> None:
		...
		self.cls_create_log.log_message(
			logger=self.logger,
			message=(f"B3BdiDerivativesMarkToMarket: page {int_page} fetched ({int_rows} rows)"),
			log_level="info",
		)
	...
	def transform_data(self, data: dict) -> pd.DataFrame:
		...
		row may have more elements than column definitions — the extra element is always
