...
class B3BdiIndexesIbovGreatestUpsides(ABCIngestionOperations):
	"""B3 BDI IBOVESPA equities with the largest percentage gains ingestion class."""
	...
	def __init__(
		...
		int_page_max : int, optional
			...
			typically returns all results on a single page.
		...
		self.url_tpl = (
			f"https://arquivos.b3.com.br/bdi/table/IbovespaStockBiggestHighs/"
	...
	def run(
		...
		str_table_name: str = "br_b3_bdi_indexes_ibov_greatest_upsides",
		...
		str_table_name : str, optional
			The name of the table, by default "br_b3_bdi_indexes_ibov_greatest_upsides".
	...
	def _log_page_progress(self, int_page: int, int_rows: int) -> None:
		...
		self.cls_create_log.log_message(
			logger=self.logger,
			message=(
				f"B3BdiIndexesIbovGreatestUpsides: page {int_page} fetched ({int_rows} rows)"
			),
			log_level="info",
		)
	...
	def transform_data(self, data: dict) -> pd.DataFrame:
		...
		Column names are converted from PascalCase to UPPER_SNAKE_CASE. Each values row
		contains 5 elements but only 4 column definitions; the 5th element (always null)
		is dropped by selecting only the named columns.
