...
class BVMFVBOVTradingVolumes(ABCIngestionOperations):
	...
	def __init__(
		...
		self.date_ref = date_ref or self.cls_dates_br.add_working_days(
			self.cls_dates_current.curr_date(), -7
		)
		self.url = (
	...
	def get_response(
		...
		while not bool_parsed_tables:
			...
			if len_tables > 0:
				...
				raise ValueError(f"Failed to get response after {int_max_retries} attempts.")
	...
	def transform_data(self, html_root: bs4.BeautifulSoup) -> pd.DataFrame:
		...
		for i, tr in enumerate(html_table.find_all("tr")):
			if i >= 2:
				list_td.extend([self._td_parser(el) for el in tr.find_all("td")])
	...
	def _td_parser(self, el: bs4.element.Tag) -> str:
		...
		return (
			...
			else self.cls_str_handler.remove_diacritics(el.get_text())
			.strip()
			.replace(" de ", " ")
			.replace(" do ", " ")
			.replace(" a ", " ")
