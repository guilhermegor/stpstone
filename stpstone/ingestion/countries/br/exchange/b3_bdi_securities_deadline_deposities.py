...
class B3BdiSecuritiesDeadlineDeposities(ABCIngestionOperations):
	"""B3 BDI deadline for deposit of securities ingestion class."""
	...
	def __init__(
		...
		int_page_max : int
			...
			returns all deadline rows on a single page.
		...
		self.url_tpl = (
			f"https://arquivos.b3.com.br/bdi/table/DeadlineDepositSecurities/"
	...
	def run(
		...
		str_table_name: str = "br_b3_bdi_securities_deadline_deposities",
		...
		str_table_name : str
			The name of the table, by default "br_b3_bdi_securities_deadline_deposities".
	...
	def _log_page_progress(self, int_page: int, int_rows: int) -> None:
		...
		self.cls_create_log.log_message(
			logger=self.logger,
			message=(
				f"B3BdiSecuritiesDeadlineDeposities: page {int_page} fetched ({int_rows} rows)"
			),
			log_level="info",
		)
