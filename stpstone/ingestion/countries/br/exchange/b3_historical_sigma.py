...
class B3HistoricalSigma(ABCIngestionOperations):
	...
	def __init__(
		...
		self.fstr_url_1 = (
			"https://sistemaswebb3-listados.b3.com.br/securitiesVolatilityProxy/"
	...
	def parse_raw_file(
		self, list_resp_req: Union[list[Response], list[PlaywrightPage], list[SeleniumWebDriver]]
		...
		for resp_req in list_resp_req:
			...
			list_ = self.cls_dicts_handler.add_key_value_to_dicts(
				list_ser=json_["results"], key=[json_["page"]]
			)
			list_ = self.cls_dicts_handler.add_key_value_to_dicts(
				list_ser=list_, key="URL", value=resp_req.url
			)
			list_ser.extend(list_)

		return list_ser

	def transform_data(self, list_ser: list[dict[str, Union[str, int, float]]]) -> pd.DataFrame:
