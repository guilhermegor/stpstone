...
class B3BdiEquitiesGreatestDownsides(ABCIngestionOperations):
	"""B3 BDI cash market equities with the largest losses ingestion class."""
	...
	def __init__(
		...
		int_page_max : int, optional
			...
			typically returns all equities with the largest losses in the cash
			market on a single page.
		...
		self.url_tpl = (
			f"https://arquivos.b3.com.br/bdi/table/InCashMarketBiggestLow/"
	...
	def run(
		...
		str_table_name: str = "br_b3_bdi_equities_greatest_downsides",
		...
		str_table_name : str, optional
			...
			"br_b3_bdi_equities_greatest_downsides".
	...
	def _log_page_progress(self, int_page: int, int_rows: int) -> None:
		...
		self.cls_create_log.log_message(
			logger=self.logger,
			message=(f"B3BdiEquitiesGreatestDownsides: page {int_page} fetched ({int_rows} rows)"),
			log_level="info",
		)
