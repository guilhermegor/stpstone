...
class B3BdiSecuritiesFungibleCustody(ABCIngestionOperations):
	"""B3 BDI fungible custody securities subscription deadlines ingestion class."""
	...
	def __init__(
		...
		int_page_max : int, optional
			...
			returns all custody rows on a single page.
		...
		self.url_tpl = (
			f"https://arquivos.b3.com.br/bdi/table/FugibleCustody/"
	...
	def run(
		...
		str_table_name: str = "br_b3_bdi_securities_fungible_custody",
		...
		str_table_name : str, optional
			The name of the table, by default "br_b3_bdi_securities_fungible_custody".
		...
		dict_dtypes = {
			"SBSCRPTN_DEADLINE": "date",
			"DEADLINE_ASSGNMNT": "date",
			"TCKR_SYMB": str,
			"ISIN_ORIGIN": str,
			"ISIN_RIGHT": str,
			"ISIN_SBSCRPTN": str,
	...
	def _log_page_progress(self, int_page: int, int_rows: int) -> None:
		...
		self.cls_create_log.log_message(
			logger=self.logger,
			message=(f"B3BdiSecuritiesFungibleCustody: page {int_page} fetched ({int_rows} rows)"),
			log_level="info",
		)
	...
	def transform_data(self, data: dict) -> pd.DataFrame:
		...
		row has 7 elements but only 6 column definitions — the 7th element is always
		...
		-------
		pd.DataFrame
			DataFrame with UPPER_SNAKE_CASE column names (6 columns), or empty
