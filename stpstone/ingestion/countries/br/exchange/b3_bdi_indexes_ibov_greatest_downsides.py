...
class B3BdiIndexesIbovGreatestDownsides(ABCIngestionOperations):
	"""B3 BDI IBOVESPA equities with the largest percentage losses ingestion class."""
	...
	def __init__(
		...
		int_page_max : int, optional
			...
			typically returns all rows on a single page.
		...
		self.url_tpl = (
			f"https://arquivos.b3.com.br/bdi/table/IbovespaStockBiggestLow/"
	...
	def run(
		...
		str_table_name: str = "br_b3_bdi_indexes_ibov_greatest_downsides",
		...
		str_table_name : str, optional
			The name of the table, by default "br_b3_bdi_indexes_ibov_greatest_downsides".
	...
	def _log_page_progress(self, int_page: int, int_rows: int) -> None:
		...
		self.cls_create_log.log_message(
			logger=self.logger,
			message=(
				f"B3BdiIndexesIbovGreatestDownsides: page {int_page} fetched ({int_rows} rows)"
			),
			log_level="info",
		)
	...
	def transform_data(self, data: dict) -> pd.DataFrame:
		...
		The 5th element in each values row (always null) is dropped by selecting
		only the columns defined in the columns list.
