...
class B3BdiBtbRenewals(ABCIngestionOperations):
	"""B3 BDI securities lending contract renewals ingestion class."""
	...
	def __init__(
		...
		int_page_max : int, optional
			Last page to fetch inclusive; defaults to 1 for the initial fetch.
		...
		self.url_tpl = (
			f"https://arquivos.b3.com.br/bdi/table/Renewals/"
	...
	def run(
		...
		str_table_name: str = "br_b3_bdi_btb_renewals",
		...
		str_table_name : str, optional
			The name of the table, by default "br_b3_bdi_btb_renewals".
		...
		dict_dtypes = {
			"DT_REF": "date",
			"TCKR_SYMB": str,
			"ASSET_OBJECT_ISIN": str,
			"BTBCONTRACT_QUANTITY": int,
			"BTBCONTRACT_FINANCIAL_VALUE": float,
			"REMUNERATION_RATE_PERCENT": float,
			"TRADE_NUMBER": int,
			"MARKET_TYPE": str,
			"RENEWAL_TYPE": str,
			"LENDER_PARTICIPANT_CODE": str,
			"LENDER_PARTICIPANT_NAME": str,
			"BORROWER_PARTICIPANT_CODE": str,
			"BORROWER_PARTICIPANT_NAME": str,
			"URL": str,
		}
		...
		df_["BTBCONTRACT_FINANCIAL_VALUE"] = df_["BTBCONTRACT_FINANCIAL_VALUE"].apply(
			lambda x: Decimal(str(x)).quantize(Decimal("0.01"), rounding=ROUND_DOWN)
			if pd.notna(x)
			else x
		)
		df_["REMUNERATION_RATE_PERCENT"] = df_["REMUNERATION_RATE_PERCENT"].apply(
			lambda x: Decimal(str(x)).quantize(Decimal("0.01"), rounding=ROUND_DOWN)
			if pd.notna(x)
			else x
		)
	...
	def _log_page_progress(self, int_page: int, int_rows: int) -> None:
		...
		self.cls_create_log.log_message(
			logger=self.logger,
			message=(f"B3BdiBtbRenewals: page {int_page} fetched ({int_rows} rows)"),
			log_level="info",
		)
	...
	def transform_data(self, data: dict) -> pd.DataFrame:
		...
		Column names are converted from PascalCase to UPPER_SNAKE_CASE. Group columns
		(isGroup: true) appear as nulls in the values array; trailing extra nulls beyond
		the defined column count are dropped by selecting only the named columns.
		...
		-------
		pd.DataFrame
			DataFrame with UPPER_SNAKE_CASE column names, or empty DataFrame when
