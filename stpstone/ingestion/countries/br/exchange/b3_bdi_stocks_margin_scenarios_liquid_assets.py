"""B3 BDI Margin Scenarios for Liquid Assets ingestion."""
...
class B3BdiStocksMarginScenariosLiquidAssets(ABCIngestionOperations):
	"""B3 BDI Margin Scenarios for Liquid Assets ingestion class."""
	...
	def __init__(
		...
		int_page_max : Optional[int], optional
			Last page to fetch inclusive; defaults to None which fetches until the
		...
		self.url_tpl = (
			f"https://arquivos.b3.com.br/bdi/table/MarginScenarios/"
	...
	def run(
		...
		str_table_name: str = "br_b3_bdi_stocks_margin_scenarios_liquid_assets",
		...
		str_table_name : str, optional
			...
			"br_b3_bdi_stocks_margin_scenarios_liquid_assets".
	...
	def _log_page_progress(self, int_page: int, int_rows: int) -> None:
		...
		self.cls_create_log.log_message(
			logger=self.logger,
			message=(
				f"B3BdiStocksMarginScenariosLiquidAssets: page {int_page} fetched "
				f"({int_rows} rows)"
			),
			log_level="info",
		)
	...
	def transform_data(self, data: dict) -> pd.DataFrame:
		...
		Column names are converted from PascalCase to UPPER_SNAKE_CASE. When the API
		returns more values per row than declared columns (e.g. a hidden RptDt column
		at position 0), the extra values are silently dropped because the rename map
		only covers the declared column positions.
