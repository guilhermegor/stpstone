...
class B3BdiDerivativesSummaryTrades(ABCIngestionOperations):
	"""B3 BDI derivatives transactions summary ingestion class."""
	...
	def __init__(
		...
		int_page_max : int
			...
			typically returns all summary rows on a single page.
	...
	def run(
		...
		str_table_name: str = "br_b3_bdi_derivatives_summary_trades",
		...
		str_table_name : str
			...
			"br_b3_bdi_derivatives_summary_trades".
	...
	def _log_page_progress(self, int_page: int, int_rows: int) -> None:
		...
		self.cls_create_log.log_message(
			logger=self.logger,
			message=(f"B3BdiDerivativesSummaryTrades: page {int_page} fetched ({int_rows} rows)"),
			log_level="info",
		)
	...
	def transform_data(self, data: dict) -> pd.DataFrame:
		...
		row has 11 elements but only 10 column definitions — the 11th element is always
		...
		-------
		pd.DataFrame
			DataFrame with UPPER_SNAKE_CASE column names (10 columns), or empty
