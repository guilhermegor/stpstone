...
class B3BdiBtbLoanBalance(ABCIngestionOperations):
	"""B3 BDI securities lending registered loan balance ingestion class."""
	...
	def __init__(
		...
		int_page_max : int, optional
			...
			returns all loan balance rows on a single page.
		...
		self.url_tpl = (
			f"https://arquivos.b3.com.br/bdi/table/BTBLoanBalance/"
	...
	def run(
		...
		str_table_name: str = "br_b3_bdi_btb_loan_balance",
		...
		str_table_name : str, optional
			The name of the table, by default "br_b3_bdi_btb_loan_balance".
		...
		for col in (
			...
			df_[col] = df_[col].apply(
				lambda x: Decimal(str(x)).quantize(Decimal("0.0001"), rounding=ROUND_DOWN)
				if pd.notna(x)
				else x
			)
		df_["BRLVALUE"] = df_["BRLVALUE"].apply(
	...
	def _log_page_progress(self, int_page: int, int_rows: int) -> None:
		...
		self.cls_create_log.log_message(
			logger=self.logger,
			message=(f"B3BdiBtbLoanBalance: page {int_page} fetched ({int_rows} rows)"),
			log_level="info",
		)
	...
	def transform_data(self, data: dict) -> pd.DataFrame:
		...
		(isGroup: true) appear as nulls in the values array and are included in the
		column mapping so the positional selection works correctly.
