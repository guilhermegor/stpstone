"""B3 BDI Daily Average Stocks ingestion."""
...
class B3BdiStocksSummary(ABCIngestionOperations):
	"""B3 BDI Daily Average Stocks ingestion class."""
	...
	def __init__(
		...
		int_page_max : int
			...
			is non-paginated (returns the same 9 summary rows on every page).
		...
		self.url_tpl = (
			f"https://arquivos.b3.com.br/bdi/table/DailyAverageStocks/"
	...
	def run(
		...
		str_table_name: str = "br_b3_bdi_stocks_summary",
		...
		str_table_name : str
			The name of the table, by default "br_b3_bdi_stocks_summary".
	...
	def _log_page_progress(self, int_page: int, int_rows: int) -> None:
		...
		self.cls_create_log.log_message(
			logger=self.logger,
			message=(f"B3BdiStocksSummary: page {int_page} fetched ({int_rows} rows)"),
			log_level="info",
		)
