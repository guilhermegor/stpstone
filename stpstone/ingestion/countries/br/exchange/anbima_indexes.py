...
class AnbimaIndexesMTM(ABCIngestionOperations):
    """Anbima Indexes Market-to-Market."""
    ...
    def parse_raw_file(
        ...
        for line in file_io:
            ...
            if line.strip() == '':
                break
            # Append the line as-is, removing trailing whitespace
    ...
    def transform_data(
        ...
        """
        return pd.read_csv(file, sep="@", skiprows=3, engine="python", decimal=",", na_values="--",
...
class AnbimaIndexesPortfComp(ABCIngestionOperations):
    """Anbima Indexes Portfolio Composition."""
    ...
    def parse_raw_file(
        ...
        file_io.seek(0)
        for line in file_io:
            # Check for blank line to start collecting data
            if line.strip() == "":
                bool_blank_row_found = True
                continue
            # Skip lines before the first blank line
            if not bool_blank_row_found:
                continue
            # Append non-blank lines after the first blank line
            list_filtered_lines.append(line.rstrip())
    ...
    def transform_data(
        ...
        pd.DataFrame
            The transformed DataFrame.
        """
        return pd.read_csv(file, sep="@", skiprows=2, engine="python", decimal=",", na_values="--",
                           encoding="latin-1", on_bad_lines="skip",
                           names=[
                               "DATA_REFERENCIA",
                               "INDICE",
