...
class FIFFactSheet(ABCIngestionOperations):
    ...
    def parse_raw_file(
        ...
        # Look for the main fact sheet file (lamina_fi_YYYYMM.csv)
        main_csv_file = None
        main_filename = None
        ...
        if main_csv_file is None:
            # If exact match not found, try to find any file starting with lamina_fi_
            for file_content, filename in files_list:
        ...
        if main_csv_file is None:
            raise ValueError(f"Main fact sheet file {expected_main_filename} not found in the downloaded ZIP")
    ...
    def transform_data(
        ...
        # For now, we'll use the date reference to construct it
