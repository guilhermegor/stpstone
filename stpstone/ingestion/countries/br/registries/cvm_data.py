...
class CVMDataBanksRegistry(ABCIngestionOperations):
    ...
    def parse_raw_file(
        ...
        # Look for the financial intermediaries file (cad_intermed.csv)
        ...
        if intermed_csv_file is None:
            # If exact match not found, try to find any CSV file
            for file_content, filename in files_list:
                if filename.lower().endswith('.csv'):
                    intermed_csv_file = file_content
                    intermed_filename = filename
        ...
        if intermed_csv_file is None:
            raise ValueError("Financial intermediaries file cad_intermed.csv not found in the downloaded ZIP")
        ...
        self.cls_create_log.log_message(
            self.logger,
            f"Successfully extracted and decoded financial intermediaries CSV file: {intermed_filename}",
            "info"
        )
    ...
    def transform_data(
        ...
        try:
            # Read CSV with semicolon separator and handle potential data issues
            df_ = pd.read_csv(file, sep=";", dtype=str)
        ...
        active_institutions = df_[df_["SIT"] == "EM FUNCIONAMENTO NORMAL"].shape[0] if "SIT" in df_.columns else 0
        cancelled_institutions = df_[df_["SIT"] == "CANCELADA"].shape[0] if "SIT" in df_.columns else 0
        
        self.cls_create_log.log_message(
            self.logger,
            f"Financial intermediaries summary: {active_institutions} active, {cancelled_institutions} cancelled",
            "info"
        )
