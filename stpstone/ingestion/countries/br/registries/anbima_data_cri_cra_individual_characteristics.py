...
class AnbimaDataCRICRAIndividualCharacteristics(ABCIngestionOperations):
	...
	def get_response(
		...
		with sync_playwright() as p:
			...
			for asset_code in self.list_asset_codes:
				self.cls_create_log.log_message(
					self.logger, f"📊 Fetching characteristics for: {asset_code}...", "info"
				)
	...
	def _extract_characteristics_data(
		self, page: PlaywrightPage, asset_code: str
	) -> dict[str, Any]:
	...
	def _extract_single_field(
		self, page: PlaywrightPage, xpath: str, field_name: str, asset_code: str
	) -> Optional[str]:
	...
	def _extract_coordinators(self, page: PlaywrightPage, asset_code: str) -> CoordinatorResult:
		...
		try:
			...
			self.cls_create_log.log_message(
				self.logger, f"📋 Found {len(names)} coordinators for asset {asset_code}", "info"
			)
