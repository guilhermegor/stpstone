...
class CVMDataBanksRegistry(ABCIngestionOperations):
	...
	def parse_raw_file(
		...
		if intermed_csv_file is None:
			raise ValueError(
				"Financial intermediaries file cad_intermed.csv not found in the downloaded ZIP"
			)
	...
	def transform_data(
		...
		active_institutions = (
			df_[df_["SIT"] == "EM FUNCIONAMENTO NORMAL"].shape[0] if "SIT" in df_.columns else 0
		)
