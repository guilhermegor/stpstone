...
class AnbimaDataDebenturesDocuments(ABCIngestionOperations):
	...
	def get_response(
		...
		if not self.debenture_codes:
			self.cls_create_log.log_message(
				self.logger, "No debenture codes provided. Cannot scrape documents.", "warning"
			)
		...
		with sync_playwright() as p:
			...
			for debenture_code in self.debenture_codes:
				self.cls_create_log.log_message(
					self.logger, f"Fetching documents for: {debenture_code}...", "info"
				)
	...
	def _extract_debenture_data(self, page: PlaywrightPage, nome: str) -> list[dict]:
	...
	def _find_document_containers(
		...
		if len(document_containers) == 0:
			self.cls_create_log.log_message(
				self.logger, f"No document containers found for {codigo_debenture}", "warning"
			)
		else:
			self.cls_create_log.log_message(
				self.logger,
			...
			if len(link_elements) > 0:
				...
				return [
					self._create_document_record(
						codigo_debenture, emissor, setor, nome_documento, data_divulgacao, None
					)
				]
	...
	def _extract_document_name(self, container: Locator) -> Optional[str]:
		...
		"""
		nome_documento = self._try_extract_by_xpath(container, './/p[@class="large-text-bold"]')
	...
	def _try_extract_by_xpath(self, container: Locator, xpath: str) -> Optional[str]:
	...
	def _extract_document_name_from_link(self, container: Locator) -> Optional[str]:
		...
		try:
			...
			try:
				...
				self.cls_create_log.log_message(
					self.logger, f"Error extracting document name from URL: {e}", "warning"
				)

		except Exception as e:
			self.cls_create_log.log_message(
				self.logger, f"Error extracting document name from link URL: {e}", "warning"
			)

		return None

	def _extract_document_date(self, container: Locator) -> Optional[str]:
	...
	def _process_document_links(
		...
		self.cls_create_log.log_message(
			self.logger,
			f"Document {container_idx + 1} ({nome_documento}): Found {len(link_elements)} links",
			"info",
		)
	...
	def _extract_link_url(
		self, link_element: Locator, page: PlaywrightPage, link_idx: int, total_links: int
	) -> Optional[str]:
		...
		try:
			...
			self.cls_create_log.log_message(
				self.logger, f"  Link {link_idx + 1}/{total_links}: {link_document}", "info"
			)
			...
			self.cls_create_log.log_message(
				self.logger, f"  Error extracting link {link_idx + 1}: {e}", "warning"
			)
			return None
