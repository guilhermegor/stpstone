...
class AnbimaDataCRICRAPricesWS(ABCIngestionOperations):
		...
		with sync_playwright() as p:
			...
			for page_num in range(self.start_page, self.end_page + 1):
				...
				for idx, _ in enumerate(price_articles):
					try:
						price_data = self._extract_price_data(page, idx + 1, data_referencia)
	...
	def _click_precos_button(self, page: PlaywrightPage, timeout_ms: int) -> None:
		...
		try:
			...
			if button_xpath.count() > 0 and button_xpath.is_visible(timeout=5_000):
				...
				self.cls_create_log.log_message(
					self.logger, "✅ Clicked on Preços button (method 1: XPath)", "info"
				)
				return
		except Exception as e:
			self.cls_create_log.log_message(self.logger, f"Method 1 failed: {e}", "warning")
		...
		try:
			...
			if button.count() > 0 and button.is_visible(timeout=5_000):
				...
				self.cls_create_log.log_message(
					self.logger, "✅ Clicked on Preços button (method 2: CSS selector)", "info"
				)
				return
		except Exception as e:
			self.cls_create_log.log_message(self.logger, f"Method 2 failed: {e}", "warning")
		...
		try:
			...
			if button_text.count() > 0 and button_text.is_visible(timeout=5_000):
				...
				self.cls_create_log.log_message(
					self.logger, "✅ Clicked on Preços button (method 3: aria role)", "info"
				)
				return
		except Exception as e:
			self.cls_create_log.log_message(self.logger, f"Method 3 failed: {e}", "warning")

		self.cls_create_log.log_message(
			self.logger, "❌ Could not click Preços button with any method", "error"
		)
	...
	def _extract_data_referencia(self, page: PlaywrightPage) -> Optional[str]:
		...
		try:
			...
			self.cls_create_log.log_message(
				self.logger, f"Error extracting DATA_REFERENCIA: {e}", "warning"
			)

		return None

	def _extract_price_data(
		self, page: PlaywrightPage, item_index: int, data_referencia: Optional[str]
	) -> dict:
		...
		base_xpath = f'//*[@id="__next"]/main/div/div/div/div/div[3]/article[{item_index}]'
	...
	def _extract_tipo_price(self, page: PlaywrightPage, item_index: int) -> dict:
		...
		try:
			...
			self.cls_create_log.log_message(
				self.logger, f"Error extracting IS_CRI_CRA for item {item_index}: {e}", "warning"
			)

		return result

	def _extract_pu_indicativo(
		self, page: PlaywrightPage, item_index: int, base_xpath: str
	) -> dict:
	...
	def _extract_link(self, page: PlaywrightPage, item_index: int, base_xpath: str) -> dict:
		...
		try:
			...
			self.cls_create_log.log_message(
				self.logger, f"Error extracting link for item {item_index}: {e}", "warning"
			)
	...
	def transform_data(self, raw_data: list) -> pd.DataFrame:
		...
		for col_ in ["DATA_REFERENCIA", "DATA_EMISSAO", "DATA_VENCIMENTO", "DATA_REFERENCIA_NTNB"]:
