...
class AnbimaDataDebenturesEvents(ABCIngestionOperations):
	...
	def get_response(
		...
		if not self.debenture_codes:
			self.cls_create_log.log_message(
				self.logger, "No debenture codes provided. Cannot scrape events.", "warning"
			)
		...
		with sync_playwright() as p:
			...
			for debenture_code in self.debenture_codes:
				self.cls_create_log.log_message(
					self.logger, f"Fetching events for: {debenture_code}...", "info"
				)
	...
	def _extract_debenture_events(
		...
		event_records = self._extract_event_rows(page, codigo_debenture, emissor, setor, url)
	...
	def _extract_event_rows(
		...
		if not data_evento_elements:
			self.cls_create_log.log_message(
				self.logger, f"No event data found for {codigo_debenture}", "warning"
			)
			return event_records
