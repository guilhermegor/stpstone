...
class AnbimaDataDebenturesAvailable(ABCIngestionOperations):
	...
	def get_response(
		...
		self.cls_create_log.log_message(
			self.logger, f"Scraping finished. Total: {len(list_pages_data)} pages found.", "info"
		)

		return list_pages_data

	def _extract_debenture_data(self, page: PlaywrightPage, id_number: str, nome: str) -> dict:
