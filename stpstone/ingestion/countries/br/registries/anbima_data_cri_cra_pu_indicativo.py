...
class AnbimaDataCRICRAPUIndicativo(ABCIngestionOperations):
	...
	def get_response(
		...
		with sync_playwright() as p:
			...
			for asset_code in self.list_asset_codes:
				self.cls_create_log.log_message(
					self.logger, f"📊 Fetching PU Indicativo prices for: {asset_code}...", "info"
				)
				...
				try:
					...
					prices_data = self._extract_pu_indicativo_prices(page, asset_code, url)
	...
	def _extract_price_rows(
		...
		if not data_ref_elements:
			self.cls_create_log.log_message(
				self.logger, f"No PU Indicativo price data found for {cod_ativo}", "warning"
			)
			return price_records
