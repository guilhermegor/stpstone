...
class AnbimaDataCRICRAEvents(ABCIngestionOperations):
	...
	def get_response(
		...
		if not self.list_asset_codes:
			self.cls_create_log.log_message(
				self.logger, "⚠️ No asset codes provided. Cannot scrape events.", "warning"
			)
		...
		with sync_playwright() as p:
			...
			self.cls_create_log.log_message(
				self.logger,
				f"🚀 Starting events scraping for {len(self.list_asset_codes)} CRI/CRA assets...",
				"info",
			)

			for asset_code in self.list_asset_codes:
				self.cls_create_log.log_message(
					self.logger, f"📊 Fetching events for: {asset_code}...", "info"
				)
				...
				try:
					...
					for page_num in range(1, total_pages + 1):
						...
						events_data = self._extract_cri_cra_events(page, asset_code, current_url)
	...
	def _extract_cri_cra_events(
		...
		event_records = self._extract_event_rows(page, cod_ativo, is_cri_cra, url)
	...
	def _extract_event_rows(
		...
		if not data_evento_elements:
			self.cls_create_log.log_message(
				self.logger, f"No event data found for {cod_ativo}", "warning"
			)
			return event_records

		self.cls_create_log.log_message(
			self.logger, f"Found {len(data_evento_elements)} event records for {cod_ativo}", "info"
		)
