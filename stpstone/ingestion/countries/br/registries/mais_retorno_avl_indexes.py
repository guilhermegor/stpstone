"""Mais Retorno Available Indexes Listing."""
...
class MaisRetornoAvlIndexes(ABCIngestionOperations):
	...
	def __init__(
		...
		"""Initialize the Mais Retorno Available Indexes ingestion class.
	...
	def run(
		...
		str_table_name: str = "br_mais_retorno_available_indexes",
		...
		str_table_name : str
			The name of the table, by default 'br_mais_retorno_available_indexes'.
	...
	def transform_data(self, scraper_playwright: PlaywrightScraper) -> pd.DataFrame:
		"""Scrape paginated index listings across all configured page slugs.
		...
		-------
		pd.DataFrame
			DataFrame with index listing data across all pages.
		"""
		list_ser = []
			...
			for slug in self.list_slugs:
				...
				while True:
					if not scraper_playwright.selector_exists(
						self._XPATH_P_INDEX_CODE.format(i),
						selector_type="xpath",
	...
	def _extract_row(self, i: int, scraper: PlaywrightScraper) -> dict:
		"""Extract a single index row from the current page.
		...
		p_index_name = scraper.get_element(
			self._XPATH_P_INDEX_NAME.format(i), selector_type="xpath"
		)
		p_index_code = scraper.get_element(
			self._XPATH_P_INDEX_CODE.format(i), selector_type="xpath"
		)
		href_index_url = scraper.get_element_attrb(
			self._XPATH_HREF_INDEX_URL.format(i),
			str_attribute="href",
			selector_type="xpath",
		)
		...
		return {
			...
			"URL_INDEX": "https://maisretorno.com/" + href_index_url
			if href_index_url is not None
			else None,
			"STATUS": p_status.get("text", None) if p_status else None,
			"PAGE_POSITION": i,
		}
