"""Brazilian IRS (Receita Federal) simplified taxation system (Simples) open data ingestion.

Fetches the single ZIP file (Simples.zip) from the Receita Federal NextCloud
...
class IRSBRSimplifiedTaxation(ABCIngestionOperations):
	"""Brazilian IRS (Receita Federal) simplified taxation system (Simples) ingestion class."""
	...
	def run(
		...
		str_table_name: str = "br_irs_simp_tax_sys",
	) -> Optional[pd.DataFrame]:
		"""Run the ingestion process.

		Fetches ``Simples.zip``, parses the CSV inside, and returns the
		resulting DataFrame. If a database session is provided the data
		is inserted into the database; otherwise the transformed DataFrame
		is returned.
		...
		str_table_name : str
			The name of the table, by default 'br_irs_simp_tax_sys'.
