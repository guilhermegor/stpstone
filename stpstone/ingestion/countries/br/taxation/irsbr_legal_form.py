"""Brazilian IRS (Receita Federal) legal form (Naturezas) open data ingestion.

Fetches the single ZIP file (Naturezas.zip) from the Receita Federal NextCloud
...
class IRSBRLegalForm(ABCIngestionOperations):
	"""Brazilian IRS (Receita Federal) legal form (Naturezas) open data ingestion class."""
	...
	def run(
		...
		str_table_name: str = "br_irs_legal_form",
	) -> Optional[pd.DataFrame]:
		"""Run the ingestion process.

		Fetches ``Naturezas.zip``, parses the CSV inside, and returns the
		...
		str_table_name : str
			The name of the table, by default 'br_irs_legal_form'.
