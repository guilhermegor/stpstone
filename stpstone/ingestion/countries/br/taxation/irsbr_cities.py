"""Brazilian IRS (Receita Federal) cities (Municipios) open data ingestion.

Fetches the single ZIP file (Municipios.zip) from the Receita Federal
NextCloud share for a given reference month, extracts the semicolon-delimited
CSV inside the archive, and returns a DataFrame with standardised column names
and types.
"""
...
class IRSBRCities(ABCIngestionOperations):
	"""Brazilian IRS (Receita Federal) cities (Municipios) open data ingestion class."""
	...
	def run(
		...
		str_table_name: str = "br_irs_cities",
	) -> Optional[pd.DataFrame]:
		"""Run the ingestion process.

		Fetches ``Municipios.zip``, parses the CSV inside, and returns a
		DataFrame. If a database session is provided the data is inserted
		into the database; otherwise the transformed DataFrame is returned.
		...
		str_table_name : str
			The name of the table, by default 'br_irs_cities'.
