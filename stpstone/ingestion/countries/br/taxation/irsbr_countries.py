"""Brazilian IRS (Receita Federal) countries (Paises) open data ingestion.

Fetches the single ZIP file (Paises.zip) from the Receita Federal NextCloud
...
class IRSBRCountries(ABCIngestionOperations):
	"""Brazilian IRS (Receita Federal) countries (Paises) open data ingestion class."""
	...
	def run(
		...
		str_table_name: str = "br_irs_countries",
	) -> Optional[pd.DataFrame]:
		"""Run the ingestion process.

		Fetches ``Paises.zip``, parses the CSV inside, and returns the
		...
		str_table_name : str
			The name of the table, by default 'br_irs_countries'.
