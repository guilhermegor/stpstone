"""Brazilian IRS (Receita Federal) businesses (Estabelecimentos) open data ingestion.

Fetches the ten numbered ZIP files (Estabelecimentos0.zip through
Estabelecimentos9.zip) from the Receita Federal NextCloud share for a given
reference month, extracts the semicolon-delimited CSV inside each archive, and
returns a single concatenated DataFrame with standardised column names and types.
"""
...
_FILE_PREFIX: str = "Estabelecimentos"
...
_COLUMN_NAMES: list[str] = [
	...
	"ID_REASON_REGISTRATION_STATUS",
	"CITY_FOREIGN_COUNTRY",
	"COUNTRY",
	"DT_INCEPTION",
	"NCEA_MAIN",
	"NCEA_SECONDARY",
...
class IRSBRBusinesses(ABCIngestionOperations):
	"""Brazilian IRS (Receita Federal) businesses (Estabelecimentos) open data ingestion class."""
	...
	def run(
		...
		str_table_name: str = "br_irs_businesses",
	) -> Optional[pd.DataFrame]:
		"""Run the ingestion process.

		Iterates over files ``Estabelecimentos0.zip`` through
		``Estabelecimentos9.zip``, fetches each ZIP, parses the CSV inside,
		and concatenates all results into a single DataFrame. If a database
		session is provided the data is inserted into the database; otherwise
		...
		str_table_name : str
			The name of the table, by default 'br_irs_businesses'.
