"""Brazilian IRS (Receita Federal) companies (Empresas) open data ingestion.

Fetches the ten numbered ZIP files (Empresas0.zip through Empresas9.zip) from the
...
class IRSBRCompanies(ABCIngestionOperations):
	"""Brazilian IRS (Receita Federal) companies (Empresas) open data ingestion class."""
	...
	def run(
		...
		str_table_name: str = "br_irs_companies",
	) -> Optional[pd.DataFrame]:
		"""Run the ingestion process.

		Iterates over files ``Empresas0.zip`` through ``Empresas9.zip``,
		fetches each ZIP, parses the CSV inside, and concatenates all
		results into a single DataFrame. If a database session is
		provided the data is inserted into the database; otherwise the
		...
		str_table_name : str
			The name of the table, by default 'br_irs_companies'.
