"""Brazilian IRS (Receita Federal) registration status (Motivos) open data ingestion.

Fetches the single ZIP file (Motivos.zip) from the Receita Federal NextCloud
...
class IRSBRRegistrationStatus(ABCIngestionOperations):
	"""Brazilian IRS (Receita Federal) registration status (Motivos) open data ingestion class."""
	...
	def run(
		...
		str_table_name: str = "br_irs_registration_status",
	) -> Optional[pd.DataFrame]:
		"""Run the ingestion process.

		Fetches ``Motivos.zip``, parses the CSV inside, and returns the
		...
		str_table_name : str, optional
			The name of the table, by default 'br_irs_registration_status'.
