...
class B3BdiFixedIncomeDebenturesRepurchaseAgreements(ABCIngestionOperations):
	...
	def run(
		...
		str_table_name : str, optional
			The name of the table, by default "br_b3_bdi_fixed_income_debentures_repurchase_agreements".
	...
	def _log_page_progress(self, int_page: int, int_rows: int) -> None:
		...
		self.cls_create_log.log_message(
			logger=self.logger,
			message=(
				f"B3BdiFixedIncomeDebenturesRepurchaseAgreements: page {int_page} fetched ({int_rows} rows)"
			),
			log_level="info",
		)
