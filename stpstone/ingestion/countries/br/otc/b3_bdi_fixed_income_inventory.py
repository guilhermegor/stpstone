"""B3 BDI OTC Position Inventory ingestion."""
...
class B3BdiFixedIncomeInventory(ABCIngestionOperations):
	"""B3 BDI OTC Position Inventory ingestion class."""
	...
	def __init__(
		...
		self.url_tpl = (
			f"https://arquivos.b3.com.br/bdi/table/Stock/"
	...
	def run(
		...
		str_table_name: str = "br_b3_bdi_fixed_income_inventory",
		...
		str_table_name : str
			The name of the table, by default "br_b3_bdi_fixed_income_inventory".
		...
		dict_dtypes = {
			...
			"QUANTITY_ISSUED": int,
			"QUANTITY_MARKET": int,
			"QUANTITY_DEPOSITED": int,
			"VOLUME_MARKET": float,
			"VOLUME_DEPOSITED": float,
	...
	def _log_page_progress(self, int_page: int, int_rows: int) -> None:
		...
		self.cls_create_log.log_message(
			logger=self.logger,
			message=(f"B3BdiFixedIncomeInventory: page {int_page} fetched ({int_rows} rows)"),
			log_level="info",
		)
