...
class B3BdiDerivativesSwaps(ABCIngestionOperations):
	"""B3 BDI OTC flexible swap contracts ingestion class."""
	...
	def __init__(
		...
		self.url_tpl = (
			f"https://arquivos.b3.com.br/bdi/table/SwapFlex/"
	...
	def run(
		...
		str_table_name: str = "br_b3_bdi_derivatives_swaps",
		...
		str_table_name : str, optional
			The name of the table, by default "br_b3_bdi_derivatives_swaps".
		...
		for col in ("NATIONAL_VOL", "AVRG_DAILY_RATE"):
	...
	def _log_page_progress(self, int_page: int, int_rows: int) -> None:
		...
		self.cls_create_log.log_message(
			logger=self.logger,
			message=(f"B3BdiDerivativesSwaps: page {int_page} fetched ({int_rows} rows)"),
			log_level="info",
		)
	...
	def transform_data(self, data: dict) -> pd.DataFrame:
		...
		row has 6 elements but only 5 column definitions — the 6th element is always
		...
		-------
		pd.DataFrame
			DataFrame with UPPER_SNAKE_CASE column names (5 columns), or empty
