...
class B3BdiDerivativesWccpRegistries(ABCIngestionOperations):
	"""B3 BDI OTC derivatives registration without CCP ingestion class."""
	...
	def __init__(
		...
		int_page_max : int
			...
			returns all registration rows on a single page.
		...
		self.url_tpl = (
			f"https://arquivos.b3.com.br/bdi/table/OTCRegistrationWCCP/"
	...
	def run(
		...
		str_table_name: str = "br_b3_bdi_derivatives_wccp_registries",
		...
		str_table_name : str
			...
			"br_b3_bdi_derivatives_wccp_registries".
		...
		df_["FINANCIAL_VOL"] = df_["FINANCIAL_VOL"].apply(
	...
	def _log_page_progress(self, int_page: int, int_rows: int) -> None:
		...
		self.cls_create_log.log_message(
			logger=self.logger,
			message=(f"B3BdiDerivativesWccpRegistries: page {int_page} fetched ({int_rows} rows)"),
			log_level="info",
		)
