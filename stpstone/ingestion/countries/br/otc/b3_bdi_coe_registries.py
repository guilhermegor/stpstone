...
class B3BdiCoeRegistries(ABCIngestionOperations):
	"""B3 BDI COE registration ingestion class."""
	...
	def __init__(
		...
		self.url_tpl = (
			f"https://arquivos.b3.com.br/bdi/table/COERegistration/"
	...
	def run(
		...
		str_table_name: str = "br_b3_bdi_coe_registries",
		...
		str_table_name : str
			The name of the table, by default "br_b3_bdi_coe_registries".
	...
	def _log_page_progress(self, int_page: int, int_rows: int) -> None:
		...
		self.cls_create_log.log_message(
			logger=self.logger,
			message=(f"B3BdiCoeRegistries: page {int_page} fetched ({int_rows} rows)"),
			log_level="info",
		)
