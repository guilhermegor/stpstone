"""B3 BDI OTC Consolidated Tradings ingestion."""
...
class B3BdiFixedIncomeConsolidatedTrades(ABCIngestionOperations):
	"""B3 BDI OTC Consolidated Tradings ingestion class."""
	...
	def __init__(
		...
		self.url_tpl = (
			f"https://arquivos.b3.com.br/bdi/table/ConsolidatedRecords/"
	...
	def run(
		...
		str_table_name: str = "br_b3_bdi_fixed_income_consolidated_trades",
		...
		str_table_name : str, optional
			The name of the table, by default "br_b3_bdi_fixed_income_consolidated_trades".
	...
	def _log_page_progress(self, int_page: int, int_rows: int) -> None:
		...
		self.cls_create_log.log_message(
			logger=self.logger,
			message=(
				f"B3BdiFixedIncomeConsolidatedTrades: page {int_page} fetched ({int_rows} rows)"
			),
			log_level="info",
		)
