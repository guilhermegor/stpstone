...
class B3BdiDerivativesElectronicForwards(ABCIngestionOperations):
	"""B3 BDI OTC electronic forward contracts ingestion class."""
	...
	def __init__(
		...
		self.url_tpl = (
			f"https://arquivos.b3.com.br/bdi/table/EletronicTerm/"
	...
	def run(
		...
		str_table_name: str = "br_b3_bdi_derivatives_electronic_forwards",
		...
		for col in ("NATIONAL_VOL", "INTERNATIONAL_VOL"):
	...
	def _log_page_progress(self, int_page: int, int_rows: int) -> None:
		...
		self.cls_create_log.log_message(
			logger=self.logger,
			message=(
				f"B3BdiDerivativesElectronicForwards: page {int_page} fetched ({int_rows} rows)"
			),
			log_level="info",
		)
