...
class B3BdiSettlement(ABCIngestionOperations):
	...
	def run(
		...
		str_table_name: str = "br_b3_bdi_settlement",
		...
		str_table_name : str, optional
			The name of the table, by default "br_b3_bdi_settlement".
	...
	def _log_page_progress(self, int_page: int, int_rows: int) -> None:
		...
		self.cls_create_log.log_message(
			logger=self.logger,
			message=(f"B3BdiSettlement: page {int_page} fetched ({int_rows} rows)"),
			log_level="info",
		)
