...
class CvmMonthlyReport:
	...
	# ------------------------------------------------------------------
	# XML building helpers (top-down call order)  # noqa: ERA001
	# ------------------------------------------------------------------
	...
	def _build_otc_lines(self, lista_otc: Optional[list[OtcOperation]], ind: str) -> list[str]:
	...
	def _parse_xml_tree(self, root: Element) -> PerfilMensalDocument:
		...
		rows = [
			self._parse_xml_row(row_el) for row_el in self._findall(perfil_mensal, "ROW_PERFIL")
		]
	...
	def _parse_xml_row(self, row_el: Element) -> PerfilMensalRow:
		...
		return PerfilMensalRow(
			...
			total_recurs_exter=self._parse_decimal(self._text(row_el, "TOTAL_RECURS_EXTER")),
			...
			var_patrim_fdo_n_preco_acoes=self._opt_decimal(row_el, "VAR_PATRIM_FDO_N_PRECO_ACOES"),
			...
			tot_ativos_p_relac=self._parse_decimal(self._text(row_el, "TOT_ATIVOS_P_RELAC")),
			lista_emissores_tit_cred_priv=self._parse_issuers_list(row_el),
			tot_ativos_cred_priv=self._parse_decimal(self._text(row_el, "TOT_ATIVOS_CRED_PRIV")),
			ved_regul_cobr_taxa_perform=self._opt_text(row_el, "VED_REGUL_COBR_TAXA_PERFORM"),
	...
	def _parse_nr_client(self, row_el: Element) -> ClientCount:
		...
		return ClientCount(
			...
			nr_soc_captlz_arrendm_merc=int(self._text(nr_el, "NR_SOC_CAPTLZ_ARRENDM_MERC")),
	...
	def _parse_distr_patrim(self, row_el: Element) -> Optional[PatrimonyDistribution]:
		...
		return PatrimonyDistribution(
			...
			pr_soc_captlz_arrendm_merc=self._opt_decimal(dp_el, "PR_SOC_CAPTLZ_ARRENDM_MERC"),
	...
	def _parse_var_outros(self, row_el: Element) -> Optional[VarOutros]:
		...
		return VarOutros(
			...
			val_percent_outros=self._parse_decimal(self._text(vo_el, "VAL_PERCENT_OUTROS")),
		)
	...
	def _parse_nominal_risk(self, row_el: Element) -> Optional[NominalRiskBlock]:
		...
		fnocs = [
			NominalRiskFactor(
				...
				val_fator_risco_noc_short=int(self._text(fn_el, "VAL_FATOR_RISCO_NOC_SHORT")),
			)
	...
	def _records_to_df(
		...
		if isinstance(records[0], BaseModel):
			...
			raise TypeError("List elements must be BaseModel instances or dicts.")
