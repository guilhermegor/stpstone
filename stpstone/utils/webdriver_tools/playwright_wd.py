...
class PlaywrightScraper(metaclass=TypeChecker):
	...
	def _minimize_window_os_level(self) -> None:
		...
		try:
			...
			CreateLog().log_message(self.logger, "Browser window minimized via OS command", "info")
	...
	def selector_exists(
		self, selector: str, timeout: Optional[int] = None, visible: Optional[bool] = None
	) -> bool:
	...
	def get_element_attrb(
		self, selector: str, attribute: str = "href", timeout: Optional[int] = None
	) -> Optional[str]:
	...
	def get_elements(self, selector: str, timeout: Optional[int] = None) -> list[ReturnGetElement]:
