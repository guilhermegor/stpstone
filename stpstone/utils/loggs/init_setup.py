...
def initiate_logging(logger: Logger, path_log: Optional[str] = None) -> None:
	...
	if path_log is not None:
		...
		log_creator.log_message(logger, f"Logs parent directory: {path_log}", "info")
		if dispatch is True:
			log_creator.log_message(logger, "Logs parent directory created successfully.", "info")
		elif dispatch is False:
			log_creator.log_message(logger, "Logs parent directory could not be created.", "info")
		else:
			raise RuntimeError(f"Unexpected dispatch value: {dispatch}") from None

	log_creator.log_message(
		logger, f"Routine started at {DatesBRAnbima().curr_datetime()}", "info"
	)
	log_creator.log_message(logger, f"Routine operator {getuser()}", "info")
