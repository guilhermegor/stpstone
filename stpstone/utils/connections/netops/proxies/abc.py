...
class ABCSession(ABC, metaclass=ABCMetaClass):
    ...
    def _test_proxy(self, str_ip:str, int_port:int, bool_return_availability:bool=True) -> bool:
        try:
        ...
        def retrieve_proxy():
            ...
            for dict_proxy in list_ser:
                ...
                if all([x is not None for x in [str_ip, int_port]]) == True:
                    if self._test_proxy(str_ip, int_port, bool_return_availability=True) == True:
    ...
    def ip_infos(self, session:Session, bool_return_availability:bool=False,
        ...
        if bool_return_availability == True:
            return True
        else:
