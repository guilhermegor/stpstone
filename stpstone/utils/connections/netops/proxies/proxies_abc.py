...
class ABCSession(ABC, metaclass=ABCTypeCheckerMeta):
	...
	def _validate_proxy_structure(
		self, list_proxies: list[dict[str, Union[str, int, float, bool]]]
	) -> None:
	...
	def _test_proxy(
		self, str_ip: str, int_port: int, bool_return_availability: bool = True
		...
		def retrieve_proxies() -> Optional[list[dict[str, str]]]:
			...
			for dict_proxy in list_proxies:
				...
				if str_ip and int_port:
					bool_test_proxy = self._test_proxy(
						str_ip, int(int_port), bool_return_availability=True
					)
			...
			self.cls_create_log.log_message(
				self.logger, f"Number of working proxies: {len(list_result)}", log_level="info"
			)
	...
	def _filtered_proxies(self) -> list[dict[str, Union[str, int, float, bool]]]:
		...
		self.cls_create_log.log_message(
			self.logger, f"Number of available proxies: {len(list_proxies)}", log_level="info"
		)
		...
		for key, value, strategy in [
			...
			(
				"ratio_times_alive_dead",
				self.float_min_ratio_times_alive_dead,
			...
			if value is not None:
				...
				list_proxies = HandlingDicts().filter_list_ser(
					list_proxies, key, value, str_filter_type=strategy
				)
				self.cls_create_log.log_message(
					self.logger,
	...
	def _dict_proxy(self, str_ip: str, int_port: int) -> dict[str, str]:
		...
		if int_port <= 0:
			raise ValueError("int_port must be positive")
		return {"http": f"http://{str_ip}:{int_port}", "https": f"http://{str_ip}:{int_port}"}
	...
	def session(self) -> Session:
		...
		dict_proxy = (
			self.dict_proxies
			if self.dict_proxies is not None
			else (self._dict_proxy(proxy["ip"], proxy["port"]) if proxy is not None else None)
		)
