...
class YieldFreeProxy(metaclass=TypeChecker):
	...
	def __next__(self) -> Session:
		...
		while (
			len(list_proxies) == 0 and time.time() - self._last_cache_time < self.timeout_session
		):
			self.cached_sessions = self._cache()
			list_proxies = self.cached_sessions
		if len(list_proxies) == 0 and time.time() - self._last_cache_time > self.timeout_session:
			self.create_logs.log_message(
				self.logger,
