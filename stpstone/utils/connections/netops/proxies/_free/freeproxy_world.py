			...
			try:
				...
				for i_tr in list_range:
					ip = cls_selenium_wd.find_element(
						web_driver, f"{self.xpath_tr}[{i_tr}]/td[1]"
					).text
					port = cls_selenium_wd.find_element(
						web_driver, f"{self.xpath_tr}[{i_tr}]/td[2]/a"
					).text
					country = cls_selenium_wd.find_element(
						web_driver, f"{self.xpath_tr}[{i_tr}]/td[3]//span[@class='table-country']"
					).text
					city = cls_selenium_wd.find_element(
						web_driver, f"{self.xpath_tr}[{i_tr}]/td[4]"
					).text
					speed = cls_selenium_wd.find_element(
						web_driver, f"{self.xpath_tr}[{i_tr}]//div[@class='n-bar-wrapper']/p/a"
					).text
					type_ = cls_selenium_wd.find_element(
						web_driver, f"{self.xpath_tr}[{i_tr}]/td[6]/a"
					).text
					anonymity = cls_selenium_wd.find_element(
						web_driver, f"{self.xpath_tr}[{i_tr}]/td[7]/a"
					).text.lower()
					anonymity = "elite" if anonymity == "high" else "transparent"
					last_checked = cls_selenium_wd.find_element(
						web_driver, f"{self.xpath_tr}[{i_tr}]/td[8]"
					).text
					...
					proxy_data: ReturnAvailableProxies = {
						...
						"ip_name": "",
						"timezone": ", ".join(
							WWTimezones().get_timezones_by_country_code(self.str_country_code)
						),
						"continent": WWGeography().get_continent_by_country_code(
							self.str_country_code
						),
						"continent_code": WWGeography()
						.get_continent_code_by_country_code(self.str_country_code)
						.upper(),
						"country": country,
