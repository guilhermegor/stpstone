...
class FreeProxyNet(ABCSession):
	...
	def _available_proxies(self) -> list[ReturnAvailableProxies]:
		...
		try:
			resp_req = request("GET", "https://free-proxy-list.net/", headers=headers, timeout=10)
			...
			for el_tr in el_trs:
				...
				str_protocol = (
					"https"
					if self.str_parser.match_string_like(str_protocol_code, "yes*")
					else "http"
				)
