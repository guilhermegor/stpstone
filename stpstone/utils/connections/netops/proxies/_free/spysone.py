...
class SpysOneCountry(ABCSession):
	...
	def _parse_proxy_row(
		self, row: WebElement, cls_selenium_wd: SeleniumWD
	) -> ReturnAvailableProxies:
		...
		timeout, uptime, ts_check_date = self._parse_performance_metrics(row, cls_selenium_wd)
	...
	def _parse_anonymity(self, row: WebElement, cls_selenium_wd: SeleniumWD) -> str:
		...
		return (
			"elite"
			if anonymity_text == "hia"
			else ("anonymous" if anonymity_text == "anm" else "transparent")
		)
