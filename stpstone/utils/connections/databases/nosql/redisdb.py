...
class RedisClient(metaclass=TypeChecker):
	...
	def __init__(
		self, str_host: str = "localhost", int_port: int = 6379, bl_decode_resp: bool = True
	) -> None:
	...
	def __new__(
		...
		**kwargs : Any
		...
		if not cls._instance:
			host = (
				kwargs.get("str_host", "localhost")
				if kwargs
				else (args[0] if len(args) > 0 else "localhost")
			)
			port = kwargs.get("int_port", 6379) if kwargs else (args[1] if len(args) > 1 else 6379)
			decode_resp = (
				kwargs.get("bl_decode_resp", True)
				if kwargs
				else (args[2] if len(args) > 2 else True)
			)
			try:
	...
	def _load_config(str_host: str, int_port: int, bl_decode_resp: bool) -> ReturnLoadConfig:
		...
		ReturnLoadConfig
			Configuration dictionary for Redis connection
		"""
		return {"host": str_host, "port": int_port, "decode_responses": bl_decode_resp}
