...
class MongoConn(metaclass=TypeChecker):
	...
	def __init__(
		...
		# Only initialize if not already initialized (singleton pattern)  # noqa: ERA001
		if not MongoConn._initialized:
			# Set attributes from the FIRST initialization (singleton pattern)  # noqa: ERA001
		...
		try:
			...
			if self.logger is not None:
				CreateLog().log_message(
					self.logger, f"Error: {err}; mongoDB injestion aborted", "info"
				)
