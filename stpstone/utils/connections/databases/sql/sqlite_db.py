...
class SQLiteDB(ABCDatabase):
	...
	def insert(
		...
		try:
			...
			CreateLog().log_message(
				self.logger,
				f"Successful commit in db {self.db_path} " + f"/ table {str_table_name}.",
				"info",
			)
		except Exception as err:
			self.conn.rollback()
	...
	def backup(self, str_backup_dir: str, str_bkp_name: Optional[str] = None) -> str:
		...
		if not self.check_bkp_tool():
			error_msg = (
				"Backup tool is required for backups, but it is not available and "
		...
		try:
			...
			backup_file = os.path.join(
				str_backup_dir, str_bkp_name or f"{os.path.basename(self.db_path)}.backup"
			)
	...
	def check_bkp_tool(self) -> bool:
		...
		try:
			if system == "linux":
				...
				error_msg = (
					f"Unsupported operating system: {system}. Please install sqlite3 manually."
				)
				CreateLog().log_message(self.logger, error_msg, "error")
				return False
