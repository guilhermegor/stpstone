...
class MT5(metaclass=TypeChecker):
	"""Adapter that fulfils the ``IMT5`` port for the MetaTrader5 trading platform.
	...
	------
	NotImplementedError
		If this class does not satisfy the ``IMT5`` port — i.e., if a required
	...
	def __init__(self, logger: Logger) -> None:
		...
		------
		NotImplementedError
			If this class does not satisfy the ``IMT5`` port.
		"""
		self.logger = logger
	...
	def package_info(self) -> None:
		...
		CreateLog().log_message(
			self.logger,
			f"MetaTrader5 package author: {mt5.__author__}",
			"info",
		)
		CreateLog().log_message(
			self.logger,
			f"MetaTrader5 package version: {mt5.__version__}",
			"info",
		)
