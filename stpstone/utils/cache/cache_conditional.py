...
class ConditionalCacheReset(metaclass=TypeChecker):
    ...
    def _validate_inputs(
        self, cache_clear_method: str, condition_file: Optional[str]
    ) -> None:
        ...
        if not isinstance(cache_clear_method, str):
            raise TypeError(f"cache_clear_method must be of type str, "
                            f"got {type(cache_clear_method).__name__}")
        ...
        if condition_file is not None and not isinstance(condition_file, str):
            raise TypeError("condition_file must be one of types: "
