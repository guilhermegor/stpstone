...
class HtmlHandler(metaclass=TypeChecker):
	...
	def bs_parser(
		self, resp_req: Response, parser: str = "html.parser"
	...
	def parse_html_to_string(
		...
		for element in soup.find_all(True):
			if element.name not in ["table", "tr", "td", "th"] and not element.find_parent(
				"table"
			):
...
class HtmlBuilder(metaclass=TypeChecker):
	...
	def tag(self, name: str, *content: str, cls: Optional[str] = None, **attrs: str) -> str:
		...
		**attrs : str
		...
		attr_str = (
			"".join(f' {attr}="{value}"' for attr, value in sorted(attrs.items())) if attrs else ""
		)
