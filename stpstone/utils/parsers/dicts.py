...
class HandlingDicts(metaclass=TypeChecker):
	...
	def multikeysort(self, items: list[dict], columns: list[str]) -> list[dict]:
		...
		def comparer(left: dict, right: dict) -> int:
			...
			"""
			comparer_iter = (self.cmp(fn(left), fn(right)) * mult for fn, mult in list_comparers)
	...
	def sum_values_selected_keys(
		...
		if list_keys_merge is not None:
			...
			if bool_sum_values_key:
				return {k: (sum(v) if isinstance(v, list) else v) for k, v in dict_.items()}
			return dict_
	...
	def filter_list_ser(
		...
		return [dict_ for dict_ in list_ser if filter_ops[str_filter_type](dict_[foreigner_key])]

	def merge_values_foreigner_keys(
		self, list_ser: list[dict], foreigner_key: str, list_keys_merge_dict: list[str]
	) -> list[dict]:
		...
		return [
			self.sum_values_selected_keys(
				self.filter_list_ser(list_ser, foreigner_key, key), list_keys_merge_dict
			)
			for key in list_foreinger_keys
		]

	def n_smallest(self, list_ser: list[dict], key_: str, n: int) -> list[dict]:
	...
	def n_largest(self, list_ser: list[dict], key_: str, n: int) -> list[dict]:
	...
	def add_key_value_to_dicts(
		...
		value: Optional[
			Union[Callable[..., Union[int, float, str]], Union[int, float, str]]
		] = None,
	...
	def pair_keys_with_values(
		...
		for sublist in list_lists:
			if len(sublist) != len(list_keys):
				raise ValueError(f"Sublist length {len(sublist)} != keys length {len(list_keys)}")
