...
class DealingPd:
    ...
    def append_df_to_Excel(
        ...
        Args:
            ...
            bool_header (bool): Whether to write headers
            bool_index (int): Whether to write index
            mode (str): Mode to open Excel file
            label_sensitivity (str): Label sensitivity
            bool_set_sensitivity_label (bool): Whether to set sensitivity label
            bool_debug_mode (bool): Whether to print debug messages
        ...
        if bool_debug_mode == True:
            ...
            print("BL_INDEX: {}".format(bool_index))
        with pd.ExcelWriter(filename, engine="xlsxwriter", mode=mode) as writer:
            for df_, sheet_name in list_tup_df_sheet_name:
                if bool_index == 0:
                    df_.reset_index(drop=True, inplace=True)
                df_.to_excel(writer, sheet_name, index=bool_index, headebool_header)
        if (bool_set_sensitivity_label == True) and (os.name == "nt"):
    ...
    def export_xl(
        ...
        Args:
            ...
            - bool_adjust_layout (bool): Whether to adjust layout of Excel file
            - bool_debug_mode (bool): Whether to print debug messages
    ...
    def settingup_pandas(
        ...
        Args:
            ...
            bool_wrap_repr (bool): Whether to wrap repr(DataFrame) across additional lines
    ...
    def convert_datetime_columns(
        ...
        Args:
            ...
            bool_pandas_convertion (bool): Whether to use pandas conversion or excel format
