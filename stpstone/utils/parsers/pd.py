...
class DealingPd(metaclass=TypeChecker):
	...
	def export_xl(
		...
		if not blame_xpt:
			CreateLog().log_message(
				logger, f"File not saved to hard drive: {path_xlsx}", "warning"
			)
	...
	def convert_datetime_columns(
		...
		if bool_pandas_convertion:
			for col_date in list_col_date:
				df_[col_date] = pd.to_datetime(df_[col_date]).dt.date.astype(object)
		else:
	...
	def merge_dfs_into_df(
		...
		df_merge = df_2.merge(df_intersec, how="outer", on=list_cols, indicator=True)
	...
	def max_chrs_per_column(self, df_: pd.DataFrame) -> dict[str, int]:
		...
		return {col_: df_[col_].astype(str).str.len().max() for col_ in df_.columns}
