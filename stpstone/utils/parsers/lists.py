...
class ListHandler(metaclass=TypeChecker):
	...
	def get_list_until_invalid_occurrences(
		self, list_: list[Any], list_invalid_values: list[Any]
	) -> list[Any]:
		...
		for el in list_:
			...
			if any(
				self.str_handler.match_string_like(
					el_processed, self.str_handler.remove_diacritics(str(inv)).lower()
				)
	...
	def get_lower_upper_bound(
		self, sorted_list: list[Number], value_to_put_in_between: Number
		...
		return {"lower_bound": sorted_list[bounds[0]], "upper_bound": sorted_list[bounds[1]]}
	...
	def closest_bound(self, sorted_list: list[Number], value_to_put_in_between: Number) -> Number:
	...
	def closest_number_within_list(self, list_: list[Number], number: Number) -> Number:
	...
	def first_occurrence_like(self, list_: list[str], str_like: str) -> int:
		...
		try:
			return list_.index(next(x for x in list_ if x.lower() == str_like_lower))
	...
	def nth_smallest_numbers(
		self, list_numbers: list[Number], nth_smallest: int
	) -> NDArray[np.float64]:
	...
	def chunk_list(
		...
		if str_join_char is None:
			return [
				list_to_chunk[x : x + int_chunk] for x in range(0, len(list_to_chunk), int_chunk)
			]
		...
		if len(list_position_chunks) > 2:
			for lim in list_position_chunks[2:]:
				if str_join_char is None:
					...
					list_chunked.append(str_join_char.join(list_to_chunk[inf_limit:sup_limit]))
				inf_limit = sup_limit
				sup_limit = lim
			if str_join_char is None:
				list_chunked.append(
					list_to_chunk[list_position_chunks[-2] : list_position_chunks[-1]]
				)
			else:
				list_chunked.append(
					str_join_char.join(
	...
	def cartesian_product(
		...
		for tup in products:
			...
			if (truncated not in result) and all(
				truncated[i] != truncated[i - 1] for i in range(1, len(truncated))
			):
	...
	def sort_alphanumeric(self, list_: list[str]) -> list[str]:
		...
		@type_checker
		def alphanum_key(key: str) -> list[Any]:  # noqa ANN401: typing.Any is not allowed
	...
	def validate_lists_same_length(self, dict_lists_data: dict[str, list[Union[Any]]]) -> None:
		...
		if len(unique_lengths) > 1:
			...
			self.cls_create_log.log_message(self.logger, error_msg, "error")
