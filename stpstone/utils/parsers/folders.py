...
class DirFilesManagement(metaclass=TypeChecker):
	...
	def compress_to_7z_file(
		self, file_path_7z: str, object_to_compress: str, method: str = "w"
	) -> bool:
	...
	def walk_folder_subfolder_w_rule(
		self, root_directory: str, list_name_like: list[str]
	) -> list[str]:
	...
	def loop_files_w_rule(
		...
		list_files_last_edition = [
			self.time_last_edition(
				os.path.join(directory, file_name), bool_to_datetime=bool_to_datetime
			)
		...
		return [
			...
			for dict_ in HandlingDicts().multikeysort(
				list_ser_file_name_last_edition, [f"-{key_file_last_edition}"]
			)
		]

	def find_project_root(
		self, marker: str = "pyproject.toml", start_path: Optional[str] = None
	) -> Path:
	...
	def get_zip_from_web_in_memory(
		self, resp_req: Response, bool_io_interpreting: bool = False
	...
	def recursive_unzip_in_memory(
		self, file: Union[BytesIO, ZipExtFile]
		...
		for idx, str_entry in enumerate(list_entries):
			...
			if bool_is_directory:
				...
				str_tree_structure += self.generate_tree(
					str_entry_path, str_prefix=str_new_prefix, bool_include_root=False
				)
