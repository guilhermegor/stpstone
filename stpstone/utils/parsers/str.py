...
TypeCaseTo = TypeVar(
	"TypeCaseTo",
	bound=Literal[
		"camel", "pascal", "snake", "kebab", "upper_constant", "lower_constant", "upper_first"
	],
)
...
class StrHandler(metaclass=TypeChecker):
	...
	def alphabetic_range(self, case: str = "upper") -> list:
		...
		if case == "upper":
			...
			raise ValueError(f"case must be either 'upper' or 'lower', got '{case}'")
	...
	def replace_respecting_case(self, str_: str, str_replaced: str, str_replace: str) -> str:
		...
		return re.sub(re.escape(str_replaced), matchcase, str_, flags=re.IGNORECASE)
	...
	def convert_case(self, str_: str, from_case: TypeCaseFrom, to_case: TypeCaseTo) -> str:
	...
	def get_url_query(
		self, url: str, bool_include_fragment: bool = False
