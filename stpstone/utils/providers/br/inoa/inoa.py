...
This module provides a class for interacting with the INOA Alpha Tools API to
fetch fund information and quotes data. It handles authentication, request
retries, and data formatting using pandas DataFrames.
...
class AlphaTools(metaclass=TypeChecker):
	"""Adapter that fulfils the ``IAlphaToolsClient`` port for INOA Alpha Tools.
	...
	------
	NotImplementedError
		If this class does not satisfy the ``IAlphaToolsClient`` port — i.e.,
	...
	def __init__(
		...
		------
		NotImplementedError
			If this class does not satisfy the ``IAlphaToolsClient`` port.
		"""
		...
		self.cls_dates = ABCCalendarOperations()
	...
	def _validate_dates(self, date_start: datetime, date_end: datetime) -> None:
		...
		------
		ValueError
			If start date is after end date or dates are not datetime objects.
		"""
	...
	def quotes(self, list_ids: list[int]) -> pd.DataFrame:
		...
		------
		ValueError
			If fund ID list is empty or API response is malformed.
		"""
		if not list_ids:
		...
		try:
			...
			df_quotes["date"] = [
				self.cls_dates.str_date_to_datetime(d, self.str_fmt_date_output)
