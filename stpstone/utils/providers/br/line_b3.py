...
class ConnectionApi(metaclass=TypeChecker):
    ...
    def access_token(
        ...
        bool_str_dict_params: bool = False,
        bool_verify: bool = False,
        i_retrieves: int = 0,
        i_aux: int = 0,
    ...
    def app_request(
        ...
        if bool_parse_dict_params_data == True:
            if dict_params != None:
        ...
        if bool_retry_if_error == True:
            while (bool_retry_request == True) and (i <= int_max_retrieves):
                if bool_debug_mode == True:
...
class Operations(ConnectionApi):
    ...
    def groups_authorized_markets(
        ...
        return self.app_request(
            self.token, method, app, bool_retry_if_errobool_retry_if_error
        )
    ...
    def intruments_per_group(
        ...
        group_id: str,
        method: str = "POST",
        str_bool_settled: str = "true",
        bool_parse_dict_params_data: bool = True,
        float_secs_sleep: Optional[float] = None,
        app: str = "/api/v1.0/exchangeLimits/findInstruments",
    ) -> Union[List[Dict[str, Any]], int]:
        dict_payload = {
            "authorizedMarketGroupId": group_id,
            "isLimitSetted": str_bool_settled,
        }
        return self.app_request(
            self.token,
            method,
...
class AccountsData(ConnectionApi):
    ...
    def spxi_instrument_post(
        ...
        Args:
            ...
            bool_parse_dict_params_data (bool): parse dictionary params data
            method (str): method
            app (str): app
...
class DocumentsData(ConnectionApi):
    ...
    def block_unblock_doc(
        ...
        """
        # payload
        dict_params = {"id": str(doc_id), "isBlocked": bool_isblocked}
        # retrieving json
        return self.app_request(
            self.token,
            method,
    ...
    def is_protection_mode(
        ...
        dict_payload = {
            "id": str(doc_id),
            "isProtected": str(bool_protect).lower(),
        }
        #   retrieving json
        return self.app_request(
            self.token,
    ...
    def doc_profile(
        ...
        json_doc = self.app_request(
            self.token, method, app.format(doc_id), bool_retry_if_errobool_retry_if_error
        )
...
class ProfilesData(ConnectionApi):
    ...
    def entities_associated_profile(
        ...
        return self.app_request(
            self.token, method, app, dict_params, bool_retry_if_errobool_retry_if_error
        )
    ...
    def profile_market_limits_get(
        ...
        """
        return self.app_request(
            self.token, method, app.format(prof_id), bool_retry_if_errobool_retry_if_error
        )
...
class SystemEventManagement(ConnectionApi):
    ...
    def report(
        ...
        method: str = "POST",
        bool_parse_dict_params_data: str = True,
        float_secs_sleep: Optional[float] = None,
