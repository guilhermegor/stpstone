...
class DealingExcel(metaclass=TypeChecker):
	...
	def paste_values_column(
		self, sheet_name: str, str_range: str, excel_app: object, active_wb: object
	) -> None:
	...
	def delete_cells_w_specific_data(
		...
		try:
			...
			excel_app.Worksheets(sheet_name).Cells.Find("DELETAR").EntireColumn.SpecialCells(
				2
			).EntireRow.Delete()
		except Exception as err:
	...
	def attr_value_cel(
		self, sheet_name: str, str_range: str, value: str, excel_app: object, active_wb: object
	) -> None:
	...
	def restore_corrupted_xl(self, filename_orig: str, filename_output: str) -> bool:
...
class ExcelChart(metaclass=TypeChecker):
	...
	def __init__(
		self, excel: object, workbook: object, chartname: str, after_sheet: object
	) -> None:
	...
	def set_category_axis_range(self, min_value: float, max_value: float) -> None:
...
class ExcelWorksheet(metaclass=TypeChecker):
	...
	def set_font(self, row: int, col: int, font: str, size: float) -> None:
	...
	def get_font(self, row: int, col: int) -> tuple[str, float]:
...
class ExcelWriter(metaclass=TypeChecker):
	...
	def __init__(
		self, file_name: str, default_sheet_name: str, make_visible: bool = False
	) -> None:
	...
	def add_to_cell(
		self, row: int, col: int, content: str, sheet: Optional[object] = None
	) -> None:
	...
	def add_row(
		...
		try:
			...
			insert_range = self.get_range_by_cells(
				(row_i, start_col), (row_i, last_col), target_sheet
			)
	...
	def add_multiple_rows(
		...
		if not all(
			isinstance(tup, tuple) and all(isinstance(item, str) for item in tup)
		...
		try:
			...
			last_col = start_col + col_n - 1
			insert_range = self.get_range_by_cells(
				(start_row, start_col), (last_row, last_col), target_sheet
			)
	...
	def fit_cols(self, col_start: int, col_sup: int, sheet: Optional[object] = None) -> None:
		...
		if not isinstance(col_sup, int) or col_sup < col_start:
			raise ValueError(
				"End column must be a positive integer greater than or equal to " + "start column"
			)
		try:
			target_sheet = sheet if sheet is not None else self.default_sheet
	...
	def fit_col(self, col_n: int, sheet: Optional[object] = None) -> None:
		...
		try:
			target_sheet = sheet if sheet is not None else self.default_sheet
			target_sheet.Range(
				target_sheet.Cells(1, col_n), target_sheet.Cells(1, col_n)
			).EntireColumn.AutoFit()
		except Exception as err:
			raise ValueError(f"Failed to fit column: {str(err)}") from err

	def set_col_width(self, col_n: int, width: float, sheet: Optional[object] = None) -> None:
		"""Set column width.
	...
	def format_range(self, range_obj: object, style: TypeStyleXLObject) -> None:
	...
	def reset_color_pallet(self, color_index: int, color: int) -> None:
