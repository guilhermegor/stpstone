...
def streaming_pipeline(
	generator: Iterable[str], functions: list[Callable[[str], str]]
) -> Iterator[str]:
	...
	for func in functions:
		if not callable(func):
			raise TypeError(
				f"All transformation functions must be callable, but got {type(func).__name__}"
			)
	...
	for data in generator:
		for func in functions:
			...
			if not isinstance(result, str):
				raise TypeError(
					f"Function {func.__name__} must return a str, but got {type(result).__name__}"
				)
			data = result
		yield data
