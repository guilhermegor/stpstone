...
class ABCCalendar(ABC, metaclass=ABCTypeCheckerMeta):
	...
	def get_holidays_raw(
		self, timeout: Union[int, float, tuple[float, float], tuple[int, int]] = (12.0, 21.0)
	) -> pd.DataFrame:
...
class DateManipulation(CalendarCore):
	...
	def str_date_to_date(
		self, str_date: str, format_input: TypeDateFormatInput = "DD/MM/YYYY"
	) -> date:
	...
	def excel_float_to_date(self, numeric_excel_date: Union[int, float]) -> date:
...
class DateTimezoneAware(DateManipulation):
	...
	def change_timezone(
		self, date_: TypeDatetimeDate, target_tz: str = "UTC", source_tz: Optional[str] = None
	) -> datetime:
	...
	def date_to_datetime(self, date_: date, str_timezone: Optional[str] = "UTC") -> datetime:
	...
	def to_unix_timestamp(
		self, date_: Union[TypeDatetimeDate, time], str_timezone: Optional[str] = "UTC"
	) -> int:
	...
	def unix_timestamp_to_datetime(
		self, unix_timestamp: Union[float, int], str_timezone: Optional[str] = "UTC"
	) -> datetime:
	...
	def iso_to_unix_timestamp(
		self, iso_timestamp: str, str_timezone: Optional[str] = "UTC"
	) -> int:
	...
	def excel_float_to_datetime(
		self, float_date: float, str_timezone: Optional[str] = "UTC"
	) -> datetime:
...
class DatesCurrent(DatesRangeDelta):
	...
	def curr_datetime(self, str_timezone: Optional[str] = "UTC") -> datetime:
	...
	def curr_time(self, str_timezone: Optional[str] = "UTC") -> time:
	...
	def current_timestamp_string(
		self, format_output: str = "%Y%m%d_%H%M%S", str_timezone: Optional[str] = "UTC"
	) -> str:
...
class DateFormatter(DatesCurrent):
	...
	def get_platform_locale(
		self, str_locale: Optional[str] = None, str_timezone: Optional[str] = None
	) -> str:
	...
	def month_number(
		self, date_: TypeDatetimeDate, bool_month_mm: bool = False
	) -> Union[int, str]:
