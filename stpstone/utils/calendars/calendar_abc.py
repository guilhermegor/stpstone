...
class ABCCalendar(ABC, metaclass=ABCTypeCheckerMeta):
    ...
    def get_platform_locale(
        ...
        # normalize locale for platform OS
        base_locale = str_locale.replace("_", "-").replace(".UTF-8", "")
        if platform.system() == "Windows":
            normalized_locale = base_locale
        else:
            normalized_locale = f"{base_locale.replace('-', '_')}.UTF-8"
    ...
    def add_working_days(
        ...
        while int_step < int_target_days:
            date_current = date_current + timedelta(days=1 if int_days_to_add > 0 else -1)
    ...
    def unix_timestamp_to_datetime(
        ...
        return datetime.fromtimestamp(unix_timestamp, tz=str_timezone)
    ...
    def unix_timestamp_to_date(
        ...
        return datetime.fromtimestamp(unix_timestamp, tz=str_timezone).date()
    ...
    def timestamp_to_datetime(
        ...
        try:
            ...
            raise ValueError(f"Failed to parse timestamp '{timestamp_}' in format "
                             f"'YYYY-MM-DD{substr_timestamp}HH:MM:SS': {str(err)}") from err
    ...
    def years_between_dates(
        ...
        return set(self.year_number(date_) for date_ in list_)
    ...
    def delta_working_hours(
        ...
        y_start, mt_start, d_start = (
            int(timestamp_start.split(substr_timestamp)[0].split("-")[0]),
            int(timestamp_start.split(substr_timestamp)[0].split("-")[1]),
            int(timestamp_start.split(substr_timestamp)[0].split("-")[2]),
        )
        h_start, m_start, s_start = (
            int(timestamp_start.split(substr_timestamp)[1].split(":")[0]),
            int(timestamp_start.split(substr_timestamp)[1].split(":")[1]),
            int(timestamp_start.split(substr_timestamp)[1].split(":")[2]),
        )
        y_end, mt_end, d_end = (
            int(timestamp_end.split(substr_timestamp)[0].split("-")[0]),
            int(timestamp_end.split(substr_timestamp)[0].split("-")[1]),
            int(timestamp_end.split(substr_timestamp)[0].split("-")[2]),
        )
        h_end, m_end, s_end = (
            int(timestamp_end.split(substr_timestamp)[1].split(":")[0]),
            int(timestamp_end.split(substr_timestamp)[1].split(":")[1]),
            int(timestamp_end.split(substr_timestamp)[1].split(":")[2]),
        )
        timestamp_start = datetime(y_start, mt_start, d_start, h_start, m_start, s_start)
        timestamp_end = datetime(y_end, mt_end, d_end, h_end, m_end, s_end)
        
        for y in range(timestamp_start.year, timestamp_end.year + 1):
        ...
        return businesshrs.difference(timestamp_start, timestamp_end).timedelta
    ...
    def get_start_end_day_month(
        ...
        date_start = self.build_date(date_.year, date_.month, 1)
        date_start = self.nearest_working_day(date_start, bool_next=True) \
            if bool_working_days else date_start
        
        date_end = self.build_date(date_.year, date_.month + 1, 1) if date_.month < 12 \
            else self.build_date(date_.year + 1, 1, 1)
    ...
    def get_dates_weekday_month(
        ...
        list_ = []
        date_start = self.build_date(year, month, 1)
        date_end = self.build_date(year, month + 1, 1) if month < 12 \
            else self.build_date(year + 1, 1, 1)
        date_ = date_start
