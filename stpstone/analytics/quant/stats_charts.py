...
class ProbStatsCharts(metaclass=TypeChecker):
	...
	def confusion_mtx_2x2(
		...
		cm = LogLinearRegressions().logistic_regression_logit(
			x_array_real_numbers, y_vector_real_numbers, c_positive_floating_point_number
		)["confusion_matrix"]
	...
	def plot_precision_recall_vs_threshold(
		...
		plt.plot(
			thresholds, recalls[:-1], line_style_recall, label=label_recall, linewidth=line_width
		)
	...
	def histogram(
		...
		if ncols is not None and nrows is not None:
			...
			if len(sample_vector) > ncols * nrows:
				raise ValueError(
					f"Number of histograms ({len(sample_vector)}) exceeds "
	...
	def add_identity(
		self, axes: plt.Axes, *line_args: tuple[Any, ...], **line_kwargs: dict[str, Any]
	) -> plt.Axes:
