...
class StatisticalDescription(metaclass=TypeChecker):
	...
	def statistical_description(
		self, array_data: NDArray[np.float64]
	...
	def covariance(
		self, array_data_1: NDArray[np.float64], array_data_2: NDArray[np.float64]
	) -> float:
		...
		return float(
			np.dot(array_data_2.T - array_data_2.mean(), array_data_1 - array_data_1.mean())
	...
	def mean_squared_error_fitting_precision(
		self, array_observations: NDArray[np.float64], array_predictions: NDArray[np.float64]
	) -> ReturnErrorMetrics:
		...
		if len(array_observations) != len(array_predictions):
			raise ValueError("Input arrays must have the same length")
		if not np.all(np.isfinite(array_observations)) or not np.all(
			np.isfinite(array_predictions)
		):
			raise ValueError("Input arrays contain NaN or infinite values")
	...
	def quantile(
		self, array_data: NDArray[np.float64], q: float, axis: Literal[0, 1] = 0
	) -> NDArray[np.float64]:
