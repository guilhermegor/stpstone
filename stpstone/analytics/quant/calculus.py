...
class Calculus(metaclass=TypeChecker):
	...
	def differentiation(
		self, f: Expr, variable_: Symbol, nth_derivative: int = 1, *args_symbols: Symbol
	) -> Expr:
	...
	def trapz_integration(self, f: npt.ArrayLike, variable_: npt.ArrayLike) -> float:
	...
	def cumtrapz_integration(
		self, f: npt.ArrayLike, variable_: npt.ArrayLike
	...
	def sum_of_squares_gradient(
		self, array_x: Union[list[float], npt.NDArray[np.float64]]
	) -> npt.NDArray[np.float64]:
	...
	def least_gradient_vector(
		self, step_size: float = -0.01, iter: int = 1000, epsilon: float = 0.001
	) -> list[float]:
