...
class Interpolation(metaclass=TypeChecker):
	...
	def linear_interpolation(
		self, array_x: Union[list[float], np.ndarray], array_y: Union[list[float], np.ndarray]
	) -> interp1d:
	...
	def cubic_spline(
		...
		if bc_type == "periodic" and not np.allclose(array_y[0], array_y[-1]):
			raise ValueError("First and last y values must match for periodic boundary conditions")
