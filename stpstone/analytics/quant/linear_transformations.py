...
class LinearAlgebra(metaclass=TypeChecker):
	...
	def scalar_multiply(
		self, scalar: Number, array: Union[npt.ArrayLike, Sequence[Number]]
	) -> npt.NDArray[np.float64]:
	...
	def cholesky_decomposition(
		self, array_data: npt.NDArray[np.float64], bool_lower_triangle: bool = True
	) -> npt.NDArray[np.float64]:
	...
	def eigenvalue_eigenvector(
		self, array_data: npt.NDArray[np.float64]
	...
	def matrix_multiplication(
		self, array_1: npt.NDArray[np.float64], array_2: npt.NDArray[np.float64]
	) -> npt.NDArray[np.float64]:
	...
	def angle_between_two_arrays(
		...
		"""
		return np.arccos(
			np.dot(array_1, array_2.T) / (np.linalg.norm(array_1) * np.linalg.norm(array_2))
		)

	def determinant(self, array_data: npt.NDArray[np.float64]) -> float:
		"""Matrix determinant.
