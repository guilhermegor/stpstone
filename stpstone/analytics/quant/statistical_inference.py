...
@type_checker
def validate_array(array: NDArray[np.float64], name: str, min_samples: int = 1) -> None:
...
@type_checker
def validate_scalar(
	value: float, name: str, min_val: Optional[float] = None, max_val: Optional[float] = None
) -> None:
...
class MultipleRegressionHT(metaclass=TypeChecker):
	...
	def vif_iv(
		self, vector_x: NDArray[np.float64], array_y: NDArray[np.float64]
	) -> tuple[float, float]:
	...
	def normality_error_dist_test(
		self, array_x: NDArray[np.float64], array_y: NDArray[np.float64], alpha: float = 0.05
	...
	def breusch_godfrey(
		self, array_x: NDArray[np.float64], array_y: NDArray[np.float64], p: Optional[int] = None
	...
	def test_joint_coeff(self, sse_r: float, sse_unr: float, q: int, n: int, k: int) -> float:
...
class NormalityHT(metaclass=TypeChecker):
	...
	def kolmogorov_smirnov_test(
		...
		cdf_vals = np.array([NormalDistribution().cdf(x, mean_val, std_dev) for x in sorted_x])
		test_1 = np.abs(empirical_func - cdf_vals)
		test_2 = np.array(
			[cdf_vals[0]] + [cdf_vals[i] - empirical_func[i - 1] for i in range(1, len(sorted_x))]
		)
	...
	def anderson_darling(
		self, array_data: NDArray[np.float64], alpha: float = 0.05
	) -> NormalityTestResults:
...
class CorrelationHT(metaclass=TypeChecker):
	...
	def pearsons_correlation_coefficient(
		self, x1_array: NDArray[np.float64], x2_array: NDArray[np.float64], alpha: float = 0.05
...
class StationaryHT(metaclass=TypeChecker):
	...
	def augmented_dickey_fuller(
		self, array_x: NDArray[np.float64], alpha: float = 0.05
...
class StatisticalDistributionsHT(metaclass=TypeChecker):
	...
	def benford_law(
		self, array_data: NDArray[np.float64], bool_list_number_occurrencies: bool = False
	) -> BenfordResults:
		...
		if not bool_list_number_occurrencies:
			...
			for i in range(1, 10):
				array_percentual_occurrence[i - 1] = list_first_digits.count(i) / len(
					list_first_digits
				)
		else:
			if array_data.sum() == 0:
