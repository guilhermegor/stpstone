...
class BondDuration:
    ...
    def __init__(self, list_cfs: List[float], float_ytm: float, float_fv: float, 
                 float_coupo_rate: float, str_when: str = "end") -> None:
        self.list_cfs = list_cfs
        self.float_ytm = float_ytm
        self.float_fv = float_fv
        self.float_coupo_rate = float_coupo_rate
        self.str_when = str_when
        self.fm = FinancialMath()

    @property
    def macaulay(self) -> float:
        array_nper, array_discounted_cfs = self.fm.pv_cfs(self.list_cfs, self.float_ytm, 
    ...
    def dollar(self, float_y: float, int_n: int) -> float:
        _, array_discounted_cfs = self.fm.pv_cfs(self.list_cfs, self.float_ytm, 
                                              str_capitalization="simple", str_when="end")
    ...
    def effective(self, float_delta_y: float) -> float:
        ...
        float_pv_minus = np.sum([
            self.fm.pv(self.float_ytm - float_delta_y, int_t, float_cf) 
