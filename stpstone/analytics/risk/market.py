...
class MarkowitzPortf(metaclass=TypeChecker):
    ...
    def cov(self) -> NDArray[np.float64]:
        ...
        for t in range(1, self.array_r.shape[0]):
            ...
            array_cov = (
                self.float_lambda * array_cov 
                + (1 - self.float_lambda) * np.dot(array_r_t, array_r_t.T)
            )
        return array_cov
...
class VaR(metaclass=TypeChecker):
    ...
    def parametric_var(self) -> float:
        ...
        """
        return (
            self.float_mu * self.int_t 
            - self.float_z * self.float_sigma * np.sqrt(self.int_t)
        )
    ...
    def monte_carlo_var(
        self,
        int_simulations: Optional[int] = 10_000,
        float_portf_nv: Optional[float] = 1_000_000
    ) -> float:
        ...
        array_simulated_r = np.random.normal(
            loc=self.float_mu,
            scale=self.float_sigma,
            size=(int_simulations, self.int_t)
        )
