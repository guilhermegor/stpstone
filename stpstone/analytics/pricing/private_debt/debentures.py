...
class DebenturesPricer(metaclass=TypeChecker):
	...
	def _calculate_coupon_dates(self, dt_settlement: date) -> tuple[date, date]:
		...
		dt_next_coupon = dt_last_coupon.replace(
			month=dt_last_coupon.month + int_months_to_add_between
		)
	...
	def _count_days_between(self, date_start: date, date_end: date) -> int:
		...
		return (
			(date_end.year - date_start.year) * 360
			+ (date_end.month - date_start.month) * 30
			+ (dt_d2 - dt_d1)
		)

	def _calculate_ytc_price(
		self, ytc_rate: float, float_call_price: float, call_periods: int
	) -> float:
	...
	def calculate_price(self) -> float:
		...
		float_coupon_payment: float = (
			self.float_fv * self.float_coupon_r / self.int_coupon_freq
		) * (1 - self.float_tax_rate)
		for t in range(1, int_periods + 1):
			float_cf: float = (
				float_coupon_payment + self.float_fv if t == int_periods else float_coupon_payment
			)
	...
	def current_yield(self, float_market_price: Optional[float] = None) -> float:
		...
		float_price: float = (
			float_market_price if float_market_price is not None else self.calculate_price()
		)
		float_annual_coupon: float = (
			self.float_fv * self.float_coupon_r * (1 - self.float_tax_rate)
		)
	...
	def total_return(self, float_market_price: Optional[float] = None) -> float:
		...
		float
		"""
		return self.current_yield(float_market_price) + self.capital_gain_yield(float_market_price)
	...
	def dirty_price(
		...
		"""
		clean_price: float = (
			float_clean_price if float_clean_price is not None else self.calculate_price()
		)
	...
	def holding_period_return(
		self, float_purchase_price: float, float_sale_price: float, int_holding_period: float
	) -> float:
		...
		# calculate return  # noqa: ERA001
		float_total_return: float = (
			float_sale_price - float_purchase_price
		...
		for _ in range(int_max_iterations):
			...
			for t in range(1, int_periods + 1):
				...
				float_duration += (
					t
					* float_cash_flow
					/ (float_discount_rate * (1 + float_ytc / self.int_coupon_freq))
				)
			...
			# ensure yield stays reasonable - prevent negative yields < -99%  # noqa: ERA001
			float_ytc = max(float_ytc, -0.99)
		raise ValueError(
			"Yield to call calculation did not converge after "
