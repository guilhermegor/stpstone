...
class ComeCotasCalculator(metaclass=TypeChecker):
	...
	def __init__(
		self, str_fund_type: Literal["FIA", "FIRF", "FIM", "FIDC", "FIP", "FI-IE", "FII"]
	) -> None:
	...
	def _validate_inputs(
		...
		if float_acquisition_basis is not None and not isinstance(
			float_acquisition_basis, (float, int)
		):
	...
	def is_tax_day(self, date_ref: Optional[date] = None) -> bool:
		...
		# if date_ref is falsy (None, 0, False, "", [], etc.), it uses date.today()  # noqa: ERA001
		# only keeps date_ref if it is truthy (e.g., a valid date, 1, True, "hello")  # noqa: ERA001,E501
