...
class MTMFromDailySettlement(metaclass=TypeChecker):
    ...
    def jse(self, float_daily_settlement: float, float_qty: float) -> float:
        """JSE - Future contract of BVMF Index FTSE/JSE TOP 40 in BRL (JSE).
        
        Parameters
        ----------
        float_daily_settlement : float
            Daily settlement value of the contract in JPY
        float_qty : float
            Number of contracts (quantity)
        float_xcg_usdbrl : float
            Exchange rate between USD and BRL (USD/BRL)
        float_xcg_parity_jpyusd : float
            Exchange parity between JPY and USD (JPY/USD)
    ...
    def klbni(self, float_daily_settlement: float, float_qty: float) -> float:
        """KLBN - Future contract of KLBN11 in BRL (KLBNI).
    ...
    def lreno(self, float_daily_settlement: float, float_qty: float) -> float:
        """LREN - Future contract of LREN3 in BRL (LREN).
    ...
    def mbr(self, float_daily_settlement: float, float_qty: float) -> float:
        """MBR - Micro future contract of Ibovespa B3 BR+ in BRL (MBR).
    ...
    def mex(
        ...
        """MEX - Future contract of Mexican Peso in USD (MEX).
    ...
    def mgluo(self, float_daily_settlement: float, float_qty: float) -> float:
        """MGLU - Future contract of MGLU3 in BRL (MGLUO).
    ...
    def mxn(self, float_daily_settlement: float, float_qty: float) -> float:
        """MXN - Future contract of Mexican Peso in BRL (MXN).
    ...
    def nok(
        ...
        """NOK - Future contract of Norwegian Krone in USD (NOK).
    ...
    def nzd(self, float_daily_settlement: float, float_qty: float) -> float:
        """NZD - Future contract of New Zealand Dollar in BRL (NZD).
    ...
    def nzl(
        ...
        """NZL - Future contract of New Zealand Dollar in USD (NZL).
    ...
    def oc1(self, float_daily_settlement: float, float_qty: float) -> float:
        """OC1 - Future contract of OC1 in BRL (OC1).

        Effective interest rate until the contract's maturity, defined by the compounding of the 
        average daily rates of interbank deposits (overnight) operations in the period between the 
        trade date, inclusive, and the contract's last trading daym inclusive.
    ...
    def pcaro(self, float_daily_settlement: float, float_qty: float) -> float:
        """PCAR - Future contract of PCAR3 in BRL (PCARO).
    ...
    def petrp(self, float_daily_settlement: float, float_qty: float) -> float:
        """PETR - Future contract of PETR4 in BRL (PETRP).
    ...
    def prioo(self, float_daily_settlement: float, float_qty: float) -> float:
        """PRIO - Future contract of PRIO3 in BRL (PRIOO).
    ...
    def pssao(self, float_daily_settlement: float, float_qty: float) -> float:
        """PSSA - Future contract of PSSA3 in BRL (PSSAO).
    ...
    def radlo(self, float_daily_settlement: float, float_qty: float) -> float:
        """RADL - Future contract of RADL3 in BRL (RADLO).
    ...
    def railo(self, float_daily_settlement: float, float_qty: float) -> float:
        """RAIL - Future contract of RAIL3 in BRL (RAILO).
    ...
    def rdoro(self, float_daily_settlement: float, float_qty: float) -> float:
        """RDO - Future contract of RDO3 in BRL (RDORO).
    ...
    def rento(self, float_daily_settlement: float, float_qty: float) -> float:
        """RENT - Future contract of RENT3 in BRL (RENTO).
    ...
    def soy(
        ...
        """SOY - Future contract of Soy Bean in BRL (SOY).
    ...
    def suzbo(self, float_daily_settlement: float, float_qty: float) -> float:
        """SUZB - Future contract of SUZB3 in BRL (SUZBO).
    ...
    def swi(
        ...
        """SWI - Future contract of Swiss Franc in USD (SWI).
    ...
    def t10(
        ...
        """T10 - Future contract of 10-Year Treasury Note in USD (T10).
    ...
    def tie(
        ...
        Mexican Interbank Equilibrium Rate for One-Day Funding (TIIE), published by the Bank of 
        Mexico, until the contract's maturity, traded on the exchange market of B3 S.A.
    ...
    def try_(self, float_daily_settlement: float, float_qty: float) -> float:
        """TRY - Future contract of Turkish Lira in BRL (TRY).
    ...
    def tuq(
        ...
        """TUQ - Future contract of Turkish Lira in USD (TUQ).
    ...
    def usima(self, float_daily_settlement: float, float_qty: float) -> float:
        """USIM - Future contract of USIM5 Index in BRL (USIMA).
    ...
    def valeo(self, float_daily_settlement: float, float_qty: float) -> float:
        """VALE - Future contract of VALE3 in BRL (VALEO).
    ...
    def vbbro(self, float_daily_settlement: float, float_qty: float) -> float:
        """VBBR - Future contract of VBBR3 in BRL (VBBRO).
    ...
    def vivto(self, float_daily_settlement: float, float_qty: float) -> float:
        """VIVT - Future contract of VIVT3 in BRL (VIVTO).
    ...
    def wdo(self, float_daily_settlement: float, float_qty: float) -> float:
        """WDO - Future contract of US Dollar in BRL (WDO).
    ...
    def wegeo(self, float_daily_settlement: float, float_qty: float) -> float:
        """WEGE - Future contract of WEGE3 in BRL (WEGEO).
    ...
    def weu(self, float_daily_settlement: float, float_qty: float) -> float:
        """WEU - Mini future contract of Euro in BRL (WEU).
    ...
    def win(self, float_daily_settlement: float, float_qty: float) -> float:
        """WIN - Mini future contract of Ibovespa Index in BRL (WIN).
    ...
    def wsp(
        ...
        """WSP - Micro future contract of S&P 500 in BRL (WSP).
    ...
    def xfi(self, float_daily_settlement: float, float_qty: float) -> float:
        """XFI - Future contract of IFIX Index in BRL (XFI).
    ...
    def zar(self, float_daily_settlement: float, float_qty: float) -> float:
        """ZAR - Future contract of South African Rand in BRL (ZAR).
