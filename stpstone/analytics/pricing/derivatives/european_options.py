...
class BlackScholesMerton(metaclass=TypeChecker):
	...
	def general_opt_price(
		...
		if opt_type == "call":
			if t == 0:
				return max(s - k, 0)
			return s * np.exp((b - r) * t) * self.cls_normal_dist.cdf(
			...
			) - s * np.exp((b - r) * t) * self.cls_normal_dist.cdf(-self.d1(s, k, b, t, sigma, q))
...
class MonteCarlo(metaclass=TypeChecker):
	...
	def vanilla_mc_price(
		...
		array_z: NDArray[np.float64] = np.random.standard_normal((n_simulations, n_steps))

		array_s: NDArray[np.float64] = np.zeros((n_simulations, n_steps + 1))
		array_s[:, 0] = s

		# simulate trajectories using geometric brownian motion
		for int_t in range(1, n_steps + 1):
			array_s[:, int_t] = array_s[:, int_t - 1] * np.exp(
				(r - 0.5 * sigma**2) * dt + sigma * np.sqrt(dt) * array_z[:, int_t - 1]
			)
		...
		float_se: float = np.std(array_payoff) / np.sqrt(n_simulations) * np.exp(-r * t)
	...
	def monte_carlo_barrier_option(
		...
		array_barrier_hit: NDArray[np.bool_] = np.zeros(n_simulations, dtype=bool)
		...
		for int_t in range(1, n_steps + 1):
			...
			# check barrier (permanent memory with |=)  # noqa: ERA001
			#   |= accumulates hits flags over time steps  # noqa: ERA001
...
class Greeks(BlackScholesMerton):
	...
	def strike_from_delta(
		...
		if opt_type == "call":
			return s * np.exp(
				-self.cls_normal_dist.inv_cdf(delta * np.exp((r - b) * t)) * sigma * t**0.5
				+ (b + sigma**2 / 2.0) * t
			)
		elif opt_type == "put":
			return s * np.exp(
	...
	def gamma(
		self, s: float, k: float, r: float, t: float, sigma: float, q: float, b: float
	) -> float:
		"""Gamma.
		...
		"""
		return (
			self.cls_normal_dist.pdf(self.d1(s, k, b, t, sigma, q))
			* np.exp((b - r) * t)
			/ (s * sigma * t**0.5)
		)
		...
		if opt_type == "call":
			return (
				...
				* self.cls_normal_dist.cdf(self.d1(s, k, b, t, sigma, q))
				- r * k * np.exp(-r * t) * self.cls_normal_dist.cdf(self.d2(s, k, b, t, sigma, q))
			)
		elif opt_type == "put":
			return (
				-(
	...
	def time_to_maturity_maximum_vega(
		self, s: float, k: float, r: float, sigma: float, b: float
	) -> float:
		...
		return (
			2
			* (1.0 + (1.0 + (8.0 * r / sigma**2 + 1.0) * np.log(s / k) ** 2) ** 0.5)
			/ (8.0 * r + sigma**2)
		)

	def vega_global_maximum(
		self, k: float, r: float, sigma: float, b: float
	) -> ResultVegaGlobalMax:
		"""Vega global maximum.
		...
		return {"t_max_global_vega": t, "s_max_global_vega": s, "vega_max_global": vega}
	...
	def vega_delta_relationship(
		...
		return (
			np.exp((b - r) * t)
			* self.cls_normal_dist.pdf(
				self.cls_normal_dist.inv_cdf(
	...
	def rho(
		...
		"""
		if opt_type == "call":
			return t * k * np.exp(-r * t) * self.cls_normal_dist.cdf(self.d2(s, k, b, t, sigma, q))
		elif opt_type == "put":
			return (
				-t * k * np.exp(-r * t) * self.cls_normal_dist.cdf(-self.d2(s, k, b, t, sigma, q))
			)
	...
	def lambda_greek(
		...
		"""
		return (
			self.delta(s, k, r, t, sigma, q, opt_type)
			* s
			/ self.general_opt_price(s=s, k=k, r=r, t=t, sigma=sigma, q=q, b=b, opt_type=opt_type)
		)
	...
	def vanna_vol(
		...
		"""
		return (
			self.vanna(s=s, k=k, r=r, t=t, sigma=sigma, q=q, b=b, opt_type=opt_type)
			* (1.0 / sigma)
			* (
	...
	def charm(
		...
		if opt_type == "call":
			return -np.exp((b - r) * t) * (
				self.cls_normal_dist.pdf(self.d1(s, k, b, t, sigma, q))
				* (b / (sigma * t**0.5) - self.d2(s, k, b, t, sigma, q) / (2.0 * t))
				+ (b - r) * self.cls_normal_dist.cdf(self.d1(s, k, b, t, sigma, q))
			)
		elif opt_type == "put":
			return -np.exp((b - r) * t) * (
				self.cls_normal_dist.pdf(self.d1(s, k, b, t, sigma, q))
	...
	def zomma(
		...
		"""
		return (
			self.gamma(s, k, r, t, sigma, q, b)
			* (self.d1(s, k, b, t, sigma, q) * self.d2(s, k, b, t, sigma, q) - 1)
			/ sigma
		)
	...
	def speed(
		...
		"""
		return (
			-self.gamma(s=s, k=k, r=r, t=t, sigma=sigma, q=q, b=b)
			* (1.0 + self.d1(s, k, b, t, sigma, q) / (sigma * t**0.5))
			/ s
		)
	...
	def color(
		...
		return -self.gamma(s=s, k=k, r=r, t=t, sigma=sigma, q=q, b=b) * (
			r
			- b
			+ b * self.d1(s, k, b, t, sigma, q) / (sigma * t**0.5)
			+ (1.0 - self.d1(s, k, b, t, sigma, q) * self.d2(s, k, b, t, sigma, q)) / (2.0 * t)
		)
	...
	def vomma_p(
		...
		"""
		return (
			self.vega_p(s=s, k=k, r=r, t=t, sigma=sigma, q=q, b=b)
			* self.d1(s, k, b, t, sigma, q)
			* self.d2(s, k, b, t, sigma, q)
			/ sigma
		)
	...
	def vomma_positive_outside_interval(
		...
		if bool_spot:
			...
			sign_ = -1
			s_k = k
		# return vomma 0 boundaries  # noqa: ERA001
		return {
	...
	def ultima(
		...
		"""
		return (
			self.vomma(s=s, k=k, r=r, t=t, sigma=sigma, q=q, b=b)
			/ sigma
			* (
				self.d1(s, k, b, t, sigma, q) * self.d2(s, k, b, t, sigma, q)
				- self.d1(s, k, b, t, sigma, q) / self.d2(s, k, b, t, sigma, q)
				- self.d2(s, k, b, t, sigma, q) / self.d1(s, k, b, t, sigma, q)
				- 1.0
			)
		)
	...
	def variance_vanna(
		...
		return (
			-s
			* np.exp((b - r) * t)
			* self.pdf(self.d1(s, k, b, t, sigma, q))
			* self.d2(s, k, b, t, sigma, q)
			/ (2.0 * sigma)
		)
	...
	def variance_ultima(
		...
		return (
			...
			* (
				(self.d1(s, k, b, t, sigma, q) * self.d2(s, k, b, t, sigma, q) - 1.0)
				* (self.d1(s, k, b, t, sigma, q) * self.d2(s, k, b, t, sigma, q) - 3.0)
				- (self.d1(s, k, b, t, sigma, q) ** 2 + self.d2(s, k, b, t, sigma, q) ** 2)
			)
		)
	...
	def dbsm_dohm(
		...
		The Complete Guide To Option Pricing Formulas Espen Gaarder Haug
		"""
		return s * self.cls_normal_dist.pdf(
			(np.log(s / k) + t * sigma**2 / 2.0) / (sigma * t**0.5)
		)
	...
	def driftless_theta(
		...
		"""
		return (
			-s * self.cls_normal_dist.pdf(self.d1(s, k, b, t, sigma, q)) * sigma / (2.0 * t**0.5)
		)
	...
	def bleed_offset_volatility(
		...
		"""
		return self.theta(s=s, k=k, r=r, t=t, sigma=sigma, q=q, b=b) / self.vega(
	...
	def theta_gamma_relationship_driftless(
		...
		return (
			-2.0
			* self.driftless_theta(s=s, k=k, r=r, t=t, sigma=sigma, q=q, b=b)
			/ (s**2 * sigma**2)
		)
	...
	def strike_given_risk_neutral_prob(
		...
		if opt_type == "call":
			return s * np.exp(
				-self.cls_normal_dist.inv_cdf(p) * sigma * t**0.5 + (b - sigma**2 / 2.0) * t
			)
		elif opt_type == "call":
			return s * np.exp(
				self.cls_normal_dist.inv_cdf(p) * sigma * t**0.5 + (b - sigma**2 / 2.0) * t
	...
	def d_zeta_d_vol(
		...
		if opt_type == "call":
			return (
				-self.cls_normal_dist.pdf(self.d2(s, k, b, t, sigma, q))
				* self.d1(s, k, b, t, sigma, q)
				/ sigma
			)
		elif opt_type == "put":
	...
	def d_zeta_d_time(
		...
		"""
		if opt_type == "call":
			return self.pdf(self.d2(s, k, b, t, sigma, q)) * (
				b / (sigma * t**0.5) - self.d1(s, k, b, t, sigma, q) / (2.0 * t)
			)
		elif opt_type == "put":
			return -self.pdf(self.d2(s, k, b, t, sigma, q)) * (
				b / (sigma * t**0.5) - self.d1(s, k, b, t, sigma, q) / (2.0 * t)
	...
	def probability_ever_getting_itm(
		...
		if opt_type == "call":
			return (k / s) ** (mu + lambda_) * self.cls_normal_dist.cdf(-z) + (k / s) ** (
				mu - lambda_
			) * self.cls_normal_dist.cdf(-z + 2 * lambda_ * sigma * t**0.5)
		elif opt_type == "put":
			return (k / s) ** (mu + lambda_) * self.cls_normal_dist.cdf(z) + (k / s) ** (
				mu - lambda_
			) * self.cls_normal_dist.cdf(z - 2 * lambda_ * sigma * t**0.5)

	def net_weighted_vega_exposure(
		self, psi_r: float, *dicts_opts: list[dict[str, float]]
	) -> float:
...
class IterativeMethods(Greeks):
	...
	def binomial_pricing_model(
		...
		for i in np.arange(int(n), 0, -1):
			array_cp = disc * (q * array_cp[1 : i + 1] + (1.0 - q) * array_cp[0:i])
	...
	def crr_method(
		...
		for i in np.arange(int(n), 0, -1):
			for j in range(0, i):
				array_cp[j] = disc * (q * array_cp[j + 1] + (1 - q) * array_cp[j])
		# returning the no-arbitrage price at node 0  # noqa: ERA001
		return array_cp[0]
	...
	def eqp_method(
		...
		for i in np.arange(int(n), 0, -1):
			for j in range(0, i):
				array_cp[j] = disc * (pu * array_cp[j + 1] + pd * array_cp[j])
		# returning the no-arbitrage price at node 0  # noqa: ERA001
		return array_cp[0]
...
class EuropeanOptions(IterativeMethods):
	...
	def implied_volatility(
		...
		if method == "newton_raphson":
			...
			while epsilon > tolerance:
				...
				dif_calc_market = self.general_opt_price(s, k, r, t, imp_vol, q, b, opt_type) - cp0
				...
				try:
					imp_vol = -dif_calc_market / self.vega(s, k, r, t, imp_vol, q, b) + imp_vol
				except RuntimeWarning:
					return imp_vol, flag_max_iter_hitten
				epsilon = abs((imp_vol - orig_vol) / imp_vol)
			# returning implied volatility and maximum iterations hitten  # noqa: ERA001
			return imp_vol, flag_max_iter_hitten
			...
			while (float_high - float_low) > epsilon:
				if (
					self.general_opt_price(
						s, k, r, t, float(float_high + float_low) / 2.0, q, b, opt_type
					)
					> cp0
				):
			...
			func = lambda sigma: self.func_non_linear(s, k, r, t, sigma, q, b, cp0, opt_type)  # noqa: E731 - do not assign a `lambda` expression, use a def
			...
			return minimize(
				self.func_non_linear(s, k, r, t, sigma, q, b, cp0, opt_type), orig_vol, method="CG"
			), False
	...
	def moneyness(self, s: float, k: float, r: float, t: float, sigma: float, q: float) -> float:
		...
		Mercado de Opções, Conceitos e Estratégias / Author: Luiz Maurício da Silva /
			Pgs. 74, 75, 76, 77, 78
		"""
		return (self.d1(s, k, r, t, sigma, q) + self.d2(s, k, r, t, sigma, q)) / 2
	...
	def iaotm(
		...
		if abs(self.moneyness(s, k, r, t, sigma, q)) < pct_moneyness_atm:
			return "ATM"
		elif (self.moneyness(s, k, r, t, sigma, q) < pct_moneyness_atm and opt_type == "call") or (
			self.moneyness(s, k, r, t, sigma, q) > pct_moneyness_atm and opt_type == "put"
		):
			return "OTM"
		elif (self.moneyness(s, k, r, t, sigma, q) > pct_moneyness_atm and opt_type == "call") or (
			self.moneyness(s, k, r, t, sigma, q) < pct_moneyness_atm and opt_type == "put"
