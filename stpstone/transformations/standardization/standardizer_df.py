...
TypeFillnaStrategy = TypeVar(
	"TypeFillnaStrategy", bound=Optional[dict[str, Literal["bfill", "ffill"]]]
)
TypeErrorActionAsTypeDataFrame = TypeVar(
	"TypeErrorActionAsTypeDataFrame", bound=Literal["raise", "ignore", "coerce"]
)
TypeKeepDuplicatedDataFrame = TypeVar(
	"TypeKeepDuplicatedDataFrame", bound=Literal["first", "last", False]
)
...
class DFStandardization(metaclass=TypeChecker):
	...
	def __init__(
		...
		self.str_dt_fillna = (
			...
			if str_dt_fillna
				...
				"2099-12-31"
				if self.str_fmt_dt == "YYYY-MM-DD"
				else "31/12/2099"
				if self.str_fmt_dt == "DD/MM/YYYY"
				else "31/12/99"
				if self.str_fmt_dt == "DD/MM/YY"
				else "31122099"
				if self.str_fmt_dt == "DDMMYYYY"
				else "311299"
				if self.str_fmt_dt == "DDMMYY"
				else "20991231"
				if self.str_fmt_dt == "YYYYMMDD"
				else 4102358400
				if self.str_fmt_dt == "unix_ts"
				else "31122099"
			)
		)
	...
	def limit_columns_to_dtypes(self, df_: pd.DataFrame) -> pd.DataFrame:
		...
		"""
		list_cols_excluded = [
			x for x in list(df_.columns) if x not in list(self.dict_dtypes.keys())
		]
		self.cls_create_log.log_message(
			self.logger, f"list cols dataframe before filtering: {list(df_.columns)}", "info"
		)
		self.cls_create_log.log_message(
			self.logger, f"list cols to filter: {list(self.dict_dtypes.keys())}", "info"
		)
		self.cls_create_log.log_message(
			self.logger, f"list of columns excluded: {list_cols_excluded}", "info"
		)
	...
	def change_dtypes(self, df_: pd.DataFrame) -> pd.DataFrame:
		...
		list_cols_missing = [
			...
			if c
			not in self.cls_list_handler.extend_lists(list(dict_dtypes.keys()), self.list_cols_dt)
		]
	...
	def strip_hidden_characters(self, df_: pd.DataFrame) -> pd.DataFrame:
		...
		string_cols = [
			c
			for c in df_.select_dtypes(include=["object", "category", "string"]).columns
		...
		for col_ in string_cols:
			try:
				...
				is_object_or_category = df_[col_].dtype in ["object", "category"]
				is_string_dtype = pd.api.types.is_string_dtype(df_[col_])
				if is_object_or_category or is_string_dtype:
					# Additional check: ensure the column has string values  # noqa: ERA001
					sample_values = df_[col_].dropna().head(5)
					if len(sample_values) > 0 and all(
						isinstance(val, str) for val in sample_values
					):
				...
				# Log the error and continue with other columns  # noqa: ERA001
				if self.logger:
					self.cls_create_log.log_message(
						self.logger,
	...
	def strip_data(self, df_: pd.DataFrame) -> pd.DataFrame:
		...
		list_cols = [
			col_
			for col_ in df_.select_dtypes(["object", "string"]).columns
