...
class DimensionalityReduction(metaclass=TypeChecker):
	...
	def iv_woe_iter(
		...
		if (
			...
			and name_missing_data in df_tmp_groupby[col_min_value].to_numpy()
		):
	...
	def var_iter(
		...
		for col in df_.columns:
			if col != target_col:
				try:
					col_is_numeric = np.issubdtype(df_[col].dtype, np.number)
				except TypeError:
					col_is_numeric = False
				if col_is_numeric and df_[col].nunique() > min_bins_monotonic:
