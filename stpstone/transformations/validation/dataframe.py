...
class DataFrameValidator:
    ...
    def check_missing_values(self):
        ...
        self.create_log.warnings(self.logger, 'Missing Values:\n', missing_summary[missing_summary > 0])

    @property
    def check_duplicates(self):
        duplicate_count = self.df_.duplicated().sum()
        if duplicate_count:
            self.create_log.warnings(self.logger, f"Found {duplicate_count} duplicate rows.")

    def validate_ranges(self, dict_rng_constraints:Dict[str, Tuple[Number, Number]]) -> None:
    ...
    def validate_categorical_values(self, col_:Optional[str]=None, 
                                    list_allowed_values:Optional[str]=None):
