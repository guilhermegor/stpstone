...
def validate_type(
	...
	if hasattr(expected_type, "__annotations__") and hasattr(expected_type, "__total__"):
		...
		if not isinstance(value, dict):
			raise TypeError(
				f"{param_name} must be a dictionary for TypedDict, "
				+ f"got {type(value).__name__}"
			)
		return

	# Alternative check for TypedDict (more robust, Python 3.9+)  # noqa: ERA001
	try:
	...
	# (since bool is a subclass of int in Python, we need to check this first)  # noqa: ERA001
	...
	if origin is Literal:
		...
		if value not in args:
			...
			raise TypeError(f"{param_name} must be one of: {allowed_values}, got {repr(value)}")
		return

	# handle Union types (including Optional)  # noqa: ERA001
	if origin is Union:
		args = get_args(expected_type)
		...
		raise TypeError(
			f"{param_name} must be one of types: {', '.join(type_names)}, "
			+ f"got {type(value).__name__}"
		)

	# handle generic types like List[Callable[...]]  # noqa: ERA001
	elif origin is list:
		if not isinstance(value, list):
			raise TypeError(f"{param_name} must be of type list, " + f"got {type(value).__name__}")
	...
	# handle generic types like List, dict, etc.
		...
		if not isinstance(value, origin):
			raise TypeError(
				f"{param_name} must be of type {expected_type}, " + f"got {type(value).__name__}"
			)
		return
		...
		**kwargs : Any
		...
		for param_name, param in params.items():
			...
			if param.kind == inspect.Parameter.VAR_POSITIONAL:
				...
				varargs_type = (
					get_args(varargs_type)[0] if get_origin(varargs_type) is list else varargs_type
				)
...
class TypeChecker(type):
	...
	def __new__(
		cls: "TypeChecker", name: str, bases: tuple, dict_: dict[str, Any]
	) -> "TypeChecker":
...
class AdvancedTypeChecker(type):
	...
	def __new__(
		cls: "AdvancedTypeChecker", name: str, bases: tuple, dict_: dict[str, Any]
	) -> "AdvancedTypeChecker":
		...
		def create_advanced_type_checked_method(
			...
			def wrapper(
				...
				**kwargs : Any
				...
				# validate return type if requested  # noqa: ERA001
...
class ConfigurableTypeChecker(type):
	...
	def __new__(
		cls: "ConfigurableTypeChecker", name: str, bases: tuple, dict_: dict[str, Any]
