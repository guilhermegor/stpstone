"""Unit tests for B3BdiFixedIncomeSettlements class."""
...
from stpstone.ingestion.countries.br.otc.b3_bdi_fixed_income_settlements import (
	B3BdiFixedIncomeSettlements,
)
...
@pytest.fixture
def instance(sample_date: date) -> B3BdiFixedIncomeSettlements:
	"""Fixture providing a B3BdiFixedIncomeSettlements instance.
	...
	"""
	return B3BdiFixedIncomeSettlements(date_ref=sample_date)
...
def test_init_with_valid_inputs(sample_date: date) -> None:
	...
	None
	"""
	inst = B3BdiFixedIncomeSettlements(date_ref=sample_date, int_page_size=500)
	...
	assert "SaleOff" in inst.url_tpl
...
def test_init_default_page_size(sample_date: date) -> None:
	...
	None
	"""
	inst = B3BdiFixedIncomeSettlements(date_ref=sample_date)
...
def test_init_without_date_ref(mocker: MockerFixture) -> None:
	...
	inst = B3BdiFixedIncomeSettlements()
...
def test_init_logger_propagated() -> None:
	...
	inst = B3BdiFixedIncomeSettlements(date_ref=date(2026, 4, 17), logger=mock_logger)
	assert inst.logger is mock_logger


def test_get_response_success(
	instance: B3BdiFixedIncomeSettlements, mocker: MockerFixture
) -> None:
	"""Test get_response posts to the correct URL and returns the response.

	Parameters
	----------
	instance : B3BdiFixedIncomeSettlements
		Initialized instance.
	mocker : MockerFixture
		Pytest-mock fixture.
...
def test_transform_data_normal(
	instance: B3BdiFixedIncomeSettlements, sample_table_dict: dict
) -> None:
	...
	assert df_["TCKR_SYMB"].iloc[0] == "CDCA"
	assert df_["QUANTITY"].iloc[0] == 281
	assert df_["VOLUME"].iloc[0] == pytest.approx(299226.16)


def test_transform_data_empty_values(
	instance: B3BdiFixedIncomeSettlements, empty_table_dict: dict
) -> None:
