"""Unit tests for B3BdiStocksMostTraded class."""
...
@pytest.fixture
def instance(sample_date: date) -> B3BdiStocksMostTraded:
	"""Fixture providing a B3BdiStocksMostTraded instance.
	...
	"""
	return B3BdiStocksMostTraded(date_ref=sample_date)
...
def sample_table_dict() -> dict:
	...
	-------
	dict
		Sample table dict mimicking the BDI InCash API response.
	"""
	return {
		"columns": [
			{"name": "TckrSymb"},
		...
		"values": [
			["PETR4", "PN      N2", 4828085.575, 11.947079906739575, None],
		],
	}
...
def test_init_with_valid_inputs(sample_date: date) -> None:
	...
	None
	"""
	inst = B3BdiStocksMostTraded(date_ref=sample_date, int_page_size=500)
...
def test_init_default_page_size(sample_date: date) -> None:
	...
	None
	"""
	inst = B3BdiStocksMostTraded(date_ref=sample_date)
...
def test_init_logger_propagated() -> None:
	...
	inst = B3BdiStocksMostTraded(date_ref=date(2026, 4, 17), logger=mock_logger)
	assert inst.logger is mock_logger


def test_get_response_success(
	instance: B3BdiStocksMostTraded,
	mocker: MockerFixture,
) -> None:
...
def test_parse_raw_file_missing_table_key(instance: B3BdiStocksMostTraded) -> None:
...
def test_transform_data_normal(
	...
	assert len(df_) == 1
	assert list(df_.columns) == ["TCKR_SYMB", "COD", "VLM_TRADED_DAY", "PARTICIPATION"]
	assert df_["TCKR_SYMB"].iloc[0] == "PETR4"
	assert df_["COD"].iloc[0] == "PN      N2"
	assert df_["VLM_TRADED_DAY"].iloc[0] == 4828085.575


def test_transform_data_multiple_rows(instance: B3BdiStocksMostTraded) -> None:
	...
	table = {
		...
		"values": [
			["PETR4", "PN      N2", 4828085.575, 11.947079906739575, None],
			["VALE3", "ON      NM", 3621074.123, 8.943210123456789, None],
			["ITUB4", "PN      N1", 2905432.789, 7.182345678901234, None],
		],
	}
	df_ = instance.transform_data(table)
	assert len(df_) == 3
	assert set(df_["TCKR_SYMB"].tolist()) == {"PETR4", "VALE3", "ITUB4"}
...
def test_module_reload(sample_date: date) -> None:
	...
	import stpstone.ingestion.countries.br.exchange.b3_bdi_stocks_most_traded as mod

	importlib.reload(mod)
	inst = mod.B3BdiStocksMostTraded(date_ref=sample_date)
	assert inst.date_ref == sample_date
	assert "InCash" in inst.url_tpl
