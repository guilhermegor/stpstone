"""Unit tests for B3BdiFixedIncomeInstrumentsRegistration class."""
...
from stpstone.ingestion.countries.br.registries.b3_bdi_fixed_income_instruments_registration import (  # noqa: E501
...
@pytest.fixture
def instance(sample_date: date) -> B3BdiFixedIncomeInstrumentsRegistration:
	"""Fixture providing a B3BdiFixedIncomeInstrumentsRegistration instance.
	...
	"""
	return B3BdiFixedIncomeInstrumentsRegistration(date_ref=sample_date)
...
def sample_table_dict() -> dict:
	...
	return {
		...
		"values": [
			["2026-04-17T00:00:00", "IT0122H5UK9", "ITAU UNIBANCO S.A.", "COE"],
		],
	}
...
def empty_table_dict() -> dict:
	...
	"""
	return {
		"columns": [{"name": "DtRef"}, {"name": "CodISIN"}],
		"values": [],
	}
...
def test_init_with_valid_inputs(sample_date: date) -> None:
	...
	None
	"""
	inst = B3BdiFixedIncomeInstrumentsRegistration(date_ref=sample_date, int_page_size=500)
...
def test_init_default_page_size(sample_date: date) -> None:
	...
	None
	"""
	inst = B3BdiFixedIncomeInstrumentsRegistration(date_ref=sample_date)
...
def test_init_without_date_ref(mocker: MockerFixture) -> None:
	...
	inst = B3BdiFixedIncomeInstrumentsRegistration()
...
def test_init_logger_propagated() -> None:
	...
	inst = B3BdiFixedIncomeInstrumentsRegistration(date_ref=date(2026, 4, 17), logger=mock_logger)
	assert inst.logger is mock_logger


def test_get_response_success(
	instance: B3BdiFixedIncomeInstrumentsRegistration, mocker: MockerFixture
) -> None:
	"""Test get_response posts to the correct URL and returns the response.

	Parameters
	----------
	instance : B3BdiFixedIncomeInstrumentsRegistration
		Initialized instance.
	mocker : MockerFixture
		Pytest-mock fixture.
...
def test_transform_data_normal(
	instance: B3BdiFixedIncomeInstrumentsRegistration, sample_table_dict: dict
) -> None:
	...
	assert len(df_) == 1
	assert list(df_.columns) == ["DT_REF", "COD_ISIN", "NM_EMISSOR", "TP_PRODUTO"]
	assert df_["COD_ISIN"].iloc[0] == "IT0122H5UK9"


def test_transform_data_empty_values(
	instance: B3BdiFixedIncomeInstrumentsRegistration, empty_table_dict: dict
) -> None:
...
def test_run_without_db_paginates(
	...
	assert len(result) == 1
	assert "COD_ISIN" in result.columns
