"""Unit tests for B3BdiCoeInventory class."""
...
from stpstone.ingestion.countries.br.otc.b3_bdi_coe_inventory import (
	B3BdiCoeInventory,
)
...
@pytest.fixture
def instance(sample_date: date) -> B3BdiCoeInventory:
	"""Fixture providing a B3BdiCoeInventory instance.
	...
	"""
	return B3BdiCoeInventory(date_ref=sample_date)
...
def sample_table_dict() -> dict:
	...
	-------
	dict
		Sample table dict mimicking the BDI COEInventory API response.
	"""
	return {
		"columns": [
			{"name": "TckrSymb"},
		...
		"values": [
			["COE", 433569, 101672275346.26, None],
		],
	}
...
def test_init_with_valid_inputs(sample_date: date) -> None:
	...
	None
	"""
	inst = B3BdiCoeInventory(date_ref=sample_date, int_page_size=500)
	...
	assert "COEInventory" in inst.url_tpl
...
def test_init_default_page_size(sample_date: date) -> None:
	...
	None
	"""
	inst = B3BdiCoeInventory(date_ref=sample_date)
...
def test_init_logger_propagated() -> None:
	...
	inst = B3BdiCoeInventory(date_ref=date(2026, 4, 17), logger=mock_logger)
	assert inst.logger is mock_logger


def test_get_response_success(
	instance: B3BdiCoeInventory,
	mocker: MockerFixture,
) -> None:
...
def test_parse_raw_file_missing_table_key(instance: B3BdiCoeInventory) -> None:
...
def test_transform_data_normal(
	...
	assert df_["TCKR_SYMB"].iloc[0] == "COE"
	assert df_["TRAD_QTY"].iloc[0] == 433569
	assert df_["FINANCIAL_VOL"].iloc[0] == 101672275346.26


def test_transform_data_multiple_rows(instance: B3BdiCoeInventory) -> None:
	...
	table = {
		...
		"values": [
			["COE", 433569, 101672275346.26, None],
			["COE2", 1000, 500000.0, None],
			["COE3", 250, 98765432.10, None],
		],
	}
	df_ = instance.transform_data(table)
	assert len(df_) == 3
	assert set(df_["TCKR_SYMB"].tolist()) == {"COE", "COE2", "COE3"}
...
def test_run_without_db(
	...
	assert result["FINANCIAL_VOL"].iloc[0] == Decimal("101672275346.26")
...
def test_module_reload(sample_date: date) -> None:
	...
	import stpstone.ingestion.countries.br.otc.b3_bdi_coe_inventory as mod

	importlib.reload(mod)
	inst = mod.B3BdiCoeInventory(date_ref=sample_date)
	assert inst.date_ref == sample_date
	assert "COEInventory" in inst.url_tpl
