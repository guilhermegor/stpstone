"""Unit tests for FMPEtfsOhlcvYesterday ingestion class."""
...
@pytest.fixture
def sample_etf_data() -> list:
	"""Fixture providing sample batch ETF quote records.

	Returns
	-------
	list
		List of sample ETF quote dictionaries.
	"""
	return [
		{"symbol": "SPY", "price": 500.0, "change": 1.5, "volume": 80000000.0},
		{"symbol": "QQQ", "price": 420.0, "change": -0.8, "volume": 45000000.0},
	]


@pytest.fixture
def mock_response_success(sample_etf_data: list) -> Response:
	"""Fixture providing a successful HTTP response mock.

	Parameters
	----------
	sample_etf_data : list
		Sample ETF data list.
	...
	mock_resp.json.return_value = sample_etf_data
	return mock_resp
...
def mock_dir_files(mocker: MockerFixture) -> MagicMock:
	...
	mock_cls = mocker.patch(
		...
		".fmp_etfs_ohlcv_yesterday.DirFilesManagement"
	)
...
class TestFMPEtfsOhlcvYesterday:
	"""Test cases for FMPEtfsOhlcvYesterday class."""
	...
	def test_init_with_explicit_date(
		...
		instance = FMPEtfsOhlcvYesterday(token="tok", date_ref=test_date)  # noqa: S106
	...
	def test_init_url_contains_endpoint(
		...
		"""Test that the constructed URL contains the batch-etf-quotes endpoint.
		...
		None
		"""
		instance = FMPEtfsOhlcvYesterday(token="mytoken")  # noqa: S106

		assert "batch-etf-quotes" in instance.url
	...
	def test_get_response_success(
		...
		instance = FMPEtfsOhlcvYesterday(token="tok")  # noqa: S106
	...
	def test_run_returns_dataframe_without_db(
		...
		"""
		mock_requests_get.return_value = mock_response_success
		standardized_df = pd.DataFrame({"SYMBOL": ["SPY"], "PRICE": [500.0]})
	...
	def test_run_with_cls_db_calls_insert(
		...
		"""
		mock_requests_get.return_value = mock_response_success
		standardized_df = pd.DataFrame({"SYMBOL": ["SPY"]})

		instance = FMPEtfsOhlcvYesterday(token="tok", cls_db=mock_db_session)  # noqa: S106
		mocker.patch.object(instance, "standardize_dataframe", return_value=standardized_df)
