"""Unit tests for TradingEconNonFarmPayrollForecasts class."""
...
from stpstone.ingestion.countries.ww.macroeconomics.trading_econ_non_farm_payroll_forecasts import (  # noqa: E501
...
@pytest.fixture
def instance(sample_date: date) -> TradingEconNonFarmPayrollForecasts:
	"""Provide a TradingEconNonFarmPayrollForecasts instance for testing.
	...
	"""
	return TradingEconNonFarmPayrollForecasts(date_ref=sample_date)
...
def test_init_default_date_ref() -> None:
	...
	with patch.object(DatesBRAnbima, "add_working_days", return_value=date(2025, 1, 1)):
		obj = TradingEconNonFarmPayrollForecasts()
	assert isinstance(obj.date_ref, date)


def test_init_sets_expected_attributes(instance: TradingEconNonFarmPayrollForecasts) -> None:
	"""Test initialization sets expected helper attributes.

	Parameters
	----------
	instance : TradingEconNonFarmPayrollForecasts
		Ingestion instance.
...
def test_transform_data_quits_driver(
	...
	fixed_texts = ["2025-01-01", "08:30", "USD", "Dec", "256K", "212K", "200K", "210K"]
	with (
		patch.object(
			TradingEconNonFarmPayrollForecasts,
			"_list_web_elements",
