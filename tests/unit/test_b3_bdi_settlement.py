"""Unit tests for B3BdiSettlement class."""
...
@pytest.fixture
def instance(sample_date: date) -> B3BdiSettlement:
	"""Fixture providing a B3BdiSettlement instance.
	...
	"""
	return B3BdiSettlement(date_ref=sample_date)
...
def test_init_with_valid_inputs(sample_date: date) -> None:
	...
	None
	"""
	inst = B3BdiSettlement(date_ref=sample_date, int_page_size=500)
...
def test_init_default_page_size(sample_date: date) -> None:
	...
	None
	"""
	inst = B3BdiSettlement(date_ref=sample_date)
...
def test_init_logger_propagated() -> None:
	...
	inst = B3BdiSettlement(date_ref=date(2026, 4, 17), logger=mock_logger)
	assert inst.logger is mock_logger


def test_get_response_success(instance: B3BdiSettlement, mocker: MockerFixture) -> None:
	"""Test get_response posts to the correct URL and returns the response.

	Parameters
	----------
	instance : B3BdiSettlement
		Initialized instance.
	mocker : MockerFixture
		Pytest-mock fixture.
...
def test_get_response_http_error(instance: B3BdiSettlement, mocker: MockerFixture) -> None:
...
def test_transform_data_normal(instance: B3BdiSettlement, sample_table_dict: dict) -> None:
...
def test_transform_data_empty_values(
	instance: B3BdiSettlement, empty_table_dict: dict
) -> None:
