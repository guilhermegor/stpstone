...
def test_clear_multiple_caches_partial_failure() -> None:
    ...
    decorated_method = clear_multiple_caches(
        mock_instance.test_method, 
        ["clear_cache", "bad_method", "clear_cache_a"]
    )
...
def test_validate_method_cache_pairs_empty_cache_method_names(
    ...
    with pytest.raises(
        ValueError, 
        match=f"Cache method at index {expected_i}, {expected_j} cannot be empty"
    ):
