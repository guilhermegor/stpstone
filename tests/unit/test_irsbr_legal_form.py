"""Unit tests for IRSBRLegalForm class.
...
from stpstone.ingestion.countries.br.taxation.irsbr_legal_form import (
	_COLUMN_NAMES,
	IRSBRLegalForm,
)
...
# Helpers
# --------------------------
def _build_zip_bytes(csv_content: str, csv_filename: str = "Naturezas.csv") -> BytesIO:
	...
	csv_filename : str, optional
		The name of the CSV entry, by default "Naturezas.csv".
...
def _sample_csv_row() -> str:
	...
	"""
	return "2046;SOCIEDADE ANONIMA ABERTA"
...
@pytest.fixture
def irsbr_instance(sample_date: date) -> IRSBRLegalForm:
	"""Fixture providing IRSBRLegalForm instance.
	...
	-------
	IRSBRLegalForm
		Initialized IRSBRLegalForm instance.
	"""
	return IRSBRLegalForm(date_ref=sample_date)
...
def test_init_with_valid_inputs(sample_date: date) -> None:
	...
	Verifies
	--------
	- The IRSBRLegalForm instance is initialized correctly.
	...
	None
	"""
	instance = IRSBRLegalForm(date_ref=sample_date)
...
def test_transform_data(
	...
	assert len(df_) == 3
	assert df_["CODE"].iloc[0] == "2046"
	assert df_["DESCRIPTION"].iloc[0] == "SOCIEDADE ANONIMA ABERTA"


def test_transform_data_none_input(irsbr_instance: IRSBRLegalForm) -> None:
...
def test_transform_data_invalid_zip(irsbr_instance: IRSBRLegalForm) -> None:
...
def test_transform_data_zip_without_csv(irsbr_instance: IRSBRLegalForm) -> None:
...
def test_run_without_db(
	...
	"""
	mock_requests_get.return_value = mock_response
	sample_df = pd.DataFrame({"CODE": ["2046"], "DESCRIPTION": ["SOCIEDADE ANONIMA ABERTA"]})
	with (
...
def test_run_single_file_url(
	...
	"""Test that run fetches exactly one file (Naturezas.zip).

	Verifies
	--------
	- requests.get is called exactly once.
	- The URL contains Naturezas.zip (no numeric suffix).
	...
	with (
		...
		assert "Naturezas.zip" in call_url
...
def test_reload_module() -> None:
	...
	import stpstone.ingestion.countries.br.taxation.irsbr_legal_form

	original_instance = IRSBRLegalForm(date_ref=date(2025, 3, 15))
	importlib.reload(stpstone.ingestion.countries.br.taxation.irsbr_legal_form)
	new_instance = IRSBRLegalForm(date_ref=date(2025, 3, 15))
