"""Unit tests for IRSBRCompanies class.
...
# Helpers
# --------------------------
def _build_zip_bytes(csv_content: str, csv_filename: str = "Empresas0.csv") -> BytesIO:
...
def _sample_csv_row() -> str:
	...
	"""
	return "12345678;Empresa Teste LTDA;2062;50;1000.00;05;"
...
@pytest.fixture
def irsbr_instance(sample_date: date) -> IRSBRCompanies:
	"""Fixture providing IRSBRCompanies instance.
	...
	-------
	IRSBRCompanies
		Initialized IRSBRCompanies instance.
	"""
	return IRSBRCompanies(date_ref=sample_date)
...
def test_init_with_valid_inputs(sample_date: date) -> None:
	...
	Verifies
	--------
	- The IRSBRCompanies instance is initialized correctly.
	...
	None
	"""
	instance = IRSBRCompanies(date_ref=sample_date)
...
def test_transform_data(
	...
	assert len(df_) == 3
	assert df_["EIN_COMPANY"].iloc[0] == "12345678"
	assert df_["COMPANY_NAME"].iloc[0] == "Empresa Teste LTDA"


def test_transform_data_none_input(irsbr_instance: IRSBRCompanies) -> None:
...
def test_transform_data_invalid_zip(irsbr_instance: IRSBRCompanies) -> None:
...
def test_transform_data_zip_without_csv(irsbr_instance: IRSBRCompanies) -> None:
...
def test_run_without_db(
	...
	"""
	mock_requests_get.return_value = mock_response
	sample_df = pd.DataFrame({"EIN_COMPANY": ["12345678"]})
	with (
...
def test_run_iterates_over_all_files(
	...
	with (
		...
		for i in range(_NUM_FILES):
			...
			assert f"Empresas{i}.zip" in call_url
