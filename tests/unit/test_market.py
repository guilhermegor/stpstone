...
@pytest.fixture
def markowitz_portf(
	sample_portfolio_returns: NDArray[np.float64], sample_portfolio_weights: NDArray[np.float64]
) -> MarkowitzPortf:
...
def var_instance(sample_returns: NDArray[np.float64]) -> VaR:
	...
	"""
	return VaR(float_mu=0.001, float_sigma=0.02, array_r=sample_returns, float_cl=0.95, int_t=1)
...
class TestRiskStats:
	...
	def test_variance_ewma_invalid_lambda(self, risk_stats: RiskStats, lambda_val: float) -> None:
...
class TestMarkowitzPortf:
	...
	def test_init_with_valid_inputs(
		self, sample_returns: NDArray[np.float64], sample_weights: NDArray[np.float64]
	) -> None:
	...
	def test_init_with_dataframes(
		self, sample_returns_df: pd.DataFrame, sample_weights_df: pd.DataFrame
	) -> None:
		...
		None
		"""
		portf = MarkowitzPortf(sample_returns_df, sample_weights_df, bool_validate_w=True)
		assert isinstance(portf.array_r, np.ndarray)
		assert isinstance(portf.array_w, np.ndarray)

	def test_weight_validation_enabled(self, sample_returns: NDArray[np.float64]) -> None:
	...
	def test_weight_validation_disabled(self, sample_returns: NDArray[np.float64]) -> None:
...
class TestVaR:
	...
	@pytest.mark.parametrize("shock_type,shock", [("relative", 0.1), ("absolute", 0.01)])
	def test_historic_var_stress_test(
		self, var_instance: VaR, shock_type: Literal["absolute", "relative"], shock: float
	) -> None:
	...
	def test_historic_var_stress_test_invalid_type(self, var_instance: VaR) -> None:
...
class TestRiskMeasures:
	...
	def risk_measures(self, sample_returns: NDArray[np.float64]) -> RiskMeasures:
		...
		"""
		return RiskMeasures(
			float_mu=0.001, float_sigma=0.02, array_r=sample_returns, float_cl=0.95, int_t=1
		)
	...
	def test_tracking_error_calculation(
		self, risk_measures: RiskMeasures, sample_returns: NDArray[np.float64]
	) -> None:
		...
		None
		"""
		tracking_error = risk_measures.tracking_error(sample_returns, sample_returns * 0.9)
...
class TestQuoteVar:
	...
	def test_different_std_methods(
		self, sample_returns: NDArray[np.float64], method: Literal["std", "ewma_std"]
	) -> None:
...
class TestPortfVar:
	...
	def test_mismatched_shapes_raises_error(self, sample_returns: NDArray[np.float64]) -> None:
