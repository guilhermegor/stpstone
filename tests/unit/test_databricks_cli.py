...
class TestJobsCLIValidation:
	...
	def test_validate_job_id_non_positive(
		self, jobs_cli_instance: JobsCLI, invalid_job_id: int
	) -> None:
	...
	def test_validate_filename_valid(
		self, jobs_cli_instance: JobsCLI, valid_filename: str
	) -> None:
...
class TestJobsCLIOperations:
	...
	def test_job_run_without_params(
		...
		with patch("builtins.open", MagicMock()), patch("json.load", return_value=output_data):
...
class TestDbfsCLIValidation:
	...
	@pytest.mark.parametrize(
		"valid_path", ["dbfs:/path/to/file", "dbfs:/FileStore/tables/data.csv"]
	)
...
class TestDbfsCLIOperations:
	...
	def test_list_files_detailed(
		...
		mock_output = (
			"drwxr-xr-x 0 dbfs:/path/to/list/file1.txt\n-rw-r--r-- 1 "
...
class TestDbfsCLICopyAndRun:
	...
	def test_copy_and_run_success(
		...
		with (
			patch.object(dbfs_cli_instance, "copy") as mock_copy,
			...
			with patch(
				"stpstone.utils.connections.databases.spark.databricks_cli.JobsCLI"
			) as mock_jobs_cli:
