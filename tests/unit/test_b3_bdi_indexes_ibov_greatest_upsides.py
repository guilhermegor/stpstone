"""Unit tests for B3BdiIndexesIbovGreatestUpsides class."""
...
@pytest.fixture
def instance(sample_date: date) -> B3BdiIndexesIbovGreatestUpsides:
	"""Fixture providing a B3BdiIndexesIbovGreatestUpsides instance.
	...
	"""
	return B3BdiIndexesIbovGreatestUpsides(date_ref=sample_date)
...
def sample_table_dict() -> dict:
	...
	-------
	dict
		Sample table dict mimicking the BDI IbovespaStockBiggestHighs API response.
	"""
	return {
		"columns": [
			{"name": "TckrSymb"},
...
def test_init_with_valid_inputs(sample_date: date) -> None:
	...
	None
	"""
	inst = B3BdiIndexesIbovGreatestUpsides(date_ref=sample_date, int_page_size=500)
	...
	assert "IbovespaStockBiggestHighs" in inst.url_tpl
...
def test_init_default_page_size(sample_date: date) -> None:
	...
	None
	"""
	inst = B3BdiIndexesIbovGreatestUpsides(date_ref=sample_date)
...
def test_init_without_date_ref(mocker: MockerFixture) -> None:
	...
	inst = B3BdiIndexesIbovGreatestUpsides()
...
def test_init_logger_propagated() -> None:
	...
	inst = B3BdiIndexesIbovGreatestUpsides(date_ref=date(2026, 4, 17), logger=mock_logger)
	assert inst.logger is mock_logger


def test_get_response_success(
	instance: B3BdiIndexesIbovGreatestUpsides,
	mocker: MockerFixture,
) -> None:
...
def test_parse_raw_file_missing_table_key(instance: B3BdiIndexesIbovGreatestUpsides) -> None:
...
def test_transform_data_normal(
	...
	assert len(df_) == 1
	assert list(df_.columns) == ["TCKR_SYMB", "COD", "PRICE", "OSCILLATION"]
	assert df_["TCKR_SYMB"].iloc[0] == "POMO4"
	assert df_["COD"].iloc[0] == "PN      N2"
	assert df_["PRICE"].iloc[0] == 7.01
	assert df_["OSCILLATION"].iloc[0] == 2.18


def test_transform_data_multiple_rows(instance: B3BdiIndexesIbovGreatestUpsides) -> None:
	...
	table = {
		...
		"values": [
			["POMO4", "PN      N2", 7.01, 2.18, None],
			["VALE3", "ON      NM", 89.75, 2.64, None],
			["VAMO3", "ON      NM", 4.41, 6.26, None],
		],
	}
	df_ = instance.transform_data(table)
	assert len(df_) == 3
	assert set(df_["TCKR_SYMB"].tolist()) == {"POMO4", "VALE3", "VAMO3"}
...
def test_module_reload(sample_date: date) -> None:
	...
	import stpstone.ingestion.countries.br.exchange.b3_bdi_indexes_ibov_greatest_upsides as mod

	importlib.reload(mod)
	inst = mod.B3BdiIndexesIbovGreatestUpsides(date_ref=sample_date)
	assert inst.date_ref == sample_date
	assert "IbovespaStockBiggestHighs" in inst.url_tpl
