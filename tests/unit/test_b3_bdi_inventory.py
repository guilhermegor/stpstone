"""Unit tests for B3BdiInventory class."""
...
@pytest.fixture
def instance(sample_date: date) -> B3BdiInventory:
	"""Fixture providing a B3BdiInventory instance.
	...
	"""
	return B3BdiInventory(date_ref=sample_date)
...
def test_init_with_valid_inputs(sample_date: date) -> None:
	...
	None
	"""
	inst = B3BdiInventory(date_ref=sample_date, int_page_size=500)
...
def test_init_default_page_size(sample_date: date) -> None:
	...
	None
	"""
	inst = B3BdiInventory(date_ref=sample_date)
...
def test_init_logger_propagated() -> None:
	...
	inst = B3BdiInventory(date_ref=date(2026, 4, 17), logger=mock_logger)
	assert inst.logger is mock_logger


def test_get_response_success(instance: B3BdiInventory, mocker: MockerFixture) -> None:
	"""Test get_response posts to the correct URL and returns the response.

	Parameters
	----------
	instance : B3BdiInventory
		Initialized instance.
	mocker : MockerFixture
		Pytest-mock fixture.
...
def test_get_response_http_error(instance: B3BdiInventory, mocker: MockerFixture) -> None:
...
def test_transform_data_normal(instance: B3BdiInventory, sample_table_dict: dict) -> None:
	...
	assert df_["TCKR_SYMB"].iloc[0] == "CCB"
	assert df_["QUANTITY_ISSUED"].iloc[0] == 3128125015
	assert df_["QUANTITY_MARKET"].iloc[0] == 3128122915


def test_transform_data_with_null_volumes(instance: B3BdiInventory) -> None:
	"""Test transform_data handles rows with None VolumeMarket/VolumeDeposited.

	Rows where VolumeMarket and VolumeDeposited are null (e.g. instrument CDAWA)
	must survive transform without raising an error.
	...
	df_ = instance.transform_data(table_dict_with_nulls)
	assert isinstance(df_, pd.DataFrame)
	assert len(df_) == 1
	assert df_["TCKR_SYMB"].iloc[0] == "CDAWA"
	assert pd.isna(df_["VOLUME_MARKET"].iloc[0])
	assert pd.isna(df_["VOLUME_DEPOSITED"].iloc[0])
	assert df_["QUANTITY_INSTRUMENT"].iloc[0] == 352


def test_transform_data_empty_values(instance: B3BdiInventory, empty_table_dict: dict) -> None:
