"""Unit tests for B3TradingHoursRealDenominatedInterestRates ingestion class."""
...
from stpstone.ingestion.countries.br.exchange.b3_trading_hours_real_denominated_interest_rates import (  # noqa: E501
...
def mock_dataframe() -> pd.DataFrame:
	...
	"""
	return pd.DataFrame({"CONTRACT": ["DI1"], "TICKER": ["DI1F26"]})
...
def test_init_correct_url() -> None:
	"""Test that the correct B3 interest rates URL is set on initialization.

	Returns
	-------
	None
	"""
	instance = B3TradingHoursRealDenominatedInterestRates(date_ref=date(2025, 9, 12))
	assert instance.url == (
		"https://www.b3.com.br/en_us/solutions/platforms/puma-trading-system/"
		"for-members-and-traders/trading-hours/derivatives/interest-rates/"
	)


def test_init_inherits_from_core() -> None:
	"""Test that B3TradingHoursRealDenominatedInterestRates inherits from B3TradingHoursCore.
...
def test_run_returns_dataframe_without_db(
	...
	None
	"""
	instance = B3TradingHoursRealDenominatedInterestRates(date_ref=date(2025, 9, 12), cls_db=None)
	mocker.patch("requests.get", return_value=mock_response)
...
def test_run_with_db_returns_none(
	...
	instance = B3TradingHoursRealDenominatedInterestRates(
