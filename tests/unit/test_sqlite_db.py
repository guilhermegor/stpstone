...
def test_init_invalid_path_type(
    mocker: MockerFixture, 
    mock_logger: Mock
) -> None:
...
def test_read_with_date_conversion(
    ...
    None
    """
    mocker.patch.object(ABCCalendarOperations, "str_date_to_datetime", 
...
def test_backup_subprocess_error(
    ...
    mocker.patch("subprocess.run", side_effect=subprocess.CalledProcessError(
        1, ["sqlite3"], stderr="Backup failed"))
...
def test_check_bkp_tool_install_success(
    mocker: MockerFixture, 
    sqlite_db: SQLiteDB
) -> None:
