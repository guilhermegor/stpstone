"""Unit tests for IRSBRShareholdersEducation class.
...
# Helpers
# --------------------------
def _build_zip_bytes(csv_content: str, csv_filename: str = "Qualificacoes.csv") -> BytesIO:
	...
	csv_filename : str, optional
		The name of the CSV entry, by default "Qualificacoes.csv".
...
def _sample_csv_row() -> str:
	...
	-------
	str
		One complete data row with 2 fields.
	"""
	return "05;ADMINISTRADOR"
...
@pytest.fixture
def irsbr_instance(sample_date: date) -> IRSBRShareholdersEducation:
	"""Fixture providing IRSBRShareholdersEducation instance.
	...
	-------
	IRSBRShareholdersEducation
		Initialized IRSBRShareholdersEducation instance.
	"""
	return IRSBRShareholdersEducation(date_ref=sample_date)
...
def test_init_with_valid_inputs(sample_date: date) -> None:
	...
	Verifies
	--------
	- The IRSBRShareholdersEducation instance is initialized correctly.
	...
	None
	"""
	instance = IRSBRShareholdersEducation(date_ref=sample_date)
...
def test_init_with_default_date() -> None:
	...
	with patch.object(DatesBRAnbima, "add_working_days", return_value=date(2025, 3, 14)):
		instance = IRSBRShareholdersEducation()
...
def test_get_response_success(
	irsbr_instance: IRSBRShareholdersEducation,
	...
	----------
	irsbr_instance : IRSBRShareholdersEducation
		Instance of IRSBRShareholdersEducation.
	mock_requests_get : object
...
def test_transform_data(
	...
	assert len(df_) == 3
	assert df_["CODE"].iloc[0] == "05"
	assert df_["DESCRIPTION"].iloc[0] == "ADMINISTRADOR"


def test_transform_data_none_input(irsbr_instance: IRSBRShareholdersEducation) -> None:
...
def test_transform_data_invalid_zip(irsbr_instance: IRSBRShareholdersEducation) -> None:
...
def test_transform_data_zip_without_csv(irsbr_instance: IRSBRShareholdersEducation) -> None:
...
def test_run_without_db(
	...
	"""
	mock_requests_get.return_value = mock_response
	sample_df = pd.DataFrame({"CODE": ["05"], "DESCRIPTION": ["ADMINISTRADOR"]})
	with (
...
def test_run_http_error(
	...
	"""Test run method when HTTP request fails.

	Verifies
	--------
	- Returns empty DataFrame when the file download fails.
...
def test_run_single_file_url(
	irsbr_instance: IRSBRShareholdersEducation,
	mock_requests_get: object,
	mock_response: Response,
	mock_backoff: object,
) -> None:
	"""Test that run fetches exactly one file (Qualificacoes.zip).

	Verifies
	--------
	- requests.get is called exactly once.
	- The URL contains Qualificacoes.zip (no numeric suffix).
	...
	with (
		...
		call_url = mock_requests_get.call_args_list[0][0][0]
		assert "Qualificacoes.zip" in call_url
		assert "2025-03" in call_url
...
def test_reload_module() -> None:
	...
	import stpstone.ingestion.countries.br.taxation.irsbr_shareholders_education

	original_instance = IRSBRShareholdersEducation(date_ref=date(2025, 3, 15))
	importlib.reload(stpstone.ingestion.countries.br.taxation.irsbr_shareholders_education)
	new_instance = IRSBRShareholdersEducation(date_ref=date(2025, 3, 15))
