...
def test_init_default(b3_instance: B3FinancialIndicators) -> None:
    ...
    assert b3_instance.url == "https://sistemaswebb3-derivativos.b3.com.br/financialIndicatorsProxy/FinancialIndicators/GetFinancialIndicators/eyJsYW5ndWFnZSI6InB0LWJyIn0="
...
@pytest.mark.parametrize("invalid_response", [
    MagicMock(spec=PlaywrightPage),
    MagicMock(spec=SeleniumWebDriver)
])
...
def test_transform_data(
    ...
    assert list(df_.columns) == [
        "securityIdentificationCode", "description", "groupDescription",
        "value", "rate", "lastUpdate"
    ]
...
def test_insert_table_db(
    b3_instance: B3FinancialIndicators, mock_db_session: MagicMock
) -> None:
