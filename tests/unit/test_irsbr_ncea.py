"""Unit tests for IRSBRNCEA class.
...
from stpstone.ingestion.countries.br.taxation.irsbr_ncea import (
	_COLUMN_NAMES,
	IRSBRNCEA,
)
...
# Helpers
# --------------------------
def _build_zip_bytes(csv_content: str, csv_filename: str = "Cnaes.csv") -> BytesIO:
	...
	csv_filename : str, optional
		The name of the CSV entry, by default "Cnaes.csv".
...
def _sample_csv_row() -> str:
	...
	"""
	return "0111301;Cultivo de arroz"
...
@pytest.fixture
def irsbr_instance(sample_date: date) -> IRSBRNCEA:
	"""Fixture providing IRSBRNCEA instance.
	...
	-------
	IRSBRNCEA
		Initialized IRSBRNCEA instance.
	"""
	return IRSBRNCEA(date_ref=sample_date)
...
def test_init_with_valid_inputs(sample_date: date) -> None:
	...
	Verifies
	--------
	- The IRSBRNCEA instance is initialized correctly.
	...
	None
	"""
	instance = IRSBRNCEA(date_ref=sample_date)
...
def test_transform_data(
	...
	assert len(df_) == 3
	assert df_["CODE"].iloc[0] == "0111301"
	assert df_["DESCRIPTION"].iloc[0] == "Cultivo de arroz"


def test_transform_data_none_input(irsbr_instance: IRSBRNCEA) -> None:
...
def test_transform_data_invalid_zip(irsbr_instance: IRSBRNCEA) -> None:
...
def test_transform_data_zip_without_csv(irsbr_instance: IRSBRNCEA) -> None:
...
def test_run_without_db(
	...
	"""
	mock_requests_get.return_value = mock_response
	sample_df = pd.DataFrame({"CODE": ["0111301"], "DESCRIPTION": ["Cultivo de arroz"]})
	with (
...
def test_run_single_file_url(
	...
	"""Test that run fetches exactly one file (Cnaes.zip).

	Verifies
	--------
	- requests.get is called exactly once.
	- The URL contains Cnaes.zip (no numeric suffix).
...
def test_reload_module() -> None:
	...
	import stpstone.ingestion.countries.br.taxation.irsbr_ncea

	original_instance = IRSBRNCEA(date_ref=date(2025, 3, 15))
	importlib.reload(stpstone.ingestion.countries.br.taxation.irsbr_ncea)
	new_instance = IRSBRNCEA(date_ref=date(2025, 3, 15))
