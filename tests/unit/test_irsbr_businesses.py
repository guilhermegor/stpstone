"""Unit tests for IRSBRBusinesses class.
...
from stpstone.ingestion.countries.br.taxation.irsbr_businesses import (
	_COLUMN_NAMES,
	_NUM_FILES,
	IRSBRBusinesses,
)
...
# Helpers
# --------------------------
def _build_zip_bytes(csv_content: str, csv_filename: str = "Estabelecimentos0.csv") -> BytesIO:
	...
	csv_filename : str, optional
		The name of the CSV entry, by default "Estabelecimentos0.csv".
...
def _sample_csv_row() -> str:
	...
	-------
	str
		One complete data row with 30 fields.
	"""
	return (
		"12345678;0001;00;1;Empresa Teste LTDA;02;20200115;01;"
		";;20150310;4781400;;AVENIDA;PAULISTA;1000;SALA 10;"
		"BELA VISTA;01310100;SP;7107;11;99999999;11;88888888;"
		"11;77777777;teste@email.com;;"
	)
...
@pytest.fixture
def irsbr_instance(sample_date: date) -> IRSBRBusinesses:
	"""Fixture providing IRSBRBusinesses instance.
	...
	-------
	IRSBRBusinesses
		Initialized IRSBRBusinesses instance.
	"""
	return IRSBRBusinesses(date_ref=sample_date)
...
def test_init_with_valid_inputs(sample_date: date) -> None:
	...
	Verifies
	--------
	- The IRSBRBusinesses instance is initialized correctly.
	...
	None
	"""
	instance = IRSBRBusinesses(date_ref=sample_date)
...
def test_get_response_success(
	irsbr_instance: IRSBRBusinesses,
...
def test_transform_data(
	...
	assert df_["COMPANY_NAME"].iloc[0] == "Empresa Teste LTDA"


def test_transform_data_none_input(irsbr_instance: IRSBRBusinesses) -> None:
...
def test_transform_data_invalid_zip(irsbr_instance: IRSBRBusinesses) -> None:
...
def test_transform_data_zip_without_csv(irsbr_instance: IRSBRBusinesses) -> None:
...
def test_run_iterates_over_all_files(
	...
	with (
		...
		for i in range(_NUM_FILES):
			...
			assert f"Estabelecimentos{i}.zip" in call_url
...
def test_reload_module() -> None:
	...
	import stpstone.ingestion.countries.br.taxation.irsbr_businesses

	original_instance = IRSBRBusinesses(date_ref=date(2025, 3, 15))
	importlib.reload(stpstone.ingestion.countries.br.taxation.irsbr_businesses)
	new_instance = IRSBRBusinesses(date_ref=date(2025, 3, 15))
