...
def sample_json_data() -> list[dict[str, Any]]:
	...
	"""
	return [{"id": 1, "name": "Alice", "age": 30}, {"id": 2, "name": "Bob", "age": 25}]
...
class TestPostgreSQLDBConnection:
	...
	def test_init_with_valid_config(
		self, mock_connect: Mock, postgresql_db_config: dict[str, Any], mock_connection: MagicMock
	) -> None:
	...
	def test_init_connection_failure(
		self, mock_connect: Mock, postgresql_db_config: dict[str, Any]
	) -> None:
...
class TestReadMethod:
	...
	def test_read_with_type_conversion(
		...
		result = db.read("SELECT * FROM users", dict_type_cols={"id": "int64", "name": "string"})
	...
	def test_read_with_date_conversion(
		...
		_ = db.read("SELECT * FROM users", list_cols_dt=["created_at"], str_fmt_dt="YYYY-MM-DD")
