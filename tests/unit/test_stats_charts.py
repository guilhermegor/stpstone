...
class TestVisualizationMethods:
	...
	def test_confusion_mtx_2x2_valid(
		...
		mocker.patch.object(
			...
			return_value={"confusion_matrix": np.array([[1, 0], [0, 1]], dtype=np.int64)},
		)
	...
	def test_confusion_mtx_nxn_zero_row_sums(
		self, prob_stats_charts: ProbStatsCharts, mocker: MockerFixture
	) -> None:
	...
	def test_ecdf_chart_valid(
		...
		"""
		mocker.patch.object(
			NormalDistribution, "ecdf", return_value=(np.array([1, 2]), np.array([0.2, 0.8]))
		)
		mocker.patch.object(plt, "show")
		prob_stats_charts.ecdf_chart(sample_ecdf_data)
		plt.show.assert_called_once()

	def test_ecdf_chart_invalid_dict(self, prob_stats_charts: ProbStatsCharts) -> None:
	...
	def test_pandas_histogram_columns_invalid_bins(
		self, prob_stats_charts: ProbStatsCharts, sample_dataframe: pd.DataFrame
	) -> None:
	...
	def test_plot_precision_recall_vs_threshold_valid(
		...
		mocker.patch(
			...
			return_value=(np.array([0.9, 0.8]), np.array([0.7, 0.6]), np.array([0.1, 0.2])),
		)
		mocker.patch.object(plt, "show")
	...
	def test_plot_roc_curve_valid(
		...
		mocker.patch(
			...
			return_value=(np.array([0, 0.5, 1]), np.array([0, 0.5, 1]), np.array([0.1, 0.2])),
		)
		mocker.patch.object(plt, "show")
	...
	def test_abline_valid(self, prob_stats_charts: ProbStatsCharts, mocker: MockerFixture) -> None:
	...
	def test_classification_plot_2d_ivs_valid(
		...
		mocker.patch.object(
			mock_model, "predict", return_value=np.zeros(np.prod(expected_shape), dtype=np.float64)
		)
		mocker.patch.object(plt, "show")
	...
	def test_classification_plot_2d_ivs_invalid_shape(
		self, prob_stats_charts: ProbStatsCharts, mock_scaler: Mock, mock_model: Mock
	) -> None:
