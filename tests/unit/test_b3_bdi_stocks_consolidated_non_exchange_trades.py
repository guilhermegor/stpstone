"""Unit tests for B3BdiStocksConsolidatedNonExchangeTrades class."""
...
@pytest.fixture
def instance(sample_date: date) -> B3BdiStocksConsolidatedNonExchangeTrades:
	"""Fixture providing a B3BdiStocksConsolidatedNonExchangeTrades instance.
	...
	"""
	return B3BdiStocksConsolidatedNonExchangeTrades(date_ref=sample_date)
...
def sample_table_dict() -> dict:
	...
	-------
	dict
		Sample table dict mimicking the BDI ConsolidatedTradesRVAfter API response.
	"""
	return {
		"columns": [
			{"name": "RptDt"},
...
def mock_response(sample_table_dict: dict) -> Response:
	"""Mock Response with a two-row table payload.
...
def test_init_with_valid_inputs(sample_date: date) -> None:
	...
	None
	"""
	inst = B3BdiStocksConsolidatedNonExchangeTrades(date_ref=sample_date, int_page_size=500)
	...
	assert "ConsolidatedTradesRVAfter" in inst.url_tpl
...
def test_init_default_page_size(sample_date: date) -> None:
	...
	None
	"""
	inst = B3BdiStocksConsolidatedNonExchangeTrades(date_ref=sample_date)
...
def test_get_response_success(
	instance: B3BdiStocksConsolidatedNonExchangeTrades,
	mocker: MockerFixture,
) -> None:
...
def test_transform_data_normal(
	...
	assert df_["TCKR_SYMB"].iloc[0] == "FIXA11"
	assert df_["ISIN"].iloc[0] == "BRFIXACTF002"
	assert df_["TRAD_AVRG_PRIC"].iloc[0] == 19.2
	assert df_["FIN_INSTRM_QTY"].iloc[0] == 1
