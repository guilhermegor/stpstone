"""Unit tests for B3BdiStocksSummary class."""
...
@pytest.fixture
def instance(sample_date: date) -> B3BdiStocksSummary:
	"""Fixture providing a B3BdiStocksSummary instance.
	...
	"""
	return B3BdiStocksSummary(date_ref=sample_date)
...
def sample_table_dict() -> dict:
	...
	-------
	dict
		Sample table dict mimicking the BDI DailyAverageStocks API response structure.
	"""
	return {
		"columns": [
			{"name": "TckrSymb"},
...
def test_init_with_valid_inputs(sample_date: date) -> None:
	...
	None
	"""
	inst = B3BdiStocksSummary(date_ref=sample_date, int_page_size=500)
	...
	assert "DailyAverageStocks" in inst.url_tpl
...
def test_init_default_page_size(sample_date: date) -> None:
	...
	None
	"""
	inst = B3BdiStocksSummary(date_ref=sample_date)
...
def test_init_logger_propagated() -> None:
	...
	inst = B3BdiStocksSummary(date_ref=date(2026, 4, 17), logger=mock_logger)
	assert inst.logger is mock_logger


def test_get_response_success(instance: B3BdiStocksSummary, mocker: MockerFixture) -> None:
	"""Test get_response posts to the correct URL and returns the response.

	Parameters
	----------
	instance : B3BdiStocksSummary
		Initialized instance.
	mocker : MockerFixture
		Pytest-mock fixture.
...
def test_get_response_http_error(instance: B3BdiStocksSummary, mocker: MockerFixture) -> None:
...
def test_transform_data_normal(instance: B3BdiStocksSummary, sample_table_dict: dict) -> None:
	...
	assert len(df_) == 1
	assert list(df_.columns) == ["TCKR_SYMB", "NMBR_TRADES_DAY", "VLM_TRADED_DAY", "COL_ORDER"]
	assert df_["TCKR_SYMB"].iloc[0] == "Dia"
	assert df_["NMBR_TRADES_DAY"].iloc[0] == 4296181
	assert df_["VLM_TRADED_DAY"].iloc[0] == 81405.95


def test_transform_data_all_periods(instance: B3BdiStocksSummary) -> None:
	"""Test transform_data handles all 9 period rows correctly.
	...
	table = {
		...
		"values": [
			["Dia", 4296181, 81405.95, 1],
			["Mês", 85923620, 1628119.00, 2],
			["Mês anterior", 84001200, 1596000.00, 3],
			["Trimestre", 257000000, 4884000.00, 4],
			["Trimestre anterior", 250000000, 4750000.00, 5],
			["Semestre", 514000000, 9768000.00, 6],
			["Semestre anterior", 510000000, 9690000.00, 7],
			["Ano", 1028000000, 19536000.00, 8],
			["Ano anterior", 1020000000, 19380000.00, 9],
		],
	}
...
def test_transform_data_empty_values(instance: B3BdiStocksSummary, empty_table_dict: dict) -> None:
