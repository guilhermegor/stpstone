...
def test_get_response_success(
	...
	mock_requests_get.assert_called_once_with(bvmf_instance.url, timeout=(12.0, 21.0), verify=True)
...
def test_get_response_empty_tables(
	...
	mock_requests_get.return_value = mock_response
	with (
		patch.object(bvmf_instance, "parse_raw_file", return_value=mock_empty_html_root),
...
def test_parse_raw_file(
	...
	"""
	with patch.object(
		bvmf_instance.cls_html_handler, "bs_parser", return_value=mock_html_root
	) as mock_parser:
...
def test_run_without_db(
	...
	mock_requests_get.return_value = mock_response
	with (
		patch.object(bvmf_instance, "parse_raw_file", return_value=mock_html_root),
...
def test_run_with_db(
	...
	with (
		...
		result = bvmf_instance.run(
			timeout=(12.0, 21.0), bool_verify=True, str_table_name="test_table"
		)
