"""Unit tests for B3BdiSecuritiesFungibleCustody class."""
...
@pytest.fixture
def instance(sample_date: date) -> B3BdiSecuritiesFungibleCustody:
	"""Fixture providing a B3BdiSecuritiesFungibleCustody instance.
	...
	"""
	return B3BdiSecuritiesFungibleCustody(date_ref=sample_date)
...
def sample_table_dict() -> dict:
	...
	-------
	dict
		Sample table dict mimicking the BDI FugibleCustody API response.
	"""
	return {
		"columns": [
			{"name": "SbscrptnDeadline"},
			{"name": "DeadlineAssgnmnt"},
			{"name": "TckrSymb"},
			{"name": "IsinOrigin"},
			{"name": "IsinRight"},
			{"name": "IsinSbscrptn"},
		...
		"values": [
			[
				"2026-04-20T00:00:00",
				"2026-04-20T00:00:00",
				"ZAGROS MULTIESTRATEGIA FDO DE INV IMOB",
				"BRZAGHCTF007 127",
				"BRZAGHD02M11 127",
				"BRZAGHR03M14 128",
				None,
...
def test_init_with_valid_inputs(sample_date: date) -> None:
	...
	None
	"""
	inst = B3BdiSecuritiesFungibleCustody(date_ref=sample_date, int_page_size=500)
	...
	assert "FugibleCustody" in inst.url_tpl
...
def test_init_default_page_size(sample_date: date) -> None:
	...
	None
	"""
	inst = B3BdiSecuritiesFungibleCustody(date_ref=sample_date)
...
def test_init_without_date_ref(mocker: MockerFixture) -> None:
	...
	inst = B3BdiSecuritiesFungibleCustody()
...
def test_init_logger_propagated() -> None:
	...
	inst = B3BdiSecuritiesFungibleCustody(date_ref=date(2026, 4, 17), logger=mock_logger)
	assert inst.logger is mock_logger


def test_get_response_success(
	instance: B3BdiSecuritiesFungibleCustody,
	mocker: MockerFixture,
) -> None:
...
def test_parse_raw_file_missing_table_key(instance: B3BdiSecuritiesFungibleCustody) -> None:
...
def test_transform_data_normal(
	...
	instance : B3BdiSecuritiesFungibleCustody
		Initialized instance.
	sample_table_dict : dict
		Sample table dict with one row (7 values, 6 columns).
	...
	assert df_["TCKR_SYMB"].iloc[0] == "ZAGROS MULTIESTRATEGIA FDO DE INV IMOB"
	assert df_["ISIN_ORIGIN"].iloc[0] == "BRZAGHCTF007 127"


def test_transform_data_multiple_rows(instance: B3BdiSecuritiesFungibleCustody) -> None:
	...
	assert len(df_) == 2
	assert "ZAGROS MULTIESTRATEGIA FDO DE INV IMOB" in df_["TCKR_SYMB"].tolist()
	assert "INFRACOMMERCE CXAAS S.A." in df_["TCKR_SYMB"].tolist()
...
def test_run_without_db(
	...
	assert len(result) == 1
	assert "SBSCRPTN_DEADLINE" in result.columns
	assert "DEADLINE_ASSGNMNT" in result.columns
	assert "TCKR_SYMB" in result.columns
	assert "ISIN_ORIGIN" in result.columns
	assert "ISIN_RIGHT" in result.columns
	assert "ISIN_SBSCRPTN" in result.columns
	assert "URL" in result.columns
	assert result["ISIN_ORIGIN"].iloc[0] == "BRZAGHCTF007 127"
...
def test_module_reload(sample_date: date) -> None:
	...
	import stpstone.ingestion.countries.br.exchange.b3_bdi_securities_fungible_custody as mod

	importlib.reload(mod)
	inst = mod.B3BdiSecuritiesFungibleCustody(date_ref=sample_date)
	assert inst.date_ref == sample_date
	assert "FugibleCustody" in inst.url_tpl
