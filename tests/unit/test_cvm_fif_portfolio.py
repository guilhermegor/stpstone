"""Unit tests for CvmFIFPortfolio ingestion module."""
...
def mock_zip_response() -> MagicMock:
	...
	with zipfile.ZipFile(zip_buffer, "w") as zip_file:
		zip_file.writestr("lamina_fi_carteira_202312.csv", "CNPJ;TP_ATIVO\n12345678000195;ACAO")
...
def sample_csv_content() -> str:
	...
	str
		Sample CSV data.
	"""
	return "CNPJ_FUNDO_CLASSE;TP_ATIVO;PR_PL_ATIVO\n12345678000195;ACAO;0.5"


@pytest.fixture
def sample_dataframe() -> pd.DataFrame:
	"""Provide sample FIF portfolio DataFrame.
...
class TestFIFPortfolio:
	...
	def test_init_url_construction(self, mock_logger: MagicMock, sample_date: date) -> None:
		...
		None
		"""
		instance = CvmFIFPortfolio(date_ref=sample_date, logger=mock_logger)
	...
	def test_get_response_success(
		...
		"""
		with patch(
			"stpstone.ingestion.countries.br.registries.cvm_fif_portfolio.requests.get"
		) as mock_get:
	...
	def test_parse_raw_file_extracts_carteira(
		self,
		mock_logger: MagicMock,
		sample_date: date,
		mock_zip_response: MagicMock,
	) -> None:
		"""Test parse_raw_file extracts the carteira CSV file.
		...
		"""
		with patch(
			"stpstone.ingestion.countries.br.registries.cvm_fif_portfolio.DirFilesManagement"
		) as mock_dir_files:
	...
	def test_parse_raw_file_carteira_not_found_raises(
		self,
		mock_logger: MagicMock,
		sample_date: date,
		mock_zip_response: MagicMock,
	) -> None:
		"""Test ValueError raised when carteira file not found.
		...
		with patch(
			...
			mock_dir_files_instance = MagicMock()
			mock_dir_files_instance.recursive_unzip_in_memory.return_value = [
				(BytesIO(b"binary data"), "unrelated_file.bin")
			]
			mock_dir_files.return_value = mock_dir_files_instance
	...
	def test_transform_data_adds_file_name(
		...
		"""Test transform_data adds FILE_NAME with carteira format.
		...
		assert result_df["FILE_NAME"].iloc[0] == "lamina_fi_carteira_202312.csv"
	...
	def test_run_without_db(
		...
		"""
		with (
			patch.object(CvmFIFPortfolio, "get_response") as mock_get_response,
			patch.object(CvmFIFPortfolio, "parse_raw_file") as mock_parse,
			patch.object(CvmFIFPortfolio, "transform_data") as mock_transform,
			patch.object(CvmFIFPortfolio, "standardize_dataframe") as mock_standardize,
		):
	...
	def test_run_with_db(
		...
		with (
			...
			patch.object(CvmFIFPortfolio, "transform_data") as mock_transform,
			patch.object(CvmFIFPortfolio, "standardize_dataframe") as mock_standardize,
			patch.object(CvmFIFPortfolio, "insert_table_db") as mock_insert_db,
		):
			mock_get_response.return_value = MagicMock()
			mock_parse.return_value = (StringIO(), "test.csv")
			mock_transform.return_value = sample_dataframe
			...
			mock_insert_db.assert_called_once_with(
				...
				str_table_name="br_cvm_fif_portfolio",
