"""Unit tests for CvmFIFDailyInfos ingestion module."""
...
class TestFIFDailyInfos:
	"""Test cases for CvmFIFDailyInfos class."""
	...
	def test_init_with_default_date(self, mock_logger: MagicMock) -> None:
		...
		with (
			patch(
				"stpstone.ingestion.countries.br.registries.cvm_fif_daily_infos.DatesCurrent"
			) as mock_dates_current,
			patch(
				"stpstone.ingestion.countries.br.registries.cvm_fif_daily_infos.DatesBRAnbima"
			) as mock_dates_br,
		):
			...
			instance = CvmFIFDailyInfos(logger=mock_logger)
	...
	def test_init_with_custom_date(self, mock_logger: MagicMock, sample_date: date) -> None:
		...
		None
		"""
		instance = CvmFIFDailyInfos(date_ref=sample_date, logger=mock_logger)
	...
	def test_get_response_success(
		...
		"""
		with patch(
			"stpstone.ingestion.countries.br.registries.cvm_fif_daily_infos.requests.get"
		) as mock_get:
	...
	def test_parse_raw_file_success(
		...
		"""
		with patch(
			"stpstone.ingestion.countries.br.registries.cvm_fif_daily_infos.DirFilesManagement"
		) as mock_dir_files:
	...
	def test_run_without_db(
		...
		"""
		with (
			patch.object(CvmFIFDailyInfos, "get_response") as mock_get_response,
			patch.object(CvmFIFDailyInfos, "parse_raw_file") as mock_parse,
			patch.object(CvmFIFDailyInfos, "transform_data") as mock_transform,
			patch.object(CvmFIFDailyInfos, "standardize_dataframe") as mock_standardize,
		):
	...
	def test_run_with_db(
		...
		with (
			...
			patch.object(CvmFIFDailyInfos, "transform_data") as mock_transform,
			patch.object(CvmFIFDailyInfos, "standardize_dataframe") as mock_standardize,
			patch.object(CvmFIFDailyInfos, "insert_table_db") as mock_insert_db,
		):
			mock_get_response.return_value = MagicMock()
			mock_parse.return_value = (StringIO(), "test.csv")
			mock_transform.return_value = sample_dataframe
