...
def test_singleton_pattern() -> None:
    ...
    assert conn1.str_host == "localhost"
...
def test_save_df_insert_failure(mock_mongo_client: MagicMock, sample_dataframe: pd.DataFrame, mock_logger: Logger) -> None:
...
def test_reload_module(mock_mongo_client: MagicMock) -> None:
    ...
    import importlib
    conn1 = MongoConn()
    importlib.reload(sys.modules["stpstone.mongodb.mongo_conn"])
    conn2 = MongoConn()
    assert conn1 is conn2


def test_none_logger(mock_mongo_client: MagicMock, sample_dataframe: pd.DataFrame) -> None:
    ...
    conn._collection = Mock()
    conn.save_df(sample_dataframe)
    assert not CreateLog().log_message.called
...
def test_numeric_edge_cases(mock_mongo_client: MagicMock) -> None:
    ...
    assert conn1.int_port == 1
    conn2 = MongoConn(int_port=65535)
    assert conn2.int_port == 65535