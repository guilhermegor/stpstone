...
def test_save_df_valid(
	mongo_conn: MongoConn, sample_dataframe: pd.DataFrame, mocker: MockerFixture
) -> None:
	...
	assert mock_collection.insert_many.call_args[0][0] == sample_dataframe.to_dict(
		orient="records"
	)
...
def test_save_df_insertion_failure(
	...
	if mongo_conn.logger:
		mock_logger.assert_called_with(
			mongo_conn.logger, "ERROR Insertion failed, MONGODB INJECTION ABORTED", "info"
		)
