"""Unit tests for B3BdiFixedIncomeRepos class."""
...
from stpstone.ingestion.countries.br.otc.b3_bdi_fixed_income_repos import (
	B3BdiFixedIncomeRepos,
)
...
@pytest.fixture
def instance(sample_date: date) -> B3BdiFixedIncomeRepos:
	"""Fixture providing a B3BdiFixedIncomeRepos instance.
	...
	"""
	return B3BdiFixedIncomeRepos(date_ref=sample_date)
...
def sample_table_dict() -> dict:
	...
	return {
		"columns": [
			...
			{"name": "TckrSymb"},
			{"name": "AverageTerm"},
			{"name": "AverageFinancial"},
			{"name": "NumberOfDealings"},
			{"name": "FinancialValue"},
		],
		"values": [
			["2026-04-17T00:00:00", "CDCA", 28, 155532002.6, 1, 155532002.6],
		],
	}
...
def test_init_with_valid_inputs(sample_date: date) -> None:
	...
	None
	"""
	inst = B3BdiFixedIncomeRepos(date_ref=sample_date, int_page_size=500)
	...
	assert "RepurchaseDealings" in inst.url_tpl
...
def test_init_default_page_size(sample_date: date) -> None:
	...
	None
	"""
	inst = B3BdiFixedIncomeRepos(date_ref=sample_date)
...
def test_init_logger_propagated() -> None:
	...
	inst = B3BdiFixedIncomeRepos(date_ref=date(2026, 4, 17), logger=mock_logger)
...
def test_get_response_success(instance: B3BdiFixedIncomeRepos, mocker: MockerFixture) -> None:
	...
	Parameters
	----------
	instance : B3BdiFixedIncomeRepos
		Initialized instance.
	mocker : MockerFixture
		Pytest-mock fixture.
...
def test_transform_data_normal(instance: B3BdiFixedIncomeRepos, sample_table_dict: dict) -> None:
	...
	assert df_["AVERAGE_TERM"].iloc[0] == 28
	assert df_["NUMBER_OF_DEALINGS"].iloc[0] == 1


def test_transform_data_empty_values(
	instance: B3BdiFixedIncomeRepos, empty_table_dict: dict
) -> None:
