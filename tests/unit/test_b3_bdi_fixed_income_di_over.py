"""Unit tests for B3BdiFixedIncomeDiOver class."""
...
@pytest.fixture
def instance(sample_date: date) -> B3BdiFixedIncomeDiOver:
	"""Fixture providing a B3BdiFixedIncomeDiOver instance.
	...
	"""
	return B3BdiFixedIncomeDiOver(date_ref=sample_date)
...
def test_init_with_valid_inputs(sample_date: date) -> None:
	...
	None
	"""
	inst = B3BdiFixedIncomeDiOver(date_ref=sample_date, int_page_size=500)
...
def test_init_default_page_size(sample_date: date) -> None:
	...
	None
	"""
	inst = B3BdiFixedIncomeDiOver(date_ref=sample_date)
...
def test_init_logger_propagated() -> None:
	...
	inst = B3BdiFixedIncomeDiOver(date_ref=date(2026, 4, 17), logger=mock_logger)
	assert inst.logger is mock_logger


def test_get_response_success(instance: B3BdiFixedIncomeDiOver, mocker: MockerFixture) -> None:
	"""Test get_response posts to the correct URL and returns the response.

	Parameters
	----------
	instance : B3BdiFixedIncomeDiOver
		Initialized instance.
	mocker : MockerFixture
		Pytest-mock fixture.
...
def test_get_response_http_error(instance: B3BdiFixedIncomeDiOver, mocker: MockerFixture) -> None:
...
def test_transform_data_normal(instance: B3BdiFixedIncomeDiOver, sample_table_dict: dict) -> None:
