"""Unit tests for IRSBRCities class.
...
from stpstone.ingestion.countries.br.taxation.irsbr_cities import (
	_COLUMN_NAMES,
	_NUM_FILES,
	IRSBRCities,
)
...
# Helpers
# --------------------------
def _build_zip_bytes(csv_content: str, csv_filename: str = "Municipios.csv") -> BytesIO:
	...
	csv_filename : str
		The name of the CSV entry, by default "Municipios.csv".
...
@pytest.fixture
def irsbr_instance(sample_date: date) -> IRSBRCities:
	"""Fixture providing IRSBRCities instance.
	...
	-------
	IRSBRCities
		Initialized IRSBRCities instance.
	"""
	return IRSBRCities(date_ref=sample_date)
...
def test_init_with_valid_inputs(sample_date: date) -> None:
	...
	Verifies
	--------
	- The IRSBRCities instance is initialized correctly.
	...
	None
	"""
	instance = IRSBRCities(date_ref=sample_date)
...
def test_transform_data(
	...
	assert len(df_) == 3
	assert df_["CODE"].iloc[0] == "7107"
	assert df_["NAME"].iloc[0] == "SAO PAULO"


def test_transform_data_none_input(irsbr_instance: IRSBRCities) -> None:
...
def test_transform_data_invalid_zip(irsbr_instance: IRSBRCities) -> None:
...
def test_transform_data_zip_without_csv(irsbr_instance: IRSBRCities) -> None:
...
def test_run_without_db(
	...
	"""
	mock_requests_get.return_value = mock_response
	sample_df = pd.DataFrame({"CODE": ["7107"], "NAME": ["SAO PAULO"]})
	with (
...
def test_run_http_error(
	...
	"""Test run method when the HTTP request fails.
...
def test_run_single_file_url(
	...
	"""Test that run fetches a single Municipios.zip file (no numbered suffix).

	Verifies
	--------
	- requests.get is called exactly once.
	- The URL contains Municipios.zip without a numeric index.
	...
	with (
		...
		assert "Municipios.zip" in call_url
...
def test_reload_module() -> None:
	...
	import stpstone.ingestion.countries.br.taxation.irsbr_cities

	original_instance = IRSBRCities(date_ref=date(2025, 3, 15))
	importlib.reload(stpstone.ingestion.countries.br.taxation.irsbr_cities)
	new_instance = IRSBRCities(date_ref=date(2025, 3, 15))
