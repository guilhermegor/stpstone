...
class TestGPT:
	...
	def mixed_prompt(self) -> list[tuple]:
		...
		"""
		return [("text", "Describe this image"), ("image_url", "http://example.com/image.jpg")]

	def test_init_with_valid_params(
		self, valid_init_params: dict[str, Any], mock_openai_client: MagicMock
	) -> None:
	...
	@pytest.mark.parametrize(
		"api_key,type_error", [(None, TypeError), ("", ValueError), (123, TypeError)]
	)
	...
	@pytest.mark.parametrize(
		"model,type_error", [(None, TypeError), ("", ValueError), (123, TypeError)]
	)
	...
	def test_init_invalid_max_tokens(
		self, max_tokens: int, valid_init_params: dict[str, Any], mock_openai_client: MagicMock
	) -> None:
	...
	def test_run_prompt_with_image(
		...
		with patch("stpstone.utils.llms.gpt.gpt.OpenAI", return_value=mock_openai_client):
			...
			assert (
				call_args["messages"][-1]["content"][0]["image_url"]["url"]
