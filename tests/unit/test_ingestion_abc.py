...
def sample_regex_patterns() -> dict[str, dict[str, str]]:
	...
	"""
	return {"event1": {"pattern1": r"test\d+"}, "event2": {"pattern2": r"sample\d+"}}
...
def test_get_response_timeout_variations(
	ingestion_instance: ConcreteIngestion, timeout: Union[int, float, tuple]
) -> None:
...
def test_standardize_dataframe_valid_input(
	ingestion_instance: ConcreteIngestion, sample_dataframe: pd.DataFrame, mocker: MockerFixture
) -> None:
	...
	mock_df_standardization = mocker.patch.object(
		DFStandardization, "pipeline", return_value=sample_dataframe
	)
	...
	result = ingestion_instance.standardize_dataframe(
		df_=sample_dataframe, date_ref=date_ref, dict_dtypes=dict_dtypes
	)
...
def test_standardize_dataframe_invalid_df(
	ingestion_instance: ConcreteIngestion, invalid_df: Union[None, str, int]
) -> None:
	...
	with pytest.raises(TypeError, match="df_ must be of type DataFrame"):
		ingestion_instance.standardize_dataframe(
			df_=invalid_df, date_ref=date(2023, 1, 1), dict_dtypes={"col1": str}
		)
...
def test_standardize_dataframe_invalid_date(
	...
	with pytest.raises(TypeError, match="date_ref must be of type date"):
		ingestion_instance.standardize_dataframe(
			df_=sample_dataframe, date_ref=invalid_date, dict_dtypes={"col1": str}
		)
...
def test_insert_table_db_invalid_session(
	...
	with pytest.raises(TypeError, match="cls_db must be of type Session"):
		ingestion_instance.insert_table_db(
			cls_db=invalid_db, str_table_name="test_table", df_=sample_dataframe
		)


@pytest.mark.parametrize(
	"invalid_table_name,expected_exception,expected_message",
	[
...
def test_insert_table_db_invalid_table_name(
	...
	with pytest.raises(expected_exception, match=expected_message):
		ingestion_instance.insert_table_db(
			cls_db=mock_session, str_table_name=invalid_table_name, df_=sample_dataframe
		)
...
def test_insert_table_db_empty_table_name(
	...
	with pytest.raises(ValueError, match="str_table_name cannot be empty"):
		ingestion_instance.insert_table_db(
			cls_db=mock_session, str_table_name="", df_=sample_dataframe
		)
...
def test_paginate_text_blocks_valid(
	ingestion_instance: ConcreteIngestion, mock_fitz_document: fitz.Document, mocker: MockerFixture
) -> None:
	...
	"""
	mocker.patch.object(
		StrHandler, "remove_diacritics_nfkd", side_effect=lambda x, **kwargs: x.lower()
	)
...
def test_paginate_text_blocks_empty_document(
	ingestion_instance: ConcreteIngestion, mocker: MockerFixture
) -> None:
...
def test_paginate_text_blocks_invalid_pages_join(
	...
	if not isinstance(invalid_pages_join, int):
		with pytest.raises(TypeError, match="int_pages_join must be of type int"):
			ingestion_instance.paginate_text_blocks(
				mock_fitz_document, int_pages_join=invalid_pages_join
			)
	else:
...
def test_get_file_invalid_response(
	ingestion_instance: ConcreteIngestion, invalid_resp: Union[None, str, int]
) -> None:
	...
	"""
	with pytest.raises(
		TypeError, match="resp_req must be one of types: Response, Page, WebDriver"
	):
...
def test_pdf_docx_tables_response_invalid_file(
	ingestion_instance: ConcreteIngestion, invalid_file: Union[None, str, int]
) -> None:
...
def test_regex_patterns_match_valid(
	...
	result = ingestion_instance._regex_patterns_match(
		list_blocks_pages=["test123"], dict_regex_patterns=sample_regex_patterns
	)
	assert isinstance(result, list)
...
def test_regex_patterns_match_no_matches(
	...
	result = ingestion_instance._regex_patterns_match(
		list_blocks_pages=["no match"], dict_regex_patterns=sample_regex_patterns
	)
...
def test_regex_patterns_match_invalid_blocks(
	...
	with pytest.raises(TypeError, match="list_blocks_pages must be of type list"):
		ingestion_instance._regex_patterns_match(
			list_blocks_pages=invalid_blocks, dict_regex_patterns=sample_regex_patterns
		)
