"""Unit tests for B3BdiBtbRenewals class."""
...
@pytest.fixture
def instance(sample_date: date) -> B3BdiBtbRenewals:
	"""Fixture providing a B3BdiBtbRenewals instance.
	...
	"""
	return B3BdiBtbRenewals(date_ref=sample_date)
...
def sample_table_dict() -> dict:
	...
	-------
	dict
		Sample table dict mimicking the BDI Renewals API response.
	"""
	return {
		"columns": [
			{"name": "DtRef"},
			{"name": "TckrSymb"},
			{"name": "AssetObjectISIN"},
			{"name": "BTBContractQuantity"},
			{"name": "BTBContractFinancialValue"},
			{"name": "RemunerationRatePercent"},
			{"name": "TradeNumber"},
			{"name": "MarketType"},
			{"name": "RenewalType"},
			{"name": "LenderParticipantCode"},
			{"name": "LenderParticipantName"},
			{"name": "BorrowerParticipantCode"},
			{"name": "BorrowerParticipantName"},
			{"name": "Borrower"},
			{"name": "Lender"},
		],
		"values": [
...
def test_init_with_valid_inputs(sample_date: date) -> None:
	...
	None
	"""
	inst = B3BdiBtbRenewals(date_ref=sample_date, int_page_size=500)
	...
	assert "Renewals" in inst.url_tpl
...
def test_init_default_page_size(sample_date: date) -> None:
	...
	None
	"""
	inst = B3BdiBtbRenewals(date_ref=sample_date)
...
def test_init_logger_propagated() -> None:
	...
	inst = B3BdiBtbRenewals(date_ref=date(2026, 4, 17), logger=mock_logger)
	assert inst.logger is mock_logger


def test_get_response_success(
	instance: B3BdiBtbRenewals,
	mocker: MockerFixture,
) -> None:
...
def test_parse_raw_file_missing_table_key(instance: B3BdiBtbRenewals) -> None:
...
def test_transform_data_normal(
	...
	"""Test transform_data builds a DataFrame with correct UPPER_SNAKE columns.
	...
	expected_cols = [
		...
		"TCKR_SYMB",
		"ASSET_OBJECT_ISIN",
		"BTBCONTRACT_QUANTITY",
		"BTBCONTRACT_FINANCIAL_VALUE",
		"REMUNERATION_RATE_PERCENT",
		"TRADE_NUMBER",
		"MARKET_TYPE",
		"RENEWAL_TYPE",
		"LENDER_PARTICIPANT_CODE",
		"LENDER_PARTICIPANT_NAME",
		"BORROWER_PARTICIPANT_CODE",
		"BORROWER_PARTICIPANT_NAME",
		"BORROWER",
		"LENDER",
	]
	assert list(df_.columns) == expected_cols
	assert df_["TCKR_SYMB"].iloc[0] == "GGBR4"
	assert df_["BTBCONTRACT_QUANTITY"].iloc[0] == 182621
	assert df_["BTBCONTRACT_FINANCIAL_VALUE"].iloc[0] == 3953744.65
...
def test_run_without_db(
	...
	assert "BTBCONTRACT_FINANCIAL_VALUE" in result.columns
...
def test_module_reload(sample_date: date) -> None:
	...
	import stpstone.ingestion.countries.br.exchange.b3_bdi_btb_renewals as mod

	importlib.reload(mod)
	inst = mod.B3BdiBtbRenewals(date_ref=sample_date)
	assert inst.date_ref == sample_date
	assert "Renewals" in inst.url_tpl
