"""Unit tests for B3BdiStocksMarginScenariosLiquidAssets class."""
...
@pytest.fixture
def instance(sample_date: date) -> B3BdiStocksMarginScenariosLiquidAssets:
	"""Fixture providing a B3BdiStocksMarginScenariosLiquidAssets instance.
	...
	"""
	return B3BdiStocksMarginScenariosLiquidAssets(date_ref=sample_date)
...
def sample_table_dict() -> dict:
	...
	-------
	dict
		Sample table dict mimicking the BDI MarginScenarios API response structure.
	"""
	return {
		"columns": [
			{"name": "TckrSymb"},
...
def test_init_with_valid_inputs(sample_date: date) -> None:
	...
	None
	"""
	inst = B3BdiStocksMarginScenariosLiquidAssets(date_ref=sample_date, int_page_size=500)
	...
	assert "MarginScenarios" in inst.url_tpl
...
def test_init_default_page_size(sample_date: date) -> None:
	...
	None
	"""
	inst = B3BdiStocksMarginScenariosLiquidAssets(date_ref=sample_date)
	assert inst.int_page_size == 1_000


def test_init_default_page_max_is_none(sample_date: date) -> None:
	"""Test that default int_page_max is None (unlimited pagination).
...
def test_init_without_date_ref(mocker: MockerFixture) -> None:
	...
	inst = B3BdiStocksMarginScenariosLiquidAssets()
...
def test_init_logger_propagated() -> None:
	...
	inst = B3BdiStocksMarginScenariosLiquidAssets(date_ref=date(2026, 4, 17), logger=mock_logger)
	assert inst.logger is mock_logger


def test_get_response_success(
	instance: B3BdiStocksMarginScenariosLiquidAssets, mocker: MockerFixture
) -> None:
	"""Test get_response posts to the correct URL and returns the response.

	Parameters
	----------
	instance : B3BdiStocksMarginScenariosLiquidAssets
		Initialized instance.
	mocker : MockerFixture
		Pytest-mock fixture.
...
def test_transform_data_normal(
	instance: B3BdiStocksMarginScenariosLiquidAssets, sample_table_dict: dict
) -> None:
	...
	assert len(df_) == 1
	assert list(df_.columns) == ["TCKR_SYMB", "VRTX_CD", "SCNRO_ID", "PRFVAL", "TP_SHCK"]
	assert df_["TCKR_SYMB"].iloc[0] == "VLDIRR310"
	assert df_["VRTX_CD"].iloc[0] == 216
	assert df_["SCNRO_ID"].iloc[0] == 9998
	assert df_["PRFVAL"].iloc[0] == 0.3494
	assert df_["TP_SHCK"].iloc[0] == "A"


def test_transform_data_with_extra_values_column(
	instance: B3BdiStocksMarginScenariosLiquidAssets,
) -> None:
	"""Test transform_data silently drops extra values beyond declared columns.

	The API may return 6 values per row while only 5 columns are declared
	(the hidden RptDt at position 0 is absent from columns metadata).
	...
	table = {
		...
		"values": [
			["VLDIRR310", 216, 9998, 0.3494, "A", None],
		],
	}
	df_ = instance.transform_data(table)
	assert isinstance(df_, pd.DataFrame)
	assert len(df_) == 1
	assert list(df_.columns[:5]) == ["TCKR_SYMB", "VRTX_CD", "SCNRO_ID", "PRFVAL", "TP_SHCK"]
	assert df_["TCKR_SYMB"].iloc[0] == "VLDIRR310"


def test_transform_data_empty_values(
	instance: B3BdiStocksMarginScenariosLiquidAssets, empty_table_dict: dict
) -> None:
