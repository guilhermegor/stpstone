"""Unit tests for FMPCommoditiesOhlcvYesterday ingestion class."""
...
@pytest.fixture
def sample_commodity_data() -> list:
	"""Fixture providing sample batch commodity quote records.

	Returns
	-------
	list
		List of sample commodity quote dictionaries.
	"""
	return [
		{"symbol": "GCUSD", "price": 2350.0, "change": 5.0, "volume": 120000.0},
		{"symbol": "SIUSD", "price": 28.5, "change": -0.3, "volume": 75000.0},
	]


@pytest.fixture
def mock_response_success(sample_commodity_data: list) -> Response:
	"""Fixture providing a successful HTTP response mock.

	Parameters
	----------
	sample_commodity_data : list
		Sample commodity data list.
	...
	mock_resp.json.return_value = sample_commodity_data
	return mock_resp
...
def mock_dates_current(mocker: MockerFixture) -> MagicMock:
	...
	mock_cls = mocker.patch(
		...
		".fmp_commodities_ohlcv_yesterday.DatesCurrent"
	)
...
def mock_dates_br(mocker: MockerFixture, mock_dates_current: MagicMock) -> MagicMock:
	...
	mock_cls = mocker.patch(
		"stpstone.ingestion.countries.ww.exchange.markets"
		".fmp_commodities_ohlcv_yesterday.DatesBRAnbima"
	)
...
def mock_dir_files(mocker: MockerFixture) -> MagicMock:
	...
	mock_cls = mocker.patch(
		"stpstone.ingestion.countries.ww.exchange.markets"
		".fmp_commodities_ohlcv_yesterday.DirFilesManagement"
	)
...
def mock_create_log(mocker: MockerFixture) -> MagicMock:
	...
	mock_cls = mocker.patch(
		"stpstone.ingestion.countries.ww.exchange.markets"
		".fmp_commodities_ohlcv_yesterday.CreateLog"
	)
...
class TestFMPCommoditiesOhlcvYesterday:
	"""Test cases for FMPCommoditiesOhlcvYesterday class."""

	def test_init_with_explicit_date(
		self,
		mock_dates_current: MagicMock,
		...
		"""Test initialization with an explicit date_ref sets the attribute correctly.
		...
		instance = FMPCommoditiesOhlcvYesterday(token="tok", date_ref=test_date)  # noqa: S106

		assert instance.token == "tok"  # noqa: S105
		assert instance.date_ref == test_date

	def test_init_url_contains_endpoint(
		self,
		mock_dates_current: MagicMock,
		mock_dates_br: MagicMock,
		mock_dir_files: MagicMock,
		...
		"""Test that the constructed URL contains the batch-commodity-quotes endpoint.
		...
		None
		"""
		instance = FMPCommoditiesOhlcvYesterday(token="mytoken")  # noqa: S106

		assert "batch-commodity-quotes" in instance.url
		assert "mytoken" in instance.url
	...
	def test_get_response_success(
		...
		instance = FMPCommoditiesOhlcvYesterday(token="tok")  # noqa: S106
	...
	def test_run_returns_dataframe_without_db(
		...
		"""
		mock_requests_get.return_value = mock_response_success
		standardized_df = pd.DataFrame({"SYMBOL": ["GCUSD"], "PRICE": [2350.0]})
	...
	def test_run_with_cls_db_calls_insert(
		...
		"""
		mock_requests_get.return_value = mock_response_success
		standardized_df = pd.DataFrame({"SYMBOL": ["GCUSD"]})

		instance = FMPCommoditiesOhlcvYesterday(token="tok", cls_db=mock_db_session)  # noqa: S106
		mocker.patch.object(instance, "standardize_dataframe", return_value=standardized_df)
