"""Unit tests for B3BdiInstrumentRegistration class."""
...
@pytest.fixture
def instance(sample_date: date) -> B3BdiInstrumentRegistration:
	"""Fixture providing a B3BdiInstrumentRegistration instance.
	...
	"""
	return B3BdiInstrumentRegistration(date_ref=sample_date)
...
def test_init_with_valid_inputs(sample_date: date) -> None:
	...
	None
	"""
	inst = B3BdiInstrumentRegistration(date_ref=sample_date, int_page_size=500)
...
def test_init_default_page_size(sample_date: date) -> None:
	...
	None
	"""
	inst = B3BdiInstrumentRegistration(date_ref=sample_date)
...
def test_init_without_date_ref(mocker: MockerFixture) -> None:
	...
	inst = B3BdiInstrumentRegistration()
...
def test_init_logger_propagated() -> None:
	...
	inst = B3BdiInstrumentRegistration(date_ref=date(2026, 4, 17), logger=mock_logger)
	assert inst.logger is mock_logger


def test_get_response_success(
	instance: B3BdiInstrumentRegistration, mocker: MockerFixture
) -> None:
	"""Test get_response posts to the correct URL and returns the response.

	Parameters
	----------
	instance : B3BdiInstrumentRegistration
		Initialized instance.
	mocker : MockerFixture
		Pytest-mock fixture.
...
def test_transform_data_normal(
	instance: B3BdiInstrumentRegistration, sample_table_dict: dict
) -> None:
...
def test_transform_data_empty_values(
	instance: B3BdiInstrumentRegistration, empty_table_dict: dict
) -> None:
