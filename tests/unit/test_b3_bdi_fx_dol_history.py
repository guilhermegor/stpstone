"""Unit tests for B3BdiFxDolHistory class."""
...
from stpstone.ingestion.countries.br.macroeconomics.b3_bdi_fx_dol_history import (
	B3BdiFxDolHistory,
)
...
def instance(
	...
	"""Fixture providing a B3BdiFxDolHistory instance.
	...
	"""
	return B3BdiFxDolHistory(date_ref=sample_date)
...
def sample_table_dict() -> dict:
	...
	-------
	dict
		Sample table dict mimicking the BDI HistoricalExchange API response structure.
	"""
	return {
		"columns": [
			{"name": "RptDt"},
		...
		"values": [
			["2026-04-17T00:00:00", "DOL", "RTDOLD2", "Indicadores gerais", 4.989],
			["2026-04-17T00:00:00", "DOL", "RTDOLD1", "Indicador Economico", 4.9878],
		],
	}
...
def test_init_with_valid_inputs(sample_date: date) -> None:
	...
	None
	"""
	inst = B3BdiFxDolHistory(date_ref=sample_date, int_page_size=500)
	...
	assert "HistoricalExchange" in inst.url_tpl
...
def test_init_default_page_max(sample_date: date) -> None:
	...
	None
	"""
	inst = B3BdiFxDolHistory(date_ref=sample_date)
...
def test_init_logger_propagated() -> None:
	...
	inst = B3BdiFxDolHistory(date_ref=date(2026, 4, 17), logger=mock_logger)
	assert inst.logger is mock_logger


def test_get_response_success(
	instance: B3BdiFxDolHistory,
	mocker: MockerFixture,
) -> None:
...
def test_transform_data_normal(
	...
	assert len(df_) == 2
	assert "RPT_DT" in df_.columns
	assert "TCKR_SYMB" in df_.columns
	assert "SYMB" in df_.columns
	assert "ECNC_IND_DESC" in df_.columns
	assert "PRIC_VAL" in df_.columns
	assert df_["TCKR_SYMB"].iloc[0] == "DOL"
	assert df_["PRIC_VAL"].iloc[0] == 4.989
