...
def mock_response() -> Response:
	...
	response.content = (
		b"TITULO VENCIMENTO PRECO_UNITARIO PRECO_RETORNO POSICAO_CUSTODIA"
...
def test_get_response_success(
	...
	mock_requests_get.assert_called_once_with(
		anbima_instance.url, timeout=(12.0, 21.0), verify=True
	)
...
def test_parse_raw_file(anbima_instance: Anbima550Listing, mock_response: Response) -> None:
	...
	"""
	with patch.object(
		anbima_instance, "get_file", return_value=StringIO("test content")
	) as mock_get_file:
...
def test_run_without_db(
	...
	with (
		patch.object(
			anbima_instance, "parse_raw_file", return_value=StringIO("test content")
		) as mock_parse,
		patch.object(
			anbima_instance, "transform_data", return_value=pd.DataFrame({"TITULO": [1]})
		) as mock_transform,
		patch.object(
			anbima_instance, "standardize_dataframe", return_value=pd.DataFrame({"TITULO": [1]})
		) as mock_standardize,
	):
