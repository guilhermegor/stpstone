...
@pytest.fixture
def mock_psutil_net_connections(
    mocker: MockerFixture
) -> Any: # noqa ANN401: typing.Any not allowed
...
def test_get_open_ports(
    ...
    mock_psutil_net_connections.return_value = [
        psutil._common.sconn(fd=-1, family=socket.AF_INET, type=socket.SOCK_STREAM, 
                            laddr=psutil._common.addr(ip="127.0.0.1", port=80), 
                            raddr=None, status=psutil.CONN_LISTEN, pid=None),
        psutil._common.sconn(fd=-1, family=socket.AF_INET, type=socket.SOCK_STREAM,
                            laddr=psutil._common.addr(ip="127.0.0.1", port=443),
                            raddr=None, status=psutil.CONN_LISTEN, pid=None),
        psutil._common.sconn(fd=-1, family=socket.AF_INET, type=socket.SOCK_STREAM,
                            laddr=psutil._common.addr(ip="127.0.0.1", port=8080),
                            raddr=None, status=psutil.CONN_ESTABLISHED, pid=None),
        psutil._common.sconn(fd=-1, family=socket.AF_INET, type=socket.SOCK_STREAM,
                            laddr=None, raddr=None, status=psutil.CONN_LISTEN, pid=None),
    ]
...
def test_module_reload(mocker: MockerFixture) -> None:
    ...
    assert hasattr(sys.modules["stpstone.utils.connections.netops.diagnostics.network_info"], 
                   "NetworkInfo")