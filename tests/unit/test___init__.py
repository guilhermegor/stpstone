...
def mock_version_not_found(mocker: MockerFixture) -> type[Any]:
	...
	"""
	return mocker.patch("importlib.metadata.version", side_effect=PackageNotFoundError)
...
def mock_metadata_success(mocker: MockerFixture, expected_version: str) -> type[Any]:
	...
	"""
	return mocker.patch("importlib.metadata.metadata", return_value={"version": expected_version})
...
def mock_metadata_not_found(mocker: MockerFixture) -> type[Any]:
	...
	type[Any]
		Mocked metadata function
	"""
	return mocker.patch("importlib.metadata.metadata", side_effect=PackageNotFoundError)
...
def mock_metadata_import_error(mocker: MockerFixture) -> type[Any]:
	...
	type[Any]
		Mocked metadata function
	"""
	return mocker.patch("importlib.metadata.metadata", side_effect=ImportError)
