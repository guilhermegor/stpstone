"""Unit tests for AnbimaDataIDKAPre1A module.

Tests the ingestion functionality for AnbimaDataIDKAPre1A including:
...
def sample_dataframe() -> pd.DataFrame:
	...
	-------
	pd.DataFrame
		Sample DataFrame with expected IDKAPre1A structure (7 columns)
	"""
	return pd.DataFrame(
		{
			"INDICE": ["TEST"],
...
class TestAnbimaDataIDKAPre1A:
	...
	@pytest.fixture
	def instance_no_db(self, sample_date: date, mock_logger: MagicMock) -> AnbimaDataIDKAPre1A:
		"""Fixture providing AnbimaDataIDKAPre1A instance without database.
		...
		"""
		return AnbimaDataIDKAPre1A(date_ref=sample_date, logger=mock_logger, cls_db=None)
	...
	def test_init_without_date(self, mock_logger: MagicMock, mock_db_session: MagicMock) -> None:
		...
		with patch.object(DatesBRAnbima, "add_working_days", return_value=date(2023, 1, 1)):
			instance = AnbimaDataIDKAPre1A(logger=mock_logger, cls_db=mock_db_session)
	...
	def test_run_without_db(
		self,
		instance_no_db: AnbimaDataIDKAPre1A,
		mock_requests_get: MagicMock,
		mock_response: Response,
	...
	def test_run_with_db(
		...
		instance.insert_table_db.assert_called_once_with(
			...
			str_table_name="br_anbima_data_indexes_idka_pre_1a",
			df_=sample_dataframe,
			bool_insert_or_ignore=False,
		)

	def test_run_invalid_timeout(self, instance: AnbimaDataIDKAPre1A) -> None:
...
def test_module_reload() -> None:
	...
	"""
	importlib.reload(
		sys.modules["stpstone.ingestion.countries.br.exchange.anbima_data_idka_pre_1a"]
	)
	assert hasattr(
		sys.modules["stpstone.ingestion.countries.br.exchange.anbima_data_idka_pre_1a"],
		"AnbimaDataIDKAPre1A",
