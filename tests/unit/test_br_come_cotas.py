...
@pytest.fixture(params=[
    "FIA", "FIRF", "FIM", "FIDC", "FIP", "FI-IE", "FII"
])
...
class TestIsTaxDay:
    ...
    def test_is_tax_day_may(self, calculator_firf: type[Any], tax_day_may: date) -> None:
        """Test May tax day returns True for applicable funds."""
        assert calculator_firf.is_tax_day(tax_day_may) is True

    def test_is_tax_day_november(
        self, calculator_firf: type[Any], tax_day_november: date
    ) -> None:
        """Test November tax day returns True for applicable funds."""
        assert calculator_firf.is_tax_day(tax_day_november) is True

    def test_is_tax_day_non_tax_day(
        self, calculator_firf: type[Any], non_tax_day: date
    ) -> None:
    ...
    def test_is_tax_day_default_current_date(
        self, calculator_firf: type[Any], mocker: MockerFixture
    ) -> None:
    ...
    def test_is_tax_day_fund_without_come_cotas(
        self, calculator_fia: type[Any], tax_day_may: date
    ) -> None:
    ...
    def test_is_tax_day_fund_without_taxation_day(
        self, calculator_fip: type[Any], tax_day_may: date
    ) -> None:
...
class TestCalculateTax:
    ...
    def test_calculate_tax_tax_day(
        self, calculator_firf: type[Any], tax_day_may: date
    ) -> None:
        """Test tax calculation on tax day."""
        position_value = 10000.0
        acquisition_basis = 8000.0
        expected_tax = (position_value - acquisition_basis) * 0.20
        result = calculator_firf.calculate_tax(
            position_value, acquisition_basis, tax_day_may
        )
        assert result == pytest.approx(expected_tax)

    def test_calculate_tax_non_tax_day(
        self, calculator_firf: type[Any], non_tax_day: date
    ) -> None:
        """Test tax calculation returns 0 on non-tax day."""
        result = calculator_firf.calculate_tax(
            10000.0, 8000.0, non_tax_day
        )
        assert result == 0.0
    ...
    def test_calculate_tax_fund_without_come_cotas(
        ...
        result = calculator_fia.calculate_tax(
            10000.0, 8000.0, tax_day_may
        )
        assert result == 0.0
    ...
    def test_calculate_tax_position_less_than_basis(
        ...
        with pytest.raises(ValueError, match="Position value cannot be less than"):
            calculator_firf.calculate_tax(
                8000.0, 10000.0, tax_day_may
            )

    def test_calculate_tax_zero_profit(
        self, calculator_firf: type[Any], tax_day_may: date
    ) -> None:
        """Test tax calculation with zero profit."""
        result = calculator_firf.calculate_tax(
            10000.0, 10000.0, tax_day_may
        )
        assert result == 0.0
    ...
    def test_calculate_tax_precision(
        self, calculator_firf: type[Any], tax_day_may: date
    ) -> None:
        """Test tax calculation with floating point precision."""
        result = calculator_firf.calculate_tax(
            10000.12345, 8000.54321, tax_day_may
        )
...
class TestTypeValidation:
    ...
    def test_invalid_position_value_type(
        self, calculator_firf: type[Any], tax_day_may: date
    ) -> None:
