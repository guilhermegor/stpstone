"""Unit tests for GlobalRatesCentralBanks class."""
...
@pytest.fixture
def instance(sample_date: date) -> GlobalRatesCentralBanks:
	"""Provide a GlobalRatesCentralBanks instance for testing.
	...
	"""
	return GlobalRatesCentralBanks(date_ref=sample_date)
...
def test_init_with_valid_date(sample_date: date) -> None:
	...
	None
	"""
	obj = GlobalRatesCentralBanks(date_ref=sample_date)
	...
	assert "central-banks" in obj.url
...
def test_init_with_default_date() -> None:
	...
	with patch.object(DatesBRAnbima, "add_working_days", return_value=date(2025, 1, 1)):
		obj = GlobalRatesCentralBanks()
		assert obj.date_ref == date(2025, 1, 1)


def test_get_response_success(
	instance: GlobalRatesCentralBanks,
...
def test_run_without_db(
	...
	with (
		...
		patch.object(
			instance, "transform_data", return_value=pd.DataFrame({"CENTRAL_BANK": ["FED"]})
		),
		patch.object(
			instance,
			"standardize_dataframe",
			return_value=pd.DataFrame({"CENTRAL_BANK": ["FED"]}),
		),
	):
		result = instance.run()
...
def test_reload_module() -> None:
	...
	import stpstone.ingestion.countries.ww.macroeconomics.global_rates_central_banks as mod

	importlib.reload(mod)
	obj = mod.GlobalRatesCentralBanks(date_ref=date(2025, 1, 1))
	assert obj.date_ref == date(2025, 1, 1)
