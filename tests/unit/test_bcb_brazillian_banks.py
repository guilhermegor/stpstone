...
def test_init_with_default_date(mocker: MockerFixture) -> None:
	...
	mock_date = mocker.patch.object(
		DatesBRAnbima, "add_working_days", return_value=date(2025, 10, 21)
	)
...
def test_transform_data(mock_pdf_content: BytesIO, mocker: MockerFixture) -> None:
	...
	mocker.patch.object(
		BCBBanksCodesCompensation, "pdf_doc_tables_response", return_value=input_df
	)
	instance = BCBBanksCodesCompensation()
	...
	assert all(pd.api.types.is_string_dtype(result[col]) for col in expected_columns)
	assert result.notna().all().all()


def test_run_without_db(
	mock_response: Response, mock_pdf_content: BytesIO, mocker: MockerFixture
) -> None:
	...
	mocker.patch.object(
		BCBBanksCodesCompensation, "standardize_dataframe", return_value=transformed_df
	)
...
@pytest.mark.parametrize("timeout", [None, 10, 10.5, (12.0, 21.0), (10, 15)])
...
@pytest.mark.parametrize("invalid_input", ["not a response", 123, None])
...
def test_pdf_doc_tables_response_empty_pdf(
	mock_pdf_content: BytesIO, mocker: MockerFixture
) -> None:
...
def test_transform_data_empty_input(mock_pdf_content: BytesIO, mocker: MockerFixture) -> None:
	...
	mocker.patch.object(
		BCBBanksCodesCompensation, "pdf_doc_tables_response", return_value=empty_df
	)
	instance = BCBBanksCodesCompensation()
	result = instance.transform_data(mock_pdf_content)
	assert isinstance(result, pd.DataFrame)
...
def test_reload_module(mocker: MockerFixture, sample_date: date) -> None:
	...
	instance = BCBBanksCodesCompensation(date_ref=sample_date)
	importlib.reload(
		sys.modules["stpstone.ingestion.countries.br.registries.bcb_brazillian_banks"]
	)
