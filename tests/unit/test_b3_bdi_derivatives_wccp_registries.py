"""Unit tests for B3BdiDerivativesWccpRegistries class."""
...
@pytest.fixture
def instance(sample_date: date) -> B3BdiDerivativesWccpRegistries:
	"""Fixture providing a B3BdiDerivativesWccpRegistries instance.
	...
	"""
	return B3BdiDerivativesWccpRegistries(date_ref=sample_date)
...
def sample_table_dict() -> dict:
	...
	-------
	dict
		Sample table dict mimicking the BDI OTCRegistrationWCCP API response.
	"""
	return {
		"columns": [
			{"name": "TckrSymb"},
		...
		"values": [
			["Opção Flexível", 1421, 40332422575.83, None],
		],
	}
...
def test_init_with_valid_inputs(sample_date: date) -> None:
	...
	None
	"""
	inst = B3BdiDerivativesWccpRegistries(date_ref=sample_date, int_page_size=500)
	...
	assert "OTCRegistrationWCCP" in inst.url_tpl
...
def test_init_default_page_size(sample_date: date) -> None:
	...
	None
	"""
	inst = B3BdiDerivativesWccpRegistries(date_ref=sample_date)
...
def test_init_without_date_ref(mocker: MockerFixture) -> None:
	...
	inst = B3BdiDerivativesWccpRegistries()
...
def test_init_logger_propagated() -> None:
	...
	inst = B3BdiDerivativesWccpRegistries(date_ref=date(2026, 4, 17), logger=mock_logger)
	assert inst.logger is mock_logger


def test_get_response_success(
	instance: B3BdiDerivativesWccpRegistries,
	mocker: MockerFixture,
) -> None:
...
def test_parse_raw_file_missing_table_key(instance: B3BdiDerivativesWccpRegistries) -> None:
...
def test_transform_data_normal(
	...
	assert df_["TCKR_SYMB"].iloc[0] == "Opção Flexível"
	assert df_["TRAD_QTY"].iloc[0] == 1421
	assert df_["FINANCIAL_VOL"].iloc[0] == 40332422575.83


def test_transform_data_multiple_rows(instance: B3BdiDerivativesWccpRegistries) -> None:
	...
	table = {
		...
		"values": [
			["BOX", 0, 0.0, None],
			["Opção Flexível", 1421, 40332422575.83, None],
			["Swap", 373, 6273961201.48, None],
			["Termo", 1690, 24529831669.99, None],
		],
	}
...
def test_run_without_db(
	...
	assert result["FINANCIAL_VOL"].iloc[0] == Decimal("40332422575.83")
...
def test_module_reload(sample_date: date) -> None:
	...
	import stpstone.ingestion.countries.br.otc.b3_bdi_derivatives_wccp_registries as mod

	importlib.reload(mod)
	inst = mod.B3BdiDerivativesWccpRegistries(date_ref=sample_date)
	assert inst.date_ref == sample_date
	assert "OTCRegistrationWCCP" in inst.url_tpl
