"""Unit tests for B3BdiDiOver class."""
...
@pytest.fixture
def instance(sample_date: date) -> B3BdiDiOver:
	"""Fixture providing a B3BdiDiOver instance.
	...
	"""
	return B3BdiDiOver(date_ref=sample_date)
...
def sample_table_dict() -> dict:
	...
	return {
		...
		"values": [
			["2026-04-17T00:00:00", 125340, 1234567890, 10.65, 1.00054266, 10.50],
		],
	}
...
def test_init_with_valid_inputs(sample_date: date) -> None:
	...
	None
	"""
	inst = B3BdiDiOver(date_ref=sample_date, int_page_size=500)
...
def test_init_default_page_size(sample_date: date) -> None:
	...
	None
	"""
	inst = B3BdiDiOver(date_ref=sample_date)
...
def test_init_logger_propagated() -> None:
	...
	inst = B3BdiDiOver(date_ref=date(2026, 4, 17), logger=mock_logger)
	assert inst.logger is mock_logger


def test_get_response_success(instance: B3BdiDiOver, mocker: MockerFixture) -> None:
	"""Test get_response posts to the correct URL and returns the response.

	Parameters
	----------
	instance : B3BdiDiOver
		Initialized instance.
	mocker : MockerFixture
		Pytest-mock fixture.
...
def test_get_response_http_error(instance: B3BdiDiOver, mocker: MockerFixture) -> None:
...
def test_get_response_timeout_error(instance: B3BdiDiOver, mocker: MockerFixture) -> None:
...
def test_transform_data_normal(instance: B3BdiDiOver, sample_table_dict: dict) -> None:
	...
	assert df_["NUMBER_OF_OPERATIONS"].iloc[0] == 125340
	assert df_["FINANCIAL_VOLUME"].iloc[0] == 1234567890
	assert df_["AVERAGE"].iloc[0] == 10.65
	assert df_["DAILY_FACTOR"].iloc[0] == 1.00054266
	assert df_["SELIC_RATE"].iloc[0] == 10.50


def test_transform_data_empty_values(instance: B3BdiDiOver, empty_table_dict: dict) -> None:
...
def test_run_without_db_paginates(
	...
	assert len(result) == 1
	assert "NUMBER_OF_OPERATIONS" in result.columns
