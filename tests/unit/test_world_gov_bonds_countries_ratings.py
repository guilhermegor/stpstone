"""Unit tests for WorldGovBondsCountriesRatings class."""
...
@pytest.fixture
def instance(sample_date: date) -> WorldGovBondsCountriesRatings:
    """Provide a WorldGovBondsCountriesRatings instance for testing.
    ...
    """
    return WorldGovBondsCountriesRatings(date_ref=sample_date)
...
def test_init_with_valid_date(sample_date: date) -> None:
    ...
    None
    """
    obj = WorldGovBondsCountriesRatings(date_ref=sample_date)
    ...
    assert "world-credit-ratings" in obj.url
...
def test_init_with_default_date() -> None:
    ...
    with patch.object(DatesBRAnbima, "add_working_days", return_value=date(2025, 1, 1)):
        obj = WorldGovBondsCountriesRatings()
        assert obj.date_ref == date(2025, 1, 1)


def test_get_response_returns_none(instance: WorldGovBondsCountriesRatings) -> None:
    """Test get_response returns None as Playwright handles navigation internally.

    Parameters
    ----------
    instance : WorldGovBondsCountriesRatings
        Ingestion instance.
...
def test_parse_raw_file_passthrough(instance: WorldGovBondsCountriesRatings) -> None:
...
def test_run_without_db(instance: WorldGovBondsCountriesRatings) -> None:
...
def test_run_with_db(instance: WorldGovBondsCountriesRatings) -> None:
...
def test_reload_module() -> None:
    ...
    import stpstone.ingestion.countries.ww.macroeconomics.world_gov_bonds_countries_ratings as mod

    importlib.reload(mod)
    obj = mod.WorldGovBondsCountriesRatings(date_ref=date(2025, 1, 1))
    assert obj.date_ref == date(2025, 1, 1)
