"""Unit tests for IRSBRRegistrationStatus class.
...
# Helpers
# --------------------------
def _build_zip_bytes(csv_content: str, csv_filename: str = "Motivos.csv") -> BytesIO:
	...
	csv_filename : str, optional
		The name of the CSV entry, by default "Motivos.csv".
...
def _sample_csv_row() -> str:
	...
	"""
	return "01;EXTINCAO POR ENCERRAMENTO LIQUIDACAO VOLUNTARIA"
...
@pytest.fixture
def irsbr_instance(sample_date: date) -> IRSBRRegistrationStatus:
	"""Fixture providing IRSBRRegistrationStatus instance.
	...
	-------
	IRSBRRegistrationStatus
		Initialized IRSBRRegistrationStatus instance.
	"""
	return IRSBRRegistrationStatus(date_ref=sample_date)
...
def test_init_with_valid_inputs(sample_date: date) -> None:
	...
	Verifies
	--------
	- The IRSBRRegistrationStatus instance is initialized correctly.
	...
	None
	"""
	instance = IRSBRRegistrationStatus(date_ref=sample_date)
...
def test_init_with_default_date() -> None:
	...
	with patch.object(DatesBRAnbima, "add_working_days", return_value=date(2025, 3, 14)):
		instance = IRSBRRegistrationStatus()
...
def test_get_response_success(
	irsbr_instance: IRSBRRegistrationStatus,
	...
	----------
	irsbr_instance : IRSBRRegistrationStatus
		Instance of IRSBRRegistrationStatus.
	mock_requests_get : object
...
def test_transform_data(
	...
	assert len(df_) == 3
	assert df_["CODE"].iloc[0] == "01"
	assert df_["DESCRIPTION"].iloc[0] == "EXTINCAO POR ENCERRAMENTO LIQUIDACAO VOLUNTARIA"


def test_transform_data_none_input(irsbr_instance: IRSBRRegistrationStatus) -> None:
...
def test_transform_data_invalid_zip(irsbr_instance: IRSBRRegistrationStatus) -> None:
...
def test_transform_data_zip_without_csv(irsbr_instance: IRSBRRegistrationStatus) -> None:
...
def test_run_without_db(
	...
	sample_df = pd.DataFrame(
		{
			"CODE": ["01"],
			"DESCRIPTION": ["EXTINCAO POR ENCERRAMENTO LIQUIDACAO VOLUNTARIA"],
		}
	)
	with (
...
def test_run_single_file_url(
	...
	"""Test that run fetches exactly one file (Motivos.zip).

	Verifies
	--------
	- requests.get is called exactly once.
	- The URL contains Motivos.zip (no numeric suffix).
	...
	with (
		...
		assert "Motivos.zip" in call_url
...
def test_reload_module() -> None:
	...
	import stpstone.ingestion.countries.br.taxation.irsbr_registration_status

	original_instance = IRSBRRegistrationStatus(date_ref=date(2025, 3, 15))
	importlib.reload(stpstone.ingestion.countries.br.taxation.irsbr_registration_status)
	new_instance = IRSBRRegistrationStatus(date_ref=date(2025, 3, 15))
