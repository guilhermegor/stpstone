"""Unit tests for FIFCDA ingestion module."""
...
class TestFIFCDA:
	...
	def test_get_response_success(
		...
		None
		"""
		with patch("stpstone.ingestion.countries.br.registries.fif_cda.requests.get") as mock_get:
	...
	def test_parse_raw_file_success(
		...
		"""
		with patch(
			"stpstone.ingestion.countries.br.registries.fif_cda.DirFilesManagement"
		) as mock_dir_files:
	...
	def test_run_without_db(
		...
		with (
			patch.object(FIFCDA, "get_response") as mock_get_response,
			patch.object(FIFCDA, "parse_raw_file") as mock_parse,
			patch.object(FIFCDA, "transform_data") as mock_transform,
			patch.object(FIFCDA, "standardize_dataframe") as mock_standardize,
		):
	...
	def test_run_with_db(
		...
		with (
			...
			patch.object(FIFCDA, "transform_data") as mock_transform,
			patch.object(FIFCDA, "standardize_dataframe") as mock_standardize,
			patch.object(FIFCDA, "insert_table_db") as mock_insert_db,
		):
			mock_get_response.return_value = MagicMock()
			mock_parse.return_value = files_list
			mock_transform.return_value = sample_dataframe
			...
			instance = FIFCDA(date_ref=sample_date, logger=mock_logger, cls_db=mock_db_session)
			result = instance.run()
