"""Unit tests for B3BdiClearingAdrCustody class."""
...
@pytest.fixture
def instance(sample_date: date) -> B3BdiClearingAdrCustody:
	"""Fixture providing a B3BdiClearingAdrCustody instance.
	...
	"""
	return B3BdiClearingAdrCustody(date_ref=sample_date)
...
def sample_table_dict() -> dict:
	...
	-------
	dict
		Sample table dict mimicking the BDI Custody API response.
	"""
	return {
		"columns": [
			{"name": "TckrSymb"},
...
def test_init_with_valid_inputs(sample_date: date) -> None:
	...
	None
	"""
	inst = B3BdiClearingAdrCustody(date_ref=sample_date, int_page_size=500)
	...
	assert "Custody" in inst.url_tpl
...
def test_init_default_page_size(sample_date: date) -> None:
	...
	None
	"""
	inst = B3BdiClearingAdrCustody(date_ref=sample_date)
...
def test_init_logger_propagated() -> None:
	...
	inst = B3BdiClearingAdrCustody(date_ref=date(2026, 4, 17), logger=mock_logger)
	assert inst.logger is mock_logger


def test_get_response_success(
	instance: B3BdiClearingAdrCustody,
	mocker: MockerFixture,
) -> None:
...
def test_parse_raw_file_missing_table_key(instance: B3BdiClearingAdrCustody) -> None:
...
def test_transform_data_normal(
	...
	assert len(df_) == 1
	assert list(df_.columns) == ["TCKR_SYMB", "TYPE", "QTY_STOCKS"]
	assert df_["TCKR_SYMB"].iloc[0] == "AMBEV S.A."
	assert df_["TYPE"].iloc[0] == "ON"
	assert df_["QTY_STOCKS"].iloc[0] == 1021800141


def test_transform_data_multiple_rows(instance: B3BdiClearingAdrCustody) -> None:
	...
	table = {
		...
		"values": [
			["AMBEV S.A.", "ON", 1021800141, None],
			["VALE S.A.", "ON      NM", 1366713815, None],
			["ITAU UNIBANCO HOLDING S/A", "PN      N1", 1271094678, None],
		],
	}
	df_ = instance.transform_data(table)
	assert len(df_) == 3
	assert "AMBEV S.A." in df_["TCKR_SYMB"].tolist()
	assert "VALE S.A." in df_["TCKR_SYMB"].tolist()
...
def test_run_without_db(
	...
	assert "QTY_STOCKS" in result.columns
	assert "URL" in result.columns
	assert result["QTY_STOCKS"].iloc[0] == 1021800141
...
def test_module_reload(sample_date: date) -> None:
	...
	import stpstone.ingestion.countries.br.exchange.b3_bdi_clearing_adr_custody as mod

	importlib.reload(mod)
	inst = mod.B3BdiClearingAdrCustody(date_ref=sample_date)
	assert inst.date_ref == sample_date
	assert "Custody" in inst.url_tpl
