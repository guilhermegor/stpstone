"""Unit tests for B3BdiDerivativesSummaryTrades class."""
...
@pytest.fixture
def instance(sample_date: date) -> B3BdiDerivativesSummaryTrades:
	"""Fixture providing a B3BdiDerivativesSummaryTrades instance.
	...
	"""
	return B3BdiDerivativesSummaryTrades(date_ref=sample_date)
...
def sample_table_dict() -> dict:
	...
	-------
	dict
		Sample table dict mimicking the BDI DerivativesOperation2 API response.
	"""
	return {
		"columns": [
			{"name": "TckrSymb"},
...
def test_init_with_valid_inputs(sample_date: date) -> None:
	...
	None
	"""
	inst = B3BdiDerivativesSummaryTrades(date_ref=sample_date, int_page_size=500)
	...
	assert "DerivativesOperation2" in inst.url_tpl
...
def test_init_default_page_size(sample_date: date) -> None:
	...
	None
	"""
	inst = B3BdiDerivativesSummaryTrades(date_ref=sample_date)
...
def test_init_without_date_ref(mocker: MockerFixture) -> None:
	...
	inst = B3BdiDerivativesSummaryTrades()
...
def test_init_logger_propagated() -> None:
	...
	inst = B3BdiDerivativesSummaryTrades(date_ref=date(2026, 4, 17), logger=mock_logger)
	assert inst.logger is mock_logger


def test_get_response_success(
	instance: B3BdiDerivativesSummaryTrades,
	mocker: MockerFixture,
) -> None:
...
def test_parse_raw_file_missing_table_key(instance: B3BdiDerivativesSummaryTrades) -> None:
...
def test_transform_data_normal(
	...
	instance : B3BdiDerivativesSummaryTrades
		Initialized instance.
	sample_table_dict : dict
		Sample table dict with two rows (11 values, 10 columns).
	...
	assert df_["TCKR_SYMB"].iloc[0] == "BGI: BOI GORDO - FUTURO"
	assert df_["NMBR_TRADES_DAY"].iloc[0] == 4502


def test_transform_data_multiple_rows(instance: B3BdiDerivativesSummaryTrades) -> None:
...
def test_module_reload(sample_date: date) -> None:
	...
	import stpstone.ingestion.countries.br.exchange.b3_bdi_derivatives_summary_trades as mod

	importlib.reload(mod)
	inst = mod.B3BdiDerivativesSummaryTrades(date_ref=sample_date)
	assert inst.date_ref == sample_date
	assert "DerivativesOperation2" in inst.url_tpl
