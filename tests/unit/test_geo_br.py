...
@patch("stpstone.utils.geography.geo_br.requests.get")
def test_states_http_error_raises(
    mock_get: MagicMock,
    geo_br: BrazilGeo
) -> None:
...
@patch("stpstone.utils.geography.geo_br.requests.get")
def test_states_timeout_error_raises(
    mock_get: MagicMock,
    geo_br: BrazilGeo
) -> None:
...
def test_zip_code_invalid_zip_empty_raises(
    invalid_zip: str,
    geo_br: BrazilGeo
) -> None:
...
def test_return_types(geo_br: BrazilGeo, sample_zip_code_response: ReturnZipCode) -> None:
    ...
    None
    """
    with patch("stpstone.utils.geography.geo_br.requests.get") as mock_get, patch(
        "stpstone.utils.geography.geo_br.sleep", return_value=None
    ):
        mock_resp = MagicMock()
        mock_resp.raise_for_status.return_value = None
        mock_resp.json.return_value = sample_zip_code_response
