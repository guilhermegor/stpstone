"""Unit tests for B3BdiBtbLendingOpenPositions class."""
...
@pytest.fixture
def instance(sample_date: date) -> B3BdiBtbLendingOpenPositions:
	"""Fixture providing a B3BdiBtbLendingOpenPositions instance.
	...
	"""
	return B3BdiBtbLendingOpenPositions(date_ref=sample_date)
...
def sample_table_dict() -> dict:
	...
	-------
	dict
		Sample table dict mimicking the BDI BTBLendingOpenPosition API response.
	"""
	return {
		"columns": [
			{"name": "RptDt"},
...
def test_init_with_valid_inputs(sample_date: date) -> None:
	...
	None
	"""
	inst = B3BdiBtbLendingOpenPositions(date_ref=sample_date, int_page_size=500)
	...
	assert "BTBLendingOpenPosition" in inst.url_tpl
...
def test_init_default_page_size(sample_date: date) -> None:
	...
	None
	"""
	inst = B3BdiBtbLendingOpenPositions(date_ref=sample_date)
...
def test_init_without_date_ref(mocker: MockerFixture) -> None:
	...
	inst = B3BdiBtbLendingOpenPositions()
...
def test_init_logger_propagated() -> None:
	...
	inst = B3BdiBtbLendingOpenPositions(date_ref=date(2026, 4, 17), logger=mock_logger)
	assert inst.logger is mock_logger


def test_get_response_success(
	instance: B3BdiBtbLendingOpenPositions,
	mocker: MockerFixture,
) -> None:
...
def test_parse_raw_file_missing_table_key(instance: B3BdiBtbLendingOpenPositions) -> None:
...
def test_transform_data_normal(
	...
	assert df_["TCKR_SYMB"].iloc[0] == "A1DM34"
	assert df_["STOCK_BALANCE"].iloc[0] == 12
	assert df_["AVG_PRIC"].iloc[0] == 349.86
...
def test_run_without_db(
	...
	assert "AVG_PRIC" in result.columns
...
def test_module_reload(sample_date: date) -> None:
	...
	import stpstone.ingestion.countries.br.exchange.b3_bdi_btb_lending_open_positions as mod

	importlib.reload(mod)
	inst = mod.B3BdiBtbLendingOpenPositions(date_ref=sample_date)
	assert inst.date_ref == sample_date
	assert "BTBLendingOpenPosition" in inst.url_tpl
