"""Unit tests for B3TheoricalPortfolioIBRX50 module.

Tests the ingestion functionality for the B3 IBRX50 theoretical portfolio, covering:
...
def sample_dataframe() -> pd.DataFrame:
	"""Provide a sample DataFrame matching the IBRX50 portfolio structure.
...
# Tests
# --------------------------
class TestB3TheoricalPortfolioIBRX50:
	"""Test cases for B3TheoricalPortfolioIBRX50."""
	...
	def instance_no_db(
		...
		"""
		return B3TheoricalPortfolioIBRX50(date_ref=sample_date, logger=mock_logger, cls_db=None)
	...
	def test_init_with_valid_inputs(
		...
		- url resolves to the IBRX50 class-level constant
		- Helper instances are created

		Parameters
		----------
		instance : B3TheoricalPortfolioIBRX50
	...
	def test_init_none_date_ref(self, mock_logger: MagicMock) -> None:
		...
		with patch.object(DatesBRAnbima, "add_working_days", return_value=date(2023, 1, 1)):
			inst = B3TheoricalPortfolioIBRX50(date_ref=None, logger=mock_logger)
	...
	def test_transform_data_normal(self, instance: B3TheoricalPortfolioIBRX50) -> None:
	...
	def test_transform_data_empty(self, instance: B3TheoricalPortfolioIBRX50) -> None:
	...
	def test_run_without_db(
		self,
		instance_no_db: B3TheoricalPortfolioIBRX50,
		mock_requests_get: MagicMock,
		mock_response: MagicMock,
	...
	def test_run_with_db(
		...
		instance.insert_table_db.assert_called_once_with(
			...
			str_table_name="br_b3_theorical_portfolio_ibrx50",
	...
	def test_run_invalid_table_name_raises(
		...
		None
		"""
		inst = B3TheoricalPortfolioIBRX50(
			date_ref=sample_date, logger=mock_logger, cls_db=mock_db_session
		)
		original = B3TheoricalPortfolioIBRX50._TABLE_NAME
		B3TheoricalPortfolioIBRX50._TABLE_NAME = ""
		try:
			mock_requests_get.return_value = mock_response
			...
			B3TheoricalPortfolioIBRX50._TABLE_NAME = original
...
def test_module_reload() -> None:
	...
	None
	"""
	importlib.reload(sys.modules["stpstone.ingestion.countries.br.exchange.b3_theor_portf_ibrx50"])
	assert hasattr(
		sys.modules["stpstone.ingestion.countries.br.exchange.b3_theor_portf_ibrx50"],
		"B3TheoricalPortfolioIBRX50",
	)
