"""Unit tests for FIFCADFI ingestion module."""
...
def sample_dataframe() -> pd.DataFrame:
	"""Provide sample FIFCADFI DataFrame.
...
class TestFIFCADFI:
	...
	def test_init_uses_current_date_when_none(self, mock_logger: MagicMock) -> None:
		...
		"""
		with patch(
			"stpstone.ingestion.countries.br.registries.fif_cad_fi.DatesCurrent"
	...
	def test_get_response_success(self, mock_logger: MagicMock, sample_date: date) -> None:
		...
		"""
		with patch(
			"stpstone.ingestion.countries.br.registries.fif_cad_fi.requests.get"
		) as mock_get:
	...
	def test_run_without_db(
		...
		"""
		with (
			patch.object(FIFCADFI, "get_response") as mock_get_response,
			patch.object(FIFCADFI, "parse_raw_file") as mock_parse,
			patch.object(FIFCADFI, "transform_data") as mock_transform,
			patch.object(FIFCADFI, "standardize_dataframe") as mock_standardize,
		):
	...
	def test_run_with_db(
		...
		with (
			...
			patch.object(FIFCADFI, "transform_data") as mock_transform,
			patch.object(FIFCADFI, "standardize_dataframe") as mock_standardize,
			patch.object(FIFCADFI, "insert_table_db") as mock_insert_db,
		):
			mock_get_response.return_value = MagicMock()
			mock_parse.return_value = (StringIO(), "cad_fi.csv")
			mock_transform.return_value = sample_dataframe
			...
			instance = FIFCADFI(date_ref=sample_date, logger=mock_logger, cls_db=mock_db_session)
			result = instance.run()
