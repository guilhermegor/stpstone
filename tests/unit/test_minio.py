...
def test_init_valid_inputs(
	minio_client: MinioClient, mock_minio: Mock, mock_create_log: Mock
) -> None:
...
def test_make_bucket_s3error(
	...
	result = minio_client.make_bucket("test-bucket")
	mock_create_log.return_value.log_message.assert_called_once_with(
		None, f"Error creating bucket test-bucket: {s3err}", "critical"
	)
...
def test_put_object_from_file_success(
	minio_client: MinioClient, mock_minio: Mock, mock_create_log: Mock, sample_file: str
) -> None:
	...
	mock_create_log.return_value.log_message.assert_called_with(
		None, f"File '{sample_file}' uploaded as 'test-object' to bucket 'test-bucket'.", "info"
	)
	assert result is True


def test_put_object_from_file_nonexistent(
	minio_client: MinioClient, mock_create_log: Mock
) -> None:
...
def test_put_object_from_stream_success(
	minio_client: MinioClient, mock_minio: Mock, mock_create_log: Mock, sample_stream: BytesIO
) -> None:
...
def test_put_object_from_stream_s3error(
	...
	mock_create_log.return_value.log_message.assert_called_with(
		None, f"Error uploading stream as 'test-object': {s3err}", "critical"
	)
	assert result is False
...
def test_put_object_from_bytes_success(
	minio_client: MinioClient, mock_minio: Mock, mock_create_log: Mock, sample_bytes: bytes
) -> None:
...
def test_put_object_from_bytes_s3error(
	...
	result = minio_client.put_object_from_bytes("test-bucket", "test-object", sample_bytes)
	mock_create_log.return_value.log_message.assert_called_with(
		None, f"Error uploading bytes as 'test-object': {s3err}", "critical"
	)
	assert result is False
...
def test_get_object_to_file_s3error(
	...
	result = minio_client.get_object_to_file("test-bucket", "test-object", file_path)
	mock_create_log.return_value.log_message.assert_called_with(
		None, f"Error downloading object 'test-object': {s3err}", "critical"
	)
	assert result is False
...
def test_list_objects_s3error(
	...
	mock_create_log.return_value.log_message.assert_called_with(
		None, f"Error listing objects in bucket 'test-bucket': {s3err}", "critical"
	)
	assert result is None
...
# --------------------------
def test_put_object_from_stream_invalid_type(minio_client: MinioClient, mock_minio: Mock) -> None:
...
def test_put_object_from_bytes_invalid_type(minio_client: MinioClient, mock_minio: Mock) -> None:
...
def test_put_object_from_file_invalid_metadata(
	minio_client: MinioClient, mock_minio: Mock, sample_file: str
) -> None:
	...
	"""
	with pytest.raises(TypeError):
		minio_client.put_object_from_file(
			"test-bucket", "test-object", sample_file, metadata="not a dict"
		)
