...
def databricks_instance(valid_query: str, valid_dsns: list[str], mock_logger: Mock) -> Databricks:
	...
	"""
	return Databricks(
		str_query=valid_query, list_dsns=valid_dsns, logger=mock_logger, max_error_attempts=3
	)
...
def test_init_invalid_query(
	invalid_query: Optional[Union[str, int, list]], valid_dsns: list[str]
) -> None:
...
def test_init_invalid_dsns(
	invalid_dsns: Optional[Union[list, int, str]], valid_query: str
) -> None:
...
def test_init_invalid_max_attempts(
	invalid_attempts: Union[int, str], valid_query: str, valid_dsns: list[str]
) -> None:
	...
	if isinstance(invalid_attempts, int):
		with pytest.raises(ValueError, match="max_error_attempts must be a positive integer"):
			Databricks(
				str_query=valid_query, list_dsns=valid_dsns, max_error_attempts=invalid_attempts
			)
	else:
...
def test_validate_dsn_conn_invalid(
	databricks_instance: Databricks, invalid_dsn: Optional[Union[str, int, list]]
) -> None:
...
def test_validate_int_timeout_invalid(
	databricks_instance: Databricks, invalid_timeout: Optional[Union[int, str]]
) -> None:
...
def test_fetch_data_from_databricks_success(
	mock_read_sql: Mock, databricks_instance: Databricks
) -> None:
...
def test_conn_databricks_connection_error(
	mock_conn_dsn: Mock, mock_log_message: Mock, databricks_instance: Databricks
) -> None:
...
def test_conn_databricks_kill_process(
	...
	with pytest.raises(ValueError, match="Connection to Databricks could not be established"):
		databricks_instance.conn_databricks(
			int_max_wait=10, bool_kill_process_when_databricks_down=True
		)
	mock_log_message.assert_any_call(
		databricks_instance.logger,
		f"Connection to Databricks could not be established in any of the DSNs, "
...
def test_retrieve_query_data_success(
	mock_conn_databricks: Mock, mock_log_message: Mock, databricks_instance: Databricks
) -> None:
	...
	mock_log_message.assert_called_once_with(
		databricks_instance.logger, "#0 Attempting connection with DSNs to Databricks", "info"
	)
...
def test_retrieve_query_data_retries(
	...
	mock_log_message.assert_any_call(
		databricks_instance.logger, "#2 Attempting connection with DSNs to Databricks", "info"
	)
...
def test_module_reload(
	databricks_instance: Databricks, valid_query: str, valid_dsns: list[str]
) -> None:
