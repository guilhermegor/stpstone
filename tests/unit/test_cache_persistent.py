...
def test_decorator_no_thread_safety(
    temp_cache_dir: Path, 
    mocker: MockerFixture
) -> None:
...
# --------------------------
def test_load_cache_existing_file(
    temp_cache_dir: Path, 
    mocker: MockerFixture
) -> None:
...
def test_load_cache_non_existing_file(
    temp_cache_dir: Path, 
    mocker: MockerFixture
) -> None:
...
def test_save_cache_no_persist(
    temp_cache_dir: Path, 
    mocker: MockerFixture
) -> None:
...
def test_save_cache_pickle_error(
    ...
    mocker.patch.object(
        PickleFiles, 
        "dump_message", 
        side_effect=pickle.PickleError("pickle error")
    )
    mocker.patch.object(CreateLog, "log_message")
    decorator = PersistentCacheDecorator(
        path_cache=str(cache_path),
...
# --------------------------
def test_clear_cache_existing_file(
    temp_cache_dir: Path, 
    mocker: MockerFixture
) -> None:
...
def test_clear_cache_non_existing_file(
    temp_cache_dir: Path, 
    mocker: MockerFixture
) -> None:
...
def test_clear_cache_file_deletion_error(
    ...
    with pytest.raises(
        ValueError, 
        match=f"Failed to delete cache file {cache_path}: deletion error"
    ):
        decorator.clear_cache()
    assert decorator.cache == {}
    CreateLog.log_message.assert_called_once_with(
...
def test_decorator_cache_miss_persist(
    temp_cache_dir: Path, 
    mocker: MockerFixture
) -> None:
...
def test_decorator_cache_miss_no_persist(
    temp_cache_dir: Path, 
    mocker: MockerFixture
) -> None:
...
# --------------------------
def test_decorator_empty_args_kwargs(
    temp_cache_dir: Path, 
    mocker: MockerFixture
) -> None:
...
def test_decorator_unicode_args(
    temp_cache_dir: Path, 
    mocker: MockerFixture
) -> None:
...
def test_reload_preservation(
    ...
    mocker.patch.object(
        PickleFiles, 
        "load_message", 
        return_value={"test_key:(1,):{}": "cached_result"}
    )
