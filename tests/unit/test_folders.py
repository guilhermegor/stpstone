...
class TestDirFilesManagement:
	...
	def test_loop_files_w_rule(
		...
		result = dir_manager.loop_files_w_rule(temp_dir, "*.txt", bool_first_last_edited=False)
		assert isinstance(result, list)
		assert len(result) == 2
	...
	def test_get_zip_from_web_in_memory(
		...
		result = dir_manager.get_zip_from_web_in_memory(mock_response, bool_io_interpreting=True)
	...
	def test_check_separator_consistency(
		...
		assert dir_manager.check_separator_consistency(
			content, int_skip_rows=1, int_skip_footer=1, list_sep=[","]
		)
...
class TestFoldersTree:
	...
	def test_generate_tree(self, folder_structure: tuple[str, list[str]]) -> None:
		...
		tree = FoldersTree(root_dir, bool_add_linebreak_markdown=True)
		result = tree.generate_tree()
		assert "<br>" in result
