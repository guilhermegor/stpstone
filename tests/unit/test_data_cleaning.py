...
class TestTestSetCheckHash:
	...
	def test_invalid_test_ratio_raises_error(self, data_cleaner: DataCleaning) -> None:
...
class TestSplitTrainTest:
	...
	def test_returns_two_dataframes(
		self, data_cleaner: DataCleaning, sample_dataframe: pd.DataFrame
	) -> None:
	...
	def test_stratified_split(
		...
		None
		"""
		train, test = data_cleaner.split_train_test(sample_dataframe, stratify_col="category")
...
class TestSplitStratifiedTrainTest:
	...
	def test_returns_two_dataframes(
		...
		None
		"""
		train, test = data_cleaner.split_stratified_train_test(sample_dataframe, "category")
		assert isinstance(train, pd.DataFrame)
		assert isinstance(test, pd.DataFrame)
...
class TestSplitTrainTestById:
	...
	def test_returns_two_dataframes(
		...
		None
		"""
		train, test = data_cleaner.split_train_test_by_id(sample_dataframe, 0.2, "id")
		assert isinstance(train, pd.DataFrame)
		assert isinstance(test, pd.DataFrame)
	...
	def test_stable_split(
		...
		None
		"""
		train1, test1 = data_cleaner.split_train_test_by_id(sample_dataframe, 0.2, "id")
		train2, test2 = data_cleaner.split_train_test_by_id(sample_dataframe, 0.2, "id")
...
class TestCreateCategoryStratifiedTrainTestSet:
	...
	def test_proper_binning(
		...
		"""
		result = data_cleaner.create_category_stratified_train_test_set(
			sample_dataframe, "feature", "new_category", [0, 0.5, 1], ["low", "high"]
		)
...
class TestRemoveCategoryStratifiedTrainTestSet:
	...
	def test_removes_category_column(
		...
		df_with_cat = data_cleaner.create_category_stratified_train_test_set(
			sample_dataframe, "feature", "temp_category", [0, 0.5, 1], ["low", "high"]
		)
...
class TestDataFrameIdColumnProportions:
	...
	def test_returns_series(
		...
		None
		"""
		result = data_cleaner.dataframe_id_column_prportions(sample_dataframe, "category")
...
class TestReplaceNaNValues:
	...
	def test_returns_dict_with_arrays(
		self, data_cleaner: DataCleaning, sample_array_with_nans: NDArray[np.float64]
	) -> None:
	...
	def test_knn_imputation(
		...
		None
		"""
		result = data_cleaner.replace_nan_values(sample_array_with_nans, n_neighbors=3)
		assert not np.isnan(result["array_after_adjustments"]).any()
	...
	def test_invalid_strategy_raises_error(
		...
		with pytest.raises(TypeError, match="must be one of"):
			data_cleaner.replace_nan_values(sample_array_with_nans, strategy="invalid")
...
class TestConvertCategoriesFromStringsToArray:
	...
	def test_one_hot_encoding(
		self, data_cleaner: DataCleaning, sample_categorical_data: NDArray[np.str_]
	) -> None:
		...
		result = data_cleaner.convert_categories_from_strings_to_array(
			sample_categorical_data, [0, 1], "one_hot_encoder"
		)
		assert np.issubdtype(result["array_data_categorized_numbers"].dtype, np.number)
	...
	def test_ordinal_encoding(
		...
		"""
		result = data_cleaner.convert_categories_from_strings_to_array(
			sample_categorical_data, [0, 1], "ordinal_encoder"
		)
		assert np.issubdtype(result["array_data_categorized_numbers"].dtype, np.number)
	...
	def test_invalid_encoder_raises_error(
		...
		with pytest.raises(TypeError, match="must be one of"):
			data_cleaner.convert_categories_from_strings_to_array(
				sample_categorical_data, [0, 1], "invalid_encoder"
			)
...
class TestRemoveNoiseFromData:
	...
	def test_output_shapes(self, data_cleaner: DataCleaning) -> None:
...
class TestShiftImage:
	...
	def test_returns_array(
		self, data_cleaner: DataCleaning, sample_image_data: NDArray[np.float64]
	) -> None:
...
class TestValidateTestRatio:
	...
	def test_valid_ratios(self, data_cleaner: DataCleaning) -> None:
	...
	def test_invalid_ratios_raise_error(self, data_cleaner: DataCleaning) -> None:
