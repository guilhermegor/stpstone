"""Unit tests for CvmFIFStatement ingestion module."""
...
class TestFIFStatement:
	"""Test cases for CvmFIFStatement class."""

	def test_init_url_uses_year_only(self, mock_logger: MagicMock, sample_date: date) -> None:
		...
		None
		"""
		instance = CvmFIFStatement(date_ref=sample_date, logger=mock_logger)

		expected_url = "https://dados.cvm.gov.br/dados/FI/DOC/EXTRATO/DADOS/extrato_fi_2023.csv"
	...
	def test_get_response_success(self, mock_logger: MagicMock, sample_date: date) -> None:
		...
		"""
		with patch(
			"stpstone.ingestion.countries.br.registries.cvm_fif_statement.requests.get"
		) as mock_get:
	...
	def test_run_without_db(
		...
		"""
		with (
			patch.object(CvmFIFStatement, "get_response") as mock_get_response,
			patch.object(CvmFIFStatement, "parse_raw_file") as mock_parse,
			patch.object(CvmFIFStatement, "transform_data") as mock_transform,
			patch.object(CvmFIFStatement, "standardize_dataframe") as mock_standardize,
		):
	...
	def test_run_with_db(
		...
		with (
			...
			patch.object(CvmFIFStatement, "transform_data") as mock_transform,
			patch.object(CvmFIFStatement, "standardize_dataframe") as mock_standardize,
			patch.object(CvmFIFStatement, "insert_table_db") as mock_insert_db,
		):
			mock_get_response.return_value = MagicMock()
			mock_parse.return_value = (StringIO(), "test.csv")
			mock_transform.return_value = sample_dataframe
