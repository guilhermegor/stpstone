"""Unit tests for B3BdiDerivativesCallsMostTraded class."""
...
@pytest.fixture
def instance(sample_date: date) -> B3BdiDerivativesCallsMostTraded:
	"""Fixture providing a B3BdiDerivativesCallsMostTraded instance.
	...
	"""
	return B3BdiDerivativesCallsMostTraded(date_ref=sample_date)
...
def test_init_with_valid_inputs(sample_date: date) -> None:
	...
	None
	"""
	inst = B3BdiDerivativesCallsMostTraded(date_ref=sample_date, int_page_size=500)
...
def test_init_default_page_size(sample_date: date) -> None:
	...
	None
	"""
	inst = B3BdiDerivativesCallsMostTraded(date_ref=sample_date)
...
def test_init_without_date_ref(mocker: MockerFixture) -> None:
	...
	inst = B3BdiDerivativesCallsMostTraded()
...
def test_init_logger_propagated() -> None:
	...
	inst = B3BdiDerivativesCallsMostTraded(date_ref=date(2026, 4, 17), logger=mock_logger)
	assert inst.logger is mock_logger


def test_get_response_success(
	instance: B3BdiDerivativesCallsMostTraded,
	mocker: MockerFixture,
) -> None:
...
def test_parse_raw_file_missing_table_key(instance: B3BdiDerivativesCallsMostTraded) -> None:
...
def test_transform_data_multiple_rows(instance: B3BdiDerivativesCallsMostTraded) -> None:
...
def test_module_reload(sample_date: date) -> None:
	...
	import stpstone.ingestion.countries.br.exchange.b3_bdi_derivatives_calls_most_traded as mod

	importlib.reload(mod)
	inst = mod.B3BdiDerivativesCallsMostTraded(date_ref=sample_date)
	assert inst.date_ref == sample_date
	assert "OptionsPurshase" in inst.url_tpl
