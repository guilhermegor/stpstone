...
def placeholder_dict() -> dict[str, str]:
	...
	return {
		"name": "{{first}} {{last}}",
		"address": "{{street}}, {{city}}",
		"nested": {"greeting": "Hello {{first}}!", "list": ["{{first}}", "{{last}}"]},
	}
...
class TestDictionaryManipulation:
	...
	def test_add_key_value_to_dicts(
		self, handler: HandlingDicts, sample_dict_list: list[dict]
	) -> None:
		...
		result = handler.add_key_value_to_dicts(
			sample_dict_list, "doubled", multiply, ["value"], {"y": 2}
		)
	...
	def test_fill_placeholders(
		self, handler: HandlingDicts, placeholder_dict: dict[str, Any]
	) -> None:
		...
		None
		"""
		replacements = {"first": "John", "last": "Doe", "street": "123 Main", "city": "Anytown"}
...
class TestGroupingAggregation:
	...
	def test_sum_values_selected_keys(
		...
		"""
		# sum selected keys
		result = handler.sum_values_selected_keys(sample_dict_list, ["value"], True)
		assert result["value"] == 50  # 10 + 20 + 15 + 5
		assert "category" in result  # Other keys should be present

		# without summing (keep as lists)
		result = handler.sum_values_selected_keys(sample_dict_list, ["value"], False)
	...
	def test_merge_values_foreigner_keys(
		...
		None
		"""
		result = handler.merge_values_foreigner_keys(sample_dict_list, "category", ["value"])
...
class TestNSmallestLargest:
	...
	def test_n_smallest(self, handler: HandlingDicts, sample_dict_list: list[dict]) -> None:
	...
	def test_n_largest(self, handler: HandlingDicts, sample_dict_list: list[dict]) -> None:
