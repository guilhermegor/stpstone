"""Unit tests for B3BdiEquitiesGreatestUpsides class."""
...
@pytest.fixture
def instance(sample_date: date) -> B3BdiEquitiesGreatestUpsides:
	"""Fixture providing a B3BdiEquitiesGreatestUpsides instance.
	...
	"""
	return B3BdiEquitiesGreatestUpsides(date_ref=sample_date)
...
def sample_table_dict() -> dict:
	...
	-------
	dict
		Sample table dict mimicking the BDI InCashMarketBiggestHighs API response.
	"""
	return {
		"columns": [
			{"name": "TckrSymb"},
...
def test_init_with_valid_inputs(sample_date: date) -> None:
	...
	None
	"""
	inst = B3BdiEquitiesGreatestUpsides(date_ref=sample_date, int_page_size=500)
	...
	assert "InCashMarketBiggestHighs" in inst.url_tpl
...
def test_init_default_page_size(sample_date: date) -> None:
	...
	None
	"""
	inst = B3BdiEquitiesGreatestUpsides(date_ref=sample_date)
...
def test_init_without_date_ref(mocker: MockerFixture) -> None:
	...
	inst = B3BdiEquitiesGreatestUpsides()
...
def test_init_logger_propagated() -> None:
	...
	inst = B3BdiEquitiesGreatestUpsides(date_ref=date(2026, 4, 17), logger=mock_logger)
	assert inst.logger is mock_logger


def test_get_response_success(
	instance: B3BdiEquitiesGreatestUpsides,
	mocker: MockerFixture,
) -> None:
...
def test_parse_raw_file_missing_table_key(instance: B3BdiEquitiesGreatestUpsides) -> None:
...
def test_transform_data_normal(
	...
	assert df_["TCKR_SYMB"].iloc[0] == "T2TD34"
	assert df_["COD"].iloc[0] == "DRN"
	assert df_["PRICE"].iloc[0] == 1.12
	assert df_["OSCILLATION"].iloc[0] == 13.13


def test_transform_data_multiple_rows(instance: B3BdiEquitiesGreatestUpsides) -> None:
	...
	table = {
		...
		"values": [
			["T2TD34", "DRN", 1.12, 13.13, None],
			["PETR4", "ON", 37.50, 5.23, None],
			["VALE3", "ON", 62.10, 3.87, None],
		],
	}
	df_ = instance.transform_data(table)
	assert len(df_) == 3
	assert set(df_["TCKR_SYMB"].tolist()) == {"T2TD34", "PETR4", "VALE3"}
...
def test_module_reload(sample_date: date) -> None:
	...
	import stpstone.ingestion.countries.br.exchange.b3_bdi_equities_greatest_upsides as mod

	importlib.reload(mod)
	inst = mod.B3BdiEquitiesGreatestUpsides(date_ref=sample_date)
	assert inst.date_ref == sample_date
	assert "InCashMarketBiggestHighs" in inst.url_tpl
