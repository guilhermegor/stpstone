...
@pytest.fixture
def temp_image_path(sample_image_data: np.ndarray) -> Path:
...
# Error Cases
# --------------------------
def test_one_hot_vectorizer_empty_corpus(
    classification: Classification
) -> None:
...
def test_classifiers_with_empty_data(
    classification: Classification
) -> None:
...
def test_img_dims_invalid_path(
    image_processing: ImageProcessing
) -> None:
...
def test_type_validation_one_hot_vectorizer(
    classification: Classification
) -> None:
