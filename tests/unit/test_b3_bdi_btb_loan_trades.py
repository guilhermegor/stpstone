"""Unit tests for B3BdiBtbLoanTrades class."""
...
@pytest.fixture
def instance(sample_date: date) -> B3BdiBtbLoanTrades:
	"""Fixture providing a B3BdiBtbLoanTrades instance.
	...
	"""
	return B3BdiBtbLoanTrades(date_ref=sample_date)
...
def sample_table_dict() -> dict:
	...
	-------
	dict
		Sample table dict mimicking the BDI BTBTrade API response.
	"""
	return {
		"columns": [
			{"name": "TckrSymb"},
...
def test_init_with_valid_inputs(sample_date: date) -> None:
	...
	None
	"""
	inst = B3BdiBtbLoanTrades(date_ref=sample_date, int_page_size=500)
	...
	assert "BTBTrade" in inst.url_tpl
...
def test_init_default_page_size(sample_date: date) -> None:
	...
	None
	"""
	inst = B3BdiBtbLoanTrades(date_ref=sample_date)
...
def test_init_logger_propagated() -> None:
	...
	inst = B3BdiBtbLoanTrades(date_ref=date(2026, 4, 17), logger=mock_logger)
	assert inst.logger is mock_logger


def test_get_response_success(
	instance: B3BdiBtbLoanTrades,
	mocker: MockerFixture,
) -> None:
...
def test_parse_raw_file_missing_table_key(instance: B3BdiBtbLoanTrades) -> None:
...
def test_transform_data_normal(
	...
	assert df_["TCKR_SYMB"].iloc[0] == "BBSE3"
	assert df_["QUANTITY"].iloc[0] == 100
	assert df_["RATE"].iloc[0] == 0.0004
...
def test_module_reload(sample_date: date) -> None:
	...
	import stpstone.ingestion.countries.br.exchange.b3_bdi_btb_loan_trades as mod

	importlib.reload(mod)
	inst = mod.B3BdiBtbLoanTrades(date_ref=sample_date)
	assert inst.date_ref == sample_date
	assert "BTBTrade" in inst.url_tpl
