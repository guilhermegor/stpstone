"""Unit tests for B3BdiStocksConsolidatedTrades class."""
...
@pytest.fixture
def instance(sample_date: date) -> B3BdiStocksConsolidatedTrades:
	"""Fixture providing a B3BdiStocksConsolidatedTrades instance.
	...
	"""
	return B3BdiStocksConsolidatedTrades(date_ref=sample_date)
...
def sample_table_dict() -> dict:
	...
	-------
	dict
		Sample table dict mimicking the BDI ConsolidatedTradesEquities API response structure.
	"""
	return {
		"columns": [
			{"name": "RptDt"},
...
def test_init_with_valid_inputs(sample_date: date) -> None:
	...
	None
	"""
	inst = B3BdiStocksConsolidatedTrades(date_ref=sample_date, int_page_size=500)
	...
	assert "ConsolidatedTradesEquities" in inst.url_tpl
...
def test_init_default_page_size(sample_date: date) -> None:
	...
	None
	"""
	inst = B3BdiStocksConsolidatedTrades(date_ref=sample_date)
...
def test_init_without_date_ref(mocker: MockerFixture) -> None:
	...
	inst = B3BdiStocksConsolidatedTrades()
...
def test_init_logger_propagated() -> None:
	...
	inst = B3BdiStocksConsolidatedTrades(date_ref=date(2026, 4, 17), logger=mock_logger)
	assert inst.logger is mock_logger


def test_get_response_success(
	instance: B3BdiStocksConsolidatedTrades,
	mocker: MockerFixture,
) -> None:
...
def test_parse_raw_file_missing_table_key(instance: B3BdiStocksConsolidatedTrades) -> None:
...
def test_transform_data_normal(
	...
	assert df_["TCKR_SYMB"].iloc[1] == "VALE3"
	assert df_["OPEN_PRIC"].iloc[0] == 38.10
	assert df_["LAST_PRIC"].iloc[1] == 62.70
...
def test_run_without_db_paginates(
	...
	Pages: page 1 returns two rows, page 2 returns empty -> loop stops after page 1.
...
def test_module_reload(sample_date: date) -> None:
	...
	import stpstone.ingestion.countries.br.exchange.b3_bdi_stocks_consolidated_trades as mod

	importlib.reload(mod)
	inst = mod.B3BdiStocksConsolidatedTrades(date_ref=sample_date)
	assert inst.date_ref == sample_date
	assert "ConsolidatedTradesEquities" in inst.url_tpl
