"""Unit tests for AnbimaIPCAForecastsNextMonth ingestion module.
...
@pytest.fixture
def anbima_ipca_next_month(
	mock_logger: Logger,
	mock_session: Session,
) -> AnbimaIPCAForecastsNextMonth:
	"""Fixture providing AnbimaIPCAForecastsNextMonth instance.
	...
	-------
	AnbimaIPCAForecastsNextMonth
		Initialized AnbimaIPCAForecastsNextMonth instance.
	"""
	return AnbimaIPCAForecastsNextMonth(logger=mock_logger, cls_db=mock_session)
...
class TestAnbimaIPCAForecastsNextMonth:
	"""Test cases for AnbimaIPCAForecastsNextMonth class."""

	def test_init(
		self,
		anbima_ipca_next_month: AnbimaIPCAForecastsNextMonth,
	) -> None:
		...
		----------
		anbima_ipca_next_month : AnbimaIPCAForecastsNextMonth
			AnbimaIPCAForecastsNextMonth instance.

		Returns
		-------
		None
		"""
		assert isinstance(anbima_ipca_next_month, AnbimaIPCACore)
		assert anbima_ipca_next_month.url == (
	...
	def test_run_returns_dataframe(
		...
		mocker.patch.object(AnbimaIPCAForecastsNextMonth, "transform_data", return_value=mock_df)
		mocker.patch.object(AnbimaIPCACore, "standardize_dataframe", return_value=mock_df)

		anbima_ipca_next_month.cls_db = None
		result = anbima_ipca_next_month.run()
	...
	def test_run_with_db_calls_insert(
		...
		assert call_kwargs["str_table_name"] == "br_anbima_ipca_forecasts_next_month"
	...
	def test_transform_data(
		...
		result = anbima_ipca_next_month.transform_data(mock_playwright_scraper)
...
def test_module_reload(mocker: MockerFixture) -> None:
	...
	importlib.reload(
		sys.modules[
			"stpstone.ingestion.countries.br.macroeconomics.anbima_ipca_forecasts_next_month"
		]
	)
	new_instance = AnbimaIPCAForecastsNextMonth()
