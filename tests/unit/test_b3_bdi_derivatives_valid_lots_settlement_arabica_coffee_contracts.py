"""Unit tests for B3BdiDerivativesValidLotsSettlementArabicaCoffeeContracts class."""
...
def instance(
    ...
    """Fixture providing a B3BdiDerivativesValidLotsSettlementArabicaCoffeeContracts instance.
    ...
    """
    return B3BdiDerivativesValidLotsSettlementArabicaCoffeeContracts(date_ref=sample_date)
...
def sample_table_dict() -> dict:
    ...
    -------
    dict
        Sample table dict mimicking the BDI ValidLotsSettle API response structure.
    """
    return {
        "columns": [
            {"name": "Tckr"},
        ...
        "values": [
            ["K26", "K26  - Vertice 6.5", 0, 0, 1, 0, 1, 0.27, 6.5, None],
            ["N26", "N26  - Vertice 6.5", 0, 1, 2, 0, 3, 0.81, 6.5, None],
        ],
    }
...
def test_init_with_valid_inputs(sample_date: date) -> None:
    ...
    - URL template contains the ValidLotsSettle endpoint name.
    ...
    None
    """
    inst = B3BdiDerivativesValidLotsSettlementArabicaCoffeeContracts(
    ...
    assert "ValidLotsSettle" in inst.url_tpl
...
def test_init_default_page_max(sample_date: date) -> None:
    ...
    None
    """
    inst = B3BdiDerivativesValidLotsSettlementArabicaCoffeeContracts(date_ref=sample_date)
...
def test_get_response_success(
    instance: B3BdiDerivativesValidLotsSettlementArabicaCoffeeContracts,
    mocker: MockerFixture,
) -> None:
...
def test_transform_data_normal(
    ...
    assert len(df_) == 2
    expected_cols = [
        "TCKR", "TCKR_SYMB", "AGIO", "BASE", "DES_AGIO",
        "NON_STANDARD", "TOTAL", "PERCE", "PERCE_DESAGIO",
    ]
    for col in expected_cols:
        assert col in df_.columns
    assert df_["TCKR"].iloc[0] == "K26"
    assert df_["AGIO"].iloc[0] == 0
    assert df_["PERCE"].iloc[0] == 0.27
...
def test_run_without_db_returns_dataframe(
    ...
    """Test run stops at int_page_max=1 and returns a DataFrame.
