...
class TestDebenturesPricer:
	...
	def test_calculate_price_zero_coupon(self, default_debenture: DebenturesPricer) -> None:
		...
		assert pytest.approx(default_debenture.calculate_price(), 2) == pytest.approx(
			expected, abs=1e-2
		)
	...
	def test_total_return(self, default_debenture: DebenturesPricer) -> None:
		...
		assert default_debenture.total_return() == pytest.approx(current + capital, abs=1e-4)
	...
	def test_accrued_interest_invalid_convention(
		self, default_debenture: DebenturesPricer
	) -> None:
