...
def test_init_with_valid_inputs(
	sample_date: datetime.date, mock_logger: Logger, mock_db_session: object
) -> None:
...
def test_init_without_date_ref(mock_logger: Logger, mocker: MockerFixture) -> None:
	...
	None
	"""
	mocker.patch.object(DatesBRAnbima, "add_working_days", return_value=datetime.date(2025, 9, 14))
...
def test_get_response_success(
	inveting_instance: InvetingComIPCAForecast, mocker: MockerFixture
) -> None:
	...
	mock_playwright_class.assert_called_once_with(
		bool_headless=True, int_default_timeout=5_000, logger=inveting_instance.logger
	)
...
def test_get_response_invalid_timeout(
	inveting_instance: InvetingComIPCAForecast, invalid_timeout: object
) -> None:
...
def test_parse_raw_file_success(
	inveting_instance: InvetingComIPCAForecast, sample_json_data: dict, mocker: MockerFixture
) -> None:
...
def test_parse_raw_file_invalid_scraper(
	inveting_instance: InvetingComIPCAForecast, invalid_scraper: object
) -> None:
...
def test_transform_data_invalid_json(
	inveting_instance: InvetingComIPCAForecast, invalid_json: object
) -> None:
...
def test_run_without_db(
	...
	mocker.patch.object(DatesBRAnbima, "unix_timestamp_to_datetime", return_value="2021-07-01")
	mocker.patch.object(inveting_instance, "standardize_dataframe", return_value=sample_dataframe)
...
def test_run_invalid_table_name(
	inveting_instance: InvetingComIPCAForecast, invalid_table_name: object, mocker: MockerFixture
) -> None:
