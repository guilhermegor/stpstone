...
class TestValidateWorkingDay:
	...
	def test_valid_working_day(
		self, airflow_plugins: AirflowPlugins, valid_context: dict[str, Any]
	) -> None:
		...
		with patch.object(DatesBRAnbima, "is_working_day", return_value=True) as mock_method:
			...
			valid_context["ti"].xcom_push.assert_called_once_with(key="bool_continue", value=True)
	...
	def test_working_day_variations(
		...
		with patch.object(DatesBRAnbima, "is_working_day", return_value=expected_result):
			...
			test_context["ti"].xcom_push.assert_called_once_with(
				key="bool_continue", value=expected_result
			)
