"""Unit tests for B3BdiDerivativesSwaps class."""
...
@pytest.fixture
def instance(sample_date: date) -> B3BdiDerivativesSwaps:
	"""Fixture providing a B3BdiDerivativesSwaps instance.
	...
	"""
	return B3BdiDerivativesSwaps(date_ref=sample_date)
...
def sample_table_dict() -> dict:
	...
	-------
	dict
		Sample table dict mimicking the BDI SwapFlex API response.
	"""
	return {
		"columns": [
			{"name": "TckrSymb"},
		...
		"values": [
			["CDI", "2026-07-01", 10, 5000000.00, 13.75, None],
		],
	}
...
def test_init_with_valid_inputs(sample_date: date) -> None:
	...
	None
	"""
	inst = B3BdiDerivativesSwaps(date_ref=sample_date, int_page_size=500)
	...
	assert "SwapFlex" in inst.url_tpl
...
def test_init_default_page_size(sample_date: date) -> None:
	...
	None
	"""
	inst = B3BdiDerivativesSwaps(date_ref=sample_date)
...
def test_init_logger_propagated() -> None:
	...
	inst = B3BdiDerivativesSwaps(date_ref=date(2026, 4, 17), logger=mock_logger)
	assert inst.logger is mock_logger


def test_get_response_success(
	instance: B3BdiDerivativesSwaps,
	mocker: MockerFixture,
) -> None:
...
def test_parse_raw_file_missing_table_key(instance: B3BdiDerivativesSwaps) -> None:
...
def test_transform_data_normal(
	...
	assert len(df_) == 1
	expected_cols = ["TCKR_SYMB", "DUE_DATE", "NMBR_BUSINESSES", "NATIONAL_VOL", "AVRG_DAILY_RATE"]
	assert list(df_.columns) == expected_cols
	assert df_["TCKR_SYMB"].iloc[0] == "CDI"
	assert df_["NMBR_BUSINESSES"].iloc[0] == 10
	assert df_["NATIONAL_VOL"].iloc[0] == 5000000.0
	assert df_["AVRG_DAILY_RATE"].iloc[0] == 13.75
...
def test_run_without_db(
	...
	assert isinstance(result["AVRG_DAILY_RATE"].iloc[0], Decimal)
...
def test_module_reload(sample_date: date) -> None:
	...
	import stpstone.ingestion.countries.br.otc.b3_bdi_derivatives_swaps as mod

	importlib.reload(mod)
	inst = mod.B3BdiDerivativesSwaps(date_ref=sample_date)
	assert inst.date_ref == sample_date
	assert "SwapFlex" in inst.url_tpl
