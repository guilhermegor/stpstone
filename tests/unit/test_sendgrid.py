...
# Constants
# --------------------------
TypeValidEmail = TypeVar("TypeValidEmail", 
                         bound=tuple[str, list[str], Optional[list[str]], str, str, str])
...
def test_validate_email_params_valid_input(
    ...
    sendgrid_instance._validate_email_params(str_sender, list_recipients, subject, html_body, 
                                             token)
...
def test_send_email_none_inputs(
    ...
    with pytest.raises(ValueError, match="API token cannot be empty"):
        sendgrid_instance.send_email(str_sender, list_recipients, list_cc, subject, html_body, 
                                     None)
...
def test_send_email_invalid_types(
    ...
    """
    str_sender, list_recipients, list_cc, subject, html_body, token = valid_email_params
    with pytest.raises(ValueError, match="Error sending email from sendgrid: 'int' "
    ...
    with pytest.raises(ValueError, match="Error sending email from sendgrid: HTTP Error 401: "
                       + "Unauthorized"):
