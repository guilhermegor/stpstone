...
def test_init_connection_error(
	mock_oracledb: Mock, valid_db_params: dict[str, Union[str, int]], logger: logging.Logger
) -> None:
...
def test_read_with_date_conversion(
	mock_oracledb: Mock, valid_db_params: dict[str, Union[str, int]], mocker: MockerFixture
) -> None:
...
def test_close(mock_oracledb: Mock, valid_db_params: dict[str, Union[str, int]]) -> None:
...
def test_close_error(
	...
	with pytest.raises(oracledb.DatabaseError, match="Close failed"):
		db.close()
	mock_log_message = CreateLog().log_message
	mock_log_message.assert_called_with(logger, "Error closing connection: Close failed", "error")
...
def test_context_manager(mock_oracledb: Mock, valid_db_params: dict[str, Union[str, int]]) -> None:
