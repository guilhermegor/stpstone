...
@pytest.fixture
def alpha_tools(valid_alpha_tools_params: dict[str, Any]) -> Any:  # noqa ANN401: typing.Any is not allowed
...
class TestInit:
	...
	def test_init_with_valid_params(self, valid_alpha_tools_params: dict[str, Any]) -> None:
...
class TestGenericReq:
	...
	def test_successful_request(
		...
		with patch(
			"stpstone.utils.providers.br.inoa.inoa.request", return_value=mock_response
		) as mock_request:
	...
	def test_request_retry_on_failure(
		...
		with (
			...
			pytest.raises(ValueError, match="API request failed"),
		):
...
class TestFunds:
	...
	def test_funds_retrieval_failure(
		...
		with (
			patch("requests.request", return_value=mock_response),
			pytest.raises(ValueError, match="Failed to process funds data"),
		):
			_ = alpha_tools.funds
...
class TestQuotes:
	...
	def test_quotes_retrieval_failure(
		...
		with (
			patch("requests.request", return_value=mock_response),
			pytest.raises(ValueError, match="Failed to process quotes data"),
		):
