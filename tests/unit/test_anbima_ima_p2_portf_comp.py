"""Unit tests for AnbimaIMAP2TheoreticalPortfolio ingestion class."""
...
class TestAnbimaIMAP2TheoreticalPortfolio:
	...
	@pytest.fixture
	def theoretical_portfolio_instance_no_db(
		self,
		sample_date: date,
		mock_logger: Logger,
	) -> AnbimaIMAP2TheoreticalPortfolio:
		"""Create AnbimaIMAP2TheoreticalPortfolio instance without DB session.
	...
	def test_init_valid_inputs(
		...
		None
		"""
		instance = AnbimaIMAP2TheoreticalPortfolio(
			date_ref=sample_date, logger=mock_logger, cls_db=mock_db_session
		)
		...
		assert not hasattr(instance, "date_ref_yymmdd")
	...
	def test_get_response_success(
		...
		result = theoretical_portfolio_instance.get_response(
			timeout=(12.0, 21.0), bool_verify=True
		)
		assert result == mock_response
		mock_requests_get.assert_called_once_with(
			theoretical_portfolio_instance.url, timeout=(12.0, 21.0), verify=True
		)
	...
	def test_parse_raw_file(
		...
		mock_get_file = mocker.patch.object(
			theoretical_portfolio_instance,
			"get_file",
	...
	def test_run_without_db(
		self,
		theoretical_portfolio_instance_no_db: AnbimaIMAP2TheoreticalPortfolio,
		mock_requests_get: MagicMock,
		mock_response: Response,
		mocker: MockerFixture,
		mock_backoff: None,
		...
		mock_requests_get.return_value = mock_response
		mocker.patch.object(
			theoretical_portfolio_instance_no_db,
			"get_file",
			return_value=StringIO("@@header\n@@\n@@20250905@IMA@LTN@data\n"),
		)
		mocker.patch.object(
			theoretical_portfolio_instance_no_db,
		...
		result = theoretical_portfolio_instance_no_db.run()
		assert isinstance(result, pd.DataFrame)
		mock_requests_get.assert_called_once()
		theoretical_portfolio_instance_no_db.standardize_dataframe.assert_called_once()
	...
	def test_run_with_db(
		...
		mocker.patch.object(
			theoretical_portfolio_instance, "standardize_dataframe", return_value=pd.DataFrame()
		)
		mock_insert = mocker.patch.object(theoretical_portfolio_instance, "insert_table_db")
		result = theoretical_portfolio_instance.run()
