...
def test_benford_law_valid_raw_data(
	stat_dist_ht: StatisticalDistributionsHT, sample_data: NDArray[np.float64]
) -> None:
...
def test_benford_law_valid_occurrence_data(
	stat_dist_ht: StatisticalDistributionsHT, occurrence_data: NDArray[np.float64]
) -> None:
	...
	"""
	result: BenfordResults = stat_dist_ht.benford_law(
		occurrence_data, bool_list_number_occurrencies=True
	)
	expected_benford = np.array([np.log10(i + 2) - np.log10(i + 1) for i in range(9)])
	assert isinstance(result, dict)
	assert "benford_expected_array" in result
	...
	assert result["real_numbers_observed_array"].shape == (9,)
	assert np.allclose(
		result["real_numbers_observed_array"], occurrence_data / occurrence_data.sum(), rtol=1e-5
	)
	assert pytest.approx(np.sum(result["real_numbers_observed_array"]), rel=1e-3) == 1.0
...
def test_benford_law_percentual_occurrence_validation(
	stat_dist_ht: StatisticalDistributionsHT, mocker: MockerFixture
) -> None:
