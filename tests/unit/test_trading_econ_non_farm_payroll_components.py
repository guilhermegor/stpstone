"""Unit tests for TradingEconNonFarmPayrollComponents class."""
...
from stpstone.ingestion.countries.ww.macroeconomics.trading_econ_non_farm_payroll_components import (  # noqa: E501
...
@pytest.fixture
def instance(sample_date: date) -> TradingEconNonFarmPayrollComponents:
	"""Provide a TradingEconNonFarmPayrollComponents instance for testing.
	...
	"""
	return TradingEconNonFarmPayrollComponents(date_ref=sample_date)
...
def test_init_default_date_ref() -> None:
	...
	with patch.object(DatesBRAnbima, "add_working_days", return_value=date(2025, 1, 1)):
		obj = TradingEconNonFarmPayrollComponents()
	assert isinstance(obj.date_ref, date)


def test_init_sets_expected_attributes(instance: TradingEconNonFarmPayrollComponents) -> None:
	"""Test initialization sets expected helper attributes.

	Parameters
	----------
	instance : TradingEconNonFarmPayrollComponents
		Ingestion instance.
...
def test_transform_data_quits_driver(
	...
	fixed_texts = ["Government", "45.0", "42.0", "K", "Dec"]
	with (
		patch.object(
			TradingEconNonFarmPayrollComponents,
			"_list_web_elements",
