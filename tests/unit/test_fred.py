...
def mock_requests_get(mocker: MockerFixture) -> object:
	...
	"""
	return mocker.patch("stpstone.ingestion.countries.us.macroeconomics.fred.requests.get")
...
def test_parse_raw_file_passthrough(fred_instance: FredUSMacro, mock_response: Response) -> None:
...
def test_run_without_db(
	...
	with (
		patch.object(fred_instance, "transform_data", return_value=sample_df) as mock_transform,
		patch.object(
			fred_instance,
...
def test_run_with_db(
	...
	sample_df = pd.DataFrame({"id": ["GNPCA"]})
	with (
		patch.object(fred_instance, "transform_data", return_value=sample_df),
...
def test_get_response_timeout_variations(
	...
	assert result is mock_response
	mock_requests_get.assert_called_once_with(fred_instance.url, timeout=timeout, verify=False)
