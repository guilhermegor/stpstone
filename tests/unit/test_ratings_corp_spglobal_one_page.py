...
def test_get_response_http_error(
    ...
    with patch("requests.post", side_effect=req_lib.exceptions.HTTPError("500")), \
...
def test_run_without_db(
    ...
    with patch("requests.post", return_value=mock_post_resp), patch.object(
...
def test_reload_module() -> None:
    ...
    with patch("requests.get") as mock_get:
        ...
        obj = mod.RatingsCorpSPGlobalOnePage(
            bearer="Bearer test", token="key", date_ref=date(2025, 1, 1)  # noqa: S106
        )
    assert obj.date_ref == date(2025, 1, 1)
