...
class TestExecute:
	...
	def test_execute_valid_query(
		self, valid_db_config: dict[str, Any], mock_connection: Mock
	) -> None:
...
class TestRead:
	...
	def test_read_with_date_conversion(
		...
		with (
			...
			patch("pandas.read_sql", return_value=sample_query_result),
			patch(
				"stpstone.utils.connections.databases.sql.sqlserver_db.ABCCalendarOperations",
			...
			_ = db.read("SELECT * FROM test_table", list_cols_dt=date_cols, str_fmt_dt=date_format)
...
class TestInsert:
	...
	def test_insert_valid_data(
		self, valid_db_config: dict[str, Any], mock_connection: Mock, mock_json_files: Mock
	) -> None:
		...
		with (
			...
			mock_to_sql.assert_called_once_with(
				table_name, mock_connection, if_exists="append", index=False
			)
...
class TestErrorLogging:
	...
	def test_error_logging_on_connection_failure(self, valid_db_config: dict[str, Any]) -> None:
		...
		"""
		with (
			patch("stpstone.utils.connections.databases.sql.sqlserver_db.connect") as mock_connect,
			patch(
