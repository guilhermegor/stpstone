"""Unit tests for B3WarrantiesInternationalSecurities ingestion class."""
...
def mock_html_handler(mocker: MockerFixture) -> HtmlHandler:
	...
	html_handler = mocker.patch(
		...
		".b3_warranties_international_securities.HtmlHandler"
	)
...
def mock_dir_files_management(mocker: MockerFixture) -> DirFilesManagement:
	...
	dir_files_management = mocker.patch(
		"stpstone.ingestion.countries.br.exchange"
		".b3_warranties_international_securities.DirFilesManagement"
	)
...
def mock_dates_br(mocker: MockerFixture) -> DatesBRAnbima:
	...
	dates_br = mocker.patch(
		"stpstone.ingestion.countries.br.exchange"
		".b3_warranties_international_securities.DatesBRAnbima"
	)
...
def mock_dates_current(mocker: MockerFixture) -> DatesCurrent:
	...
	dates_current = mocker.patch(
		"stpstone.ingestion.countries.br.exchange"
		".b3_warranties_international_securities.DatesCurrent"
	)
...
def mock_create_log(mocker: MockerFixture) -> CreateLog:
	...
	"""
	return mocker.patch(
		"stpstone.ingestion.countries.br.exchange.b3_warranties_international_securities.CreateLog"
	).return_value
...
def test_init_default_date(
	...
	None
	"""
	inst = B3WarrantiesInternationalSecurities()
...
def test_init_custom_date(
	...
	inst = B3WarrantiesInternationalSecurities(date_ref=custom_date)
...
def test_transform_data(
	...
	- Timestamp strips are applied to date columns.
	...
	sample_data.to_excel(
		excel_buffer, sheet_name="Títulos Internacionais", index=False, engine="openpyxl"
	)
	excel_buffer.seek(0)
	...
	assert "CODIGO_TITULO" in result.columns
	assert "CUSIP" in result.columns
	assert "DATA_VENCIMENTO" in result.columns
	assert "PROXIMO_PAGAMENTO_CUPOM" in result.columns
...
def test_run_without_db(
	...
	assert isinstance(result, pd.DataFrame)
	expected_columns = ["ID", "TIPO_TITULO", "CODIGO_TITULO", "CUSIP"]
...
def test_run_with_db(
	...
	mocker.patch(
		"stpstone.ingestion.abc.ingestion_abc.ABCIngestionOperations.standardize_dataframe",
		return_value=pd.DataFrame({"ID": ["INST1"]}),
	)
...
def test_module_reload(
	...
	None
	"""
	import stpstone.ingestion.countries.br.exchange.b3_warranties_international_securities

	importlib.reload(
		stpstone.ingestion.countries.br.exchange.b3_warranties_international_securities
	)
	inst = stpstone.ingestion.countries.br.exchange.b3_warranties_international_securities.B3WarrantiesInternationalSecurities()  # noqa: E501
