...
class TestConfigurationMethods:
	...
	def test_load_config_default(self) -> None:
		...
		expected = {"host": "localhost", "port": 6379, "decode_responses": True}
	...
	def test_load_config_custom(self) -> None:
		...
		expected = {"host": "redis.example.com", "port": 6380, "decode_responses": False}
		assert config == expected
...
class TestConnectionMethods:
	...
	def test_connect_success(
		self, mock_strict_redis: Mock, default_config: ReturnLoadConfig
	) -> None:
		...
		result = RedisClient._connect(
			default_config["host"], default_config["port"], default_config["decode_responses"]
		)
...
class TestGetMethod:
	...
	def test_get_returns_redis_client(
		self, redis_client_instance: RedisClient, mock_redis_client: Mock
	) -> None:
	...
	def test_get_without_initialization_raises_error(self) -> None:
		...
		with patch(
			"stpstone.utils.connections.databases.nosql.redisdb.RedisClient._connect"
		) as mock_connect:
...
class TestErrorConditions:
	...
	def test_connection_error_chaining(self, mock_strict_redis: Mock) -> None:
		...
		with pytest.raises(
			RuntimeError, match="Failed to create Redis client instance"
		) as exc_info:
