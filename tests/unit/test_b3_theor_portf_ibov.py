"""Unit tests for B3TheoricalPortfolioIBOV module.

Tests the ingestion functionality for the B3 IBOV theoretical portfolio, covering:
...
def sample_dataframe() -> pd.DataFrame:
	"""Provide a sample DataFrame matching the IBOV portfolio structure.
...
# Tests
# --------------------------
class TestB3TheoricalPortfolioIBOV:
	"""Test cases for B3TheoricalPortfolioIBOV."""
	...
	def instance_no_db(
		...
		"""
		return B3TheoricalPortfolioIBOV(date_ref=sample_date, logger=mock_logger, cls_db=None)
	...
	def test_init_with_valid_inputs(
		...
		- url resolves to the IBOV class-level constant
		- Helper instances are created

		Parameters
		----------
		instance : B3TheoricalPortfolioIBOV
	...
	def test_init_none_date_ref(self, mock_logger: MagicMock) -> None:
		...
		with patch.object(DatesBRAnbima, "add_working_days", return_value=date(2023, 1, 1)):
			inst = B3TheoricalPortfolioIBOV(date_ref=None, logger=mock_logger)
	...
	def test_transform_data_normal(self, instance: B3TheoricalPortfolioIBOV) -> None:
	...
	def test_transform_data_empty(self, instance: B3TheoricalPortfolioIBOV) -> None:
	...
	def test_run_without_db(
		self,
		instance_no_db: B3TheoricalPortfolioIBOV,
		mock_requests_get: MagicMock,
		mock_response: MagicMock,
	...
	def test_run_with_db(
		...
		instance.insert_table_db.assert_called_once_with(
			...
			str_table_name="br_b3_theorical_portfolio_ibov",
	...
	def test_run_invalid_table_name_raises(
		...
		None
		"""
		inst = B3TheoricalPortfolioIBOV(
			date_ref=sample_date, logger=mock_logger, cls_db=mock_db_session
		)
		# Temporarily clear the class-level default so the guard fires
		original = B3TheoricalPortfolioIBOV._TABLE_NAME
		B3TheoricalPortfolioIBOV._TABLE_NAME = ""
		try:
			mock_requests_get.return_value = mock_response
			...
			B3TheoricalPortfolioIBOV._TABLE_NAME = original
...
def test_module_reload() -> None:
	...
	None
	"""
	importlib.reload(sys.modules["stpstone.ingestion.countries.br.exchange.b3_theor_portf_ibov"])
	assert hasattr(
		sys.modules["stpstone.ingestion.countries.br.exchange.b3_theor_portf_ibov"],
		"B3TheoricalPortfolioIBOV",
	)
