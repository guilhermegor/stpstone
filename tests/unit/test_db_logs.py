...
@pytest.fixture
def mock_dates_br(mocker: MockerFixture) -> Any:  # noqa ANN401: typing.Any is not allowed
	...
	"""
	return mocker.patch.object(ABCCalendarOperations, "utc_log_ts", return_value=date(2025, 8, 13))


@pytest.fixture
def mock_hostname(mocker: MockerFixture) -> Any:  # noqa ANN401: typing.Any is not allowed
...
def test_audit_log_empty_df(db_logs: DBLogs, empty_df: pd.DataFrame, sample_date: date) -> None:
...
def test_audit_log_invalid_url(
	db_logs: DBLogs, sample_df: pd.DataFrame, sample_date: date
) -> None:
...
def test_log_data_insert_empty_df(
	db_logs: DBLogs, empty_df: pd.DataFrame, sample_date: date
) -> None:
