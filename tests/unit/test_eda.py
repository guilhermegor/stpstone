...
class TestPrepareBins:
	...
	def sample_data(self) -> tuple[pd.DataFrame, str, str]:
		...
		"""
		df_ = pd.DataFrame(
			{"feature": np.random.normal(size=100), "target": np.random.randint(0, 2, size=100)}
		)
		return df_, "feature", "target"

	def test_successful_monotonic_binning(
		self, eda: ExploratoryDataAnalysis, sample_data: tuple[pd.DataFrame, str, str]
	) -> None:
...
class TestEdaDatabase:
	...
	def sample_df(self) -> pd.DataFrame:
		...
		"""
		return pd.DataFrame(
			{"col1": np.random.normal(size=100), "col2": np.random.randint(0, 10, size=100)}
		)
	...
	def test_valid_bins(
		self, eda: ExploratoryDataAnalysis, sample_df: pd.DataFrame, bins: int
	) -> None:
...
class TestValidationMethods:
	...
	@pytest.mark.parametrize(
		"value,valid", [(1, True), (0, False), (-1, False), (1.5, False), ("1", False)]
	)
	def test_validate_positive_integer(
		self, eda: ExploratoryDataAnalysis, value: int, valid: bool
	) -> None:
