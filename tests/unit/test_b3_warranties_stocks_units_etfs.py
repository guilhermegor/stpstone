"""Unit tests for B3WarrantiesStocksUnitsETFs ingestion class."""
...
def mock_html_handler(mocker: MockerFixture) -> HtmlHandler:
	...
	html_handler = mocker.patch(
		"stpstone.ingestion.countries.br.exchange.b3_warranties_stocks_units_etfs.HtmlHandler"
	)
...
def mock_dir_files_management(mocker: MockerFixture) -> DirFilesManagement:
	...
	dir_files_management = mocker.patch(
		...
		".b3_warranties_stocks_units_etfs.DirFilesManagement"
	)
...
def mock_dates_br(mocker: MockerFixture) -> DatesBRAnbima:
	...
	dates_br = mocker.patch(
		"stpstone.ingestion.countries.br.exchange.b3_warranties_stocks_units_etfs.DatesBRAnbima"
	)
...
def mock_dates_current(mocker: MockerFixture) -> DatesCurrent:
	...
	dates_current = mocker.patch(
		"stpstone.ingestion.countries.br.exchange.b3_warranties_stocks_units_etfs.DatesCurrent"
	)
...
def mock_create_log(mocker: MockerFixture) -> CreateLog:
	...
	"""
	return mocker.patch(
		"stpstone.ingestion.countries.br.exchange.b3_warranties_stocks_units_etfs.CreateLog"
	).return_value
...
def test_init_default_date(
	...
	None
	"""
	inst = B3WarrantiesStocksUnitsETFs()
...
def test_init_custom_date(
	...
	inst = B3WarrantiesStocksUnitsETFs(date_ref=custom_date)
...
def test_get_href(
	...
	mock_html_handler.lxml_xpath.assert_called_once_with(
		html_content="parsed_html", str_xpath='//*[@id="panel1a"]/ul/li[2]/a'
	)
...
def test_module_reload(
	...
	None
	"""
	import stpstone.ingestion.countries.br.exchange.b3_warranties_stocks_units_etfs

	importlib.reload(stpstone.ingestion.countries.br.exchange.b3_warranties_stocks_units_etfs)
	inst = stpstone.ingestion.countries.br.exchange.b3_warranties_stocks_units_etfs.B3WarrantiesStocksUnitsETFs()  # noqa: E501
	assert "limites-de-renda-variavel-e-fixa" in inst.url
