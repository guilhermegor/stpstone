"""Unit tests for B3BdiDerivativesElectronicForwards class."""
...
@pytest.fixture
def instance(sample_date: date) -> B3BdiDerivativesElectronicForwards:
	"""Fixture providing a B3BdiDerivativesElectronicForwards instance.
	...
	"""
	return B3BdiDerivativesElectronicForwards(date_ref=sample_date)
...
def sample_table_dict() -> dict:
	...
	-------
	dict
		Sample table dict mimicking the BDI EletronicTerm API response.
	"""
	return {
		"columns": [
			{"name": "TckrSymb"},
			...
			{"name": "NmbrBusinesses"},
			{"name": "NationalVol"},
			{"name": "InternationalVol"},
		],
		"values": [
			["LFT", "2026-09-01", 5, 12000000.00, 2300000.00, None],
		],
	}
...
def test_init_with_valid_inputs(sample_date: date) -> None:
	...
	None
	"""
	inst = B3BdiDerivativesElectronicForwards(date_ref=sample_date, int_page_size=500)
	...
	assert "EletronicTerm" in inst.url_tpl
...
def test_init_default_page_size(sample_date: date) -> None:
	...
	None
	"""
	inst = B3BdiDerivativesElectronicForwards(date_ref=sample_date)
	assert inst.int_page_size == 1_000


def test_init_default_page_max_is_none(sample_date: date) -> None:
	"""Test that default int_page_max is None (unbounded pagination).
...
def test_init_without_date_ref(mocker: MockerFixture) -> None:
	...
	inst = B3BdiDerivativesElectronicForwards()
...
def test_init_logger_propagated() -> None:
	...
	inst = B3BdiDerivativesElectronicForwards(date_ref=date(2026, 4, 17), logger=mock_logger)
	assert inst.logger is mock_logger


def test_get_response_success(
	instance: B3BdiDerivativesElectronicForwards,
	mocker: MockerFixture,
) -> None:
...
def test_transform_data_normal(
	...
	assert df_["TCKR_SYMB"].iloc[0] == "LFT"
	assert df_["NMBR_BUSINESSES"].iloc[0] == 5
	assert df_["NATIONAL_VOL"].iloc[0] == 12000000.0
	assert df_["INTERNATIONAL_VOL"].iloc[0] == 2300000.0
...
def test_run_without_db(
	...
	"""Test run fetches pages until empty, then returns a DataFrame with Decimal volumes.
	...
	mocker.patch.object(instance, "get_response", side_effect=[mock_response, empty_resp])
	...
	assert isinstance(result["INTERNATIONAL_VOL"].iloc[0], Decimal)
...
def test_module_reload(sample_date: date) -> None:
	...
	import stpstone.ingestion.countries.br.otc.b3_bdi_derivatives_electronic_forwards as mod

	importlib.reload(mod)
	inst = mod.B3BdiDerivativesElectronicForwards(date_ref=sample_date)
	assert inst.date_ref == sample_date
	assert "EletronicTerm" in inst.url_tpl
