...
@pytest.fixture
def anbima_instance(mock_logger: MagicMock, sample_date: date) -> Any:  # noqa ANN401: typing.Any is not allowed
	...
	"""
	with (
		patch.object(DatesBRAnbima, "add_working_days", return_value=sample_date),
		patch.object(DatesCurrent, "curr_date", return_value=date(2023, 12, 20)),
	):
		return AnbimaFundsRedemptionProbabilityMatrix(
			date_ref=sample_date, logger=mock_logger, headless=True
		)


@pytest.fixture
def anbima_instance_with_db(mock_logger: MagicMock, mock_db_session: MagicMock) -> Any:  # noqa ANN401: typing.Any is not allowed
	...
	"""
	with (
		patch.object(DatesBRAnbima, "add_working_days", return_value=date(2023, 10, 15)),
		patch.object(DatesCurrent, "curr_date", return_value=date(2023, 12, 20)),
	):
		return AnbimaFundsRedemptionProbabilityMatrix(
			logger=mock_logger, cls_db=mock_db_session, headless=True
		)
...
class TestInitialization:
	...
	def test_init_with_date_ref(self, mock_logger: MagicMock, sample_date: date) -> None:
		...
		with (
			...
			instance = AnbimaFundsRedemptionProbabilityMatrix(
				date_ref=sample_date, logger=mock_logger
			)
	...
	def test_init_without_date_ref(self, mock_logger: MagicMock) -> None:
		...
		with (
			patch.object(DatesBRAnbima, "add_working_days", return_value=expected_date),
			patch.object(DatesCurrent, "curr_date", return_value=date(2023, 12, 20)),
		):
	...
	def test_month_mapping_correctness(self, mock_logger: MagicMock) -> None:
		...
		for test_date_, expected_month in test_cases:
			with (
				patch.object(DatesBRAnbima, "add_working_days", return_value=test_date_),
				patch.object(DatesCurrent, "curr_date", return_value=date(2023, 12, 20)),
			):
				instance = AnbimaFundsRedemptionProbabilityMatrix(
					date_ref=test_date_, logger=mock_logger
				)
...
class TestRunMethod:
	...
	def test_run_success_with_dataframe_return(
		...
		"""
		with (
			patch.object(anbima_instance, "get_response", return_value=sample_dataframe),
	...
	def test_run_success_with_database_insert(
		...
		"""
		with (
			patch.object(anbima_instance_with_db, "get_response", return_value=sample_dataframe),
			patch.object(anbima_instance_with_db, "transform_data", return_value=sample_dataframe),
			patch.object(
				anbima_instance_with_db, "standardize_dataframe", return_value=sample_dataframe
			),
	...
	def test_run_no_data_retrieved(
		...
		with (
			patch.object(anbima_instance, "get_response", return_value=empty_df),
			...
			mock_log.assert_any_call(
				anbima_instance.logger, "❌ No data retrieved from source", "error"
			)
	...
	def test_run_custom_parameters(
		...
		with (
			...
			anbima_instance.run(
				timeout_ms=300000, bool_insert_or_ignore=False, str_table_name="custom_table"
			)
...
class TestGetResponse:
	...
	def test_get_response_success(
		...
		with (
			patch(
				...
				"anbima_funds_withdraws_probability.sync_playwright"
			) as mock_playwright,
	...
	def test_get_response_month_selection_fails(
		...
		with (
			...
			assert isinstance(result, pd.DataFrame)
			mock_log.assert_any_call(
				anbima_instance.logger, "⚠ Continuing with current month selection", "warning"
			)
	...
	def test_get_response_exception_handling(
		...
		with (
			...
			patch.object(
				anbima_instance, "_initialize_browser", side_effect=Exception("Test error")
			),
			patch.object(anbima_instance.cls_create_log, "log_message") as mock_log,
		):
			mock_playwright.return_value.__enter__.return_value.chromium.launch.return_value = (
			...
			assert result.empty
			mock_log.assert_any_call(
				anbima_instance.logger, "❌ Error during scraping process: Test error", "error"
			)
			mock_browser.close.assert_called_once()
...
class TestBrowserInitialization:
	...
	def test_initialize_browser_success(
		...
		mock_page.goto.assert_called_once_with(
			anbima_instance.base_url, wait_until="load", timeout=60000
		)
	...
	def test_initialize_browser_iframe_not_found(
		...
		with patch.object(anbima_instance.cls_create_log, "log_message") as mock_log:
			...
			assert result is None
			mock_log.assert_any_call(
				anbima_instance.logger, "✗ Power BI iframe not found", "error"
			)
...
class TestSelectionMethods:
	...
	def test_select_month_already_correct(
		...
		with patch.object(anbima_instance.cls_create_log, "log_message") as mock_log:
			...
			assert result is True
			mock_log.assert_any_call(
				anbima_instance.logger, "✓ Already on target month: Outubro", "info"
			)
	...
	def test_select_month_dropdown_not_found(
		...
		with patch.object(anbima_instance.cls_create_log, "log_message") as mock_log:
			...
			mock_log.assert_any_call(anbima_instance.logger, "✗ Month dropdown not found", "error")
...
class TestDownloadMethods:
	...
	def test_download_data_fails(
		...
		with (
			...
			assert result is None
			mock_log.assert_any_call(
				anbima_instance.logger, "✗ Download failed: Download failed", "error"
			)
...
class TestDataProcessing:
	...
	def test_process_csv_data_success(
		...
		with patch.object(anbima_instance.cls_create_log, "log_message") as mock_log:
			...
			mock_log.assert_any_call(
				anbima_instance.logger, "✓ Successfully decoded with utf-8", "info"
			)
	...
	def test_process_csv_data_single_column_fix(
		...
		with patch.object(anbima_instance.cls_create_log, "log_message") as mock_log:
			...
			assert any(
				"single column" in str(call).lower() or "one column" in str(call).lower()
...
2023-10-15,Mensal,Renda Fixa,0.5678""".encode("utf-8")  # noqa UP012: unnecessary utf-8 `encoding` argument to `encode`
	...
	def test_transform_data_success(
		...
		with patch.object(anbima_instance.cls_create_log, "log_message") as mock_log:
			...
			mock_log.assert_any_call(
				anbima_instance.logger, "✓ Processed numeric column: valor", "info"
			)
	...
	def test_transform_data_empty(
		...
		with patch.object(anbima_instance.cls_create_log, "log_message") as mock_log:
			...
			assert result.empty
			mock_log.assert_any_call(anbima_instance.logger, "⚠ No data to transform", "warning")
...
class TestErrorHandling:
	...
	def test_initialization_with_none_date(self, mock_logger: MagicMock) -> None:
		...
		with (
			...
			instance = AnbimaFundsRedemptionProbabilityMatrix(date_ref=None, logger=mock_logger)
	...
	def test_download_with_temp_file_cleanup(
		...
		with (
			...
			patch(
				"stpstone.ingestion.countries.br.registries"
				+ ".anbima_funds_withdraws_probability.Path"
			) as mock_path_class,
			patch("builtins.open", mock_open(read_data=sample_csv_data)),
			patch.object(mock_page, "expect_download") as mock_expect,
		):
	...
	def test_month_selection_with_special_characters(
		...
		with (
			patch.object(DatesBRAnbima, "add_working_days", return_value=date(2023, 3, 15)),
			patch.object(DatesCurrent, "curr_date", return_value=date(2023, 12, 20)),
		):
			instance = AnbimaFundsRedemptionProbabilityMatrix(
				date_ref=date(2023, 3, 15), logger=MagicMock()
			)
...
class TestPerformance:
	...
	def test_memory_efficient_data_processing(
		...
		large_data = pd.DataFrame(
			{"data": ["2023-10-15"] * 1000, "valor": ["1,2345"] * 1000, "prazo": ["30"] * 1000}
		)

		with (
			patch.object(anbima_instance, "get_response", return_value=large_data),
