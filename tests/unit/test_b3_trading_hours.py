...
def test_b3_trading_hours_core_init_default_date(mocker: MockerFixture) -> None:
	...
	mock_dates_current = mocker.patch.object(
		DatesCurrent, "curr_date", return_value=date(2025, 9, 13)
	)
...
def test_get_response_success(
	mocker: MockerFixture, b3_trading_hours_core: B3TradingHoursCore, mock_response: Response
) -> None:
...
def test_get_response_invalid_bool_verify(
	b3_trading_hours_core: B3TradingHoursCore, invalid_bool_verify: Union[None, str, int]
) -> None:
...
def test_parse_raw_file(
	b3_trading_hours_core: B3TradingHoursCore, mock_response: Response, mocker: MockerFixture
) -> None:
...
def test_transform_data(
	...
	None
	"""
	mock_xpath = mocker.patch.object(
		HtmlHandler, "lxml_xpath", return_value=[MagicMock(text="Test")]
	)
...
def test_b3_trading_hours_stocks_transform_data(
	...
	mock_transform.assert_called_once_with(
		...
		xpath_td='//*[@id="conteudo-principal"]/div[4]/div/div/table[1]/tbody/tr[position() > 1]/td',
		na_values="-",
	)
...
def test_transform_data_empty_html(
	b3_trading_hours_core: B3TradingHoursCore, mocker: MockerFixture
) -> None:
...
def test_transform_data_invalid_inputs(
	b3_trading_hours_core: B3TradingHoursCore, invalid_input: tuple
) -> None:
...
def test_run_invalid_table_name_none(b3_trading_hours_core: B3TradingHoursCore) -> None:
	...
	with pytest.raises(TypeError, match="str_table_name must be of type str, got NoneType"):
		b3_trading_hours_core.run(dict_dtypes={"MERCADO": str}, str_table_name=None)
...
def test_run_invalid_table_name_empty_string(b3_trading_hours_core: B3TradingHoursCore) -> None:
	...
	with suppress(TypeError, ValueError):
		b3_trading_hours_core.run(dict_dtypes={"MERCADO": str}, str_table_name="")
...
def test_run_invalid_table_name_integer(b3_trading_hours_core: B3TradingHoursCore) -> None:
	...
	with pytest.raises(TypeError, match="str_table_name must be of type str, got int"):
		b3_trading_hours_core.run(dict_dtypes={"MERCADO": str}, str_table_name=123)
