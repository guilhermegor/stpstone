...
class TestBRRegressiveTaxSchedules:
	...
	def test_calculate_total_taxes(
		...
		"""
		iof, ir, total = tax_calculator.calculate_total_taxes(cddt, cddr, sample_notional)
	...
	# Test Edge Cases
	# --------------------------
	def test_very_large_notional(self, tax_calculator: BRRegressiveTaxSchedules) -> None:
	...
	def test_docstring_example(self, tax_calculator: BRRegressiveTaxSchedules) -> None:
		...
		iof, ir, total = tax_calculator.calculate_total_taxes(
			int_cddt=200, int_cddr=10, float_notional=10000.00
		)
