"""Unit tests for B3TradingHoursUSDollarDenominatedInterestRatesFutures ingestion class."""
...
def mock_dataframe() -> pd.DataFrame:
	...
	"""
	return pd.DataFrame({"CONTRACT": ["DDI"], "TICKER": ["DDI1"]})
...
def test_init_correct_url() -> None:
	...
	None
	"""
	instance = B3TradingHoursUSDollarDenominatedInterestRatesFutures(date_ref=date(2025, 9, 12))
	assert instance.url == (
...
def test_run_returns_dataframe_without_db(
	...
	None
	"""
	instance = B3TradingHoursUSDollarDenominatedInterestRatesFutures(
	...
	result = instance.run(
		str_table_name="br_b3_trading_hours_usdollar_denominated_interest_rates_futures"
	)
