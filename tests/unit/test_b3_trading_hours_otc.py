"""Unit tests for B3TradingHoursOTC ingestion class."""
...
def mock_dataframe() -> pd.DataFrame:
	...
	"""
	return pd.DataFrame({"MERCADO": ["OTC"], "NEGOCIACAO_INICIO": ["09:00"]})
...
def test_init_correct_url() -> None:
	"""Test that the correct B3 OTC URL is set on initialization.

	Returns
	-------
	None
	"""
	instance = B3TradingHoursOTC(date_ref=date(2025, 9, 12))
	assert instance.url == (
		"https://www.b3.com.br/pt_br/solucoes/plataformas/puma-trading-system/"
		"para-participantes-e-traders/horario-de-negociacao/balcao-organizado/"
	)


def test_init_inherits_from_core() -> None:
	"""Test that B3TradingHoursOTC inherits from B3TradingHoursCore.
...
def test_run_returns_dataframe_without_db(
	...
	None
	"""
	instance = B3TradingHoursOTC(date_ref=date(2025, 9, 12), cls_db=None)
	mocker.patch("requests.get", return_value=mock_response)
	...
	result = instance.run(str_table_name="br_b3_trading_hours_exchange_otc")
...
def test_run_with_db_returns_none(
	...
	instance = B3TradingHoursOTC(date_ref=date(2025, 9, 12), cls_db=mock_session)
	mocker.patch("requests.get", return_value=mock_response)
	mocker.patch.object(B3TradingHoursCore, "parse_raw_file", return_value=mock_html_element)
	mocker.patch.object(B3TradingHoursCore, "transform_data", return_value=mock_dataframe)
	mocker.patch.object(B3TradingHoursCore, "standardize_dataframe", return_value=mock_dataframe)
