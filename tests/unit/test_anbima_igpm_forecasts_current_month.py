"""Unit tests for AnbimaIGPMForecastsCurrentMonth ingestion module.
...
class TestAnbimaIGPMForecastsCurrentMonth:
	...
	def test_init(
		...
		assert anbima_igpm_current_month.url == (
	...
	def test_run_returns_dataframe(
		...
		mocker.patch.object(
			AnbimaIGPMForecastsCurrentMonth, "transform_data", return_value=mock_df
		)
	...
	def test_run_with_db_calls_insert(
		...
		assert call_kwargs["str_table_name"] == "br_anbima_igpm_forecasts_current_month"
...
def test_module_reload(mocker: MockerFixture) -> None:
	...
	importlib.reload(
		sys.modules[
			"stpstone.ingestion.countries.br.macroeconomics.anbima_igpm_forecasts_current_month"
		]
	)
	new_instance = AnbimaIGPMForecastsCurrentMonth()
