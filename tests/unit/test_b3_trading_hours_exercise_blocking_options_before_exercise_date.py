"""Unit tests for B3TradingHoursExerciseBlockingOptionsBeforeExerciseDate class."""
...
from stpstone.ingestion.countries.br.exchange.b3_trading_hours_exercise_blocking_options_before_exercise_date import (  # noqa: E501
...
def mock_dataframe() -> pd.DataFrame:
	...
	"""
	return pd.DataFrame({"CONTRACT": ["OPT"], "EXERCISE": ["09:00"]})
...
def test_init_correct_url() -> None:
	"""Test that the correct B3 exercise-and-blocking URL is set on initialization.

	Returns
	-------
	None
	"""
	instance = B3TradingHoursExerciseBlockingOptionsBeforeExerciseDate(date_ref=date(2025, 9, 12))
	assert instance.url == (
		"https://www.b3.com.br/en_us/solutions/platforms/puma-trading-system/"
		"for-members-and-traders/trading-hours/derivatives/exercise-and-blocking-of-options/"
	)


def test_init_inherits_from_core() -> None:
	"""Test that the class inherits from B3TradingHoursCore.
...
def test_run_returns_dataframe_without_db(
	...
	None
	"""
	instance = B3TradingHoursExerciseBlockingOptionsBeforeExerciseDate(
		date_ref=date(2025, 9, 12), cls_db=None
	)
	mocker.patch("requests.get", return_value=mock_response)
	...
	result = instance.run(
		str_table_name="br_b3_trading_hours_exercise_blocking_options_before_exercise_date"
	)
...
def test_run_with_db_returns_none(
	...
	instance = B3TradingHoursExerciseBlockingOptionsBeforeExerciseDate(
		date_ref=date(2025, 9, 12), cls_db=mock_session
	)
	mocker.patch("requests.get", return_value=mock_response)
	mocker.patch.object(B3TradingHoursCore, "parse_raw_file", return_value=mock_html_element)
	mocker.patch.object(B3TradingHoursCore, "transform_data", return_value=mock_dataframe)
...
def test_transform_data_calls_super_with_correct_args(
	...
	mock_transform.assert_called_once_with(
		...
		list_th=["CONTRACT", "TICKER", "EXERCISE", "PURCHASE_FOR_BLOCKING_WITH_EXERCISE_RISK"],
