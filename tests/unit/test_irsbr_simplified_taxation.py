"""Unit tests for IRSBRSimplifiedTaxation class.
...
# Helpers
# --------------------------
def _build_zip_bytes(csv_content: str, csv_filename: str = "Simples.csv") -> BytesIO:
	...
	csv_filename : str
		The name of the CSV entry, by default "Simples.csv".
...
def _sample_csv_row() -> str:
	...
	-------
	str
		One complete data row with 7 fields.
	"""
	return "12345678;01;20180101;00000000;02;20200301;00000000"
...
@pytest.fixture
def irsbr_instance(sample_date: date) -> IRSBRSimplifiedTaxation:
	"""Fixture providing IRSBRSimplifiedTaxation instance.
	...
	-------
	IRSBRSimplifiedTaxation
		Initialized IRSBRSimplifiedTaxation instance.
	"""
	return IRSBRSimplifiedTaxation(date_ref=sample_date)
...
def test_init_with_valid_inputs(sample_date: date) -> None:
	...
	Verifies
	--------
	- The IRSBRSimplifiedTaxation instance is initialized correctly.
	...
	None
	"""
	instance = IRSBRSimplifiedTaxation(date_ref=sample_date)
...
def test_init_with_default_date() -> None:
	...
	with patch.object(DatesBRAnbima, "add_working_days", return_value=date(2025, 3, 14)):
		instance = IRSBRSimplifiedTaxation()
...
def test_get_response_success(
	irsbr_instance: IRSBRSimplifiedTaxation,
	...
	----------
	irsbr_instance : IRSBRSimplifiedTaxation
		Instance of IRSBRSimplifiedTaxation.
	mock_requests_get : object
...
def test_transform_data(
	...
	assert df_["CODE_SIMPL_TX"].iloc[0] == "01"


def test_transform_data_none_input(irsbr_instance: IRSBRSimplifiedTaxation) -> None:
...
def test_transform_data_invalid_zip(irsbr_instance: IRSBRSimplifiedTaxation) -> None:
...
def test_transform_data_zip_without_csv(irsbr_instance: IRSBRSimplifiedTaxation) -> None:
...
def test_run_fetches_single_file(
	irsbr_instance: IRSBRSimplifiedTaxation,
	mock_requests_get: object,
	mock_response: Response,
	mock_backoff: object,
) -> None:
	"""Test that run fetches exactly one file (Simples.zip).

	Verifies
	--------
	- requests.get is called exactly once.
	- The URL contains 'Simples.zip'.
	...
	with (
		...
		assert "Simples.zip" in call_url
...
def test_reload_module() -> None:
	...
	import stpstone.ingestion.countries.br.taxation.irsbr_simplified_taxation

	original_instance = IRSBRSimplifiedTaxation(date_ref=date(2025, 3, 15))
	importlib.reload(stpstone.ingestion.countries.br.taxation.irsbr_simplified_taxation)
	new_instance = IRSBRSimplifiedTaxation(date_ref=date(2025, 3, 15))
