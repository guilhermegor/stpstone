"""Unit tests for B3TradingHoursPMIFuture ingestion class."""
...
def mock_dataframe() -> pd.DataFrame:
	...
	"""
	return pd.DataFrame({"CONTRACT": ["PMI"], "TICKER": ["PMI1"]})
...
def test_init_correct_url() -> None:
	...
	None
	"""
	instance = B3TradingHoursPMIFuture(date_ref=date(2025, 9, 12))
	assert instance.url == (
...
def test_init_inherits_from_core() -> None:
	"""Test that B3TradingHoursPMIFuture inherits from B3TradingHoursCore.
...
def test_run_returns_dataframe_without_db(
	...
	None
	"""
	instance = B3TradingHoursPMIFuture(date_ref=date(2025, 9, 12), cls_db=None)
	mocker.patch("requests.get", return_value=mock_response)
	...
	result = instance.run(str_table_name="br_b3_trading_hours_pmi_future")
...
def test_run_with_db_returns_none(
	...
	instance = B3TradingHoursPMIFuture(date_ref=date(2025, 9, 12), cls_db=mock_session)
	mocker.patch("requests.get", return_value=mock_response)
	mocker.patch.object(B3TradingHoursCore, "parse_raw_file", return_value=mock_html_element)
	mocker.patch.object(B3TradingHoursCore, "transform_data", return_value=mock_dataframe)
	mocker.patch.object(B3TradingHoursCore, "standardize_dataframe", return_value=mock_dataframe)
...
def test_transform_data_calls_super_with_correct_args(
	...
	None
	"""
	instance = B3TradingHoursPMIFuture()
