"""Unit tests for AnbimaDataIDALIQGeral module.

Tests the ingestion functionality for AnbimaDataIDALIQGeral including:
...
def sample_dataframe() -> pd.DataFrame:
	...
	-------
	pd.DataFrame
		Sample DataFrame with expected IDALIQGeral structure (9 columns, no PMR)
	"""
	return pd.DataFrame(
		{
			"INDICE": ["TEST"],
...
class TestAnbimaDataIDALIQGeral:
	...
	@pytest.fixture
	def instance_no_db(self, sample_date: date, mock_logger: MagicMock) -> AnbimaDataIDALIQGeral:
		"""Fixture providing AnbimaDataIDALIQGeral instance without database.
		...
		"""
		return AnbimaDataIDALIQGeral(date_ref=sample_date, logger=mock_logger, cls_db=None)
	...
	def test_init_without_date(self, mock_logger: MagicMock, mock_db_session: MagicMock) -> None:
		...
		with patch.object(DatesBRAnbima, "add_working_days", return_value=date(2023, 1, 1)):
			instance = AnbimaDataIDALIQGeral(logger=mock_logger, cls_db=mock_db_session)
	...
	def test_transform_data(self, instance: AnbimaDataIDALIQGeral, mocker: MockerFixture) -> None:
	...
	def test_run_without_db(
		self,
		instance_no_db: AnbimaDataIDALIQGeral,
		mock_requests_get: MagicMock,
		mock_response: Response,
	...
	def test_run_with_db(
		...
		instance.insert_table_db.assert_called_once_with(
			...
			str_table_name="br_anbima_data_indexes_ida_liq_geral",
			df_=sample_dataframe,
			bool_insert_or_ignore=False,
		)

	def test_run_invalid_timeout(self, instance: AnbimaDataIDALIQGeral) -> None:
...
def test_module_reload() -> None:
	...
	"""
	importlib.reload(
		sys.modules["stpstone.ingestion.countries.br.exchange.anbima_data_ida_liq_geral"]
	)
	assert hasattr(
		sys.modules["stpstone.ingestion.countries.br.exchange.anbima_data_ida_liq_geral"],
		"AnbimaDataIDALIQGeral",
