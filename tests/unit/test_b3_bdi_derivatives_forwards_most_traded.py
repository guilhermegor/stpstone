"""Unit tests for B3BdiDerivativesForwardsMostTraded class."""
...
@pytest.fixture
def instance(sample_date: date) -> B3BdiDerivativesForwardsMostTraded:
	"""Fixture providing a B3BdiDerivativesForwardsMostTraded instance.
	...
	"""
	return B3BdiDerivativesForwardsMostTraded(date_ref=sample_date)
...
def sample_table_dict() -> dict:
	...
	-------
	dict
		Sample table dict mimicking the BDI Forward API response.
	"""
	return {
		"columns": [
			{"name": "TckrSymb"},
		...
		"values": [
			["ISHARES SMALCI", "CI", 33, 125.27, 250000, 31317.5, 2.189, None],
		],
	}
...
def test_init_with_valid_inputs(sample_date: date) -> None:
	...
	None
	"""
	inst = B3BdiDerivativesForwardsMostTraded(date_ref=sample_date, int_page_size=500)
	...
	assert "Forward" in inst.url_tpl
...
def test_init_default_page_size(sample_date: date) -> None:
	...
	None
	"""
	inst = B3BdiDerivativesForwardsMostTraded(date_ref=sample_date)
...
def test_init_without_date_ref(mocker: MockerFixture) -> None:
	...
	inst = B3BdiDerivativesForwardsMostTraded()
...
def test_init_logger_propagated() -> None:
	...
	inst = B3BdiDerivativesForwardsMostTraded(date_ref=date(2026, 4, 17), logger=mock_logger)
	assert inst.logger is mock_logger


def test_get_response_success(
	instance: B3BdiDerivativesForwardsMostTraded,
	mocker: MockerFixture,
) -> None:
...
def test_parse_raw_file_missing_table_key(instance: B3BdiDerivativesForwardsMostTraded) -> None:
...
def test_transform_data_normal(
	...
	instance : B3BdiDerivativesForwardsMostTraded
		Initialized instance.
	sample_table_dict : dict
		Sample table dict with one row (8 values, 7 columns).
	...
	assert df_["TCKR_SYMB"].iloc[0] == "ISHARES SMALCI"
	assert df_["TERM"].iloc[0] == 33
	assert df_["VLM_TRADED_DAY"].iloc[0] == 31317.5


def test_transform_data_multiple_rows(instance: B3BdiDerivativesForwardsMostTraded) -> None:
	...
	table = {
		...
		"values": [
			["ISHARES SMALCI", "CI", 33, 125.27, 250000, 31317.5, 2.189, None],
			["IT NOW SPXI CI", "CI", 60, 49.24, 342812, 16880.06, 3.001, None],
			["VIBRA       ON      NM", "ON      NM", 122, 34.79, 2720000, 94628.8, 23.817, None],
		],
	}
...
def test_module_reload(sample_date: date) -> None:
	...
	import stpstone.ingestion.countries.br.exchange.b3_bdi_derivatives_forwards_most_traded as mod

	importlib.reload(mod)
	inst = mod.B3BdiDerivativesForwardsMostTraded(date_ref=sample_date)
	assert inst.date_ref == sample_date
	assert "Forward" in inst.url_tpl
