"""Unit tests for B3TradingHoursExchangeRateFutures ingestion class."""
...
def mock_dataframe() -> pd.DataFrame:
	...
	"""
	return pd.DataFrame({"CONTRACT": ["USD"], "TICKER": ["DOL1"]})
...
def test_init_correct_url() -> None:
	"""Test that the correct B3 FX URL is set on initialization.

	Returns
	-------
	None
	"""
	instance = B3TradingHoursExchangeRateFutures(date_ref=date(2025, 9, 12))
	assert instance.url == (
		"https://www.b3.com.br/en_us/solutions/platforms/puma-trading-system/"
		"for-members-and-traders/trading-hours/derivatives/foreign-exchange-and-dollar-spot/"
	)


def test_init_inherits_from_core() -> None:
	"""Test that B3TradingHoursExchangeRateFutures inherits from B3TradingHoursCore.
...
def test_run_returns_dataframe_without_db(
	...
	None
	"""
	instance = B3TradingHoursExchangeRateFutures(date_ref=date(2025, 9, 12), cls_db=None)
	mocker.patch("requests.get", return_value=mock_response)
	...
	result = instance.run(str_table_name="br_b3_trading_hours_exchange_rate_futures")
...
def test_run_with_db_returns_none(
	...
	instance = B3TradingHoursExchangeRateFutures(date_ref=date(2025, 9, 12), cls_db=mock_session)
	mocker.patch("requests.get", return_value=mock_response)
	mocker.patch.object(B3TradingHoursCore, "parse_raw_file", return_value=mock_html_element)
	mocker.patch.object(B3TradingHoursCore, "transform_data", return_value=mock_dataframe)
	mocker.patch.object(B3TradingHoursCore, "standardize_dataframe", return_value=mock_dataframe)
...
def test_transform_data_calls_super_with_correct_args(
	...
	None
	"""
	instance = B3TradingHoursExchangeRateFutures()
