...
def test_validate_ranges_empty_constraints(
...
def test_validate_ranges_invalid_bounds_type(
    dataframe_validator: DataFrameValidator
) -> None:
...
def test_validate_ranges_invalid_bounds_order(
    dataframe_validator: DataFrameValidator
) -> None:
...
def test_validate_dates_empty_column_names(
    dataframe_validator: DataFrameValidator
) -> None:
...
def test_validate_dates_nonexistent_columns(
    dataframe_validator: DataFrameValidator
) -> None:
...
def test_pipeline_partial_parameters(
    ...
    with patch.object(validator, "create_log", mock_create_log):
        ...
        result1 = validator.pipeline(
            dict_rng_constraints={"numeric_col": (1, 10)}
        )
        
        # Test with only date validation
        result2 = validator.pipeline(
            col_start="date_start",
            col_sup="date_end"
        )
...
def test_validate_ranges_type_validation(
    dataframe_validator: DataFrameValidator
) -> None:
...
def test_validate_dates_type_validation(
    dataframe_validator: DataFrameValidator
) -> None:
...
def test_dataframe_with_all_nulls(
    mock_logger: Mock,
    mock_create_log: Mock
) -> None:
