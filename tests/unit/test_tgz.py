...
class TestValidateInputPaths:
	...
	def test_no_source_provided(self, handler: HandlingTGZFiles) -> None:
		...
		"""
		with pytest.raises(
			ValueError, match="Either url_source or complete_source_path must be provided"
		):
...
class TestFetchTgzFiles:
	...
	def test_directory_creation(
		...
		None
		"""
		mock_mkdir = mocker.patch.object(DirFilesManagement, "mk_new_directory", return_value=None)
		mocker.patch.object(DirFilesManagement, "object_exists", return_value=False)

		new_dir = os.path.join(temp_dir, "new_dir")
		handler.fetch_tgz_files(new_dir, "test.tgz", complete_source_path=sample_tgz_path)
	...
	def test_url_download(
		...
		mock_download = mocker.patch.object(
			DirFilesManagement, "download_web_file", return_value="success"
		)
		...
		result = handler.fetch_tgz_files(
			temp_dir, "test.tgz", url_source="http://example.com/file.tgz"
		)

		mock_download.assert_called_once_with(
			"http://example.com/file.tgz", os.path.join(temp_dir, "test.tgz")
		)
		assert result["blame_download_tgz_file"] == "success"

	def test_local_file_handling(
		self, handler: HandlingTGZFiles, temp_dir: str, sample_tgz_path: str
	) -> None:
		...
		"""
		result = handler.fetch_tgz_files(
			temp_dir, "test.tgz", complete_source_path=sample_tgz_path
		)
	...
	def test_missing_source_file(self, handler: HandlingTGZFiles, temp_dir: str) -> None:
		...
		None
		"""
		with pytest.raises(ValueError, match=r"Source file not found: /nonexistent/path"):
			handler.fetch_tgz_files(temp_dir, "test.tgz", complete_source_path="/nonexistent/path")
	...
	def test_tgz_extraction_error(
		...
		with pytest.raises(ValueError, match="Failed to process TGZ file"):
			handler.fetch_tgz_files(temp_dir, "test.tgz", complete_source_path="/dummy/path")
...
class TestIntegration:
	...
	def test_full_workflow_url_download(
		...
		# setup mocks
		mock_download = mocker.patch.object(
			DirFilesManagement, "download_web_file", return_value="downloaded"
		)
		mocker.patch.object(DirFilesManagement, "object_exists", return_value=False)
	...
	def test_full_workflow_local_file(
		...
		assert result["blame_download_tgz_file"] == "n/a"
		assert set(result["extracted_files_names"]) == {"file0.txt", "file1.txt", "file2.txt"}
		for i in range(3):
