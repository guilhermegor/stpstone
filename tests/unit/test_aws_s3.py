...
def test_init_missing_credentials(
	...
	with pytest.raises(type_error, match=error_msg):
		S3Client(aws_access_key_id=key, aws_secret_access_key=secret, s3_bucket=bucket)
...
@pytest.mark.parametrize(
	...
	[
		...
		(ClientError({"Error": {"Code": "OtherError"}}, "operation"), "AWS Client Error: "),
		(
			EndpointConnectionError(endpoint_url="url"),
			"Failed to connect to AWS endpoint. Check internet connection.",
		),
		(FileNotFoundError("file.txt"), "The file test_key was not found locally."),
		(ValueError("generic error"), "Unexpected error during test_action: generic error"),
	],
)
def test_handle_error(
	s3_client: S3Client, mock_logger: Logger, exception: Exception, expected_message: str
) -> None:
...
def test_upload_file_success(
	s3_client: S3Client, sample_data: BytesIO, mock_boto3_client: Mock
) -> None:
	...
	mock_boto3_client.put_object.assert_called_once_with(
		Body=sample_data.getvalue(), Bucket="test_bucket", Key="test_key"
	)
...
def test_download_file_success(s3_client: S3Client, mock_boto3_client: Mock) -> None:
	...
	with patch("builtins.print") as mock_print:
		...
		mock_boto3_client.get_object.assert_called_once_with(Bucket="test_bucket", Key="test_key")
...
def test_list_objects_success(s3_client: S3Client, mock_boto3_client: Mock) -> None:
	...
	assert result == expected_result
	mock_boto3_client.list_objects.assert_called_once_with(Bucket="test_bucket", Prefix="prefix/")
...
def test_upload_file_empty_data(s3_client: S3Client, mock_boto3_client: Mock) -> None:
	...
	assert result is True
	mock_boto3_client.put_object.assert_called_once_with(
		Body=b"", Bucket="test_bucket", Key="test_key"
	)
...
def test_list_objects_empty_prefix(s3_client: S3Client, mock_boto3_client: Mock) -> None:
	...
	assert result == expected_result
	mock_boto3_client.list_objects.assert_called_once_with(Bucket="test_bucket", Prefix="")
