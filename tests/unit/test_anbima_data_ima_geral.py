"""Unit tests for AnbimaDataIMAGeral module.

Tests the ingestion functionality for AnbimaDataIMAGeral including:
...
def sample_dataframe() -> pd.DataFrame:
	...
	-------
	pd.DataFrame
		Sample DataFrame with expected IMAGeral structure (10 columns)
	"""
	return pd.DataFrame(
		{
			"INDICE": ["TEST"],
...
def test_module_reload() -> None:
	...
	None
	"""
	importlib.reload(sys.modules["stpstone.ingestion.countries.br.exchange.anbima_data_ima_geral"])
	assert hasattr(
		sys.modules["stpstone.ingestion.countries.br.exchange.anbima_data_ima_geral"],
		"AnbimaDataIMAGeral",
	)
