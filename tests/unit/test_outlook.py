...
class TestValidationMethods:
    ...
    @pytest.mark.parametrize(
        "path", 
        ["/valid/path", ["/valid/path1", "/valid/path2"]]
    )
...
class TestSendEmail:
    ...
    def test_send_email_with_attachments(
        ...
        dealing_outlook.send_email(
            "Test Subject",
            mail_attachments=[str(test_file), "nonexistent.txt"]
        )
    ...
    def test_send_email_on_behalf(
        ...
        dealing_outlook.send_email(
            "Test Subject", 
            send_behalf_of="other@example.com"
        )
...
class TestDownloadAttachment:
    ...
    def test_download_attachment_multiple_paths(
        ...
        with patch.object(DirFilesManagement, 'object_exists', return_value=True), \
            patch.object(JsonFiles, 'send_json', side_effect=lambda x: x):
                paths = [
                    str(tmp_path / "attachment1.txt"),
                    str(tmp_path / "attachment2.txt")
                ]
                result = dealing_outlook.download_attch(
                    "test@example.com",
                    "Inbox",
    ...
    def test_download_attachment_with_original_name(
        ...
        with patch.object(DirFilesManagement,'get_file_format_from_file_name', 
                          return_value="txt"), \
...
class TestEmailRetrieval:
    ...
    def test_get_body_content(
        ...
        with patch.object(
            ...
            result = dealing_outlook.get_body_content(
                "test@example.com",
                "Inbox",
                "Test"
            )
