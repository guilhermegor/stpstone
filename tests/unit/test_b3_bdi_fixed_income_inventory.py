"""Unit tests for B3BdiFixedIncomeInventory class."""
...
@pytest.fixture
def instance(sample_date: date) -> B3BdiFixedIncomeInventory:
	"""Fixture providing a B3BdiFixedIncomeInventory instance.
	...
	"""
	return B3BdiFixedIncomeInventory(date_ref=sample_date)
...
def test_init_with_valid_inputs(sample_date: date) -> None:
	...
	None
	"""
	inst = B3BdiFixedIncomeInventory(date_ref=sample_date, int_page_size=500)
...
def test_init_default_page_size(sample_date: date) -> None:
	...
	None
	"""
	inst = B3BdiFixedIncomeInventory(date_ref=sample_date)
...
def test_init_without_date_ref(mocker: MockerFixture) -> None:
	...
	inst = B3BdiFixedIncomeInventory()
...
def test_init_logger_propagated() -> None:
	...
	inst = B3BdiFixedIncomeInventory(date_ref=date(2026, 4, 17), logger=mock_logger)
	assert inst.logger is mock_logger


def test_get_response_success(instance: B3BdiFixedIncomeInventory, mocker: MockerFixture) -> None:
	"""Test get_response posts to the correct URL and returns the response.

	Parameters
	----------
	instance : B3BdiFixedIncomeInventory
		Initialized instance.
	mocker : MockerFixture
		Pytest-mock fixture.
...
def test_transform_data_with_null_volumes(instance: B3BdiFixedIncomeInventory) -> None:
