...
def test_cholesky_decomposition_non_positive_definite(linear_algebra: LinearAlgebra) -> None:
...
def test_cholesky_decomposition_invalid_type(linear_algebra: LinearAlgebra) -> None:
...
def test_eigenvalue_eigenvector_normal(
	...
	for i in range(len(eigenvalues)):
		assert np.allclose(
			valid_square_matrix @ eigenvectors[:, i], eigenvalues[i] * eigenvectors[:, i]
		)
...
# Shape Array
# --------------------------
def test_shape_array_normal(linear_algebra: LinearAlgebra, valid_matrix_2x3: np.ndarray) -> None:
...
def test_determinant_singular(linear_algebra: LinearAlgebra, singular_matrix: np.ndarray) -> None:
...
# Inverse
# --------------------------
def test_inverse_normal(linear_algebra: LinearAlgebra, valid_square_matrix: np.ndarray) -> None:
...
def test_inverse_singular(linear_algebra: LinearAlgebra, singular_matrix: np.ndarray) -> None:
...
def test_matrix_rank_singular(linear_algebra: LinearAlgebra, singular_matrix: np.ndarray) -> None:
