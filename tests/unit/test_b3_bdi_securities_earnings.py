"""Unit tests for B3BdiSecuritiesEarnings class."""
...
@pytest.fixture
def instance(sample_date: date) -> B3BdiSecuritiesEarnings:
	"""Fixture providing a B3BdiSecuritiesEarnings instance.
	...
	"""
	return B3BdiSecuritiesEarnings(date_ref=sample_date)
...
def sample_table_dict() -> dict:
	...
	-------
	dict
		Sample table dict mimicking the BDI ProventionCreditVariable API response.
	"""
	return {
		"columns": [
			{"name": "TckrSymb"},
...
def test_init_with_valid_inputs(sample_date: date) -> None:
	...
	None
	"""
	inst = B3BdiSecuritiesEarnings(date_ref=sample_date, int_page_size=500)
	...
	assert "ProventionCreditVariable" in inst.url_tpl
...
def test_init_default_page_size(sample_date: date) -> None:
	...
	None
	"""
	inst = B3BdiSecuritiesEarnings(date_ref=sample_date)
...
def test_init_logger_propagated() -> None:
	...
	inst = B3BdiSecuritiesEarnings(date_ref=date(2026, 4, 17), logger=mock_logger)
	assert inst.logger is mock_logger


def test_get_response_success(
	instance: B3BdiSecuritiesEarnings,
	mocker: MockerFixture,
) -> None:
...
def test_parse_raw_file_missing_table_key(instance: B3BdiSecuritiesEarnings) -> None:
...
def test_transform_data_normal(
	...
	assert df_["TCKR_SYMB"].iloc[0] == "EAGLE MATERIALS INC"
	assert df_["VALUE"].iloc[0] == pytest.approx(0.052634998)


def test_transform_data_multiple_rows(instance: B3BdiSecuritiesEarnings) -> None:
	...
	assert len(df_) == 2
	assert "EAGLE MATERIALS INC" in df_["TCKR_SYMB"].tolist()
	assert "IRB - BRASIL RESSEGUROS S.A." in df_["TCKR_SYMB"].tolist()
...
def test_run_without_db(
	...
	assert "EARNINGS_TYPE" in result.columns
	assert "DATE_AGE" in result.columns
	assert "VALUE" in result.columns
	assert "CREDIT_DATE" in result.columns
	assert "URL" in result.columns
	assert result["VALUE"].iloc[0] == pytest.approx(0.052634998)
...
def test_module_reload(sample_date: date) -> None:
	...
	import stpstone.ingestion.countries.br.exchange.b3_bdi_securities_earnings as mod

	importlib.reload(mod)
	inst = mod.B3BdiSecuritiesEarnings(date_ref=sample_date)
	assert inst.date_ref == sample_date
	assert "ProventionCreditVariable" in inst.url_tpl
