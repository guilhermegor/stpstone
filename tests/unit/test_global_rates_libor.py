"""Unit tests for GlobalRatesLibor class."""
...
from stpstone.ingestion.countries.ww.macroeconomics.global_rates_libor import GlobalRatesLibor
...
@pytest.fixture
def instance(sample_date: date) -> GlobalRatesLibor:
    """Provide a GlobalRatesLibor instance for testing.
    ...
    """
    return GlobalRatesLibor(date_ref=sample_date)
...
def test_init_with_valid_date(sample_date: date) -> None:
    ...
    None
    """
    obj = GlobalRatesLibor(date_ref=sample_date)
...
def test_init_with_default_date() -> None:
    ...
    with patch.object(DatesBRAnbima, "add_working_days", return_value=date(2025, 1, 1)):
        obj = GlobalRatesLibor()
        assert obj.date_ref == date(2025, 1, 1)


def test_get_response_success(
    instance: GlobalRatesLibor,
...
def test_reload_module() -> None:
    ...
    import stpstone.ingestion.countries.ww.macroeconomics.global_rates_libor as mod

    importlib.reload(mod)
    obj = mod.GlobalRatesLibor(date_ref=date(2025, 1, 1))
    assert obj.date_ref == date(2025, 1, 1)
