"""Unit tests for B3BdiIndexesIbovGreatestDownsides class."""
...
@pytest.fixture
def instance(sample_date: date) -> B3BdiIndexesIbovGreatestDownsides:
	"""Fixture providing a B3BdiIndexesIbovGreatestDownsides instance.
	...
	"""
	return B3BdiIndexesIbovGreatestDownsides(date_ref=sample_date)
...
def sample_table_dict() -> dict:
	...
	-------
	dict
		Sample table dict mimicking the BDI IbovespaStockBiggestLow API response.
	"""
	return {
		"columns": [
			{"name": "TckrSymb"},
...
def test_init_with_valid_inputs(sample_date: date) -> None:
	...
	None
	"""
	inst = B3BdiIndexesIbovGreatestDownsides(date_ref=sample_date, int_page_size=500)
	...
	assert "IbovespaStockBiggestLow" in inst.url_tpl
...
def test_init_default_page_size(sample_date: date) -> None:
	...
	None
	"""
	inst = B3BdiIndexesIbovGreatestDownsides(date_ref=sample_date)
...
def test_init_without_date_ref(mocker: MockerFixture) -> None:
	...
	inst = B3BdiIndexesIbovGreatestDownsides()
...
def test_init_logger_propagated() -> None:
	...
	inst = B3BdiIndexesIbovGreatestDownsides(date_ref=date(2026, 4, 17), logger=mock_logger)
	assert inst.logger is mock_logger


def test_get_response_success(
	instance: B3BdiIndexesIbovGreatestDownsides,
	mocker: MockerFixture,
) -> None:
...
def test_parse_raw_file_missing_table_key(instance: B3BdiIndexesIbovGreatestDownsides) -> None:
...
def test_transform_data_normal(
	...
	assert df_["TCKR_SYMB"].iloc[0] == "BRAV3"
	assert df_["COD"].iloc[0] == "ON      NM"
	assert df_["PRICE"].iloc[0] == 19.55
	assert df_["OSCILLATION_DESC"].iloc[0] == -6.27


def test_transform_data_multiple_rows(instance: B3BdiIndexesIbovGreatestDownsides) -> None:
	...
	table = {
		...
		"values": [
			["BRAV3", "ON      NM", 19.55, -6.27, None],
			["BRKM5", "PNA     N1", 8.85, -5.54, None],
			["PETR3", "ON      N2", 50.81, -5.31, None],
		],
	}
	df_ = instance.transform_data(table)
	assert len(df_) == 3
	assert set(df_["TCKR_SYMB"].tolist()) == {"BRAV3", "BRKM5", "PETR3"}
...
def test_module_reload(sample_date: date) -> None:
	...
	import stpstone.ingestion.countries.br.exchange.b3_bdi_indexes_ibov_greatest_downsides as mod

	importlib.reload(mod)
	inst = mod.B3BdiIndexesIbovGreatestDownsides(date_ref=sample_date)
	assert inst.date_ref == sample_date
	assert "IbovespaStockBiggestLow" in inst.url_tpl
