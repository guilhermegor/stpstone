...
def sample_price_df() -> pd.DataFrame:
	...
	"""
	return pd.DataFrame(
		{"date": pd.date_range("2023-01-01", periods=4), "price": [100.0, 110.0, 105.0, 120.0]}
	)
...
def test_calc_returns_from_prices_ln_return(
	financial_returns: FinancialReturns, sample_prices: list[float]
) -> None:
	...
	expected = [0.09531018, -0.04652002, 0.13353139]
...
def test_calc_returns_from_prices_single_price(financial_returns: FinancialReturns) -> None:
...
def test_pandas_returns_from_spot_prices_ln_return(
	financial_returns: FinancialReturns, sample_price_df: pd.DataFrame
) -> None:
...
def test_pandas_returns_from_spot_prices_missing_columns(
	...
	with pytest.raises(KeyError):
		financial_returns.pandas_returns_from_spot_prices(sample_price_df, "missing_price", "date")
