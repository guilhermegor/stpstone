...
class TestNumHandler:
    ...
    # multiples() tests
    # --------------------------
    @pytest.mark.parametrize("m, ceiling, expected", [
        (3, 10, [0, 3, 6, 9, 10]),
    ...
    @pytest.mark.parametrize("m, ceiling", [
        (0, 10),
        (-3, 10),
        (3, -10),
    ])
    def test_multiples_invalid_input(
        self, num_handler: TypeChecker, m: int, ceiling: int
    ) -> None:
    ...
    @pytest.mark.parametrize("number, multiple", [
        ("not a number", 3),
        (10.7, 0),
        (10.7, "not a number"),
    ])
    ...
    # round_up() tests
    # --------------------------
    @pytest.mark.parametrize("num, base, ceiling, expected", [
        (12.3, 5.0, 20.0, 15.0),
        (17.0, 5.0, 20.0, 20.0),
        (3.1, 2.0, 10.0, 4.0),
        (0.5, 1.0, 5.0, 1.0),
    ...
    @pytest.mark.parametrize("num, base, ceiling", [
        ("not a number", 5, 20),
    ...
    def test_sum_n_elements_valid(
        self, num_handler: TypeChecker, args: tuple[Union[int, float], ...], 
    ...
    def test_range_floats_valid(
        ...
        pace: float, expected: list[float]
    ) -> None:
    ...
    @pytest.mark.parametrize("epsilon, inf, sup, pace", [
        (1.0, 1.0, 0.5, 0.1),
        (1.0, 0.0, 1.0, -0.1),
    ])
    ...
    # clamp() tests
    # --------------------------
    @pytest.mark.parametrize("n, minn, maxn, expected", [
        (5.0, 0.0, 10.0, 5.0),
        (-1.0, 0.0, 10.0, 0.0),
        (15.0, 0.0, 10.0, 10.0),
    ])
