...
def mock_requests_get(mocker: MockerFixture) -> object:
    ...
    """
    return mocker.patch(
        "stpstone.ingestion.countries.ww.exchange.crypto.coinpaprika.requests.get"
    )
...
def test_run_without_db_returns_dataframe(
    ...
    with patch.object(coinpaprika_instance, "get_response", return_value=mock_response), \
            patch.object(
                coinpaprika_instance, "parse_raw_file", return_value=StringIO("[]")
            ), \
            patch.object(coinpaprika_instance, "transform_data", return_value=single_row), \
            patch.object(
                coinpaprika_instance, "standardize_dataframe", return_value=single_row
            ):
