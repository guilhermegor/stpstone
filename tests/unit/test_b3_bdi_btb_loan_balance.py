"""Unit tests for B3BdiBtbLoanBalance class."""
...
@pytest.fixture
def instance(sample_date: date) -> B3BdiBtbLoanBalance:
	"""Fixture providing a B3BdiBtbLoanBalance instance.
	...
	"""
	return B3BdiBtbLoanBalance(date_ref=sample_date)
...
def sample_table_dict() -> dict:
	...
	-------
	dict
		Sample table dict mimicking the BDI BTBLoanBalance API response.
	"""
	return {
		"columns": [
			{"name": "RptDt"},
...
def test_init_with_valid_inputs(sample_date: date) -> None:
	...
	None
	"""
	inst = B3BdiBtbLoanBalance(date_ref=sample_date, int_page_size=500)
	...
	assert "BTBLoanBalance" in inst.url_tpl
...
def test_init_default_page_size(sample_date: date) -> None:
	...
	None
	"""
	inst = B3BdiBtbLoanBalance(date_ref=sample_date)
...
def test_init_logger_propagated() -> None:
	...
	inst = B3BdiBtbLoanBalance(date_ref=date(2026, 4, 17), logger=mock_logger)
	assert inst.logger is mock_logger


def test_get_response_success(
	instance: B3BdiBtbLoanBalance,
	mocker: MockerFixture,
) -> None:
...
def test_parse_raw_file_missing_table_key(instance: B3BdiBtbLoanBalance) -> None:
...
def test_transform_data_normal(
	...
	assert df_["TCKR_SYMB"].iloc[0] == "5PREETF11"
	assert df_["DNR_MIN_RATE"].iloc[0] == 0.0025
...
def test_run_without_db(
	...
	assert "DNR_MIN_RATE" in result.columns
...
def test_module_reload(sample_date: date) -> None:
	...
	import stpstone.ingestion.countries.br.exchange.b3_bdi_btb_loan_balance as mod

	importlib.reload(mod)
	inst = mod.B3BdiBtbLoanBalance(date_ref=sample_date)
	assert inst.date_ref == sample_date
	assert "BTBLoanBalance" in inst.url_tpl
