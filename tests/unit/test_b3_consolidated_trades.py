...
@pytest.fixture
def b3_instance(sample_date: date, mock_requests_get: MagicMock) -> B3ConsolidatedTrades:
...
def test_init_without_date_ref(mocker: MockerFixture, mock_requests_get: MagicMock) -> None:
	...
	None
	"""
	mock_dates_current = mocker.patch.object(
		DatesCurrent, "curr_date", return_value=date(2025, 1, 2)
	)
	mock_dates_br = mocker.patch.object(
		DatesBRAnbima, "add_working_days", return_value=date(2025, 1, 1)
	)
...
def test_get_response_invalid_timeout(
	b3_instance: B3ConsolidatedTrades, mock_requests_get: MagicMock, mock_backoff: None
) -> None:
...
def test_parse_raw_file(b3_instance: B3ConsolidatedTrades, mock_response: Response) -> None:
	...
	"""
	with patch.object(
		b3_instance, "get_file", return_value=StringIO("test content")
	) as mock_get_file:
...
def test_transform_data(b3_instance: B3ConsolidatedTrades) -> None:
	...
	None
	"""
	sample_csv = StringIO("Header1;Header2\n2025-01-01;ABC123\n2025-01-02;XYZ789")
...
def test_run_without_db(
	...
	with (
		patch.object(
			b3_instance, "get_file", return_value=StringIO("Header1;Header2\nData1;Data2")
		),
		patch.object(
			b3_instance, "standardize_dataframe", return_value=pd.DataFrame()
		) as mock_standardize,
	):
...
def test_empty_response(
	...
	with (
		patch.object(b3_instance, "get_file", return_value=StringIO("")),
...
def test_reload_module() -> None:
	...
	import stpstone.ingestion.countries.br.exchange.b3_consolidated_trades

	importlib.reload(stpstone.ingestion.countries.br.exchange.b3_consolidated_trades)
	instance = B3ConsolidatedTrades(date_ref=date(2025, 1, 1))
...
def test_standardize_dataframe_types(b3_instance: B3ConsolidatedTrades) -> None:
	...
	with (
		patch.object(b3_instance, "get_file", return_value=sample_csv),
		patch.object(b3_instance, "standardize_dataframe", return_value=df_) as mock_standardize,
	):
		result = b3_instance.run()
		assert isinstance(result, pd.DataFrame)
...
def test_insert_table_db_called(
	...
	with (
		...
		mock_insert.assert_called_once_with(
			cls_db=mock_db, str_table_name="test_table", df_=ANY, bool_insert_or_ignore=True
		)
