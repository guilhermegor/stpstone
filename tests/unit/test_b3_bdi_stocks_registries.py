"""Unit tests for B3BdiStocksRegistries class."""
...
@pytest.fixture
def instance(sample_date: date) -> B3BdiStocksRegistries:
	"""Fixture providing a B3BdiStocksRegistries instance.
	...
	"""
	return B3BdiStocksRegistries(date_ref=sample_date)
...
def sample_table_dict() -> dict:
	...
	-------
	dict
		Sample table dict mimicking the BDI InstrumentsEquities API response structure.
	"""
	return {
		"columns": [
			{"name": "RptDt"},
		...
		"values": [
			["2026-04-17T00:00:00", "PETR4", "PETR"],
		],
	}
...
def test_init_with_valid_inputs(sample_date: date) -> None:
	...
	None
	"""
	inst = B3BdiStocksRegistries(date_ref=sample_date, int_page_size=500)
	...
	assert "InstrumentsEquities" in inst.url_tpl
...
def test_init_default_page_size(sample_date: date) -> None:
	...
	None
	"""
	inst = B3BdiStocksRegistries(date_ref=sample_date)
	assert inst.int_page_size == 1_000


def test_init_default_page_max_is_none(sample_date: date) -> None:
	"""Test that default int_page_max is None (fetch until empty).
...
def test_init_logger_propagated() -> None:
	...
	inst = B3BdiStocksRegistries(date_ref=date(2026, 4, 17), logger=mock_logger)
	assert inst.logger is mock_logger


def test_get_response_success(instance: B3BdiStocksRegistries, mocker: MockerFixture) -> None:
	"""Test get_response posts to the correct URL and returns the response.

	Parameters
	----------
	instance : B3BdiStocksRegistries
		Initialized instance.
	mocker : MockerFixture
		Pytest-mock fixture.
...
def test_get_response_http_error(instance: B3BdiStocksRegistries, mocker: MockerFixture) -> None:
...
def test_transform_data_normal(
	...
	assert len(df_) == 1
	assert list(df_.columns) == ["RPT_DT", "TCKR_SYMB", "ASST"]
	assert df_["RPT_DT"].iloc[0] == "2026-04-17T00:00:00"
	assert df_["TCKR_SYMB"].iloc[0] == "PETR4"
	assert df_["ASST"].iloc[0] == "PETR"


def test_transform_data_column_name_conversion(instance: B3BdiStocksRegistries) -> None:
	"""Test transform_data converts all 31 InstrumentsEquities column names correctly.
