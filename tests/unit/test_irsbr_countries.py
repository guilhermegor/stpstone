"""Unit tests for IRSBRCountries class.
...
from stpstone.ingestion.countries.br.taxation.irsbr_countries import (
	_COLUMN_NAMES,
	IRSBRCountries,
)
...
# Helpers
# --------------------------
def _build_zip_bytes(csv_content: str, csv_filename: str = "Paises.csv") -> BytesIO:
	...
	csv_filename : str
		The name of the CSV entry, by default "Paises.csv".
...
@pytest.fixture
def irsbr_instance(sample_date: date) -> IRSBRCountries:
	"""Fixture providing IRSBRCountries instance.
	...
	-------
	IRSBRCountries
		Initialized IRSBRCountries instance.
	"""
	return IRSBRCountries(date_ref=sample_date)
...
def test_init_with_valid_inputs(sample_date: date) -> None:
	...
	Verifies
	--------
	- The IRSBRCountries instance is initialized correctly.
	...
	None
	"""
	instance = IRSBRCountries(date_ref=sample_date)
...
def test_transform_data(
	...
	assert len(df_) == 3
	assert df_["CODE"].iloc[0] == "105"
	assert df_["NAME"].iloc[0] == "BRASIL"


def test_transform_data_none_input(irsbr_instance: IRSBRCountries) -> None:
...
def test_transform_data_invalid_zip(irsbr_instance: IRSBRCountries) -> None:
...
def test_transform_data_zip_without_csv(irsbr_instance: IRSBRCountries) -> None:
...
def test_run_without_db(
	...
	"""
	mock_requests_get.return_value = mock_response
	sample_df = pd.DataFrame({"CODE": ["105"], "NAME": ["BRASIL"]})
	with (
...
def test_run_single_file_url(
	...
	"""Test that run fetches exactly one file (Paises.zip).

	Verifies
	--------
	- requests.get is called exactly once.
	- The URL contains Paises.zip (no numeric suffix).
...
def test_reload_module() -> None:
	...
	import stpstone.ingestion.countries.br.taxation.irsbr_countries

	original_instance = IRSBRCountries(date_ref=date(2025, 3, 15))
	importlib.reload(stpstone.ingestion.countries.br.taxation.irsbr_countries)
	new_instance = IRSBRCountries(date_ref=date(2025, 3, 15))
