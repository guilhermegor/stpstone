"""Unit tests for CvmFIFFactSheet ingestion module."""
...
class TestFIFFactSheet:
	"""Test cases for CvmFIFFactSheet class."""
	...
	def test_init_url_construction(self, mock_logger: MagicMock, sample_date: date) -> None:
		...
		None
		"""
		instance = CvmFIFFactSheet(date_ref=sample_date, logger=mock_logger)
	...
	def test_get_response_success(
		...
		"""
		with patch(
			"stpstone.ingestion.countries.br.registries.cvm_fif_fact_sheet.requests.get"
		) as mock_get:
	...
	def test_parse_raw_file_success(
		...
		"""
		with patch(
			"stpstone.ingestion.countries.br.registries.cvm_fif_fact_sheet.DirFilesManagement"
		) as mock_dir_files:
	...
	def test_run_without_db(
		...
		"""
		with (
			patch.object(CvmFIFFactSheet, "get_response") as mock_get_response,
			patch.object(CvmFIFFactSheet, "parse_raw_file") as mock_parse,
			patch.object(CvmFIFFactSheet, "transform_data") as mock_transform,
			patch.object(CvmFIFFactSheet, "standardize_dataframe") as mock_standardize,
		):
	...
	def test_run_with_db(
		...
		with (
			...
			patch.object(CvmFIFFactSheet, "transform_data") as mock_transform,
			patch.object(CvmFIFFactSheet, "standardize_dataframe") as mock_standardize,
			patch.object(CvmFIFFactSheet, "insert_table_db") as mock_insert_db,
		):
			mock_get_response.return_value = MagicMock()
			mock_parse.return_value = (StringIO(), "test.csv")
			mock_transform.return_value = sample_dataframe
