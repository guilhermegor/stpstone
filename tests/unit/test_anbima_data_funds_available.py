"""Unit tests for AnbimaDataFundsAvailable ingestion class.

Tests the web scraping functionality for ANBIMA available funds listing including:
...
class TestAnbimaDataFundsAvailable:
	...
	def test_init_with_invalid_end_page(self) -> None:
		...
		"""
		with pytest.raises(
			ValueError, match="end_page must be greater or equal than the start_page"
		):
			AnbimaDataFundsAvailable(start_page=10, end_page=5)

	def test_extract_from_card_success(
		self, funds_available_instance: AnbimaDataFundsAvailable, mock_locator: MagicMock
	) -> None:
	...
	def test_extract_cod_anbima_valid_link(
		self, funds_available_instance: AnbimaDataFundsAvailable
	) -> None:
	...
	@patch(
		"stpstone.ingestion.countries.br.registries.anbima_data_funds_available.sync_playwright"
	)
	def test_get_response_success(
		self, mock_sync_playwright: MagicMock, funds_available_instance: AnbimaDataFundsAvailable
	) -> None:
		...
		mock_locator_msg.is_visible.return_value = False
	...
	def test_run_with_db_insertion(
		...
		"""
		with (
			patch.object(funds_available_instance, "get_response") as mock_get_response,
...
class TestErrorHandling:
	"""Test error handling and edge cases for AnbimaDataFundsAvailable."""

	def test_funds_available_extraction_exception_handling(self, mock_logger: MagicMock) -> None:
	...
	def test_all_classes_inheritance_validation(self) -> None:
		"""Test that AnbimaDataFundsAvailable properly inherits from base classes.
...
class TestTypeValidation:
	"""Test type validation for AnbimaDataFundsAvailable method parameters and return values."""
...
class TestPerformanceAndMocking:
	"""Test performance optimizations and proper mocking patterns for AnbimaDataFundsAvailable."""
	...
	@patch("stpstone.ingestion.countries.br.registries.anbima_data_funds_available.time.sleep")
	@patch(
		"stpstone.ingestion.countries.br.registries.anbima_data_funds_available.sync_playwright"
	)
	...
	def test_fast_dataframe_operations(self, mock_logger: MagicMock) -> None:
	...
	@patch("stpstone.ingestion.countries.br.registries.anbima_data_funds_available.pd.DataFrame")
	def test_mocked_dataframe_operations(
		self, mock_dataframe: MagicMock, funds_available_instance_perf: AnbimaDataFundsAvailable
	) -> None:
