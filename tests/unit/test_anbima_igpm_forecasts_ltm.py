"""Unit tests for AnbimaIGPMForecastsLTM ingestion module.
...
class TestAnbimaIGPMForecastsLTM:
	...
	def test_run_with_db_calls_insert(
		...
		assert call_kwargs["str_table_name"] == "br_anbima_igpm_forecasts_ltm"
	...
	def test_transform_data(
		...
		- Applies forward fill for IGPM_EFETIVO_PCT.
	...
	def test_run_timeout_validation(
		...
		anbima_igpm_ltm.cls_db = None
		with (
			patch.object(AnbimaIGPMCore, "parse_raw_file") as mock_parse,
...
class TestDebugFlatListStructure:
	...
	def test_debug_flat_list_structure(
		...
		"""Test debug_flat_list_structure logs structure analysis.
	...
	def test_debug_flat_list_structure_empty(
		...
		"""Test debug_flat_list_structure with empty list logs header only.
...
def test_module_reload(
	...
	importlib.reload(
		sys.modules["stpstone.ingestion.countries.br.macroeconomics.anbima_igpm_forecasts_ltm"]
	)
