"""Unit tests for B3BdiCoeRegistries class."""
...
@pytest.fixture
def instance(sample_date: date) -> B3BdiCoeRegistries:
	"""Fixture providing a B3BdiCoeRegistries instance.
	...
	"""
	return B3BdiCoeRegistries(date_ref=sample_date)
...
def sample_table_dict() -> dict:
	...
	-------
	dict
		Sample table dict mimicking the BDI COERegistration API response.
	"""
	return {
		"columns": [
			{"name": "TckrSymb"},
...
def test_init_with_valid_inputs(sample_date: date) -> None:
	...
	None
	"""
	inst = B3BdiCoeRegistries(date_ref=sample_date, int_page_size=500)
	...
	assert "COERegistration" in inst.url_tpl
...
def test_init_default_page_size(sample_date: date) -> None:
	...
	None
	"""
	inst = B3BdiCoeRegistries(date_ref=sample_date)
...
def test_init_logger_propagated() -> None:
	...
	inst = B3BdiCoeRegistries(date_ref=date(2026, 4, 17), logger=mock_logger)
	assert inst.logger is mock_logger


def test_get_response_success(
	instance: B3BdiCoeRegistries,
	mocker: MockerFixture,
) -> None:
...
def test_parse_raw_file_missing_table_key(instance: B3BdiCoeRegistries) -> None:
...
def test_transform_data_normal(
	...
	assert df_["TRAD_QTY"].iloc[0] == 940
	assert df_["FINANCIAL_VOL"].iloc[0] == 130650625.32


def test_transform_data_multiple_rows(instance: B3BdiCoeRegistries) -> None:
	...
	table = {
		...
		"values": [
			["COE", 940, 130650625.32, None],
			["COE2", 200, 5000000.00, None],
			["COE3", 50, 750000.00, None],
		],
	}
...
def test_run_without_db(
	...
	assert result["FINANCIAL_VOL"].iloc[0] == Decimal("130650625.32")
...
def test_module_reload(sample_date: date) -> None:
	...
	import stpstone.ingestion.countries.br.otc.b3_bdi_coe_registries as mod

	importlib.reload(mod)
	inst = mod.B3BdiCoeRegistries(date_ref=sample_date)
	assert inst.date_ref == sample_date
	assert "COERegistration" in inst.url_tpl
