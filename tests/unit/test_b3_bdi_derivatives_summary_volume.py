"""Unit tests for B3BdiDerivativesSummaryVolume class."""
...
@pytest.fixture
def instance(sample_date: date) -> B3BdiDerivativesSummaryVolume:
	"""Fixture providing a B3BdiDerivativesSummaryVolume instance.
	...
	"""
	return B3BdiDerivativesSummaryVolume(date_ref=sample_date)
...
def sample_table_dict() -> dict:
	...
	-------
	dict
		Sample table dict mimicking the BDI DailyAverageDerivatives2 API response.
	"""
	return {
		"columns": [
			{"name": "TckrSymb"},
		...
		"values": [
			["Dia", 66514811.0, 45041189.0, 1, None],
			["Mês", 54760799.41, 33619935.91, 2, None],
		],
	}
...
def test_init_with_valid_inputs(sample_date: date) -> None:
	...
	None
	"""
	inst = B3BdiDerivativesSummaryVolume(date_ref=sample_date, int_page_size=500)
	...
	assert "DailyAverageDerivatives2" in inst.url_tpl
...
def test_init_default_page_size(sample_date: date) -> None:
	...
	None
	"""
	inst = B3BdiDerivativesSummaryVolume(date_ref=sample_date)
...
def test_init_without_date_ref(mocker: MockerFixture) -> None:
	...
	inst = B3BdiDerivativesSummaryVolume()
...
def test_init_logger_propagated() -> None:
	...
	inst = B3BdiDerivativesSummaryVolume(date_ref=date(2026, 4, 17), logger=mock_logger)
	assert inst.logger is mock_logger


def test_get_response_success(
	instance: B3BdiDerivativesSummaryVolume,
	mocker: MockerFixture,
) -> None:
...
def test_parse_raw_file_missing_table_key(instance: B3BdiDerivativesSummaryVolume) -> None:
...
def test_transform_data_normal(
	...
	instance : B3BdiDerivativesSummaryVolume
		Initialized instance.
	sample_table_dict : dict
		Sample table dict with two rows (5 values, 4 columns).
	...
	assert len(df_) == 2
	assert list(df_.columns) == ["TCKR_SYMB", "TOTAL_WITH_MINIS", "TOTAL_NO_MINIS", "COL_ORDER"]
	assert df_["TCKR_SYMB"].iloc[0] == "Dia"
	assert df_["TOTAL_WITH_MINIS"].iloc[0] == 66514811.0
	assert df_["COL_ORDER"].iloc[0] == 1


def test_transform_data_multiple_rows(instance: B3BdiDerivativesSummaryVolume) -> None:
	...
	table = {
		...
		"values": [
			["Dia", 66514811.0, 45041189.0, 1, None],
			["Mês", 54760799.41, 33619935.91, 2, None],
			["Trimestre", 51234567.0, 31234567.0, 3, None],
		],
	}
	df_ = instance.transform_data(table)
	assert len(df_) == 3
	assert set(df_["TCKR_SYMB"].tolist()) == {"Dia", "Mês", "Trimestre"}
...
def test_module_reload(sample_date: date) -> None:
	...
	import stpstone.ingestion.countries.br.exchange.b3_bdi_derivatives_summary_volume as mod

	importlib.reload(mod)
	inst = mod.B3BdiDerivativesSummaryVolume(date_ref=sample_date)
	assert inst.date_ref == sample_date
	assert "DailyAverageDerivatives2" in inst.url_tpl
