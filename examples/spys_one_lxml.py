...
def scrape_proxies():
	try:
		...
		proxy_rows = html_content.xpath(
			'//tr[contains(@class, "spy1x") or contains(@class, "spy1xx")]'
		)
		...
		for row in proxy_rows:
			...
			proxy_type_element = row.xpath(
				'.//a[contains(@href, "proxy-list")]/font[@class="spy1"]/text()'
			)
			proxy_type = proxy_type_element[0] if proxy_type_element else None

			# Extract anonymity level
			anonymity_element = row.xpath(
				'.//font[@class="spy5" or @class="spy1" or @class="spy2"]/text()'
			)
			anonymity = anonymity_element[0] if anonymity_element else None

			# Extract country/region
			region_element = row.xpath(
				'.//font[@class="spy14"]/following-sibling::font[@class="spy1"]/text()'
			)
